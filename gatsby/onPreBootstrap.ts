...
export const onPreBootstrap: GatsbyNode['onPreBootstrap'] = async ({ cache }) => {
    ...
    if (process.env.GATSBY_POSTHOG_API_KEY && process.env.GATSBY_POSTHOG_API_HOST) {
        ...
        const assetHostConfig = assetHost
            ...
            : ''
        const siteExperimentFlagKey = process.env.SITE_EXPERIMENT_FLAG_KEY
        const siteExperimentExposureSnippet =
            process.env.SITE_EXPERIMENT_ENABLED === 'true' && siteExperimentFlagKey
                ? `
posthog.onFeatureFlags(function () {
    posthog.getFeatureFlag("${siteExperimentFlagKey}")
})`
                : ''
