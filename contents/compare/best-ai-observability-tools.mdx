...
- **PostHog (usage-based per event):** the first 100,000 AIO events each month are free, then you pay a volume-tiered rate that starts at $35 per 100,000 events and falls to $6 per 100,000 at the highest volumes, with no seat fees. Each generation and span is an event, so this workload is roughly 700,000 events; the ~600,000 above the free tier works out to ~$76.
