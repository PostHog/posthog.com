...
[PostHog](/) (that's us 👋) is an open source platform combining web analytics, product analytics, session replay, A/B testing, feature flags, and user surveys into one product. This means it's not only an alternative to Fathom but also tools like [Mixpanel](/blog/posthog-vs-mixpanel), [LaunchDarkly](/compare/posthog-vs-launchdarkly), and [Hotjar](/compare/posthog-vs-hotjar).
...
-   📈 **Product analytics:** Custom trends, funnels, user paths, retention analysis, and segment user cohorts. Also, direct [SQL querying](/docs/data-warehouse/sql) for full customization.
...
1. **It replaces multiple tools:** PostHog can replace Fathom (web analytics), [Fullstory](/compare/posthog-vs-fullstory) (session replay), and [LaunchDarkly](/compare/posthog-vs-launchdarkly) (feature flags and A/B testing). This simplifies workflows and ensures product data is all in one place.
...
Although both Matomo and Fathom position themselves as privacy-friendly [Google Analytics alternative](/compare/ga4-alternatives), Matomo is much more fully featured. Matomo has a range of paid add-ons for features like session replays, heatmaps, search performance, and A/B testing.

<ProductComparisonTable
    competitors={['matomo', 'fathom']}
    rows={[
        'platform.pricing.free_tier',
        'web_analytics',
        'platform.security.cookieless_tracking',
...
> 💡 **Good to know:** Like Fathom, Matomo Cloud offers a free trial (but only 21 days). Matomo On-Premise is free to self-host.
...
> #### Bottom line
>
> For Fathom users looking for other privacy-friendly, GA-like options will find Matomo a good one. Matomo Cloud does lack the free plan of other tools, but it provides a free trial, and Matomo On-Premise is free to self-host.
...
Similar to Plausible, Umami is a like-for-like alternative to Fathom. The big differences are that it includes a free tier, is open source, and can record session replays.

<ProductComparisonTable
    competitors={['umami', 'fathom']}
    rows={[
        'platform.pricing.free_tier',
        'web_analytics',
        'platform.security.cookieless_tracking',
...
We're biased, obviously, but we think PostHog is the perfect [Fathom](/compare/posthog-vs-fathom) replacement if:
