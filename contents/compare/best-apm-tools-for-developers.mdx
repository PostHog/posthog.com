...
An APM alert tells you that checkout latency doubled, but it won't necessarily follow up with which customers abandoned their carts, for example; for that information you'd need an analytics tool to start matching timestamps, releases, and user IDs by hand.
...
Increasingly, yes. PostHog now covers all three OpenTelemetry signals – traces (beta), logs, and metrics (private alpha) – plus error tracking and Core Web Vitals, and ties them to tools such as [session replay](/docs/session-replay) and [product analytics](/docs/product-analytics). 
