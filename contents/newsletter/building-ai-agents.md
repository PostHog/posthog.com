...
- [Tracing](/docs/llm-observability/traces) for every LLM call with inputs, outputs, latency, and cost

- Trace IDs that span the full conversation

- The ability to replay and debug [specific interactions](/docs/llm-observability/sessions)

- A [curated datasets](/docs/llm-observability/clusters) of real user queries

- Automated scorers like [LLM-as-judge](/blog/stop-ai-slop) and deterministic checks

Basically, [LLM Observability](/llm-observability).

Unfortunately, LLM Observability alone isn’t enough. Reality is gnarly and to deal with it, our team often looks at real usage.

They run a “traces hour,” where they meet, manually analyze LLM traces (AKA real user interactions), and find areas to improve. [Evals](/docs/llm-observability/evaluations) make the most sense when they stem from these investigations.
