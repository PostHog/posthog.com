...
Activation AKA your product's "ah-ha" moment is the second A in the [AARRR pirate funnel framework](/product-engineers/aarrr-pirate-funnel). It's the thing that lets you know if your users are going to find success with your product.
...
At PostHog, we look for a set of actions that mean someone is much more likely to be using the product 3 months after starting. In the case of [session replay](/session-replay), after analyzing multiple potential metrics, we found that these were watching 5 or more replays and setting a replay filter at least once.
...
2. Creating groups of 3-5 events that we could test together. We aimed for 5-10 different groupings, including the number of times the event occurs (e.g. watched 5 replays vs. watched 1 replay). Sometimes users have to do something more than once to reach their "ah-ha" moment.
...
> **Note:** This query gets retention at the **group** level using [group analytics](/docs/product-analytics/group-analytics). This means that anyone in the group (eg. customer, organization [what we use], work unit - whatever your groups are) can perform the event and it counts towards activation. If you want to get retention at the **person** level, you can use the same query but replace group references with person ones like changing `group_id` to `distinct_id`.

### Part 1: Get the groups who started with your product, broken down by month

Start with an event that represents when an group starts with your product. For many products, this is simply when they first signed up. 

We use this event to get all the groups who started with our product, broken down by month. We'll use the `group_id` values throughout the rest of the query.
...
Next, filter for one grouping of potential activation events.
...
Run the query for each of your potential activation metric groupings. Save the results so you can compare. Your best retention metric is the one with the most starting groups with the highest final retention.
