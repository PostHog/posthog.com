...
The trouble with activation is that it is a lot harder to define than a user signing up. For example, we have 8+ products and each requires a separate activation metric. On top of that, each product also has 10+ events that could be part of its activation metric.
...
Actions that don't make it into your activation metric can still be good, but they don't help you predict retention. For example, we want users to capture replays, but we found that doing so doesn't mean they stick around, so optimizing this isn't the best use of our time.
...
To help you out, we've included the exact query we used below. If you use PostHog, you can create a [new SQL insight](https://us.posthog.com/insights/new) and use our same exact query to explore your data and find your activation metric. First, I'll break the query into parts to explain what's happening, then I'll provide the whole thing in one chunk [at the end](#the-final-combined-query).
...
You push for it, of course! If you know you the actions you want people to do, you can:
