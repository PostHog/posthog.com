...
People are signing up for your product. Great! But before you start planning your IPO, you need to make sure they actually become users. This is the goal of **activation**, AKA the "ah-ha" moment of your product.

The trouble with activation is that it is a lot harder to define than a user signing up. For example, we have 8+ products and each requires a separate activation metric. Each also has 10+ events that we could use as an activation metric.

This means we spent a lot of time figuring out the best way to repeatedly discover a product's activation metric. We're  sharing our process here to help you do the same.

But before we jump in... what is activation _really_?

## What is an activation metric?

An activation metric is a set of actions a user performs that are correlated with greater retention after a user first starts using the product.

Imagine you have actions A through Z. If doing actions L, M, and P make users more likely to use your product down the line, then L, M, and P combined are your activation metric. We might know actions B, C, and F are good for them to do, but if they aren't correlated with retention, they aren't part of your activation metric.

![Figuring out activation metrics](https://res.cloudinary.com/dmukukwp6/image/upload/Clean_Shot_2024_11_06_at_10_32_38_2x_b39f41573f.png)

This definition solves the trickiest part of defining an activation metric: it could be _anything_. Does a pageview mean someone is activated? Does completing onboarding? What about creating their first object? Depending on your product, any of these could be it. Looking at their impact on retention, however, makes it clear what helps your users succeed.

At PostHog, we look for a set of actions where someone is much more likely to use the product 3 months after starting. For [session replay](/session-replay), after analyzing multiple potential metrics, we found that these were watching 5 or more replays and setting a replay filter at least once.
...
![Event groups](https://res.cloudinary.com/dmukukwp6/image/upload/Clean_Shot_2024_11_06_at_11_36_20_2x_89a95ebdf3.png)

3. Writing a query to test how the event groups improve retention (see below).
...
> **Note:** This query gets retention at the **company** level using [group analytics](/docs/product-analytics/group-analytics). This means that anyone in the group (e.g. customer, organization (what we use), work unit - whatever your groups are) can perform the event and it counts towards activation.
> 
...
Start with an event that represents when an company starts with your product. For many products, this is simply when they first signed up.

We use this event to get all the companies who started with our product, broken down by month. We'll use the `company_id` values throughout the rest of the query.
