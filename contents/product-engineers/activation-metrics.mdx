...
For example, we have 8+ products and each has a different set of activation metrics from launching a survey to inviting a teammate to creating two or more dashboards. What made finding activation metrics tricky was that each product had 10+ events that **could** be part of its activation metric.

This means we spent a lot of time discovering our products' activation metrics. We're sharing our process here to help you do the same.

But before we jump in... what is activation _really_?

## What is an activation metric?

Activation metrics are a set of actions a new user performs that are correlated with greater retention.

Focusing on retention solves the trickiest part of defining an activation metric: it could be _anything_. Does a pageview count as activation? How about inviting a teammate? Creating their first object? Depending on your product, any or all of these could be an activation metric. It is **retention** that makes it clear.
