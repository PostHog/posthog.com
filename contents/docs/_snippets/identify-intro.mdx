...
To link events from anonymous to specific users, use `identify`:

import IdentifyFrontendCode from "./identify-frontend-code.mdx"

<IdentifyFrontendCode />

This creates a person profile if one doesn't exist already. This means all their events are captured with person profiles, regardless of your `person_profiles` [config option](/docs/libraries/js#person-profiles-and-user-properties). 