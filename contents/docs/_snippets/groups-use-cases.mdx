...
Here's a full overview to how groups can be applied across PostHog:
...
| Experiments | Evaluate experiment results based on group aggregations. | Run an A/B test that measures activation rate for new companies. |
| Data warehouse | Join tables with or enrich queries with groups data. | Write a custom SQL query that calculates product usage based across different company sizes. |
