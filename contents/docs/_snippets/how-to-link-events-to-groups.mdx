In our client-side SDKs, any event with a [person profile](/docs/data/person) captured after calling `posthog.group` will automatically be linked to the group during that session.
