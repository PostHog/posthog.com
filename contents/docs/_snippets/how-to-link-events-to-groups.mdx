In our client-side SDKs, any event captured after calling `posthog.group` will automatically be linked to the group during that session. You also need to capture [person profiles](/docs/data/persons) for this to work.
