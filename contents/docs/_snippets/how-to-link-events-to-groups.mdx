...
> **Identified events:** Events must be [identified](/docs/data/anonymous-vs-identified-events) in order to be associated with a group. If the `$process_person_profile` event property ends up being set to `false`, the event will not be associated with the group.
