...
<CalloutBox icon="IconWarning" title="OFFSET is not allowed for API queries" type="caution">

When querying via the API with a [personal API key](/docs/api), `OFFSET` clauses are blocked and return an HTTP 400 error. Use timestamp-based pagination as shown below, or use [batch exports](/docs/cdp/batch-exports) for bulk data extraction.

</CalloutBox>

When querying large datasets like `events` or `query_log` over multiple batches, use timestamp-based pagination instead of `OFFSET`. This is both more efficient and required when using the [API](/docs/api-queries) with a personal API key.
