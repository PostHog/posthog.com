...
> **Note:** If your URLs contain sensitive or personal information (like email addresses or tokens), path cleaning alone isn't sufficient since PostHog still stores the original URL. See the [data collection](/docs/privacy/data-collection) privacy docs for ways to prevent PostHog from capturing personal data in URLs, such as using the `before_send` hook to strip URL properties before they're sent.
