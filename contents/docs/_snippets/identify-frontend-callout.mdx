> **Identifying users is required.** Call `posthog.identify('your-user-id')` after login to link events to a known user. This is what connects frontend event captures, [session replays](/docs/session-replay), [LLM traces](/docs/llm-analytics), and [error tracking](/docs/error-tracking) to the same person — and lets backend events link back too.
>
