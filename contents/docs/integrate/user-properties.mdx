...
> **Note:** PostHog versions <1.33 overrides ignore timestamps. This means `set` always overrides, `set_once` only writes when the value doesn't exist and `alias` upon property name conflict uses the value from person 1.
