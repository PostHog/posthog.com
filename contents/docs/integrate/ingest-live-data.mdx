...
-   [How autocapture works with the JavaScript library](#capture-user-events)
...
> Autocapture is presently only available in the web brower using the JavaScript library. For non-browser platforms and for situations where you wish to manually capture events, see the [capture user events](#capture-user-events) section.
