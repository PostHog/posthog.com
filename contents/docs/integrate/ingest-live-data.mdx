...
It's top-priority for us that events are fully processed and saved as soon as possible. However, there is a class of events which we _deliberately_ process with a slight delay. Specifically, an event is delayed by around a minute if it fits those three conditions:

- isn't anonymous (anonymous events are recognized by having the `distinct_id` the same as the `$device_id` property)
- isn't an `$identify` event (i.e. `posthog.identify()`)
