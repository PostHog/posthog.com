...
If you're using Cloudflare, you can use their proxy to forward requests to PostHog Cloud.

1. Create a new CNAME record for your domain. It should point to `app.posthog.com` or `eu.posthog.com` depending on your region, and have proxy enabled.
      - e.g. `CNAME, e, app.posthog.com, proxy enabled`

TODO: add in any extra configuration options needed e.g. disablying caching