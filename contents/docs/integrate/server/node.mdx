...
Witht the release of V2, the API was kept mostly the same but with some small changes and deprecations:

1. The `callback` parameter passed as an optional last argument to most of the methods is no longer supported
