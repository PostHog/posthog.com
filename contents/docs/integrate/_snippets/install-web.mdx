...
By default, the PostHog JS library will only load the core functionality, lazy-loading extensions such as Surveys or Replay when needed. This can cause issues if you have a Content Security Policy (CSP) that blocks inline scripts or if you want to optimize your bundle.
You can include all extensions in your bundle by importing them directly before initializing PostHog. In addition you can configure the SKD to never load extensions lazily.
