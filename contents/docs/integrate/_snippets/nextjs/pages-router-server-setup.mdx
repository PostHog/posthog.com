...
export async function getServerSideProps(ctx) {
  ...
  if (session) {
    ...
    // Use the same stable user ID your frontend passes to posthog.identify().
    // Avoid emails and other personal data as a distinct ID — send them as
    // person properties instead.
