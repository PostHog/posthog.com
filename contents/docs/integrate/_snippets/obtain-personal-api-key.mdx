...
4. Choose the scopes for your key. Use the **Scopes** dropdown to select a preset:
   - **Read-only access** - grants read permissions for all scopes but no write permissions. Ideal for keys that only need to query data.
   - **All access** - grants full read and write permissions for all scopes.
   
   You can also customize individual scopes after selecting a preset. We recommend selecting only the scopes you need – you can always modify them later.
