import SettingProperties from "./setting-properties-text.mdx"
import NamingTip from "./naming-tip.mdx"
import CustomEventIntro from "./intro.mdx" 

By default, PostHog automatically captures pageviews and pageleaves as well as clicks, change of inputs, and form submissions associated with `a`, `button`, `form`, `input`, `select`, `textarea`, and `label` tags. See our [autocapture docs](/docs/product-analytics/autocapture) for more details on this.

If you prefer to disable or filter these, set the appropriate values in your [configuration options](/docs/libraries/js/config).

### Custom event capture

<CustomEventIntro />

```js-web
posthog.capture('user_signed_up');
```

<NamingTip />

<SettingProperties />

```js-web
posthog.capture('user_signed_up', {
    login_type: "email",
    is_free_trial: true
})
```

### Manually capturing pageviews in single-page apps

PostHog automatically sends `$pageview` events whenever it gets loaded and `$pageleave` when they leaves. If you have a [single-page app](/tutorials/single-page-app-pageviews), that means it only sends pageview and pageleave once by default (when your app loads and when they leave).

To fix this, you can set the `capture_pageview` configuration option to `history_change`. This uses the history API to capture pageviews and pageleaves whenever the user navigates within your app.

```js
posthog.init("<ph_project_api_key>", {
  api_host: "<ph_client_api_host>",
  capture_pageview: "history_change",
})
```
