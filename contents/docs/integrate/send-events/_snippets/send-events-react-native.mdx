...
PostHog autocapture can automatically tracks the following events for you:
...
With autocapture, all touch events for children of `PosthogProvider` will be tracked, capturing a snapshot of the view hierarchy at that point. This allows you to create [insights](/docs/product-analytics/insights) in PostHog without having to add many custom events.
