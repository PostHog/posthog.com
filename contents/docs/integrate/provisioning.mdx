...
If you're integrating with PostHog as a partner platform, this API covers two things:

1. **Onboarding** – create a PostHog account for a user, provision a project, and get back the credentials to start sending events. The user gets a welcome email with a link to set their password.
2. **Deep linking** – once a user is provisioned, mint short-lived URLs that drop them straight into their PostHog project from your own UI (e.g. an "Open in PostHog" button), with no consent screen or password prompt.
