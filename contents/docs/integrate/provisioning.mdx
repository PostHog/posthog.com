...
sequenceDiagram
    ...
    PH-->>U: redirect to path or /project/N (session minted)
```
...
| Field | Type | Required | Description |
|---|---|---|---|
| `path` | string | No | The in-app path to land on after login, such as `/project/12345/replay/session-id` or `/project/12345/insights/abc123`. Must be a relative path starting with `/`. Omit to land on the project home. |
| `purpose` | string | No | A free-form label for analytics. Defaults to `dashboard`. |
...
// Examples:
// getDeepLinkUrl(token, '/project/12345/replay/session-id')  // specific recording
// getDeepLinkUrl(token, '/project/12345/insights/abc123')    // specific insight
// getDeepLinkUrl(token)                                       // project home (default)
```
