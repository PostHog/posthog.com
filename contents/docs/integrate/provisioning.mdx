...
Only unprivileged, user-grantable scopes are accepted – see [available scopes](#available-scopes) for the full list. Privileged, internal, and unrecognized scope strings are silently dropped.
...
async function getDeepLinkUrl(accessToken) {
  const res = await fetch(`${BASE_URL}/api/agentic/provisioning/deep_links`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
