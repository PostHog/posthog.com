...
PostHog v1.10.0 introduced [Feature Flags](/docs/user-guides/feature-flags), which allow you to safely deploy and roll back new features. This feature requires posthog-android version of `2.0.0` or above.
...
PostHog 1.31.0 introduced support for [group analytics](/docs/user-guides/group-analytics), which allows you to associate users and events with larger groups (teams, organizations, etc.). This feature requires posthog-android version of `2.0.0` or above.
...
```java
PostHog.with(this).group("group-type", "group-key", new Properties().putValue("name", "some-company-name"));
```
