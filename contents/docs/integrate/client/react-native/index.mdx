...
export const posthog = new PostHog("<ph_project_api_key>", {
    ...
    decidePollInterval?: number = 30000,
...
Super Properties are persisted across sessions so you have to explictly remove them if they are no longer relevant. In order to stop sending a Super Property with events, you can use `posthog.unregister`, like so:
...
PostHog will load feature flags when instantiated and will refresh periodically based on the configured options or whenever other methods are called that could affect the flag such as `.group()`. If you have the need to forcefully trigger the refresh however you can use `reloadFeatureFlagsAsync` 
```js
