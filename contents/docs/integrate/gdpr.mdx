...
Since PostHog automatically captures some data (for example, the IP address of a user's web browser) which can be considered<sup>†</sup> [Personally Identifiable Information (PII)](https://gdpr.eu/eu-gdpr-personal-data/), you must provide a mechanism for the concentual capturing of that data.
