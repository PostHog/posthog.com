...
`getFeatureFlagResult` returns `undefined` when the flag doesn't exist or hasn't loaded yet.
...
The `getFeatureFlagResult` API is available in version **1.336.0** and above. We recommend upgrading to the latest version. Otherwise, use the previous APIs:
