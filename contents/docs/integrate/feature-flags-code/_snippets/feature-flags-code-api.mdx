...
Only flags that have matching evaluation tags (or no tags at all) will be returned. For example:
- Flag with evaluation tags `["production", "api"]` + request with `["production", "web"]` = ✅ Flag evaluates ("production" matches)
- Flag with evaluation tags `["staging"]` + request with `["production", "web"]` = ❌ Flag doesn't evaluate (no matching tags)
