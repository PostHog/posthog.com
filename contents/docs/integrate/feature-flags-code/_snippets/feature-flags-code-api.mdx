...
#### Using evaluation contexts and runtime filtering without SDKs

When making direct API calls to the `/flags` endpoint, you can control which flags are evaluated using evaluation contexts and runtime filtering.
...
Only flags where at least one evaluation context matches (or flags with no contexts at all) will be returned. For example:
- Flag with evaluation contexts `["production", "api", "backend"]` + request with `["production", "web"]` = ✅ Flag evaluates ("production" matches)
- Flag with evaluation contexts `["staging", "api"]` + request with `["production", "web"]` = ❌ Flag doesn't evaluate (no contexts match)
- Flag with evaluation contexts `["web", "mobile"]` + request with `["production", "web"]` = ✅ Flag evaluates ("web" matches)
- Flag with no evaluation contexts = ✅ Always evaluates (backward compatibility)
...
##### Combining evaluation contexts and runtime filtering
...
// AND
// 2. Have evaluation contexts matching "production" OR "web" (or no contexts)
