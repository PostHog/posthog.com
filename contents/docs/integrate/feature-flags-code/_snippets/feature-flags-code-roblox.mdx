...
```lua
local payload = PostHog:GetFeatureFlagPayload(player, "checkout-config")
if payload then
...
```lua
local flag = PostHog:GetFeatureFlag(player, "checkout-config")
...
PostHog:SetGroupPropertiesForFlags(player, "company", {
    size = "enterprise",
})
```
