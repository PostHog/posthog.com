...
const MyComponent = () => {
    ...
    // Use the 'useFeatureFlagWithPayload' hook for fetching the feature flag payload
...
// Fetch the payload returns 'JsonType' or undefined if not loaded yet or if there was a problem loading
...
Or when we want to trigger the reload but we don't care about the result
