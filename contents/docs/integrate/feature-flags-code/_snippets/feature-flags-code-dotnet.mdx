...
```csharp
var isMyFlagEnabled = await posthog.IsFeatureEnabledAsync("flag-key", "distinct_id_of_your_user");
...
```csharp
var flag = await posthog.GetFeatureFlagAsync("flag-key", "distinct_id_of_your_user");

if (flag is { VariantKey: "variant-key"} ) { // replace "variant-key" with the key of your variant
...
```csharp
if (await posthog.GetFeatureFlagAsync("flag-key", "distinct_id_of_your_user")) {
    // Do something differently for this user
}
```
