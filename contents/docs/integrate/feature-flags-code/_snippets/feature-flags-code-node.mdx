...
`getFeatureFlagResult` returns `undefined` when the flag doesn't exist.
...
1. You call `client.getFeatureFlagResult()`, `client.getFeatureFlag()`, or `client.isFeatureEnabled()`, AND
...
The `getFeatureFlagResult` API is available in version **5.21.2** and above. We recommend upgrading to the latest version. Otherwise, use the previous APIs:

```node
// Boolean check
const isEnabled = await client.isFeatureEnabled('flag-key', 'distinct_id_of_your_user')

// Multivariate check
const variant = await client.getFeatureFlag('flag-key', 'distinct_id_of_your_user')

// Fetch the payload
const payload = await client.getFeatureFlagPayload('flag-key', 'distinct_id_of_your_user')
```
