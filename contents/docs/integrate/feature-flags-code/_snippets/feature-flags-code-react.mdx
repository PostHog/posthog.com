...
| Hook | Description |
| --- | --- |
| `useFeatureFlagResult` | Returns the full feature flag result (with `enabled`, `variant`, and `payload`). This sends a `$feature_flag_called` event. |
...
The `useFeatureFlagResult` API is available in `@posthog/react` version **1.8.0** and above. We recommend upgrading to the latest version. Otherwise, use the previous APIs:

```react
import { useFeatureFlagEnabled, useFeatureFlagVariantKey, useFeatureFlagPayload } from '@posthog/react'
