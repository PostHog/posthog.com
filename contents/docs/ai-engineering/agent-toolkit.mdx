...
The PostHog agent toolkit equips your AI agents with the ability to interact with the PostHog API and platform. It provides a set of standardized function calling tools that can be integrated with frameworks like Vercel's AI SDK, LangChain, and others.
...
- Write natural language SQL queries
- Create, update, or remove feature flags
- Track errors, experiments, and LLM analytics
- Search PostHog documentation
- See the [available tools](#available-tools) section for the full list 

Designed for developer productivity, the toolkit also provides:

- **State management**: Toolkit remembers your PostHog project and organization context across tool calls. 
- **Type-safety**: Zod schemas provide runtime validation and IDE autocompletion.
- **Shared schema**: Python and TypeScript use the same tool schema.
- **Error handling**: Clear, actionable errors to help debug agent failures. 

## MCP vs agent toolkit 

The PostHog [MCP server](/model-context-protocol) and agent toolkit are two complementary ways to integrate PostHog tools into your AI workflows. They share the same foundation; a set of core services and function calls that include tool definitions, schemas, API interactions, state management, error handling, etc.
...
Pick what makes sense for your use case. For example, you could use the agent toolkit for internal applications *and* the MCP server for third-party integrations.
...
A few more example prompts you can use with PostHog tool calls:
...
Check out our repo for [more integration examples](https://github.com/PostHog/mcp/tree/main/examples) and instructions for how to use our agent toolkit. 
