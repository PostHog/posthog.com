...
| `skill-get`         | Fetch a skill by name — returns the body, file manifest, and paging metadata for truncation recovery |
...
### Handling large skill bodies

Most skills fit in a single response. However, some MCP clients cap the size of tool responses, which can silently truncate a large skill body. `skill-get` includes paging metadata so agents can detect and recover from this.

Every `skill-get` response includes `body_total_length` — the full body length in characters. If the `body` you received is shorter than `body_total_length`, it was truncated in transit. Use `body_offset` and `body_length` to page through the body, and continue from `body_next_offset` until it is `null`:

```json
// First page — request 4000 characters
{ "skill_name": "my-large-skill", "body_offset": 0, "body_length": 4000 }

// Response includes paging metadata before the body
{
  "body_total_length": 12000,
  "body_next_offset": 4000,
  "body": "... first 4000 characters ..."
}

// Next page — continue from body_next_offset
{ "skill_name": "my-large-skill", "body_offset": 4000, "body_length": 4000 }

// Final page — body_next_offset is null when you've reached the end
{
  "body_total_length": 12000,
  "body_next_offset": null,
  "body": "... last characters ..."
}
```

When no paging params are passed, the full body is returned as before — existing consumers are unaffected.
