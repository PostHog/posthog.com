...
Now, when you use the OpenAI SDK, it automatically captures many properties into PostHog including `$ai_input`, `$ai_input_tokens`, `$ai_cache_read_input_tokens`, `$ai_latency`, `$ai_model`, `$ai_tools`, `$ai_model_parameters`, `$ai_output_choices`, and `$ai_output_tokens`.
