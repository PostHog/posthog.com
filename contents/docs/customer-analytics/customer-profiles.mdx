...
| Customer journeys | [Product analytics](/docs/product-analytics) | Customer journeys configured |
...
Customer profiles can display customer journey progress, showing where a person or group is in your defined funnels. Each step in the journey is marked as completed or not completed based on the customer's activity.

To use customer journeys, add funnel insights as journeys from the [customer analytics journeys page](https://app.posthog.com/customer_analytics/journeys). Once configured, the journey appears in the customer profile. If you have multiple journeys, use the dropdown to switch between them.
