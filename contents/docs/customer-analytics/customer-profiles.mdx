...
| Customer journeys | [Customer journeys](#customer-journeys)                 | Journeys configured with funnel insights       |
| Event stream      | [Event stream](#event-stream)                           | Event stream configured with Slack integration |

## Viewing a customer profile

You can access customer profiles in two main ways:

1. **From the dashboard**: Click on any customer in the [customer analytics dashboard](https://app.posthog.com/customer_analytics)
...
Customer profiles include a **Customer journey** node that shows where a person or group is within your defined journeys. Journeys are based on funnel insights you configure, letting you see a customer's progress through key conversion steps.

The journey node displays:

- Which funnel steps the customer has completed
- Which steps are incomplete
- Which steps are optional

You can switch between different journeys using the dropdown selector in the node. If no journeys are configured, click **Configure journeys** to set them up.

For details on creating and managing journeys, see [Create customer journeys](/docs/customer-analytics/create-customer-journeys).

## Event stream

Account profiles include an **Event stream** tab where you can add or remove accounts from your personal event stream. When enabled, selected events from that account are delivered to a Slack channel of your choice in real time.

Each team member configures their own stream independently. To set yours up:

1. Go to **Customer analytics** > [**Configuration**](https://app.posthog.com/customer_analytics/configuration?tab=customer-analytics-event-stream) and select the events you want to stream.
2. Connect a Slack workspace and pick a channel for delivery.
3. Open an account profile and switch to the **Event stream** tab.
4. Toggle **Include in my event stream** to start streaming that account's events.

> **Note:** Only accounts with an `external_id` can have their events matched and streamed. Accounts missing this identifier show a warning on the toggle.
