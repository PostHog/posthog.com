...
- **Selector:** Path to the element on the page. If you're not happy with the selector PostHog is using automatically, you could, for example, select elements based on their `id` or a `data-*` attribute. You can type a selector into the "Selector" field in the action creation modal to do this.

> **Tip: Avoid fragile selectors.** Selectors using `:nth-child()` or `:nth-of-type()` are position-based and will break if the page structure changes. For more stable tracking, add a `data-*` attribute to your element (like `data-analytics-attr="signup-button"`) and use that as your selector (e.g., `[data-analytics-attr="signup-button"]`). Any `data-*` attribute name works.
