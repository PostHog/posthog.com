...
1. Go to the [Heatmaps section in PostHog](https://us.posthog.com/heatmaps)
2. Type the URL of your website in the **Page URL** field
3. Optional: Use wildcard URL matching for the **Heatmap data URL** to combine data from similar pages

You can edit the **Page URL** at any time after creating a heatmap. Enter a new URL and click **Regenerate** to refresh the screenshot. The Page URL must be a concrete URL without wildcards. If you want to aggregate data from multiple pages, add wildcards to the **Heatmap data URL** instead.
...
If you have similar pages that have dynamic content based on an identifier in the URL (e.g., product pages on an ecommerce site), you can combine their heatmap data using wildcards `*` in the **Heatmap data URL** field.

For example, if the product pages on an ecommerce site use a URL format of `https://yourwebsite.com/products/product-id`, you can use the wildcard URL `https://yourwebsite.com/products/*` in the **Heatmap data URL** to display a combined heatmap from all the product pages.

> **Note:** Wildcards can only be used in the **Heatmap data URL** field, not in the **Page URL** field. The Page URL must be a concrete URL since it's the actual page that gets loaded for the heatmap visualization.
