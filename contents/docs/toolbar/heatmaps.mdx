...
The in-app heatmap viewer includes an **Advanced settings** dropdown with additional configuration options:

- **Heatmap data URL** - Override the URL used to fetch heatmap data. Use wildcards to combine data from similar pages.
- **Dismiss cookie & consent banners** - Automatically dismisses cookie consent banners and modals before capturing the screenshot, ensuring the heatmap shows actual page content without being obscured. This option is only available for screenshot heatmaps.
