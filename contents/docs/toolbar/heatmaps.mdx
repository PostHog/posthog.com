...
### Height limit for tall pages

In-app heatmaps are capped at 8,000px tall to keep rendering performant. When viewing a page where the captured scroll depth exceeds this height, an info banner appears above the heatmap to let you know that data below that point isn't shown.

This limit only affects the in-app heatmap viewer. The toolbar heatmap and your underlying event data are not affected.
