...
All configuration options available in the toolbar (aggregation, viewport accuracy, color palette, fixed positioning) are also available in the in-app version, along with a capture method toggle.

### Capture method

When using the in-app heatmap viewer, you can switch between two capture methods in **Heatmap settings**:

- **Screenshot** – Generates a full-page screenshot of your site. This works even if your site blocks iframes.
- **Iframe** – Loads your site directly in an iframe for a live view.

You can change the capture method at any time. If you switch to screenshot mode and no screenshot exists, PostHog automatically generates one.
