...
When you select live preview mode, PostHog runs a preflight check that fetches your page server-side and inspects its response headers. If the page blocks iframe embedding or returns an error, PostHog shows a specific message explaining the problem and offers to switch to screenshot mode instead.

The two most common reasons a live preview fails to load:

**Your site blocks iframe embedding.** Many platforms (including Shopify) send `Content-Security-Policy: frame-ancestors 'none'` or `X-Frame-Options: DENY` by default, which prevents the live preview from rendering your page. If you control your server's headers, you can allow PostHog to embed your site by updating your `Content-Security-Policy` header:
...
If you can't change these headers, use screenshot mode instead.

**Your site returns an HTTP error.** If your site's host or CDN returns a 4xx or 5xx status code (such as a 429 rate limit or a 403 challenge page), PostHog shows the status code and a short excerpt of the response so you can identify the issue. Check your site's rate limits and firewall rules, then try again.

### Screenshot captures an error page

When PostHog generates a screenshot heatmap, it checks the HTTP status code returned by your page during the render. If your page returns a non-2xx status (e.g., 429, 403, or 500), PostHog fails the heatmap with the actual error status rather than saving a screenshot of the error page. The error message includes the status code and identifies that it came from your site's host or CDN, not from PostHog.

### Using session recordings as a fallback

If both live preview and screenshot modes don't work for your site (due to auth walls, bot protection, or other restrictions), PostHog automatically searches for recent session recordings that visited the same page and suggests up to three of them.

Click any suggested recording to open the session player, then click **View heatmap** to view heatmap data overlaid on the recording snapshot.
