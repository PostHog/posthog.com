...
1. **Heatmap:** which capture mouse movements, clicks, and rageclicks and shows you a heatmap of these interactions.
...
The heatmap data shows where on the page users are clicking, rageclicking, or moving their mouse. Unlike the clickmap, the user doesn't have to be clicking on a clickable element, so you can see where users are trying to click but can't. There are mulitple options to configure your heatmap.
...
Some clicks will be on elements that don't move when the page scrolls. Our JavaScript SDK will attempt to detect fixed elements such as headers or modals and will therefore show those heatmap areas, ignoring the scroll value. 
