...
To use feature flags in the client-side, import PostHog into your Svelte component and check if the feature is enabled (while ensuring the code only runs in the browser).
...
To use feature flags in the server-side, import PostHog into your SvelteKit `load` function and check if the feature is enabled. 
