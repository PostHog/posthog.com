...
- **Log forwarding** – Optionally forwards `Rails.logger` output to [PostHog Logs](/docs/logs) over OpenTelemetry, automatically correlated with request context
...
`posthog-rails` can forward `Rails.logger` output to [PostHog Logs](/docs/logs) over OpenTelemetry (OTLP), automatically correlated with each request's distinct ID and session ID. This is opt-in and requires Ruby 3.3+.

### Setup

Add the OpenTelemetry gems to your `Gemfile` with `require: false` — `posthog-rails` loads them only when logs are enabled:
...
`opentelemetry-logs-sdk` must be `>= 0.6.0`; the OTLP exporter requires `LogRecordData#event_name`, which only exists from that version onward. Using `require: false` also prevents `opentelemetry-logs-sdk`'s load-time patch from adding a second logs pipeline to an existing tracing setup.

Then enable log forwarding in your PostHog initializer:
...
Logs reuse the same project token (`api_key`) and host configured in `PostHog.init` — nothing extra to set. When the OpenTelemetry gems are absent, the feature logs a single warning and no-ops.
...
`logs_level` controls which severity levels are forwarded to PostHog. It never changes what your app logs locally. Setting it below the Rails logger level (e.g. `:debug` when the app logs at `:info`) does not cause Rails or ActiveRecord to produce extra output:
...
Forwarding is capped at 6,000 records per minute by default to protect your ingestion quota. When the cap is reached, one warning record is emitted and further records are dropped for the remainder of that minute:
...
- `Rails.logger.silence` does not silence log forwarding — silenced records still ship to PostHog, because the silencer only lowers the level of loggers that support `local_level`.
- `Rails.logger.tagged` tags (including `config.log_tags` request IDs) are not attached to forwarded records, and the non-block form (`Rails.logger.tagged('X')`) returns a logger that bypasses forwarding entirely.
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `logs_enabled` | Boolean | `false` | Master switch for forwarding `Rails.logger` output to PostHog Logs over OTLP. Requires Ruby 3.3+ and the [three OpenTelemetry gems](#setup-1). |
...
| `logs_level` | Symbol/Integer | `nil` | Minimum severity to forward (`:debug`, `:info`, `:warn`, `:error`, `:fatal`, or a `Logger` constant). Inherits the Rails logger level when nil. |
| `logs_max_records_per_minute` | Integer/String | `6000` | Records-per-minute cap. Numeric strings (e.g. from `ENV`) are coerced. Set to `nil` or `0` to disable. |
...
1. **Check the OTel gems are installed:**
   ```bash
   bundle list | grep opentelemetry
   ```
   You should see `opentelemetry-sdk`, `opentelemetry-logs-sdk` (≥ 0.6.0), and `opentelemetry-exporter-otlp-logs`.
...
3. **Verify `logs_enabled` is set** in your initializer and that the initializer runs before the railtie (it should if it lives in `config/initializers/`).
