...
Events which relate to billing or where sampling/editing would break the PostHog feature are never processed by `before_send`. Our SDK ignores attempts to try to amend these events. These include:
