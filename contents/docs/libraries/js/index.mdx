...
To capture events without person profiles, update the `process_person` config option with one of the three options:

1. `process_person: 'always'` _(default)_ - We include person profiles for all events
2. `process_person: 'never'` - We don't include person profiles on any event.
3. `process_person: 'identified_only'` - We only include person profiles when you call `posthog.identify()`. Anonymous users won't get person profiles.
...
| `process_person`<br /><br />**Type:** Enum: `always`, `identified_only`, `never`<br/>**Default:** `always`                                                                                | Set whether events should have person profiles enabled |
