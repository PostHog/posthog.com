...
Sometimes you may want to test sending an event with `posthog.capture` or some other feature, but most modern web app (React, Vue, etc.) don't expose `window.posthog`. If you enable debug mode, you can expose `posthog` by doing the following:

1. In the browser console, search for `[PostHog.js] Starting in debug mode`
2. Expand the object
3. Right-click on `this` and pick "Store as global variable"
