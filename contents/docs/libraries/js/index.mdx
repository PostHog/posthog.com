...
Events whic are to do with billing or where sampling/editing would break the PostHog feature are never processed by `beforeSend`
...
posthog.init('my_token', {
    beforeSend: (event) => {
        let sampleRate = 1.1 // default to always returning the event
        if (event.event === '$heatmap) {
            sampleRate = 0.1
        }
