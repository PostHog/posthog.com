...
When initializing the SDK you can provide a `before_send` function. 
...
### Not all events are processed by `before_send`.

Events which are to do with billing or where sampling/editing would break the PostHog feature are never processed by `before_send`
