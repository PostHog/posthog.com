...
By default, PostHog captures events with person profiles. To set [user properties](/docs/data/user-properties) in these profiles, include them when capturing an event:
...
This creates a special `$set` event that is sent to PostHog. For more details on the difference between `$set` and `$set_once`, see our [user properties docs](/docs/data/user-properties#what-is-the-difference-between-set-and-set_once).

To capture events without person profiles, update the `person_profiles` config option with one of the three options:
...
Whenever you call `posthog.identify` with user properties, we automatically add these properties to flag evaluation calls to help determine the correct flag values. The same is true for when you call `posthog.group()`.
...
3. At the same time, set the user property `$feature_interaction/<flag-key>` to true. [Here's a code example](https://github.com/PostHog/posthog-js/blob/master/react/src/components/PostHogFeature.tsx#L48C10-L48C35).
...
- `persistence: "cookie"` (default): Stores all data in a cookie.

- `persistence: "localStorage+cookie"`: Stores user's distinct ID in a cookie and everything else in the browser's <a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage" target="_blank">`localStorage`</a>.
...
| `person_profiles`<br /><br />**Type:** Enum: `always`, `identified_only`, `never`<br/>**Default:** `always`                                                                                | Set whether events should have person profiles enabled |
