...
To control whether you capture person profiles, update the `person_profiles` config option:

import PersonProfilesConfig from './_snippets/person-profiles-config.mdx'

<PersonProfilesConfig />
...
Note that this does **not** set the user's properties. This only sets the properties for their events. To store user properties, see the [setting user properties docs](https://posthog.com/docs/product-analytics/user-properties).
...
| `ui_host`<br/><br/>**Type:** String<br/>**Default:** undefined                                                                                                                            | If using a [reverse proxy](/docs/advanced/proxy) for `api_host` then this should be the actual PostHog app URL (e.g. https://us.posthog.com) |
...
| `person_profiles`<br /><br />**Type:** Enum: `always`, `identified_only`<br/>**Default:** `always`                                                                                | Set whether events should have person profiles processed |
