...
`sendBeacon` is the right choice in `pagehide` / `beforeunload` because the browser guarantees delivery even after the page is gone, where a regular `fetch` may be canceled.
