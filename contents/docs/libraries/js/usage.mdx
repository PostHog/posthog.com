...
To remove person properties, use `unsetPersonProperties` with the property name (or an array of names) you want to remove. This creates a `$set` event with a `$unset` array, which removes those properties from the person's profile. See [person properties](/docs/product-analytics/person-properties#how-to-remove-person-properties) for more details.

```js
// Remove a single property
posthog.unsetPersonProperties('name')

// Remove multiple properties
posthog.unsetPersonProperties(['name', 'email'])
```
