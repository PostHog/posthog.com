...
The browser SDK ships first-class support for [PostHog Logs](/docs/logs): structured records via `posthog.logger.*` / `posthog.captureLog`, plus optional `console.*` autocapture. For setup and usage, see the [JavaScript web Logs installation guide](/docs/logs/installation/javascript). Config reference: [Configuring logs](/docs/libraries/js/config#configuring-logs).

### Manually flushing logs before unload

Records flush automatically every `flushIntervalMs`, but for an immediate drain — e.g. in a `pagehide` handler before the tab is closed — call `posthog.flushLogs()`:
...
`sendBeacon` is the right choice in `pagehide` / `beforeunload` because the browser guarantees delivery even after the page is gone, where a regular `fetch` may be cancelled.
