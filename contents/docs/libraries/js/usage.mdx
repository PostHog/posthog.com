...
To set [person properties](/docs/product-analytics/person-properties) in these profiles, include them when capturing an event:
...
This creates a special `$set` event that is sent to PostHog. For more details on the difference between `$set` and `$set_once`, see our [person properties docs](/docs/product-analytics/person-properties#what-is-the-difference-between-set-and-set_once).

To remove person properties, use `unsetPersonProperties()`. See [person properties](/docs/product-analytics/person-properties#how-to-remove-person-properties) for examples.
...
We recommend reading our docs on [alias](/docs/product-analytics/identify#alias-assigning-multiple-distinct-ids-to-the-same-user) to best understand how to correctly use this method.
...
If you evaluate feature flags outside the SDK – for example on your own server with [`posthog-node` local evaluation](/docs/feature-flags/local-evaluation), then pass the results to the browser – you can have the SDK use those values instead of requesting flags itself.

Set `advanced_disable_feature_flags: true` so PostHog stops evaluating feature flags (the reloads triggered by `identify()`, `group()`, and `setPersonProperties()` no longer fetch flags), then push your evaluated flags at runtime with `updateFlags(flags, payloads?, { merge })`:

```js-web
posthog.init('<ph_project_token>', {
    // Don't evaluate flags in the browser – we supply them ourselves.
    advanced_disable_feature_flags: true,
    // Optional: values that must be available on load, before updateFlags() runs.
    bootstrap: {
...
`updateFlags` replaces the stored flags by default; pass `{ merge: true }` to merge into the existing set instead. Use `bootstrap` for any flag values that must be available on first load, before `updateFlags()` runs.

Note that surveys rely on feature flags internally, so `advanced_disable_feature_flags` also disables surveys. If you use surveys, set `advanced_only_evaluate_survey_feature_flags: true` instead to keep only survey-related flags evaluated.
...
These events also add two charts to the flag's [**Usage** tab](/docs/feature-flags/surfaces/web-app#see-how-a-flag-is-used) – one for views and one for interactions – so you can compare how many people saw a feature against how many used it. PostHog checks for these events periodically, so the charts don't appear immediately.
...
The browser SDK ships first-class support for [PostHog Logs](/docs/logs): structured records via `posthog.logger.*` / `posthog.captureLog`, plus optional `console.*` autocapture. For setup, manual flushing, and the full config reference, see the [JavaScript web logs installation guide](/docs/logs/installation/javascript).
