...
To capture survey results properly in PostHog, you need to capture 4 types of events:
...
// 3. When a user has started but not completed a survey
posthog.capture("survey abandoned", {
  $survey_id: survey.id // required
})

// 4. When a user has responded to a survey
...
The `survey abandoned` event fires when a user answers at least one question in a multi-step survey but navigates away or closes the page before completing all questions. This differs from `survey dismissed`, which fires when a user explicitly closes the survey.
