...
The browser SDK can send structured log records to [PostHog Logs](/docs/logs). Two paths:

- **Browser console autocapture** — toggle on `console.log` / `console.warn` / `console.error` interception via the `logs.captureConsoleLogs` config option. See [Configuring logs](/docs/libraries/js/config#configuring-logs).
- **Manual capture API** — call `posthog.captureLog(...)` or `posthog.logger.{trace,debug,info,warn,error,fatal}(...)` directly. Always available regardless of the autocapture toggle.
...
Or `posthog.captureLog` for full control over level, attributes, and W3C trace context:
...
Each record is automatically tagged at capture time with the user's distinct ID, session ID, current URL, and active feature flags. Records are buffered, batched into OTLP payloads, and flushed every `flushIntervalMs` (default 3 seconds) — or sooner when the buffer fills.

### Manual flush

For an immediate drain — e.g. in a `pagehide` handler before the tab is closed:

```js
posthog.flushLogs() // uses the configured transport
posthog.flushLogs('sendBeacon') // forces sendBeacon, useful for unload handlers
```

### Filtering and rate-limiting

Configuration knobs (`flushIntervalMs`, `maxBufferSize`, `maxLogsPerInterval`, `serviceName`, etc.) are documented in [Configuring logs](/docs/libraries/js/config#configuring-logs).
