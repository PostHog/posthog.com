...
| `rageclick`<br/><br/>**Type:** Boolean or RageclickConfig<br/>**Default:** `true` | Determines if PostHog should automatically capture rage click events when users rapidly click on non-responsive elements. When `defaults: '2025-11-30'` or later is set, `content_ignorelist` is enabled by default to ignore elements with navigation-related text. When `defaults: '2026-05-30'` or later is set, `ignore_text_selection` is also enabled by default. [See below for `RageclickConfig`](#configuring-rageclick). |
...
The `.ph-no-rageclick` class automatically excludes elements from rage click tracking.

For more control, the `rageclick` config takes an object that controls rage click event detection. Partial config objects merge with the date-gated defaults rather than replacing them, so you can override specific options while keeping the others.
...
| `content_ignorelist`<br/><br/>**Type:** Boolean or Array of Strings<br/>**Default:** `true` when `defaults: '2025-11-30'` or later | Excludes elements from rage click tracking based on their text content or aria-label. When `true`, uses default keywords `['next', 'previous', 'prev', '>', '<']`. When `defaults: '2026-05-30'` or later, also includes stepper symbols `['+', '-', '−', '–']`. When set to an array of strings (max 10 items), uses custom keywords. Set to `false` to disable content-based exclusion. Word keywords (like `next`) match as substrings, but symbol-only keywords (like `+` or `>`) match exactly, so buttons with text like `C++` or `5 > 3` still capture rage clicks. |
| `ignore_text_selection`<br/><br/>**Type:** Boolean<br/>**Default:** `false` (`true` when `defaults: '2026-05-30'` or later) | Excludes text-editing surfaces from rage click detection. When enabled, rapid clicks on `textarea`, text-like `input` fields (text, search, email, password, url, tel, number), and `contenteditable` elements won't trigger rage clicks, since users typically double or triple-click to select text. Clicks on `checkbox`, `button`, and `submit` inputs still capture rage clicks normally. |
