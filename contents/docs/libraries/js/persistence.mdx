...
PostHog can adopt a shared identity during any of these checks. A persistence write can record the adoption without immediately reloading Feature Flags. When PostHog next processes the adopted identity, such as before a capture, it clears identity-bound Feature Flag state and starts a reload. [`onFeatureFlags`](/docs/libraries/js/usage#ensuring-flags-are-loaded-before-usage) callbacks run after the reload completes.
