...
If you're using [PostHog JS](/docs/libraries/js) on the frontend, configure [`tracing_headers`](/docs/libraries/js/config#tracing-headers) for your ASP.NET Core backend hostname so browser requests include the session and distinct ID headers.
...
The .NET SDK captures identified events by default. These create [person profiles](/docs/data/persons). To set [person properties](/docs/product-analytics/person-properties) in these profiles, include them when capturing an event:
