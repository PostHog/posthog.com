...
Finally, to ensure all your requests complete you can combine PostHog's `shutdown()` method with either Vercel's [`waitUntil` helper method](https://vercel.com/docs/functions/functions-api-reference/vercel-functions-package#waituntil) or Next.js's [`after` function](https://nextjs.org/docs/app/api-reference/functions/after). This ensures your events get sent to PostHog, no background processes are left running, and the function doesn't hang forever if something goes wrong.
...
PostHog can automatically capture generations, traces, spans, and more for Vercel's AI SDK. You can find out how to install it in [the LLM analytics docs](/docs/ai-engineering/observability?tab=Vercel+AI+SDK). 
