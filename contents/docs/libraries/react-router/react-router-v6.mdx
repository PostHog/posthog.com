...
This adds the `X-POSTHOG-DISTINCT-ID`, `X-POSTHOG-SESSION-ID`, and `X-POSTHOG-WINDOW-ID` headers to requests to the configured hostnames, which you can later use on the server-side.
