...
For server-side event capture beyond error tracking, you'll use `posthog-node` directly. 
...
The `@posthog/nuxt` module initializes a server-side client for error tracking only. For general event capture in Nitro routes, create your own `posthog-node` client. The module makes your config available at `runtimeConfig.public.posthog`.
...
**`useFeatureFlagEnabled(flag)`** - Returns `true`, `false`, or `undefined`:
...
**`useFeatureFlagVariantKey(flag)`** - Returns the variant string, `true`/`false`, or `undefined`:
...
**`useFeatureFlagPayload(flag)`** - Returns any JSON value or `undefined`:
