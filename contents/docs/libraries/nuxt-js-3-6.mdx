...
See the [JavaScript SDK docs](/docs/libraries/js/usage) for all usable functions, such as:
- [Capture custom event capture, identify users, and more.](/docs/libraries/js/usage#capturing-events)
- [Feature flags including variants and payloads.](/docs/libraries/js/usage#feature-flags)
...
export default defineEventHandler(async (event) => {
  const rawCookie = getCookie(event, `ph_${runtimeConfig.public.posthogToken}_posthog`)
