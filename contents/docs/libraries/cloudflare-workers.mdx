...
Next, set up the PostHog helper to create a PostHog client. We set `flushAt` to `1` and `flushInterval` to `0` to capture data without batching as this can be delayed and lost when the worker shuts down.
...
2. We `await` flags which blocks the response until we get the flag data we need. 
