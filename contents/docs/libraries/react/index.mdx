...
<details id="slim-entrypoint">
<summary>Slim entrypoint for smaller bundles (advanced)</summary>

To reduce your bundle size, use the **slim entrypoint** (`@posthog/react/slim`) instead of the standard `@posthog/react`. The slim entrypoint doesn't bundle the full `posthog-js` library, so you can pair it with the tree-shakeable `posthog-js/dist/module.slim` bundle.

This reduces your bundle size from ~180KB to ~94KB.

```react
// src/index.js
import { PostHogProvider, usePostHog } from '@posthog/react/slim'
...
- The slim entrypoint **only** accepts a pre-initialized `client` prop. The `apiKey` prop is not available.
- Always import from the same module path throughout your app to avoid duplicate bundles.
- See the [tree shaking documentation](/docs/integrate#tree-shaking) for the full list of available extension bundles.
