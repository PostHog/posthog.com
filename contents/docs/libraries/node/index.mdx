...
If your platform exposes a `waitUntil` API, pass it when you create the client. PostHog then extends the invocation until the flush completes, so events survive the function shutting down without you blocking the response:
...
You can tune the debounce with `waitUntilDebounceMs` (default `50`), which is how long the SDK waits after the last event before flushing, and `waitUntilMaxWaitMs` (default `500`), which caps that wait so rapid captures can't starve the flush.

> **Note:** `waitUntil`, `waitUntilDebounceMs`, and `waitUntilMaxWaitMs` are experimental and subject to change in a minor release.

Otherwise, we recommend using the `captureImmediate` method instead of `capture` to ensure the event is captured before the function shuts down. It guarantees the HTTP request finishes before your function continues (or shuts down).
