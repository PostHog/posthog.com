...
Exception events are the exception: a span that fills its events and then throws would otherwise lose the only record of why it failed, so `recordException` keeps a small reserve of its own beyond the event cap.
