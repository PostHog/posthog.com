...
The Node SDK captures identified events by default. These create [person profiles](/docs/data/persons). To set [person properties](/docs/product-analytics/person-properties) in these profiles, include them when capturing an event using `$set` and `$set_once`:
...
You can also use helper methods to set or remove person properties without hand-building `$set`, `$set_once`, or `$unset` payloads. See [person properties](/docs/product-analytics/person-properties) for examples.
...
If you're using [PostHog JS](/docs/libraries/js) on the frontend, configure [`tracing_headers`](/docs/libraries/js/config#tracing-headers) for your Express backend hostname so browser requests include `X-POSTHOG-SESSION-ID` and `X-POSTHOG-DISTINCT-ID`, which the Express middleware reads automatically.

## Add request context and error capture to Fastify

Use Fastify hooks with [`withContext`](#contexts) and `captureException` to capture request errors. Process-level `enableExceptionAutocapture` does not capture errors that Fastify handles.

This example uses Fastify 5 and `posthog-node` 5.52.1. Register the hooks and error handler in the same Fastify scope as your routes, before those routes.
...
`request.user` is application-owned, not a built-in Fastify identity. Populate it with a verified user and a string `id` in an earlier authentication hook. Without authentication, this example captures personless exceptions. If authentication runs later, pass the verified ID directly to `captureException` instead of `undefined`.
...
The HTTP status filter is optional application logic, not an SDK default. Remove the `if` condition to capture handled 4xx errors too. Unknown-route 404 responses do not pass through `setErrorHandler`. If you already have an error handler, add the capture call there instead of replacing your response behavior. Avoid capturing the same error again in another hook.

The example records the route pattern, not the raw URL, to avoid collecting query strings or path parameter values. Review error messages and other properties for sensitive data before capture. Fastify's default error response includes the error message, so use your application's response policy to hide internal details.

During graceful termination, call and await `app.close()` from your application's shutdown handler. Fastify then runs `onClose`, which waits for PostHog to drain buffered events. Do not call `shutdown()` per request or exit the process before it finishes. Abrupt termination can lose queued events.
...
<CalloutBox icon="IconInfo" title="Edge runtimes">
...
You can capture LLM usage and performance data by combining the `posthog-node` and `@posthog/ai` libraries. These work with LLM providers like OpenAI and Vercel's AI SDKs. Learn more in our [AI Observability docs](/docs/ai-observability).
