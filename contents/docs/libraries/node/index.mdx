...
Tracing is off until you set the `traces` option. No OpenTelemetry dependency is required.
...
If the callback throws or rejects, the span records the exception, its status is set to `error`, and your original error is rethrown unchanged.
...
| `recordException(error)` | Attach an exception event and set status to `error` |
...
Spans created inside a PostHog request context automatically carry `posthogDistinctId` and `sessionId` attributes, which is what makes a trace reachable from a person or a session replay. Use the [Express middleware](#add-request-context-to-express) or [`withContext`](#contexts):
