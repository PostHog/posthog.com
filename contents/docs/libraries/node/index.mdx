...
`shutdown()` exports spans that have already ended. Spans still open at that point are discarded, so end your spans before shutting down — `withSpan` does this for you.
