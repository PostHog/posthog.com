...
Your **backend** mints that token. The SDK never signs one itself. Sign an HS256 JWT with your project's **Feature flags secure API key** (Settings → Feature flags), with the user's `distinct_id` as `sub`, the `appId` your provider receives as `app_id`, `aud` set to `posthog:push_identity`, and a short `exp`. Then supply it through `pushIdentityProvider`:
...
- Sign with the **Feature flags secure API key** (Settings → Feature flags) specifically, not a scoped Project secret API key. Registration verifies the token against that key only, so a token signed with any other key is rejected with a 401.
