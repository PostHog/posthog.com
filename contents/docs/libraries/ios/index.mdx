...
```swift
PostHogSDK.shared.capture("signed_up", properties: ["plan": "Pro++", "$set": ["user_property_name": "your_value"] ])
```
...
```swift
PostHogSDK.shared.capture("signed_up", properties: ["plan": "Pro++", "$set_once": ["user_property_name": "your_value"] ])
```
