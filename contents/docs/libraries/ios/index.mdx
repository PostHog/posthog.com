...
// for $autocapture events, capture text input changes and edits
config.autocaptureConfig.captureTextEdits = true 
// for $autocapture events, capture gestures such as swipes and taps
config.autocaptureConfig.captureGestures = true
// for $autocapture events, capture control actions such as button presses
config.autocaptureConfig.captureControlActions = true
// for $autocapture events, capture control values or text (title for UIButton, text for UITextField, etc.)
...
> **Important:** Please note that `captureScreenViews` configuration is intented for `UIKit` use. For `SwiftUI` screen captures, please see the section below.
