...
| `bootstrap`<br/><br/>**Type:** `PostHogBootstrapConfig`<br/>**Default:** `nil` | Seeds identity (`distinctId`, `isIdentifiedId`) and feature-flag state (`featureFlags`, `featureFlagPayloads`) before the first `/flags` response. Bootstrapped identity applies to the first session; flags act as a base layer until `/flags` loads. See [bootstrapping](/docs/feature-flags/bootstrapping#behavior-on-mobile-sdks). |
