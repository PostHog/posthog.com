...
import AutocaptureConfig from './_snippets/autocapture.mdx'
...
Method swizzling is a technique that allows the SDK to intercept and modify method calls at runtime to provide advanced features like screen view tracking, element interactions, session replay, surveys, and more.

You can configure method swizzling through the `PostHogConfig` object with these options:
