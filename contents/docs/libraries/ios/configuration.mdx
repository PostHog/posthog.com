...
Setting this to `1` will send events immediately and will use more battery. This is set to `20` by default. You can also configure the flush interval with `flushIntervalSeconds` (default `30`), after which queued events are sent regardless of how many have been gathered.
