...
| `bootstrap`<br/><br/>**Type:** `PostHogBootstrapConfig`<br/>**Default:** `nil` | Seeds identity (`distinctId`, `isIdentifiedId`) and feature-flag state (`featureFlags`, `featureFlagPayloads`) before the first `/flags` response. Bootstrapped identity applies to the first session; only enabled flags are served, until the first `/flags` response replaces them. See [SDK bootstrapping](/docs/libraries/bootstrapping#behavior-on-mobile-sdks). |
