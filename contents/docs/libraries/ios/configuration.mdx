...
Deprecated aliases and no-op options still exist for backward compatibility, but aren't recommended for new code. Use `projectToken` instead of `apiKey`, `evaluationContexts` instead of `evaluationEnvironments`, `setBeforeSend` instead of `propertiesSanitizer`, and omit `remoteConfig` because remote config is always enabled.
