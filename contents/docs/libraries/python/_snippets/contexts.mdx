...
If you're using the PostHog JavaScript Web SDK on your frontend, it generates a session ID for you. Configure [`tracing_headers`](/docs/libraries/js/config#tracing-headers) for your backend hostname to add the session and distinct ID headers to browser requests automatically.
