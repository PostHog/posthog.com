...
The python SDK provides a way for you to capture events, query feature flags and track errors in your python backends or apps.
...
To capture [anonymous events](/docs/data/anonymous-vs-identified-events) without person profiles, set the event's `$process_person_profile` property to `False`, and pass an explicit distinct ID:
...
Contexts will also automatically capture exceptions, unless configured not to.
...
See our [Django docs](/docs/libraries/django) for how to set up PostHog in Django. Our Django library also includes a [contexts middleware](/docs/libraries/django#django-contexts-middleware) that can automatically capture distinct IDs, session IDs, and other properties you can set up with tags.
