...
### GeoIP Property evaluation

If your feature flag depends on geoIP properties, like the country or city, you can ask us to automatically override these properties based on the current IP address.

> *Note:* This would be the IP address of your server machine, unless you pass in the appropriate `HTTP_X_FORWARDED_FOR` (first priority) or `REMOTE_ADDR` (second priority) headers.
...
To request this IP property resolution, either initialise your SDK with `disable_geoip=False` to make this the default for all requests, or for a single specific request you can do:

```python
posthog.get_feature_flag("beta-feature", "distinct_id", disable_geoip=False)
```
