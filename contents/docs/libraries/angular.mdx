...
To use PostHog with Angular SSR (Server-Side Rendering), you need to:

1. Update the PostHog web JS client to only initialize on the client-side.
2. Initialize PostHog Node in the server-side code.
...
app.get('**', async (req, res, next) => {
  ...
  const client = new PostHog(
      'phc_w0xVqLhImmQgphdKDrz896233CEWfbp9O5ypCxaAfw0',
