...
1. **Check the browser console** for errors related to PostHog initialization
2. **Verify your project token and host** are correct in your environment
3. **Confirm the plugin is running client-side only** – the `mode: 'client'` setting in `nuxt.config.js` is required
4. **Check for ad blockers** which may block requests to PostHog – consider [setting up a reverse proxy](/docs/advanced/proxy)

<AskAIInput placeholder="Describe your Nuxt.js 2 integration issue..." />
