...
| `logs`<br/><br/>**Type:** Object<br/>**Default:** `undefined`                                       | Logs configuration. Accepts `serviceName`, `environment`, `serviceVersion`, `resourceAttributes`, `flushIntervalMs`, `maxBufferSize`, `rateCap`, and `beforeSend`. See the [logs installation docs](/docs/logs/installation/react-native) for details. |
...
Send structured log records from your React Native app to PostHog using `posthog.captureLog()` or the convenience methods on `posthog.logger`.

```react-native
// Using the logger convenience methods
...
// Using captureLog for full control
posthog.captureLog({
...
Configure logs when initializing PostHog:
...
Logs are automatically linked to the current user (`distinct_id`) and session (`session_id`), so they appear alongside [Session Replay](/docs/session-replay) without extra setup.

To flush logs manually:

```react-native
await posthog.flushLogs()
```

For the full setup guide and configuration options, see the [React Native logs installation docs](/docs/logs/installation/react-native).
