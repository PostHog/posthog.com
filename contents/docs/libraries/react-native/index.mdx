...
Capture structured log records from your React Native app to [PostHog Logs](/docs/logs). Records are buffered in memory, persisted to disk (so they survive app restarts), batched into OTLP payloads, and flushed automatically.

To set up, follow the [React Native logs installation guide](/docs/logs/installation/react-native).

### Capturing logs

Use the per-level `posthog.logger` for the common case:

```react-native
posthog.logger.trace(body, attributes?)
posthog.logger.debug(body, attributes?)
posthog.logger.info(body, attributes?)
posthog.logger.warn(body, attributes?)
posthog.logger.error(body, attributes?)
posthog.logger.fatal(body, attributes?)
```

Or `posthog.captureLog` for custom severity, attributes, and W3C trace context:
...
Each record is automatically tagged at capture time with the user's distinct ID, session ID, current screen name, app foreground/background state, and active feature flags. These ride alongside your custom attributes on the OTLP record.

### Configuration

The `logs` block on `PostHogOptions` accepts:
...
Records flush automatically on a timer, AppState change, buffer fill, or `posthog.shutdown()`. For an immediate drain – e.g. before navigating away from a critical screen, or in a custom crash handler:

```react-native
await posthog.flushLogs()
```

`posthog.flush()` drains the events queue only; `posthog.shutdown()` drains both.
