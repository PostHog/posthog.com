...
Events such as `Application Installed` and `Application Updated` that require previously persisted data weren't migrated, that was an oversight, the side effect is that you'll see much higher numbers for `Application Installed` events for a while.
