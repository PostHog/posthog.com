...
posthog.getFeatureFlagPayload('my-flag') // { color: 'blue' }
```

`updateFlags` replaces the stored flags by default; pass `{ merge: true }` to merge into the existing set instead. Values persist across app restarts, and `getFeatureFlag()` / `getFeatureFlagPayload()` read them back like any other flag.
