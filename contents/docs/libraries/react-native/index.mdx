...
On Android, `nativeCrashes` covers Java/Kotlin crashes. To also capture native C/C++ (NDK) crashes on Android 12 (API 31) or later, enable `errorTracking.autocapture.androidNdkCrashes` (requires `posthog-react-native` 4.78.0 and `@posthog/react-native-plugin` 2.11.0 or later):
