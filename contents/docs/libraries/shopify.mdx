...
We recommend _only_ using a Shopify Pixel for tracking conversions. If using a Pixel for other events (like `collection_viewed` or `product_viewed`), you may see CORS errors when viewing those webpages. While it doesn't affect the functionality of the Pixel, everything else can be done using posthog-js installed globally on your site.
