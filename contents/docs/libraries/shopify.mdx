...
- **The token is for a different project.** The capture endpoint accepts any valid project token. If you use the token of another project, your requests succeed, but the events go to that project. Compare the token in `theme.liquid` with the token in the ["Project variables" section of your project settings](https://us.posthog.com/settings/project#variables).
