...
Once initialized in `instrumentation-client.js|ts`, import `posthog` from `posthog-js` anywhere and call whatever you need on the `posthog` object.
...
Before changing to using `history_change` to capture pageviews, we suggested using a `useEffect` hook to capture pageviews. This is because it's the simplest way to accurately capture pageviews. Other approaches include:
