...
### Flushing events in serverless functions

Server-side functions in Next.js are short-lived, and can shut down before queued events are sent. On platforms that expose a `waitUntil` API, pass it to the client and PostHog extends the invocation until the flush finishes, so you don't have to block the response on `shutdown()`:
...
> **Note:** `waitUntil` is experimental and may change in a minor release. Without it, set `flushAt` to `1` and `flushInterval` to `0`, and `await posthog.shutdown()` before the function returns.
