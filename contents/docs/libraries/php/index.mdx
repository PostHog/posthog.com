...
This is an optional library you can install if you're working with PHP. It uses an internal queue to batch requests and flushes at the end of the request, and optionally does so in an async manner.
...
| `verify_batch_events_request`<br/><br/>**Type:** Boolean<br/>**Default:** `true` | Whether to verify succussful delivery of batch events (true, synchronous), or fire and forget (false, asynchronous), with the `lib_curl` consumer |
