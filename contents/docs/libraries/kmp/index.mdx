...
Follow the [Kotlin Multiplatform error tracking guide](/docs/error-tracking/installation/kmp) for setup and verification. Upload [target-specific debug symbols](/docs/error-tracking/upload-source-maps/kmp) to get readable stack traces from minified and native builds.
