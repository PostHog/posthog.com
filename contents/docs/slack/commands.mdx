...
Every configuration command above is also available as a `/posthog` slash command – `/posthog rules list`, `/posthog project <id>`, and so on. Bare `/posthog` does the same as `/posthog help`. Slash commands do configuration only: to start a task, mention `@PostHog` or [DM the bot](/docs/slack#dm-the-bot). There is no slash command for task creation.

Two commands work on one surface only:
