...
Type the repo into the rule itself, as both examples do. A slash command has nowhere to post a repo picker, so `/posthog rules add "frontend bug"` on its own is refused with a reminder to add `org/repo`.
