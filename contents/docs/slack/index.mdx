...
Any model available in [PostHog Desktop](/desktop) works, and you can request a reasoning effort the same way ("do this with max effort"). To change your default, set a personal preference in the [App Home tab](#the-app-home-tab) under **AI model**, or set a project- or user-level default in PostHog under **Settings → Environment → Task agents**. The task agent defaults apply everywhere: Slack, the task composer, and PostHog Desktop.

The precedence is: model named in the message → your personal Slack preference → your PostHog **My default model** → the **Project default model** → the built-in default.
...
Images are the fastest way to show the agent something you can't describe in words. Paste a screenshot of a broken page, a design, or a chart, then tell the agent what to do with it:
