...
What happens next depends on the report and on a threshold you control, the **auto-create-PR threshold**:

- If the report is actionable and its priority is at or above your threshold, an agent opens a **pull request** for it. The change is built in a sandbox and comes back with your CI and code review attached, like a teammate's PR.
...
The threshold is set per person, with a team default, so you decide how much the loop does on its own and how much waits for you. Since [pricing](/docs/start-here/pricing) is per pull request, the threshold also controls spend.
