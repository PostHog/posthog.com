...
- **Only the trace ID joins the two products.** `$ai_span_id` identifies a generation or a span inside LLM analytics. It is not an OpenTelemetry span ID, and it does not match `posthog.trace_spans.span_id`.
