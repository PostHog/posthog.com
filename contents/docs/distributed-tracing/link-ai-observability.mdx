---
title: Link traces to AI Observability
---

If your product sends OpenTelemetry spans to [Distributed tracing](/docs/distributed-tracing) and LLM events to [AI Observability](/docs/ai-observability), you can correlate the two. One request then shows both the application work and the model calls that the request made.
...
- [AI Observability](/docs/ai-observability/installation) sends `$ai_generation` events from the same service.
...
| AI Observability | `$ai_trace_id` | Any string that you choose |
...
- **Only the trace ID joins the two products.** `$ai_span_id` identifies a generation or a span inside AI Observability. It is not an OpenTelemetry span ID, and it does not match `posthog.trace_spans.span_id`.
...
- **[Traces](/docs/ai-observability/traces)** – how AI Observability groups generations and spans into a trace
