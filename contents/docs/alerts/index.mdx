...
9. Select how often you want the alert to be checked:
   ...
   Automatic hourly checks are scheduled between 2 and 13 minutes after the hour, and 15-minute checks between 1 and 3 minutes after each quarter hour.
   Daily, weekly, and monthly checks run at a consistent minute within their scheduled hour.
   Each alert keeps a consistent minute to spread evaluations out.
   If you select a specific minute for an hourly alert, it keeps that minute.
   The alert form shows the next planned evaluation; quiet hours and skipped weekends can move it later.
...
- If a scheduled check falls during quiet hours, it runs after the window ends. Automatic schedules can add up to 59 minutes to spread checks out, while staying outside blocked windows.
