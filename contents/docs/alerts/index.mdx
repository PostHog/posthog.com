...
A noisy alert doesn't have to mean opening PostHog on your phone. Snoozing silences an alert until a time you pick, then checks resume on their own.
...
Alerts delivered to Slack carry a **Snooze…** dropdown on the message itself, so you can snooze one without leaving Slack.

1. Open the **Snooze…** dropdown on the alert message.
2. Pick **For 1 hour**, **For 6 hours**, **For 1 day**, or **For 1 week**. To set an exact time instead, pick **Pick a date & time…**, then choose a date and time up to 31 days out.

The alert stops firing until the snooze expires, the Slack message updates in place to confirm it, and the change is recorded in the [activity log](/docs/settings/activity-logs) under your PostHog account. Snoozing needs access to the insight and membership of its project, so a Slack user with no matching PostHog account can't snooze anything.
