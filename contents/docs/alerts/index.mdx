...
Alerts are supported on [trends](/docs/product-analytics/trends) and [SQL (HogQL)](/docs/sql) insights.
...
PostHog AI can only create alerts on [trends](/docs/product-analytics/trends) and [SQL (HogQL)](/docs/sql) insights. Other insight types like funnels and retention don't support alerts.
...
Anomaly detection alerts also support [breakdowns](#alerts-on-breakdowns). When your insight has a breakdown, the detector monitors each breakdown value independently and fires on the first anomaly detected.
...
When your insight has breakdowns, the simulation displays results for each breakdown value (up to 25) in a scrollable list of mini-charts, with aggregated statistics at the top.
...
Both threshold and anomaly detection alerts support breakdowns. When you set an alert on a trend with a breakdown, the alert fires when any of the breakdown values triggers the alert condition.

For **threshold alerts**, each breakdown value is checked against the thresholds you've set. For **anomaly detection alerts**, each breakdown value (up to 25) is monitored independently by the detector, and the alert fires on the first anomaly detected. Breakdown values with insufficient data for the detector are skipped.
