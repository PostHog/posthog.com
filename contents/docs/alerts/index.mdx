...
1. In the **CDP Destinations** section, select **Slack** or **Webhook** from the dropdown.
...
For more complex notification setups, like customizing Slack message formatting with blocks, use the advanced CDP destinations:

1. Save the alert first, then open the **Advanced options** section.
2. Click **New notification** in the **Notification destinations** area.
3. Select a destination template (like Slack or webhook).
4. Configure the destination with custom options like message blocks.
5. Click **Create & enable** to save the destination.
...
<ProductScreenshot
    ...
    alt="Advanced CDP destination configuration" 
    classes="rounded"
/>
