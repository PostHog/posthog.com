...
On the configuration page, enter a **Webhook URL**. This is the HTTP endpoint that will receive the event data from PostHog. By default, it sends a `POST` request with a templated JSON body.
...
You should see event data flow into the webhook URL. 
