...
- **Don't `fetch` back to PostHog with your project token:** To prevent event-forwarding loops, `fetch()` calls that target `posthog.com` (or a subdomain) with your project's API token in the URL or body are blocked. Use the `postHogCapture()` helper to capture events back into your project from Hog code. For modifying or enriching incoming events, use a [transformation](/docs/cdp/transformations).
