...
1. A Service Account with the required BigQuery permissions.
...
- **Service account impersonation** – You provide a service account email and project ID. No key file is needed. PostHog impersonates your service account using its own credentials, so no long-lived secrets are exchanged or stored.
- **JSON key file** – You upload a service account JSON key file containing long-lived credentials.

### Service account permissions

Regardless of which authentication method you choose, your service account needs the following BigQuery permissions:
...
2. Create a role which has only the specific permissions the batch export requires:
...
3. Grant the Service Account access to run jobs in your Google Cloud project. This can be done by granting the `BigQuery Jobs User` role or the role we created in the previous step on your project.
...
4. Create a dataset within a BigQuery project (ours is called `BatchExports`, but any name will do).
5. Use the Sharing and Add Principal buttons to grant access to your dataset with your Service Account created in step 1. Next, assign either the `BigQuery Data Editor` role or your custom role created in step 2 to provide permissions for the dataset access. Read the full instructions on [granting access to the dataset in BigQuery](https://cloud.google.com/bigquery/docs/control-access-to-resources-iam#grant_access_to_a_dataset) if unclear.
...
With this method, you don't need to create or share a key file. Instead, you grant PostHog permission to impersonate your service account.

1. Add `posthog:{organization_id}` to your service account's **description** in Google Cloud. Replace `{organization_id}` with your PostHog organization ID, which you can find in your [PostHog organization settings](https://app.posthog.com/settings/organization). This is required for PostHog to verify ownership of the service account.

2. Grant PostHog's service account the **Service Account Token Creator** role (`roles/iam.serviceAccountTokenCreator`) on your service account. PostHog's service account email is:

   ```
   posthog-batch-exports@posthog-external.iam.gserviceaccount.com
   ```

   This allows PostHog to impersonate your service account.

3. Grant the `iam.serviceAccounts.get` permission to PostHog's service account on your service account. This allows PostHog to verify ownership by reading the description you set in step 1.

4. All done! When creating the batch export, select **Impersonate service account** and provide your service account email and project ID.

### Option 2: JSON key file

With this method, you create and upload a JSON key file for your service account.

1. Create a [key](https://cloud.google.com/iam/docs/keys-create-delete#creating) for the Service Account.
...
3. All done! When creating the batch export, select **Upload service account JSON key file** and upload the key file.
...
The remaining fields depend on your authentication method:
