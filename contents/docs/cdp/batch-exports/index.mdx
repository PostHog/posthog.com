...
After you start a backfill, PostHog first estimates the total number of records to export and validates the date range. You'll see a notification with the row estimate and an option to cancel. If no data matches the selected time range, you'll get a warning that the backfill will complete with nothing to export.

The backfills table includes a **Total rows** column showing how many records are being exported:

- For in-progress backfills, this shows an estimated count (prefixed with `~`) that may change as the backfill progresses.
- For completed backfills, this shows the final count of exported rows.
