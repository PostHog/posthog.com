...
The **History** tab shows a log of configuration changes to your batch export. Each entry shows who made the change, when, and what was modified.

Changes display human-readable descriptions. Schedule changes show before and after values in plain language – for example, "changed the schedule from hourly to daily at 3am (US/Pacific)". Other changes show field-level before and after values.

To view history, click the **History** tab when viewing your batch export.

For more on activity logs across PostHog, see [Activity logs](/docs/settings/activity-logs).
