...
## Sessions model <span class="bg-gray-accent-light dark:bg-gray-accent-dark text-gray font-semibold align-middle text-sm p-1 rounded">Beta</span>
...
A [session](/docs/data/sessions) in PostHog represents a series of events that make up a single use of your product or visit to your website. The sessions model is immutable, with each session uniquely identified by its `session_id`.

If you want to link events with sessions, you'll need to ensure you're capturing the `$session_id` in your events. This is done automatically by our JavaScript Web library and mobile SDKs. For server-side SDKs, refer to our [docs](https://posthog.com/docs/data/sessions#server-sdks-and-sessions).
