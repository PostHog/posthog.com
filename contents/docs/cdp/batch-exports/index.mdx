...
- [S3](/docs/cdp/batch-exports/s3) (available as **AWS S3** and **S3-compatible** in the destination picker)
...
If no data is found for a particular batch period, then the batch export run will immediately succeed with a "Completed" status. No data to export is not considered a failure, as it is expected that there may be times of low or no volume of events coming into PostHog, and it is particularly relevant when heavily filtering the batch export.
...
### Why can't I delete my integration?

If an integration is in use by a batch export, you can't delete it. Deleting an integration while it's connected to a batch export would put the batch export in a broken state.

To delete the integration, first either:

- Delete the batch export using it
- Update the batch export to use a different integration
