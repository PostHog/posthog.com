...
<CalloutBox icon="IconInfo" title="New integration-based connections for Snowflake" type="fyi">

Snowflake connections are now backed by reusable integrations, which store your credentials securely and let you reuse them across multiple exports. This is the preferred way to set up new exports going forward.
...
5. Set up a connection by clicking **Configure new Snowflake connection** in the **Connection** dropdown. This opens a setup modal where you provide a name and your credentials. Credentials are stored encrypted and can be reused across multiple exports.
6. Fill in the remaining [configuration details](#snowflake-configuration).
7. Click **Create** to finalize.
8. Done! The batch export will schedule its first run on the start of the next period.

## Authentication

When configuring a Snowflake connection, PostHog supports two authentication methods:

1. **Key pair (default):** For enhanced security, you can use a key pair to authenticate with Snowflake. You can either paste your private key directly or upload a key file. For information on how to generate a key pair, see [Snowflake's documentation](https://docs.snowflake.com/en/user-guide/key-pair-auth#configuring-key-pair-authentication). Both encrypted and unencrypted key pairs are supported.
2. **Password:** A simpler method that requires just the password for the Snowflake user.

## Snowflake configuration

### Connection fields

These fields are configured when creating a Snowflake connection (integration):

- **Name:** A name to identify this connection across exports.
...
- **Authentication type:** The authentication method to use: key pair (default) or password.
- **Private key:** The private key for the Snowflake user, if using key pair authentication. You can paste the key directly or upload a key file.
- **Private key passphrase (optional):** The passphrase for the private key, if using an encrypted key pair.
- **Password:** The password for the Snowflake user, if using password authentication.

### Destination fields

These fields are configured for each batch export:

- **Connection:** A Snowflake integration that stores your credentials. Click **Configure new Snowflake connection** to create one. Credentials are stored encrypted and can be reused across multiple exports.
...
| snowflake_ingested_timestamp | `TIMESTAMP` | The timestamp when the event was loaded into Snowflake                    |
...
CREATE TABLE IF NOT EXISTS "{database}"."{schema}"."{table_name}" (
    ...
    "site_url" STRING,
    "timestamp" TIMESTAMP,
    "snowflake_ingested_timestamp" TIMESTAMP
)
...
|         "snowflake_ingested_timestamp" TIMESTAMP_NTZ(9)                                                           |
...
### Export shows an "incompatible types" error

This error occurs when an existing Snowflake table has one or more columns defined with types that PostHog batch exports don't support. The full error message looks like:

> A Snowflake table 'your_table' was found, but one or more fields are defined with incompatible types.

PostHog batch exports support the following Snowflake column types: `STRING`, `TEXT`, `BINARY`, `INTEGER` (and aliases like `INT`, `BIGINT`, `SMALLINT`, `TINYINT`, `BYTEINT`, `FIXED`), `FLOAT` (and aliases like `FLOAT4`, `FLOAT8`, `DOUBLE PRECISION`, `REAL`), `BOOLEAN`, `VARIANT`, `TIMESTAMP` (including `TIMESTAMP_NTZ`, `TIMESTAMP_LTZ`, `TIMESTAMP_TZ`), and `ARRAY`. Any other type (for example, `DATE`, `TIME`, `OBJECT`, or `GEOGRAPHY`) causes this error.

This error won't resolve on its own, so retrying the export won't help. To fix it, either:

1. Drop the existing table and let PostHog recreate it with the correct schema (see the [Models](#models) section above for the expected schema).
2. Manually alter the incompatible columns to use supported types.
