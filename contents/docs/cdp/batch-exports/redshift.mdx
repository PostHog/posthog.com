...
- **AWS Access Key ID:** An AWS access key ID with access to the S3 bucket. Not required when using IAM role authentication.
- **AWS Secret Access Key:** An AWS secret access key with access to the S3 bucket. Not required when using IAM role authentication.
- **AWS Role ARN:** The ARN of an IAM role that PostHog can assume to access the S3 bucket. Use this as an alternative to access keys. See [IAM role authentication for S3 staging](#iam-role-authentication-for-s3-staging) for setup details.
...
### IAM role authentication for S3 staging

As an alternative to access keys, you can authenticate PostHog to your S3 staging bucket using IAM role assumption. With this approach, you provide an IAM role ARN instead of static credentials, and PostHog assumes the role at runtime to obtain temporary credentials.

During setup, PostHog provides an external ID that you must include as a condition in your role's trust policy. This ensures only PostHog can assume the role.

The IAM role must have the following permissions on your S3 staging bucket:

- `s3:PutObject` — upload staging files
- `s3:GetObject` — read manifest files
- `s3:DeleteObject` — clean up staging files after the COPY completes
- `s3:AbortMultipartUpload` — clean up failed uploads

For full details on setting up IAM role authentication, see the [S3 batch export documentation](/docs/cdp/batch-exports/s3).
