...
In addition to syncing data on a schedule, you can enable live queries on a synced ClickHouse source. This lets you query your ClickHouse database directly from the [SQL editor](https://app.posthog.com/sql) — queries run against your live database in real time without duplicating data into PostHog.

### Enabling live queries on a new source

When creating a new ClickHouse source, the source wizard offers three query modes:
...
Once enabled, you can query your ClickHouse source directly from the SQL editor.
...
- **From the SQL editor:** Open the [SQL editor](https://app.posthog.com/sql), click the connection picker in the toolbar, and select your ClickHouse source. Synced sources with live queries enabled are labeled with `· synced` (e.g., `my_source (ClickHouse · synced)`) to distinguish them from direct-only connections.
...
If your sync fails with an error containing "is not supported for conversion into Arrow data format," it means one of the selected columns has a type that ClickHouse can't serialize to Arrow format. This most commonly happens with `AggregateFunction(...)` state columns on aggregating materialized views.
...
**Solution:** [Deselect the offending column](#selecting-columns) in the schema's column settings, or sync a view that finalizes the aggregate (for example, using `finalizeAggregation()`), then resync.
