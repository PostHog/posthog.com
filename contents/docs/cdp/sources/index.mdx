...
If your replication key field allows null values, rows where that field is null won't be synced. PostHog shows a warning when you select a nullable field as your replication key.
...
Like incremental syncing, rows where the replication key field is null won't be synced.
