...
xmin does not capture deletes. If you need delete tracking, use CDC instead. See the [Postgres source documentation](/docs/cdp/sources/postgres#sync-methods) for more details.
...
Filters are applied as a `WHERE` clause on the source query, so only matching rows are fetched and synced. Row filters are not available for tables using [CDC sync](/docs/cdp/sources/postgres#how-posthog-cdc-works).
...
Row filters let you sync only the rows you need from SQL data warehouse sources. This reduces sync time and storage costs.

Row filters are available for Postgres, MySQL, MSSQL, Snowflake, BigQuery, and Redshift sources.

### Adding row filters

To add row filters during source setup:

1. In the table picker, click **Columns and row filters** next to a table
2. In the **Row filters** section, click **Add filter**
3. Select a column, operator, and value
4. Add more filters as needed – all filters are ANDed together
5. Click **Save**

To add row filters to an existing source:

1. Go to the [sources tab](https://app.posthog.com/data-management/sources) and click your source
2. Click **Configure** next to a schema
3. Under **Row filters**, add your filter conditions
4. Click **Save**

### Supported operators

Row filters support the following operators:
...
| `NOT IN` | Matches no value in a list |

For `IN` and `NOT IN`, enter the value as a comma-separated list – for example, `1, 2, 3` for an integer column or `'active', 'trialing'` for a string column.

### Supported column types

Row filters work with columns of the following types:
...
| Timestamp | `2026-01-01T12:00:00` (ISO format) |

The value you enter is validated against the column's type. For example, you can't filter an integer column with a text value like `banana`.

### Behavior

- **Filters are ANDed** – A row must match all conditions to sync.
- **Applied on next sync** – Filters take effect on the next sync. They don't remove rows that were already synced.
- **Maximum 20 filters per schema** – You can add up to 20 filter conditions per table.
- **Not available for CDC** – Row filters apply to snapshot-style syncs (full table, incremental, and append-only). They don't apply to tables synced with change data capture (CDC).
