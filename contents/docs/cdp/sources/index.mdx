...
Provide your credentials and PostHog handles the entire extract and load process. By default, data syncs automatically from your source to PostHog's storage (the **warehouse** access method).

Some managed sources also support a **direct query** access method, where PostHog queries your database directly without syncing any data. This is currently available for [PostgreSQL](/docs/cdp/sources/postgres) sources. See the [PostgreSQL docs](/docs/cdp/sources/postgres) for details on choosing an access method.
...
> **Note:** Sync settings only apply to sources using the warehouse access method. Sources using direct query don't sync data – they always query your database live.
