...
On some sources and tables, you can choose the sync method. The options are incremental, append only, full table, or webhook (for supported sources).
...
Webhook sync receives data in real-time as changes happen in the source system, rather than waiting for the next scheduled sync. This provides faster data freshness compared to scheduled syncs. Webhook sync is only available for certain sources like Stripe and requires the initial sync to be complete before it activates.
