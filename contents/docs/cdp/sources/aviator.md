...
You need an Aviator account and a user access token. The token inherits your account's repository access, so no additional scopes are required.

## Adding a data source

<SourceSetupIntro />

Aviator authenticates with a user access token (it starts with `av_uat_`). Create one from your [Aviator account settings](https://www.aviator.co/) and paste it into the **API token** field.
