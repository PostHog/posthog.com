...
The Custom REST source lets you connect any REST API to PostHog's Data Warehouse without waiting for a dedicated integration. You configure the connection using the source builder in the PostHog UI – setting the base URL, authentication, pagination, and the tables (endpoints) you want to sync – and PostHog handles the rest.
...
3. Configure your source using the builder (see [Configuration reference](#configuration-reference) below):
   ...
   - Add one or more **Tables** – each table maps an API endpoint to a table in your Data Warehouse
...
5. Configure sync settings (sync method, frequency) for each table, then start syncing.

## Configuration reference

### Base URL

The root URL that all table paths are resolved against (e.g. `https://api.example.com/v1`). On PostHog Cloud, all URLs must use `https://`.
...
Each table maps an API endpoint to a table in your Data Warehouse. You can add up to 50 tables per source.

Click **Add table** to add a new table. Each table has the following fields:

| Field                | Description                                                                                               |
| -------------------- | --------------------------------------------------------------------------------------------------------- |
| **Table name**       | The table name in PostHog. Must be unique within the source.                                              |
...
Select a **Paginator** type for each table. This tells PostHog how to fetch all pages of data from the endpoint.
...
Enable **Incremental sync** on a table to only fetch new or updated records on subsequent syncs instead of re-fetching everything.
...
3. Add a table named `users` with path `/users` and **Cursor in JSON body** pagination (cursor JSONPath: `meta.next_cursor`, cursor query param: `cursor`)
4. Add a second table named `orders` with the same pagination. Enable **Incremental sync** with cursor JSONPath `updated_at` and cursor query param `updated_since`
...
3. Add a table named `contacts` with path `/contacts`, records JSONPath `results`, and **Offset / limit query params** pagination (page size: `200`)
...
- **No tables** – You need at least one table
- **Duplicate table names** – Each table name must be unique
