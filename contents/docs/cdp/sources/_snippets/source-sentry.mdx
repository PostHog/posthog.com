...
You need an **Auth Token** to connect Sentry to PostHog.

1. In Sentry, go to your account settings and create a new token.
2. Grant the token the following scopes to sync all datasets:
   - `alerts:read`
   - `event:read`
   - `member:read`
   - `org:read`
...
For more details, see Sentry's [Authentication docs](https://docs.sentry.io/api/auth/).
