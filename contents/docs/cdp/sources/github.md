...
<CalloutBox icon="IconWarning" title="Some tables aren't backfilled" type="caution">

The `workflow_runs`, `workflow_jobs`, and `reviews` tables are **webhook-only**. Only data that arrives **after** you connect the source and configure the webhook is synced. Historical data isn't backfilled. Make sure you complete the webhook setup before you need to start capturing this activity.

</CalloutBox>

## Webhooks

The `workflow_runs`, `workflow_jobs`, `reviews`, `deployments`, and `deployment_statuses` tables require a webhook to receive data. PostHog creates one webhook per repository, all sharing the same signing secret. If automatic creation fails, your token needs webhook permissions – the **admin:repo_hook** scope on a classic token, or **Repository webhooks: read and write** on a fine-grained token – or you can set the webhooks up manually.
...
6. Under **Which events would you like to trigger this webhook?**, choose **Let me select individual events** and tick **Deployments**, **Deployment statuses**, **Workflow jobs**, **Workflow runs**, and **Pull request reviews**
7. Click **Add webhook**
