...
The `workflow_runs`, `workflow_jobs`, and `reviews` tables require a webhook to receive data. PostHog attempts to create the webhook automatically. If automatic creation fails, your token needs webhook permissions – the **admin:repo_hook** scope on a classic token, or **Repository webhooks: read and write** on a fine-grained token – or you can set the webhook up manually:
