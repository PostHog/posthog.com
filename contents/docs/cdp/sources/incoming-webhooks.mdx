...
### Viewing logs and retrying failed requests

The **Activity** section in your webhook source shows logs for each received request, including:

- **Status** - success, failure, or running
- **Request details** - the received payload
- **Source event** - link to view the original event

If a request fails, retry it from the logs:

1. Find the failed request in the Activity section
2. Click the menu (three dots) on that row
3. Select **Retry event**

To retry multiple failed requests at once:

1. Select **Select for retry** from any row's menu
2. Check the boxes for all requests you want to retry
