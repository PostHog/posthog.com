...
> NOTE: This is current in beta and is not publicly available.
...
# Recoridng specific configuration
...
```swift
    ...
    let config = PostHogRecorderConfig(
        ...
        networkRecordingEnabled: true,
        // Automatic redaction. None will 
        // -> .none = only redact views with an accessibilityId matching what is specified in `redactionTags`. 
