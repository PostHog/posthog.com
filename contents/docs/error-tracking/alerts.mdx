...
> **Beta:** Issue spiking alerts require the `ERROR_TRACKING_SPIKE_ALERTING` [feature flag](/docs/feature-flags) to be enabled.

Alert when an issue starts occurring more frequently than usual to catch problems before they impact more users.

To set this up, go to [error tracking's configuration page](https://app.posthog.com/error_tracking/configuration?tab=error-tracking-alerting#selectedSetting=error-tracking-alerting) and click **Alerting**. Click **New notification**, select **Issue spiking** as the trigger, choose your destination, and configure the alert.

The notification includes:
- The issue name and description
- The number of exceptions in the last 5 minutes
- How much higher than baseline (e.g., "3x over baseline")
