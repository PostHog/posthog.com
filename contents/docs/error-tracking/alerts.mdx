...
To stay on top of issues, you can set up alerts. These enable you to post to Slack, Discord, Teams, or an HTTP Webhook when an issue is created, reopened, or spiking.
...
You can also alert when an issue is spiking – meaning it's receiving significantly more exceptions than its baseline. This helps you catch sudden increases in error rates.

To set up a spike alert, go to [error tracking's configuration page](https://app.posthog.com/error_tracking/configuration?tab=error-tracking-alerting#selectedSetting=error-tracking-alerting), click **Alerting**, then **New notification**. Select **Issue spiking** as the trigger. You can configure alerts for Slack, Discord, or Microsoft Teams.

Spike alerts show:
- The exception count in the last five minutes
- How many times over baseline the current rate is
- A link to view the issue in PostHog
...
See the [spike detection guide](/docs/error-tracking/spikes) to learn how it works and how to configure it.
