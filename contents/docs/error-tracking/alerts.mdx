...
PostHog can also alert you when an existing issue suddenly spikes in volume — for example, after a bad deploy. This works differently from issue-created alerts: instead of triggering when a new issue is first seen, spike alerts fire when an issue's error rate in the current 5-minute window significantly exceeds its historical baseline.
