...
You can configure spike detection in the [error tracking configuration page](https://app.posthog.com/error_tracking?activeTab=configuration#selectedSetting=error-tracking-spike-detection).
...
## Viewing recent spikes

Detected spike events are stored and displayed in the **Recent Spikes** table on the [error tracking configuration page](https://app.posthog.com/error_tracking?activeTab=configuration#selectedSetting=error-tracking-spike-detection). This gives you a historical view of when spikes occurred across your issues.

The table shows the following for each spike event:

| Column | Description |
| --- | --- |
| **Issue** | The issue that spiked. |
| **Detected at** | When the spike was detected. |
| **Baseline** | The computed baseline value at the time of detection. |
| **Bucket value** | The current bucket value that triggered the spike. |
