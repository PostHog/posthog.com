...
You can group exceptions as a single issue based on their properties using custom grouping rules. As with [auto assignment rules](/docs/error-tracking/assigning-issues#automatic-issue-assignment) and [suppression rules](/docs/error-tracking/capture#suppressing-exceptions), these rules run server-side during ingestion after symbol resolution. This means you have access to properties like `$exception_sources` and `$exception_functions` with unminified file names and function names.
