...
While PostHog will make a best effort to offer basic grouping, the grouping logic may not fit every use case. For example, consider the following ways you may want to group exceptions into an issue:
...
You can also group exceptions as a single issue based on their properties using custom grouping rules. As with [auto assignment rules](#auto-assignment-rules), you have access to properties of the unminified stack trace because the rules run during ingestion. 
