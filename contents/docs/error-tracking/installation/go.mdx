...
The Go SDK resolves stack traces in-process, so captured frames include file names, line numbers, and function names without any symbol uploads. To also get the full inline call chain and source context (the surrounding lines of code in the error tracking UI), [upload debug symbols](/docs/error-tracking/upload-source-maps/go). That needs posthog-go 1.22.0 or later.
