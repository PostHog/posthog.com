...
The .NET SDK supports manual exception capture with `CaptureException`. It builds the `$exception` event for you, including exception type, message, stack frames, inner exceptions, aggregate exceptions, source context, and .NET runtime metadata.
...
You can also attach feature flag values to the exception event:
