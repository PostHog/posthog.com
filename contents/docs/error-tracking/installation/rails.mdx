...
> **Tip:** Use `Rails.application.credentials` to avoid hardcoding API keys. First, add your keys:
>
> ```bash
...
> ```
>
> Then reference them in your initializer:
>
> ```ruby file=config/initializers/posthog.rb
...
|--------|---------|--------|---------|
| `auto_capture_exceptions` | `false` | All exceptions are automatically captured and sent to PostHog | No automatic error tracking — you must manually call `PostHog.capture_exception` |
| `report_rescued_exceptions` | `false` | Capture exceptions that Rails rescues and shows error pages for (404s, 500s, etc.) | Only capture truly unhandled exceptions that crash your app |
