...
export default defineNuxtConfig({
  ...
  posthogConfig: {
    ...
    clientConfig: {
      capture_exceptions: true, // this enabled automatic exception capture on the client-side (Vue)
    },
    serverConfig: {
      enableExceptionAutocapture: true, // this enabled automatic exception capture on the server-side (Nitro)
    },
    sourcemaps: {
      enabled: true,
...
On the server side, you have to spawn `PostHog` yourself.
