...
Before proceeding, confirm that you can capture events using `$posthog.capture('test_event')` in your Vue components:
...
<Step title="Manually capture exceptions (optional)" subtitle="Learn how to manually capture exceptions when needed.">

The module automatically captures uncaught exceptions, but you can also manually capture errors:
...
```js file=server/api/example.js
export default defineEventHandler(async (event) => {
  try {
    const results = await DB.query.users.findMany()
    return results
  } catch (error) {
    // The module automatically captures server-side exceptions
    // But you can also manually capture if needed
    throw error
  }
})
```
