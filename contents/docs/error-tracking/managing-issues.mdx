...
Error tracking issues can also be discovered from the [Logs](https://us.posthog.com/logs) viewer. When [logs are linked to sessions](/docs/logs/link-session-replay), related errors can be viewed in the **Related errors** tab of any log entry. This is useful for investigating backend issues where both logs and errors were captured during the same session.
