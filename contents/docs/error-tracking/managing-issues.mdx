...
Error tracking issues can also be discovered from the [Logs](/docs/logs) viewer. When you [link your logs to sessions](/docs/logs/link-session-replay), you can view related errors in the **Related errors** tab of any log entry. This helps you investigate backend issues where both logs and errors were captured during the same session.
