...
Bypass rules exempt specific exceptions from both the project-wide and per-issue rate limits. Exceptions matching a bypass rule are always ingested, even when rate limits would otherwise drop them. This is useful for critical errors that must never be lost.

<CalloutBox icon="IconWarning" title="Bypass rules affect billing" type="caution">

Since bypassed exceptions are always ingested, broad bypass rules can cause unbounded volume during error spikes. Keep filters scoped to the specific errors that must never be dropped.

</CalloutBox>

Every bypass rule must include at least one filter condition — empty match-all rules are rejected. If you want to disable rate limiting entirely, remove the rate limit instead of creating a catch-all bypass rule.

You can configure bypass rules in the [Error Tracking configuration page](https://app.posthog.com/error_tracking/configuration#selectedSetting=error-tracking-rate-limits) or through the [PostHog MCP server](/docs/error-tracking/debug-errors-mcp).
