...
PostHog error tracking allows you to track and monitor errors and exceptions in your code by capturing [exception events](/docs/error-tracking/issues-and-exceptions). This can be done automatically when exceptions are thrown in your code, or manually by calling the exception capture method.

## Capturing exceptions

Exceptions are captured as special [events](/docs/data/events) in PostHog. Similar to any other event, they can be customized, filtered, and used in insights for analysis.

To help group exceptions into issues and help you debug them, PostHog will also automatically capture the following properties:
...
When capturing exceptions manually, passing `properties` to the capture exception method will add additional properties to the captured event. This is just like any other event in PostHog.
...
You can still override the [fingerprint](/docs/error-tracking/fingerprints) to [group](/docs/error-tracking/issues-and-exceptions#how-are-issues-grouped) exceptions together at capture time.
...
### Customize issue grouping rules

Other than grouping with [custom fingerprints](#overriding-exception-properties), you can also set custom properties on the exception to help you group exceptions together using [issue grouping rules](/docs/error-tracking/grouping-issues). 

For example:

- Setting fields like `db_transaction` to group exceptions together for a specific database transactions.
- Setting `feature_name` to group exceptions together for a specific feature. 
...
Note that grouping issues means that all exceptions will be grouped together under a **single issue**, and you **cannot** group exceptions into multiple issues. You can also still [filter on custom properties](/docs/error-tracking/monitoring#finding-specific-issues) across multiple issues without grouping them.
...
- A great way to understand the effectiveness of fixes is to expose release version numbers as custom properties.
