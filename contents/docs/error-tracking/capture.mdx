...
- how many recent steps to keep in memory
...
When the queue is full, SDKs should drop the oldest step first. At capture time, PostHog attaches the most recent in-order steps that fit within the configured limits and the final event size.
...
- If you expect to see the exception in a replay or session timeline, make sure the exception itself is linked to the correct user or session.
