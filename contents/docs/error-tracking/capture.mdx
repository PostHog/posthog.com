...
You can suppress exceptions using the SDK's `before_send` hook to prevent events from being sent. This is useful for performance reasons since it stops events before they leave the client.

import BeforeSendHook from './_snippets/before-send-hook.mdx'

<BeforeSendHook />

You can also configure suppression rules in PostHog settings.
