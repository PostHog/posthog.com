...
Burst protection and [suppression rules](#suppressing-exceptions) operate **client-side only** – they limit the rate at which exceptions are sent from individual browser or app instances. PostHog does not currently offer server-side rate limiting controls at the project or organization level.

To manage exception volume server-side, consider:

- Using the `before_send` hook to filter exceptions before they're sent
- Implementing rate limiting in your own application code before calling `captureException()`
- Configuring [suppression rules](#suppressing-exceptions) to drop specific exception patterns client-side
