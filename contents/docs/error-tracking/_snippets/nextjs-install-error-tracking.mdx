...
Note that `error.tsx` files take precendence over the Global error component. Therefore if you use `Error` components to handle uncaught exceptions at different levels of you route heirarchy and want to capture the associated exception you will need to do so manually:
