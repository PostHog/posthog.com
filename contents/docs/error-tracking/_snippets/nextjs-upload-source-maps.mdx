...
> **Using multiple config wrappers?** `withPostHogConfig` must be the outermost wrapper in your `next.config.js`. If another wrapper (like `withNextIntl` or `withSentryConfig`) wraps `withPostHogConfig`, the PostHog build plugin won't run and source maps won't upload. Put `withPostHogConfig` last so it wraps everything else: `withPostHogConfig(withNextIntl(nextConfig), { ... })`.
