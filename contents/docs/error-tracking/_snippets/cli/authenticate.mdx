...
For local development, the CLI also reads these variables from a `.env` or `.env.local` file in your current directory. Process environment variables take precedence over `.env`, which takes precedence over `.env.local`.
