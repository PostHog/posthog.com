...
For local development, use the `--env-file` flag to load credentials from a dotenv-style file instead of exporting them manually:

```bash
posthog-cli --env-file .env [CMD]
```

Process environment variables take precedence. If the required variables are already set in your environment, the file is not consulted.
