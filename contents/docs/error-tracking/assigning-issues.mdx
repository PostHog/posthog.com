...
To assign or unassign multiple issues at once, use bulk actions from the issue list.

1. Select issues using the checkboxes.
2. Click the **Assign** dropdown in the bulk actions bar to assign all selected issues to a role or user.
3. Click **Unassign** to remove assignees. This only appears when at least one selected issue has an assignee.
