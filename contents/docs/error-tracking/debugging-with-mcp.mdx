...
PostHog offers several tools and resources to support your [AI engineering](/docs/ai-engineering) and agentic workflows. 

One powerful option is the [PostHog MCP server](/docs/model-context-protocol). It exposes [function calling tools](/docs/agent-toolkit) to any MCP client, enabling AI agents to interact with PostHog's API via the MCP protocol.

## MCP + error tracking

When combining our MCP with error tracking, your AI agents can take actions based on PostHog data – which unlocks powerful, autonomous debugging capabilities.
...
<Caption> Use Claude Code to debug and fix errors with PostHog MCP </Caption>
