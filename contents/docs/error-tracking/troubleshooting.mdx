...
1. Use the [SDK health check](/docs/health-checks/sdk-health) to verify your SDKs are updated to the latest version
...
## 'Load failed' with no stack trace (Safari)

Safari emits `TypeError: Load failed` when a `fetch()` call can't complete. Because it's thrown from native code, no JavaScript stack trace is captured.
...
1. Check the **Network** tab in Safari's developer tools for failed requests
2. Look for CORS-related error messages in the console
3. Test with browser extensions disabled to rule out blocking
4. Use [session recordings](/docs/session-replay) to see what user actions triggered the error
...
Rules can become disabled if PostHog encounters a processing error while evaluating the rule during exception ingestion. When a rule is disabled, a banner appears in the [Error Tracking **Configuration** tab](https://app.posthog.com/error_tracking) showing the error message.

To fix and re-enable a disabled rule:

1. Go to your [custom grouping rules](/docs/error-tracking/grouping-issues#option-1-custom-grouping-rule) or [auto assignment rules](/docs/error-tracking/assigning-issues#automatic-issue-assignment) in the **Configuration** tab
