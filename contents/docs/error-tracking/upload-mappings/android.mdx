...
When enabled, the Gradle plugin's `uploadPostHogNativeSymbols<Variant>` task reads the variant's unstripped native libraries and uploads every one carrying debug info and a build ID, automatically after `assemble`, `install`, or `bundle`, for minified and non-minified builds alike. This covers native code you build with the NDK as well as `.so` files bundled from `jniLibs` or dependencies. You can also run the task explicitly without enabling the automatic upload:
