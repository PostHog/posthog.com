...
<CalloutBox icon="IconInfo" title="Using @rollup/plugin-typescript?" type="info">

If your build uses [`@rollup/plugin-typescript`](https://www.npmjs.com/package/@rollup/plugin-typescript), set `sourceMap: true` and `inlineSources: true` in its options so the PostHog plugin receives the source maps it needs to upload:
...
export default {
  plugins: [
    ...
    // ...other plugins, including posthog()
  ],
}
```
