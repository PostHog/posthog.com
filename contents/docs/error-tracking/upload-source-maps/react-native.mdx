...
**Upload fails with `content_hash_mismatch`**

This happens when a symbol set with the same identifier already exists but has different content. Common causes include:

1. **Rebuilding the same profile without JS changes** – Minor non-determinism in the build can produce a different content hash even when code hasn't changed. Pass `--skip-on-conflict` to the CLI to keep existing symbols.

2. **Different build profiles uploading to the same project** – Development and preview builds may conflict. Consider using separate PostHog projects for each configuration.

For manual uploads (like EAS Updates), add the flag directly:

```bash
posthog-cli hermes upload --directory dist --skip-on-conflict
```

Use `--force` instead if you want to overwrite existing symbols with the new content.
