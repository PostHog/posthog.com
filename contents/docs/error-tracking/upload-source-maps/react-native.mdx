...
To additionally upload native source files for source-code context, pass an options object instead of `true`. On Android, this bundles the C/C++ sources with the `.so` debug symbols, from `posthog-react-native` 4.78.0:
