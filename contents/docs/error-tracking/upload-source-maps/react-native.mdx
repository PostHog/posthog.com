...
If you enable [native crash autocapture](/docs/libraries/react-native#native-crash-autocapture) (`errorTracking.autocapture.nativeCrashes`, `errorTracking.autocapture.androidNdkCrashes`, or both), you also need to upload **native** debug symbols at build time so those crash reports are readable. This reuses the native PostHog SDKs' own tooling:

| Platform | Crashes | Symbols | Uploaded by |
| --- | --- | --- | --- |
| iOS | All native crashes (`nativeCrashes`) | dSYMs | posthog-ios's `upload-symbols.sh` (`posthog-cli dsym upload`) |
| Android | Java/Kotlin crashes (`nativeCrashes`) | ProGuard/R8 mapping files, for minified release builds | The [`com.posthog.android` Gradle plugin](/docs/error-tracking/upload-mappings/android), which also injects a matching map-id into the app |
| Android | C/C++ (NDK) crashes (`androidNdkCrashes`) | `.so` debug symbols | The same Gradle plugin |

<CalloutBox icon="IconInfo" title="Source-code context" type="fyi">

`includeSource` (or `POSTHOG_INCLUDE_SOURCE` on iOS) also uploads your native source files. On iOS, crash frames then show the surrounding code. On Android, it uploads your C/C++ sources with the `.so` debug symbols. Java/Kotlin crashes have no source-code context, because the ProGuard/R8 mapping upload doesn't include sources.
...
2. **Runtime** – enable [`errorTracking.autocapture.nativeCrashes`](/docs/libraries/react-native#native-crash-autocapture) (and `errorTracking.autocapture.androidNdkCrashes` for Android NDK crashes) and install `@posthog/react-native-plugin`.
...
Both platforms upload through `posthog-cli`, which must be installed and authenticated – the same setup covered in the **Download CLI** and **Authenticate** steps above. The Android `.so` debug symbol upload needs `posthog-cli` 0.7.32 or later. In CI/CD builds (such as EAS Build), set the `POSTHOG_CLI_API_KEY` and `POSTHOG_CLI_PROJECT_ID` environment variables (and `POSTHOG_CLI_HOST` for EU). See [authenticating the CLI](/docs/error-tracking/upload-source-maps/cli) for the full list.
...
When enabled, `expo prebuild` adds an iOS dSYM-upload build phase and applies the `com.posthog.android` Gradle plugin on Android. From `posthog-react-native` 4.78.0, it also enables the plugin's Android `.so` debug symbol upload for NDK crashes. If your project was prebuilt earlier with a `com.posthog:posthog-android-gradle-plugin` classpath older than 1.5.0, the next prebuild bumps it.

To additionally upload native source files, pass an options object instead of `true`. On iOS, this adds source-code context to crash frames. On Android, it bundles the C/C++ sources with the `.so` debug symbols, from `posthog-react-native` 4.78.0:
...
If you enable `androidNdkCrashes`, also upload the `.so` debug symbols so NDK crash stack traces resolve. This needs the Gradle plugin 1.5.0 or later. Enable the upload in `android/app/build.gradle`:
...
See [upload native debug symbols (NDK)](/docs/error-tracking/upload-mappings/android#upload-native-debug-symbols-ndk) for the full options, including source context.
