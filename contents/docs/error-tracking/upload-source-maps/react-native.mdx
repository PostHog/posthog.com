...
<CalloutBox icon="IconInfo" title="No automatic source map uploads on React Native web" type="action">

Automatic source map uploading is handled through the Gradle build process on Android and the Xcode build process on iOS. For React Native web, please use our [CLI](/docs/error-tracking/upload-source-maps/cli#upload) instead.

</CalloutBox>

1. Expo: Update the [app.json](https://docs.expo.dev/versions/latest/config/app/#plugins) file in your app's root directory so that it looks like this:
...
Plugins that add modifications can only be used with [prebuilding](https://docs.expo.dev/workflow/continuous-native-generation/) and managed [EAS Build](https://docs.expo.dev/build/introduction/).

If you can use the `posthog-react-native/expo` plugin, skip steps 2 and 3.
...
3. Web: Ensure that your React Native web build generates source maps.
