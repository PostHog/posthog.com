...
If you enable [native crash autocapture](/docs/libraries/react-native#native-crash-autocapture) (`errorTracking.autocapture.nativeCrashes` or `errorTracking.autocapture.androidNdkCrashes`), you also need to upload **native** debug symbols at build time so those crash reports are readable. This reuses the native PostHog SDKs' own tooling:
...
- **Android** – upload ProGuard/R8 mapping files via the official [`com.posthog.android` Gradle plugin](/docs/error-tracking/upload-mappings/android), which also injects a matching map-id into the app. For native C/C++ (NDK) crashes, the same plugin also uploads the app's `.so` debug symbols.

<CalloutBox icon="IconInfo" title="Source-code context on Android covers NDK code only" type="fyi">

Including native source files for source-code context (`includeSource` / `POSTHOG_INCLUDE_SOURCE`) is supported on iOS, and on Android for C/C++ sources uploaded with the `.so` debug symbols. The Android ProGuard/R8 mapping upload has no source-inclusion equivalent, so Java/Kotlin crashes have no source-code context.
