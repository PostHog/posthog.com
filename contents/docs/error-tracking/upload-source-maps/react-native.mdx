...
`includeSource` (or `POSTHOG_INCLUDE_SOURCE` on iOS) also uploads your native source files, so crash frames can show the surrounding code. It applies to iOS crashes and Android C/C++ (NDK) crashes. Android Java/Kotlin crashes have no source-code context, because the ProGuard/R8 mapping upload doesn't include sources.
