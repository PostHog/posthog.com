...
<CalloutBox icon="IconInfo" title="Git metadata on uploads" type="fyi">

When you upload source maps from a local build or from CI, PostHog will attach git metadata (branch, commit, repo, remote URL) to the release so symbol sets link back to the exact commit in the Error Tracking UI. This works automatically for Android and iOS; no configuration is required.
...
You must also disable User Script Sandboxing for the upload script to work. In Xcode, go to Build Settings, search for **User Script Sandboxing** (`ENABLE_USER_SCRIPT_SANDBOXING`) and set it to **No**. This is required because the script needs to read `.git/` for release metadata and execute `posthog-cli`, which are not allowed under Xcode's script sandbox. If you are using the Expo plugin (step 1), this is handled automatically.
...
The `posthog-react-native/expo` plugin accepts the following options:
...
**Upload succeeds but the symbol set has no git chip in the UI**

Releases in PostHog are keyed by `{release-name}@{release-version}`. If a release already exists for that key (created by an earlier upload before you enabled git metadata, or from a CI job with sandboxing issues), PostHog doesn't backfill metadata onto it — it just links your new upload to the existing record. To get git metadata on a new release, bump your build number (`CURRENT_PROJECT_VERSION` on iOS, `versionCode` on Android) or delete the stale release from the Error Tracking UI, then rebuild.

**`Server returned 0 upload keys (1 skipped as already present)`**

This means the CLI already uploaded a source map with the same chunk ID. Hermes bytecode is deterministic, so if your JS hasn't changed meaningfully, the chunk ID stays the same and the server deduplicates. To force a fresh upload:

1. Make a real change to your app's JavaScript (not just a whitespace change), or
2. Delete the existing symbol set in the Error Tracking UI and rerun the build
