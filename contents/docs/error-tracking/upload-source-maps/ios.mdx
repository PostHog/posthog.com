...
### Re-uploading dSYMs with different content

By default, if a symbol set with the same UUID already exists on the server, it is not overwritten. To force overwriting existing symbol sets (for example, to add source files to a previously source-less upload), set the `POSTHOG_FORCE` environment variable:

<MultiLanguage>

```bash file=Swift Package Manager
POSTHOG_FORCE=1 ${BUILD_DIR%/Build/*}/SourcePackages/checkouts/posthog-ios/build-tools/upload-symbols.sh
```

```bash file=CocoaPods
POSTHOG_FORCE=1 ${PODS_ROOT}/PostHog/build-tools/upload-symbols.sh
```

</MultiLanguage>

**Note:** When `POSTHOG_INCLUDE_SOURCE=1` is set, the `--force` flag is automatically enabled.
