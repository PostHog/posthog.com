...
Ensure your build generates source maps. This action does not build your project — it only injects source map metadata into the built files and uploads the source maps to PostHog. Run your usual production build first (for example, `npm run build`).
...
| `host` | No | PostHog host URL. For EU cloud set `https://eu.posthog.com` |
