...
By default, we capture 10 exceptions (bucket size) of the same type in a 10 second period before the rate limiter kicks in, after which, we capture 1 exception (refill rate) every 10 seconds.
