...
- (Optional) Configure real-time webhook sync to keep PostHog updated automatically
...
You'll see a three-step import process:
...
<Step title="Configure inbound webhook sync" badge="optional">

Once the initial import is complete, you can enable real-time syncing so that subscription preference changes in Customer.io automatically sync to PostHog.

Without webhook sync, PostHog's opt-out list can drift from Customer.io if users manage their email preferences directly through Customer.io (for example, via an unsubscribe link in an email).

### Get the webhook signing secret from Customer.io

In Customer.io, go to **Settings > Webhooks > Reporting webhooks** and copy the **Signing secret**.

### Configure the webhook in PostHog

1. In the import modal, go to the **Inbound webhook sync** step
2. Enter the **Webhook signing secret** you copied from Customer.io
3. Click **Enable sync**
4. Copy the **Webhook URL** displayed in PostHog

### Add the webhook URL to Customer.io

1. In Customer.io, go to **Integrations > Add integration > Reporting webhook**
2. Paste the webhook URL you copied from PostHog
3. Save the integration

### Supported events

The webhook processes the following Customer.io event types:

- **`unsubscribed`** – Sets a global opt-out for the recipient
- **`subscribed`** – Clears the global opt-out when a user re-subscribes
- **`cio_subscription_preferences_changed`** – Updates per-topic preferences when a user changes individual subscription settings

All other event types (like `email_opened` or `email_bounced`) are ignored.

### Manage webhook sync

You can enable or disable webhook sync at any time from the import modal without deleting your signing secret. To re-enable, toggle the sync back on.
