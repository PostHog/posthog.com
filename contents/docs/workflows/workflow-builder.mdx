...
Conversion goals let you track when users complete a desired action after entering the workflow. In the trigger configuration, you can optionally define conversion conditions based on [person properties](/docs/product-analytics/person-properties).

All conversion conditions must be met for a user to be considered converted. This uses AND logic, meaning every condition you define must be satisfied, not just one of them.
...
The **Wait until condition** step pauses the workflow until a specific condition is met. Configure two settings:

- **Conditions to wait for** - The condition that must be satisfied for the workflow to continue. This can be a [person property](/docs/product-analytics/person-properties) or other filter criteria.
- **Max time to wait for condition** - The maximum time to wait before the workflow continues. If the timeout is reached without the condition being satisfied, the workflow continues down the "no match" branch.
