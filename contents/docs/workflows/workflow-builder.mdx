...
|---------------------|-------------|
| Get ticket | Fetch the current ticket's data into workflow variables. Exposes properties like `ticket_id`, `ticket_number`, `status`, `priority`, and `channel_source`. |
| Update ticket | Change a ticket's status or priority. Status options: `new`, `open`, `pending`, `on_hold`, `resolved`. Priority options: `low`, `medium`, `high`. |
...
| `$conversation_ticket_status_changed` | Ticket status was updated (includes `old_status` and `new_status` properties) |
| `$conversation_ticket_priority_changed` | Ticket priority was updated (includes `old_priority` and `new_priority` properties) |
| `$conversation_ticket_assigned` | Ticket was assigned to a team member or AI |
| `$conversation_message_sent` | Team member sent a message on a ticket |
| `$conversation_message_received` | Customer sent a message on a ticket |
