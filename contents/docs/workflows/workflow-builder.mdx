...
<CalloutBox type="info" title="Duration validation">

Duration values must be positive whole numbers followed by a unit — for example `1m`, `2h`, or `3d`. The minimum value is 1 for any unit. Decimal or fractional values like `1.5h` or `2.7d` are not accepted.

You can save a draft workflow with an incomplete duration, but you must provide a valid duration for every delay and wait step before the workflow can be enabled.
