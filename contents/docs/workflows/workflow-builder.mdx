...
When you send emails through a workflow, PostHog captures engagement events as standard PostHog events. This lets you build insights, funnels, and dashboards based on how recipients interact with your emails.

To enable this, go to **Settings** > **Workflows** and turn on **Capture email engagement events**.

| Event                        | Description                                        |
| ---------------------------- | -------------------------------------------------- |
| `$messaging_email_sent`      | Email was sent successfully                        |
| `$messaging_email_failed`    | Email failed to send                               |
| `$messaging_email_delivered` | Email was delivered to the recipient's mail server |
| `$messaging_email_opened`    | Recipient opened the email                         |
| `$messaging_email_link_clicked` | Recipient clicked a link in the email           |
| `$messaging_email_bounced`   | Email bounced (hard or soft bounce)                |
...
| ------------------- | ---------------------------------------- |
| `$workflow_id`      | The ID of the workflow that sent the email |
| `$messaging_source` | Source of the event (`direct` or `ses`)  |

Depending on the event type, additional properties are included:

- `$messaging_email_sent` and `$messaging_email_failed` include `$email_to` and `$email_subject`
- `$messaging_email_link_clicked` includes `$link_url`
- Events from SES webhooks include `$email_to`
