...
| Update ticket | Change a ticket's status, priority, SLA, assignee, or tags | Status: `new`, `open`, `pending`, `on_hold`, `resolved`. Priority: `low`, `medium`, `high`. SLA: deadline (amount + unit) or `clear` to remove. Assignee: user or role (leave empty to keep current). Tags: select tags to set on the ticket (leave empty to keep current). |
...
- **Set SLA deadlines** - When a ticket is created, set a 4-hour response deadline
- **Route to team** - When a ticket is created, automatically assign it to a support role or specific team member
- **Auto-tag tickets** - When a message contains specific keywords, automatically add relevant tags for categorization
