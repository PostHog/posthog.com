...
Workflows doesn't require an SDK. Webhook triggers work without any code changes. To use event triggers, install a [PostHog SDK](/docs/getting-started/install) so events are captured and available as workflow triggers.

</CalloutBox>

### Event triggers

Event triggers fire on any [PostHog event](/docs/data/events). These can be manually captured or automatically [autocaptured](/docs/data/autocapture) by a PostHog SDK. To use event triggers, you need a PostHog SDK installed and capturing events. They can be filtered by the event properties and person properties attached to the event.
