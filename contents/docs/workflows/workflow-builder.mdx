...
|---------------------|-------------|---------------------|
| Get ticket | Fetch the current ticket's data into workflow variables | `ticket_id`, `ticket_number`, `status`, `priority`, `channel_source`, `sla_due_at` |
| Update ticket | Change a ticket's status, priority, or SLA due date | Status: `new`, `open`, `pending`, `on_hold`, `resolved`. Priority: `low`, `medium`, `high`. SLA: ISO 8601 datetime |
...
- **Set SLA on new tickets** - When a ticket is created, automatically set an SLA deadline based on priority or customer tier
