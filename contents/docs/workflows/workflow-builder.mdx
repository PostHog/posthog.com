...
Every workflow starts with a trigger. Triggers represent actions taken by users that kick off the workflow. There are several types of triggers:
...
| Manual trigger | Run workflow on-demand via a button click |
| Scheduled trigger | Run workflow at a specific future time |
| Batch trigger | Run workflow for each person matching filters |
| Tracking pixel trigger | Run workflow when a 1x1 tracking pixel is loaded |
| Conversation trigger | Support ticket events (when [Conversations](/docs/support) is enabled) |

For detailed information on each trigger type, including available data, filtering options, and use case examples, see the [Triggers documentation](/docs/workflows/triggers).
