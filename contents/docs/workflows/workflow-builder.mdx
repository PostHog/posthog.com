...
Workflows auto-save five seconds after your last edit — no manual save is required. New workflows created at `/workflows/new` are automatically saved as a draft, and the URL updates to `/workflows/{id}` after the first save.

Workflows have two states: **draft** and **enabled**.

| State | Description |
|-------|-------------|
| Draft | A work-in-progress workflow that auto-saves. You can build incrementally without needing a complete configuration. |
| Enabled | An active workflow that runs when triggered. Full validation is required before enabling. |

How saving works depends on the workflow state:

| Workflow state | Save behavior |
|----------------|---------------|
| Draft | Changes save directly to the workflow on auto-save. |
| Enabled (active) | Changes save as a separate draft. The live workflow is unaffected until you publish. |

### Editing an active workflow

When you edit an enabled workflow, your changes are saved as a draft. The live version continues running unchanged. An **Unpublished changes** banner appears to indicate a draft exists.

The header actions change to:

- **Publish** - Apply the draft to the live workflow
- **Discard changes** - Revert to the live state and delete the draft

The **Enable/Disable** toggle is locked while a draft exists. Publish or discard the draft first.

### Visual indicators

The workflow builder highlights unsaved changes with visual cues:

- **New or modified steps** - Dashed orange border with reduced opacity
- **Deleted steps** - Shown as "ghost nodes" with a red dashed border, strikethrough name, and reduced opacity. You can restore a deleted step using the revert button on the node. Ghost nodes are permanently removed on save or publish.

### Name and description changes

Edits to the workflow name or description save directly to the server two seconds after you stop typing. These are independent of the draft system.
...
| Update ticket | Change a ticket's status or priority | Status: `new`, `open`, `pending`, `on_hold`, `resolved`. Priority: `low`, `medium`, `high` |
