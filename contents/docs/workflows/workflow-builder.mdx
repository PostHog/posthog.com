...
| Update ticket | Change a ticket's status, priority, or SLA deadline | Status: `new`, `open`, `pending`, `on_hold`, `resolved`. Priority: `low`, `medium`, `high`. SLA: amount + unit (`minute`, `hour`, `day`) or `clear` to remove |
...
- **Set SLA deadlines** - When a new ticket is created, set an SLA deadline of 4 hours to ensure timely responses
