...
Avoid multiple workflows that respond to the same event and perform overlapping actions as this can cause duplicated messages or conflicting side-effects.
...
Use delay steps (e.g. “wait 2 days”) or conditional logic only when necessary. Overuse can lead to unpredictable states, e.g. if user attributes change during delay, or if event data evolves. 
...
Avoid side-effects where possible. E.g. don’t rely too heavily on property updates or event triggers inside a workflow, treat automations as eventual side-effects, not core business logic.
