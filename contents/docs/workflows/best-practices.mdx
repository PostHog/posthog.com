...
For actions that send messages or update user data: ensure that if some data is missing (e.g. user email, user property) the workflow safely skips or uses defaults rather than failing or sending bad messages. For message templates, see the [Liquid default value tip](/docs/workflows/library#liquid-and-merge-tags) for the recommended syntax.
...
Ensure that failures (e.g. email bounce, webhook error) are captured and surfaced. Recipients who experience hard bounces (permanent delivery failures) are automatically opted out of future emails, which helps maintain your sender reputation and deliverability. When facing a problem, check the “[Troubleshooting & FAQs](/docs/workflows/troubleshooting)” section for Workflows.
...
- Workflows with `on_error: abort` exit immediately
- Workflows with `on_error: continue` skip to the next step
