...
| Field                  | Description                                                                                                                                           |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Twilio account**     | The Twilio account you connected when you [configured your channels](/docs/workflows/configure-channels).                                             |
...
<CalloutBox type="fyi" title="Saving drafts">
...
Once the workflow is enabled, monitor it in the **Metrics** and **Logs** tabs.

### Workflow summary metrics

The **Metrics** tab shows summary metrics at the top, plus trend charts showing workflow activity over time.

| Metric | Description |
|--------|-------------|
| In progress | Workflow runs currently in progress |
| Started | Total workflow runs started in the selected time period |
| Emails sent | Total emails sent by this workflow |
| Completed | Workflow runs that finished successfully |

### Email metrics

For workflows with email dispatches, the **Metrics** tab also shows detailed email metrics:
...
| Link clicked | Links clicked in emails |
| Bounced | Emails that bounced |
| Blocked | Emails blocked by recipient servers |
| Marked as spam | Emails marked as spam |

If your workflow has multiple email dispatches, you'll also see a breakdown table showing metrics per email action (sent, delivered, opened, clicked).

### Logs

The **Logs** tab shows logs of recently triggered workflows. Check for errors here.

To confirm your workflow is delivering messages:
- Summary metrics show workflow runs starting and completing
- Email metrics show successful delivery
