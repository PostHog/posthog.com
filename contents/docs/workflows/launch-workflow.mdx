...
<CalloutBox type="fyi" title="Test sends are excluded from metrics">

Test sends triggered from this panel don't appear in the **Metrics** tab. This keeps your workflow metrics clean and ensures they reflect only production traffic.
...
- In the **Metrics** tab, you should see line charts showing the number of **succeeded** and **triggered** messages. If your workflow has a conversion goal configured, you'll also see **Converted** and **Conversion rate** cards showing how many users completed the goal. This shows production workflow activity only – test sends from the workflow builder are excluded.
