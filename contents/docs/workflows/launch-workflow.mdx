...
<CalloutBox icon="IconInfo" title="This tutorial uses event triggers" type="fyi">

This tutorial uses a `pageview` event as a workflow trigger. To use event-based triggers, you need a [PostHog SDK installed](/docs/getting-started/install) and capturing events. If you haven't installed an SDK, you can use a [webhook trigger](/docs/workflows/workflow-builder#triggers) instead.
