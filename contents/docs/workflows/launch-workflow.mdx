...
Go to [Workflows](https://app.posthog.com/workflows) and click the **+ New workflow** button. This opens a template chooser where you can browse pre-built workflow templates or start with a blank workflow.

Templates provide pre-configured workflows for common use cases like re-engaging inactive users, celebrating user milestones, sending trial ending reminders, alerting on negative survey responses, and more.

For this guide, select **Blank workflow** to build one from scratch. This opens the workflow builder:
