...
The **Metrics** tab shows an overview of your workflow's performance:

| Metric | Description |
| --- | --- |
| **Started** | Total number of workflow runs started |
| **In progress** | Total number of workflow runs currently in progress |
| **Completed** | Total number of workflow runs completed |
| **Emails sent** | Total number of emails attempted to be sent |

For email workflows, a table below the overview shows a per-email breakdown with the following columns:

| Column | Description |
| --- | --- |
| **Sent** | Total emails sent to recipients |
| **Delivered** | Successfully delivered emails (sent minus bounced and blocked) |
| **Opened** | Emails opened by recipients |
| **Clicked** | Times links in emails were clicked |

Click on an individual email node in the workflow builder to see additional per-node email metrics including **Bounced**, **Blocked**, and **Marked as spam**.

To confirm your workflow is delivering messages, look for these signs:
- In the **Metrics** tab, check that the **Started** and **Completed** counts are increasing and the email metrics table shows deliveries.
