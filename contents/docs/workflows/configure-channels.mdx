...
1. After adding your email address, PostHog shows DNS records for **SPF**, **DKIM**, and **DMARC**.
2. Copy these into your domain host (e.g. Cloudflare, GoDaddy).
3. Verification may take a few minutes to propagate for SPF and DKIM. The DMARC record will show as pending since PostHog cannot automatically verify its presence.
...
| Spam folder issues? | Ensure SPF/DKIM/DMARC records are properly configured. DMARC will show as pending in PostHog since automatic verification is not available. |
