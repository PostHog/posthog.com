...
Each sender address is a separate channel, and only a sender that exists in the project shows in the **From** dropdown of an email step in the [workflow builder](/docs/workflows/workflow-builder). To add another sender later, use the [**Channels** tab](https://app.posthog.com/workflows/channels) again. The **Settings** > **Integrations** page shows the senders that exist and lets you remove them, but you cannot add a sender there.
...
Verification applies to the domain, not to the address. If the domain is already verified for another sender in the same project, you do not need to add the DNS records again for a new address on that domain.
...
| Issue | Solution |
| --- | --- |
| No address in the **From** dropdown? | The step can only use a sender that exists in the project. Add one on the [**Channels** tab](https://app.posthog.com/workflows/channels), then select it in the step. |
