...
This is the address that recipients will see in their inbox. Each one is a separate channel, and only a sender that exists shows in the **From** dropdown of an email step in the [workflow builder](/docs/workflows/workflow-builder). To add another sender later, use the [**Channels** tab](https://app.posthog.com/workflows/channels) again.
...
<CalloutBox icon="IconInfo" title="Push notifications are in open beta" type="fyi">

Push is in open beta. It's available in every project, so you don't need to request access. Each send counts toward your workflows destinations usage and quota, the same as a webhook or other destination step. To send feedback, [reach out in-app](https://app.posthog.com/#panel=support%3Afeedback%3A%3Alow%3Atrue).
</CalloutBox>
