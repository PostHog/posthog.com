...
## No address in the email step's `From` dropdown?

An email step can only send from a sender that exists in the project. Add one in [**Settings** > **Integrations**](https://app.posthog.com/settings/environment-integrations) or on the workflows [**Channels** tab](https://app.posthog.com/workflows/channels), then select it in the step. If the domain is already verified for another sender, you do not need to add the DNS records again. See [configure a channel](/docs/workflows/configure-channels).
