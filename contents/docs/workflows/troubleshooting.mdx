...
An email step can only send from a sender that exists in the project. Add one on the workflows [**Channels** tab](https://app.posthog.com/workflows/channels), then select it in the step. The **Settings** > **Integrations** page only shows and removes senders. If the domain is already verified for another sender, you do not need to add the DNS records again. See [configure a channel](/docs/workflows/configure-channels).
