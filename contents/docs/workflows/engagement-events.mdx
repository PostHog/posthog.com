...
- **Accurate engagement rates**: when tracking consent is enabled, filter `$workflows_email_sent` to `$email_tracking_enabled = true` before calculating open or click rates. Untracked sends can never produce opens or clicks, so including them in the denominator deflates your rates.
...
- `$email_tracking_enabled` — whether this send included open and click tracking (the tracking pixel and rewritten links), on `$workflows_email_sent` and `$workflows_email_failed` only. Always set, so you can filter or break down by tracking status without handling absent values.
