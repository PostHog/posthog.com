...
If you use the `posthog-react-native/expo` config plugin, `expo prebuild` writes that override for you from `posthog-react-native` TODO_4929_VERSION. Opt out with `patchMainActivityNewIntent`:
