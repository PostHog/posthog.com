...
The setup differs per SDK. Pick your platform below. To connect the FCM or APNs channel these tokens deliver through, see [Configure a workflows channel](/docs/workflows/configure-channels?tab=Push).

## Set up push notifications

<Tab.Group tabs={['iOS', 'Android', 'React Native', 'Flutter']}>
<Tab.List>
  <Tab>iOS</Tab>
  <Tab>Android</Tab>
...
- On Flutter, `pushIdentityProvider` is a Dart callback with no `Info.plist` or manifest equivalent. If your app uses `com.posthog.posthog.AUTO_INIT`, set it to `false` and call `Posthog().setup()` explicitly, otherwise the provider is never installed.
