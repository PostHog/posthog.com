...
### CDP destination and transformation management

**Prompt:** "List all my webhook destinations"

The agent will use `cdp-functions-list` to retrieve all destinations and return a summary with names, enabled status, and template used.

**Prompt:** "Create a webhook destination that sends purchase events to https://api.example.com/purchases"

The agent will use `cdp-function-templates-list` to find the webhook template, then `cdp-functions-create` to create a destination with the webhook URL and event filters configured.

**Prompt:** "Disable the Slack notification destination called 'New signups'"

The agent will use `cdp-functions-list` to find the destination by name, then `cdp-functions-partial-update` to set `enabled: false`.

**Prompt:** "What transformation templates are available?"

The agent will use `cdp-function-templates-list` filtered by type "transformation" to show available templates like GeoIP enrichment, language detection, and custom property transformations.
