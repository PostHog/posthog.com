...
**Prompt:** "Show me recent activity in my project"

The agent will use the `activity-logs-list` tool to fetch recent changes, showing who made them, what was modified, and when.

**Prompt:** "What changes were made to feature flags this week?"

The agent will filter activity logs by the `FeatureFlag` scope and return flag creations, updates, and deletions with timestamps and user details.

**Prompt:** "Who modified the checkout-flow feature flag recently?"

The agent will query activity logs for a specific resource's change history, including what fields changed and their before/after values.
