...
| `file_download`          | [On-demand file exports](/docs/cdp/batch-exports/file-download-api)                    |
...
**Prompt:** "Export last week's pageview events as a Parquet file"

The agent will use `file-download-batch-exports-create` to start an on-demand export with the events model, Parquet format, and the appropriate date range. It then polls `file-download-batch-exports-retrieve` until the export completes, and downloads the file via the REST endpoint.

**Prompt:** "Download all session data from January 1-7 as gzipped JSONLines"

The agent will create a file download export for the sessions model with JSONLines format and gzip compression, poll for completion, and save the resulting file locally.

**Prompt:** "Cancel my running export"

The agent will use `file-download-batch-exports-cancel-create` to stop an in-progress export. Only exports in `Starting` or `Running` status can be cancelled.
