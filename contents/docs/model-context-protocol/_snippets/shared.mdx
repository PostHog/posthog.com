...
**Prompt:** "Run all the insights on my KPIs dashboard and show me the results"

The agent will use the `dashboard-insights-run` tool to execute all insights on the dashboard and return their results in an LLM-friendly text table format.

**Prompt:** "Get the latest data from my marketing dashboard with a fresh cache"

The agent will use the `dashboard-insights-run` tool with `refresh` set to `blocking` to force a cache refresh and return updated results for all dashboard insights.
