...
| `customer_analytics`     | [Customer analytics usage metrics](/docs/customer-analytics)                           |
...
**Prompt:** "List all usage metrics in my project"

The agent will use the `usage-metrics-list` tool to return all usage metrics, including their name, aggregation type (count or sum), rolling time window, and display format.

**Prompt:** "Create a usage metric called 'Weekly API Calls' that counts `api_request` events over a 7-day rolling window"

The agent will use the `usage-metrics-create` tool to create a new count-based usage metric that tracks API usage for each customer in their profile.

**Prompt:** "Create a usage metric for total revenue that sums the `amount` property on `purchase` events over 30 days"

The agent will use the `usage-metrics-create` tool with `math: sum` and `math_property: amount` to create a sum-based usage metric displayed as currency.

**Prompt:** "Update my 'Feature Usage' metric to use a 14-day rolling window instead of 7 days"

The agent will use the `usage-metrics-partial-update` tool to modify the interval field on an existing metric.
