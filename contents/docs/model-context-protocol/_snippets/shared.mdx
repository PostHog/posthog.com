...
1. Create a [personal API key](https://app.posthog.com/settings/user-api-keys?preset=mcp_server) using the **MCP Server** preset. The preset selects exactly the API scopes the PostHog MCP needs — do not hand-pick scopes, as `user:read`, `group:read`, and `organization:read` are required and easy to miss. A key without these will cause every MCP call to fail with HTTP 500.
