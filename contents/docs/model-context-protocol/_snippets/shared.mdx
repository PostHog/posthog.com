...
You can limit which tools are available using the `features` and `tools` query parameters, individually or together. If neither parameter is specified, all tools are available.
...
Use the `tools` parameter to expose specific tools by name:

```text
https://mcp.posthog.com/mcp?tools=dashboard-get,execute-sql
```

### Combining features and tools

When you provide both `features` and `tools`, they combine as a union — a tool is exposed if it matches a feature category **or** its name is in the tools list. This lets you select a feature group and add individual tools from other categories:

```text
https://mcp.posthog.com/mcp?features=flags&tools=dashboard-get,execute-sql
```
...
> **Note:** Hyphens and underscores are interchangeable in feature and tool names (e.g., `error-tracking` and `error_tracking` both work). Tool names follow the format `{feature}-{action}`, like `dashboard-get`, `feature-flag-create`, or `execute-sql`. See the [Available tools](#available-tools) section for the full list.
