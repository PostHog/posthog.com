...
<<<<<<< HEAD
Add query parameters to the MCP URL to limit which tools are available. Without filtering parameters, all tools are available.

### Filter by feature category

Use the `features` parameter to expose tools from specific feature categories:
||||||| cb5ea0e4a
