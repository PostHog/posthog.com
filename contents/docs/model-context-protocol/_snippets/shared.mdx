...
The agent will use the `query-paths` tool to analyze user journeys, returning a directed graph of the events users follow after signup. You can scope paths to specific event types like `$pageview` for page navigation, `$screen` for mobile screens, or `custom_event` for custom-tracked events.

**Prompt:** "Show me the navigation paths from the pricing page to checkout"

The agent will execute a paths query with `startPoint` set to the pricing page and `endPoint` set to checkout, revealing the most common routes users take between those two pages.

**Prompt:** "Show me pageview paths but normalize all product and user URLs into single nodes"

The agent will use path cleaning filters (`localPathCleaningFilters`) with regex patterns to collapse dynamic URL segments – for example, `/product/123` and `/product/456` both become `/product/:id` – so the visualization groups them as a single step.

**Prompt:** "What custom events do users trigger after completing onboarding, excluding heartbeat events?"

The agent will run a paths query filtered to `custom_event` types with `startPoint` set to the onboarding event and `excludeEvents` set to remove noisy events like `heartbeat` from the results.
