...
**Prompt:** "Create an LLM judge evaluation that checks if my chatbot responses are helpful and on-topic"

The agent will use `evaluation-create` to set up an LLM-as-a-judge evaluation that scores your generations for helpfulness. You can configure the model provider and scoring criteria.

**Prompt:** "List all my active evaluations and show me their pass rates"

The agent will use `evaluations-get` to fetch evaluations filtered by enabled status, returning names, types, and configurations for each.
...
**Prompt:** "Compare versions 1 and 3 of my 'support-assistant' prompt to see what changed"

The agent will use the `prompt-compare` tool to return a unified diff between the two versions, showing additions and deletions along with metadata for each version.
