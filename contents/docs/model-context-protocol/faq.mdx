...
Yes. The MCP server applies its own limits before a request reaches the PostHog API: 480 requests per minute and 4,800 requests per hour. These limits count per credential – the OAuth token or personal API key your client connects with – not per team. A request over the limit gets a `429` response with a `Retry-After` header that tells you how long to wait.

Session setup has its own budget. Opening a session, announcing that your client is ready, and reading the tool catalog count separately from tool calls. A client that reopens its session for each prompt does not spend its tool call budget on that setup.

MCP tool calls also execute against the PostHog API, so the [API rate limits](/docs/api#rate-limiting) apply on top of the MCP limits. The API limits count per team, across all users and keys in your organization.
