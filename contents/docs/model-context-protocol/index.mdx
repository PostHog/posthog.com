...
Reverse proxies route events through your own domain, improving data accuracy by avoiding ad blockers and tracking prevention.

**Prompt:** "List all reverse proxies configured for my organization"

The agent will use the `proxy-list` tool to retrieve all managed reverse proxies and return their domains, CNAME targets, and provisioning status.

**Prompt:** "Create a reverse proxy for e.example.com"

The agent will use the `proxy-create` tool to set up a new managed reverse proxy and return the CNAME target you need to add to your DNS records.

**Prompt:** "Check the status of my reverse proxy"

The agent will use the `proxy-get` tool to retrieve the proxy details, including DNS verification status and any error messages.
