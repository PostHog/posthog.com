...
**Prompt:** "Show me all cohorts in my project"

The agent will use the `cohorts-list` tool to retrieve all cohorts and display them in a sortable data table showing name, type (static or dynamic), person count, and creation date.

**Prompt:** "Open the 'power-users' cohort"

The agent will use the `cohorts-retrieve` tool to fetch the cohort details and display a visual view with metadata like person count, creation date, and created by.
