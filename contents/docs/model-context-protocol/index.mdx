...
The agent will use the `prompt-duplicate` tool to copy the latest version of the source prompt to a new prompt with the specified name. The new prompt starts at version 1. This is useful when prompt names are immutable and you need to create a variation or fork for testing.
