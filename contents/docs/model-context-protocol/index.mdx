...
There's a lot more – analytics queries, feature flag and experiment management, SQL, CDP destinations, support ticket triage, and multi-step recipes that chain it all together. You can also run governed metrics from your [semantic layer](/docs/semantic-layer) (beta), so every agent session answers "what's our MRR?" with the same canonical definition.
