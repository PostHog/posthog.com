...
The [PostHog MCP server](/docs/model-context-protocol) gives your AI coding agent direct access to PostHog analytics. Query trends, funnels, retention, paths, and custom HogQL – all from your code editor.
...
| `query-paths`                        | Run a [paths query](/docs/product-analytics/paths) to analyze the most common sequences of events or pages users navigate through. Visualize user flows and identify drop-off points.              |
| `query-paths`                        | Run a [paths query](/docs/product-analytics/paths) to analyze the most common sequences of events or pages users navigate through. Visualizes user flows as a directed graph with drop-off points. |
...
- `What are the most common paths users take after signing up?`
- `Show me the navigation paths from the pricing page to checkout.`
- `Run a HogQL query to find the most active users by event count this month.`
- `What pages do users visit after the homepage?`
- `Show me navigation paths from signup to first purchase, grouped by marketing source.`
- `Analyze custom event paths after user signed up, excluding noisy events like page_ping.`
