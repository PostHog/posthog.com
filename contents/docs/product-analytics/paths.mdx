...
Here, I've set the [start point](#changing-the-start-and-end-point) to be the "/docs" and I've [limited the number of paths](#maximum-number-of-paths) to only 5.
...
The numbers next to the paths correspond to how many distinct users passed through this specific step along the way. So out of the 745 people who viewed the home page over this period, 510 of them continued on to `/pricing` and so on.
...
<ProductScreenshot
    imageLight = {threeStepLight} 
    imageDark = {threeStepDark}
    classes="rounded"
    alt= "Path visualization with only 3 steps"
/>

As you can see, we now have 3 steps in our journey, starting with a user viewing the `/pricing` page, and then continuing on to whatever page that user viewed next, with the 3rd step being wherever the user ended up.
In this case, some users navigated directly to the `/docs` or `docs/api` or `/docs/paths` page, and as a result there is no intermediate step displayed.
However for other users, such as those who first navigated to the `homepage` page, we can see a 3rd step now, corresponding to wherever they navigated after viewing `homepage`.

> **Note: ** If a user visits `homepage`, then refreshes the page, i.e. they visit `homepage` again, we count this as the same step. In other words, no two consecutive steps ever have the same event/URL.

There are 6 users who visited `homepage` as the second step, and of which 3 users contintued to the `/docs/` page. The red portion of our flow at the second step, which corresponds to the number of users (3) who did not navigate anywhere else after viewing this page, are considered to have dropped off at this step.
...
To do this, we simply click on the number next to the name of step which shows how many users arrived at this point, which will bring up a list of all of these users.

<ProductScreenshot
    imageLight = {usersLight} 
    imageDark = {usersDark}
    classes="rounded"
