...
### 4. Person profiles and user properties
...
For user identification to work, you must be capturing person profiles. This means your JavaScript Web `person_profiles` config option must be set to `always` or `identified_only`. 

If set to `identified_only`, PostHog only captures person profiles once you call one of the following functions or methods:
...
Once you create a person profile for a user distinct ID, all events for that distinct ID count as ones with person profiles, regardless of your `person_profiles` [config option](/docs/libraries/js#person-profiles-and-user-properties). 
