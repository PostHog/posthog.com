...
import IdentifyFrameworkReact from "./_snippets/identify-framework-react.mdx"
import IdentifyFrameworkNextjs from "./_snippets/identify-framework-nextjs.mdx"
import IdentifyFrameworkNuxt from "./_snippets/identify-framework-nuxt.mdx"
import IdentifyFrameworkVue from "./_snippets/identify-framework-vue.mdx"
import IdentifyFrameworkPython from "./_snippets/identify-framework-python.mdx"
import IdentifyFrameworkNode from "./_snippets/identify-framework-node.mdx"
import IdentifyFrameworkElixir from "./_snippets/identify-framework-elixir.mdx"
...
You need to identify users to know who's using your product and what they're doing. Identifying users lets you follow users across devices and sessions, track their behavior over time in [insights](/docs/product-analytics/insights), build [cohorts](/docs/data/cohorts), and build a story across PostHog products. You can follow identified users across [session replays](/docs/session-replay/start-here), insights, [errors they encounter](/docs/error-tracking/start-here), [LLM traces](/docs/ai-observability/traces), and more. It's one of the most important things to get right when setting up PostHog.

This page will cover how to identify users across SDKs. For the broader identity strategy (how to design your anonymous-to-identified flow, cross-platform consistency, and common pitfalls), see [Identity resolution](/docs/product-analytics/identity-resolution).
...
When someone first visits your site or app, PostHog automatically assigns them an anonymous ID, stored locally, and captures their events anonymously. If they've [opted into tracking](/docs/product-analytics/privacy#complete-opt-out), this anonymous identity is tracked even across sessions. At this point you don't yet know who they are.

### Identified on the frontend

As soon as you're able to, typically when a user logs in or creates an account, you call `identify` on the frontend with a `distinct_id`. That `distinct_id` is usually something stable like a UID, their email, or their database ID. This merges the anonymous person _into_ the identified person, linking the two IDs together. From then on, looking up by either ID surfaces the user's full event history, from before and after they logged in.
...
<Tab.Group tabs={['React', 'Next.js', 'Nuxt', 'Vue']}>
...
<IdentifyFrameworkReact />

</Tab.Panel>
<Tab.Panel>

<IdentifyFrameworkNextjs />

</Tab.Panel>
<Tab.Panel>

<IdentifyFrameworkNuxt />

</Tab.Panel>
<Tab.Panel>

<IdentifyFrameworkVue />
...
Backend SDKs have no concept of an anonymous session, so there's nothing to merge. Instead, pass the same `distinct_id` you use on the frontend when capturing events. Some SDKs expose an `identify` method, while others set [person properties](/docs/product-analytics/person-properties) with `$set` on a captured event.
...
Some frameworks attach the `distinct_id` for you, often via [contexts](/docs/libraries/python#contexts). The examples below are copied directly from each framework's setup guide.

<Tab.Group tabs={['Python', 'Node', 'Elixir']}>
...
<IdentifyFrameworkPython />

</Tab.Panel>
<Tab.Panel>

<IdentifyFrameworkNode />

</Tab.Panel>
<Tab.Panel>

<IdentifyFrameworkElixir />
...
When you call `identify`, the `properties` object is where you attach [**person properties**](/docs/product-analytics/person-properties) — attributes that describe who the user is rather than what they did. Used well, they're what you filter, segment, and build cohorts on.
...
You can view whether a user can be merged into another user using `alias` when [viewing their properties](/docs/product-analytics/person-properties#how-to-view-and-edit-a-person-profile-and-properties) in the PostHog app. Under their ID, you'll see `Merge restrictions`. This will indicate whether there are merge restrictions or not, i.e. whether you can use their ID as an `alias_id` or not.
...
> **Warning:** This will treat the distinct IDs as separate users for future events. However, there is no guarantee as to how past events will be treated. They may be considered separate users, or be considered a single user for some or all events.
...
When a `User B` is merged into another `User A`, all the properties of the `User B` are added to `User A`, following the [property conflict rule](#person-properties) above. For example:
...
When an anonymous user is identified as `User A`, all the properties of the anonymous user are added to `User A`, following the [property conflict rule](#person-properties) above. For example:
