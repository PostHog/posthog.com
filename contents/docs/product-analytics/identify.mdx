...
For user identification to work, you must be processing person profiles. This means your JavaScript Web `person_profiles` config option must be set to `always` or `identified_only`. 
...
Once you create a person profile for a distinct ID, all their events are captured with person profiles, regardless of your `person_profiles` [config option](/docs/libraries/js#person-profiles-and-user-properties). 
