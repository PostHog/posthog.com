...
<CalloutBox icon="IconInfo" title="About mobile autocapture events" type="fyi">

Mobile SDKs automatically capture events like `Application Opened` and `Application Backgrounded` using Title Case with spaces. This is intentional – it makes autocaptured events easy to distinguish from your custom events. When you implement your own tracking, use `snake_case` to maintain consistency across your codebase and clearly separate your custom instrumentation from autocaptured events.
...
- Person properties set with `$set` reflect the last _ingested_ value, not the last value by timestamp. Similarly, `$set_once` is claimed by the first _ingested_ event, not the earliest by timestamp.
