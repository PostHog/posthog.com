...
- **Using a tenant, account, or organization ID** as the distinct ID in a multi-tenant app. This collapses every user in that tenant into one person profile. Identify the user and pass the tenant as a [group](/docs/product-analytics/group-analytics) instead – see [multi-tenant apps](/docs/product-analytics/multi-tenant-apps).
