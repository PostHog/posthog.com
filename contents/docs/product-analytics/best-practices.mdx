...
- **Using a tenant, account, or organization ID** as the distinct ID in a multi-tenant app. This collapses every user in that tenant into one person profile, and a busy tenant hits the per-distinct-ID rate limit above long before your project would. Identify the user and pass the tenant as a [group](/docs/product-analytics/group-analytics) instead – see [multi-tenant and B2B SaaS setup](/docs/product-analytics/multi-tenant-saas).
