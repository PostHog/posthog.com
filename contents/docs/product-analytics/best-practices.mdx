...
Your `distinct_id` is how PostHog links events to a person. If you get it wrong, you'll either fragment a single user across multiple person profiles or funnel unrelated events into the same one – both of which lead to inaccurate analytics, broken funnels, and potentially increased costs and rate limiting.
...
## 6. Prefer backend to frontend tracking
...
## 7. Be aware of client-side event buffering
...
Design your analytics with this in mind. Use event timestamps for time-based analysis rather than relying on ingestion order. Avoid workflows that assume one event will always be processed before another – if ordering matters, build that logic into your application rather than depending on ingestion sequence.
...
## 9. Use the same project for your website and app
