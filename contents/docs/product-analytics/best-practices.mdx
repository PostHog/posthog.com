...
- For event names, use the `category:object_action` framework:
  ...
  - `object` is a noun that refers to the component or location the event took place in, like `forgot_password_button` or `pricing_page`.

  - `action` is a verb that describes what happened, like `click`, `submit`, or `create`.

  - Putting all of these together, you get `account_settings:forgot_password_button_click` or `signup_flow:pricing_page_view` – The order is important as it makes it easier to find a specific event within a long list.
