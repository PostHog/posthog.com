...
export const viewUserPropertiesLight =
  "https://res.cloudinary.com/dmukukwp6/video/upload/v1710055416/posthog.com/contents/images/docs/data/user-properties/view-user-properties-light-mode.mp4";
export const viewUserPropertiesDark =
  "https://res.cloudinary.com/dmukukwp6/video/upload/v1710055416/posthog.com/contents/images/docs/data/user-properties/view-user-properties-dark-mode.mp4";
export const changeDisplayNameLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/v1710055416/posthog.com/contents/images/docs/data/user-properties/change-display-name-light-mode.png";
export const changeDisplayNameDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/v1710055416/posthog.com/contents/images/docs/data/user-properties/change-display-name-dark-mode.png";
...
Similarly to how you set properties, you can remove properties from person profiles. SDKs with helper methods provide `unsetPersonProperties()`. In other SDKs, send an event with a `$unset` array of property keys:

<MultiLanguage selector="tabs">

```js-web
posthog.unsetPersonProperties('email')
```
...
The JavaScript Web and React Native SDKs also provide a dedicated `unsetPersonProperties` method as the counterpart to `setPersonProperties`. Pass it the property name (or an array of names) you want to remove, and PostHog captures a `$set` event with the matching `$unset` array.
...
posthog.unsetPersonProperties(['email', 'plan'])
```
...
You can prevent `last_seen_at` updates by setting the `$update_person_last_seen_at` _event_ property to `false`.

This is useful for backend enrichment jobs or server-side events that update [person properties](/docs/product-analytics/person-properties) but don't represent real user activity.
...
You can find a list of all person properties in the [properties](https://app.posthog.com/data-management/properties?type=person) tab in **data management**. Just change the dropdown from **event properties** to **\*person properties**.
