...
The display name is the person property value shown in the PostHog UI to represent a user. It is their person `distinct_id` by default, but can be configured in [project settings](https://app.posthog.com/settings/project-product-analytics#person-display-name).
