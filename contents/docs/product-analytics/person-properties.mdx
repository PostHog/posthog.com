...
> **Note:** `last_seen_at` updates are temporarily behind a feature flag while we resolve an issue with them interfering with data exports. To re-enable, go to your [project settings](https://us.posthog.com/settings/project) and enable **Person Last Seen At** under **Extra settings**. We're working on restoring this as a default.
