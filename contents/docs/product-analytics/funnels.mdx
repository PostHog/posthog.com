...
When aggregating by unique sessions, click the chart or linked column values to see individual sessions that converted or dropped off. Each session shows channel type, entry path, referring domain, UTM parameters, and activity metrics like pageview count. Use the search box to filter sessions by person email or name. Click **View recording** to watch the session replay.
