...
This matches only the first time a user performs a specific event type. If this first event doesn't match the filters for that funnel step, or is outside the funnel's date range, the user is excluded from the funnel entirely. This is true even if they perform the event again later with matching filters.

**Example:** You have a funnel step looking for pageviews to `/about`. A user's first-ever pageview is to `/pricing`. PostHog will only check this first event. Since it doesn't match the `/about` filter, the user will be dropped from the funnel, even if they visit `/about` later.

##### 2. First occurrence matching filters

This matches the first time a user performs an event *that also matches the filters for that funnel step*. Any previous events of the same type that don't match the filters are ignored.
...
- **Historical trends** – shows you how your conversion rate has changed over time for users who entered the funnel on a given date. Useful for understanding how changes, fixes, and new features have affected your funnel success rate.
