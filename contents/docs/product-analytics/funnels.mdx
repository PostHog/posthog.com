...
In this case, all events, as long as they're completed sequentially, will be included in the funnel.

Consider the following entirely possible example:
...
The person will be in the Chrome breakdown as having completed all three steps: `$pageview`, `user signed up`, and `user posted`. The person will also be in the Firefox breakdown, but only for the `$pageview` step because they didn't complete the `user posted` step.
...
For the example above, when step two is chosen the value will be `Firefox`. If step two is chosen, any people who only have a `$pageview` event will be excluded from the funnel.
