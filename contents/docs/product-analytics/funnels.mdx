...
You can combine multiple events or actions into a single funnel step. This lets you treat several related events as one step without creating a custom [action](/docs/data/actions).

To add multiple events to a step, click the **+** button on a funnel step and select additional events. The combination uses logical `OR` – any occurrence of the selected events satisfies the step.

This is the same inline combination available in [Trends](/docs/product-analytics/trends/overview#combine-events-inline).
