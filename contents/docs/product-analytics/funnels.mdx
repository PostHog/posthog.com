...
For every flow throughout your product, more people will start it than complete it successfully. Funnels enable you to inspect your flows and understand where the friction points are so that you can improve them.
...
- [The steps which have the highest friction and time to convert](#graph-type)
...
Select the steps you wish to include in your funnel, ideally starting with the first event/action a user will trigger in the flow. Next, add intermediate events you believe users need to go through to be successful and finish with the event you consider to be the success for this flow. It's best to start with the simplest flow and avoid using optional steps to ensure you don't filter out or skew valuable results.
...
To refine your steps, you can filter out certain types of users or event properties. To do this, click the **filter icon**  next to the step you wish to refine further and select the filter you would like to add, or set a global filter in the **Filters** section in the bottom.
...
> **Note**: Using person properties in funnels requires [capturing person profiles](/docs/data/persons). 

### Excluding steps

You can also exclude exclude peoples who completed certain events between two specific steps. To do this, add the step you'd like excluded in the **Exclusion steps** section.

Note that these people will be completely excluded from the entire funnel.
...
<ProductScreenshot
    imageLight = "https://res.cloudinary.com/dmukukwp6/image/upload/v1718354250/posthog.com/contents/funnel_conversion_types_light.png"
    imageDark = "https://res.cloudinary.com/dmukukwp6/image/upload/v1718354250/posthog.com/contents/converesion-funnels-dark_2.png"
    classes="rounded"
    alt="Overall vs relative conversion"
/>
<Caption>Overall conversion versus relative conversion</Caption>

### Breakdowns

To understand how different types of users interact with your funnel, it's helpful to breakdown results. You can breakdown events by event and person properties. To do this, click on the **+ Add breakdown** button and select the property you want to break down by.
...
When breaking down funnels by, it's possible that the same properties don't exist on every event. For example, if you want to break down by `browser` on a funnel that's contains both of frontend and backend events.
...
- **Conversion steps** – shows you where users are dropping off in your funnel
...
Click on the bars in the chart or the linked numbers in the column values of table below the funnel to view the individual people who **COMPLETED** or **DROPPED** that step of your funnel.
...
From here, you can explore the individual people further by viewing session recordings to see exactly what they were doing when they dropped off. You can also use this feature to watch a successful user go through your funnel.
...
A common seasonality to watch out for is weekends, particularly if your product is B2B. Perhaps you have a yearly sale such as "Black Friday", which increases the number of people visiting your site and increases the expectation of discounts.
