...
Next, add events users need to complete to proceed through the funnel, ending with the event you consider to be the success for this flow. It's best to start with the simplest flow and avoid using optional steps to ensure you don't filter out or skew results.
...
If you add multiple steps with the same event type, it will require the user to **trigger that event multiple times**. Think of funnels steps as actions to be performed, not filters to be applied.
