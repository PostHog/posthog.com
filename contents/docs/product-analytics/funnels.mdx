...
You can also break down funnels by a [cohort](/docs/data/cohorts). This splits your funnel into two groups: users in the cohort and users not in it – useful for comparing how a specific group moves through your funnel versus everyone else.

To add a cohort breakdown, click **+ Add breakdown**, select **Cohorts**, and choose your cohort. The funnel shows results for both the cohort (e.g., "Power Users") and a "Not in Power Users" group.

You can only use one cohort at a time. To compare multiple cohorts, use the global filter to filter your funnel to each cohort separately.
