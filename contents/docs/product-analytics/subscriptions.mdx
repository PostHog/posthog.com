...
AI summaries automatically include any [annotations](/docs/data/annotations) that fall within the delivery's date range. This helps the AI explain metric changes by referencing events your team has already recorded – like deployments, marketing campaigns, or incidents.

For example, if your weekly subscription shows a traffic spike and an annotation marks "Launched new homepage" that week, the AI can connect the spike to the launch instead of describing the change without context.

Annotations are included automatically when they match the date range – no configuration required.
