...
You can enforce your event schemas at ingestion time by rejecting events that don't match the expected structure. When enforcement is enabled, PostHog validates incoming events against the schema's required properties and their types. Events that are missing required properties or have wrong property types are dropped before ingestion.

To enable schema enforcement:

1. Go to **Data Management** > **Events**
2. Select an event that has at least one property group attached
3. In the **Schema** section, check **Reject invalid events**

The **Reject invalid events** checkbox is disabled until at least one property group is attached to the event.

### How validation works

PostHog checks each required property defined in the event's property groups. Validation passes if:

- The property is present in the event
- The property value matches (or can be coerced to) the expected type

The supported property types and their accepted values are:

| Type | Accepted values |
|------|----------------|
| **String** | Any value |
| **Numeric** | Numbers, numeric strings (e.g., `"42"`) |
| **Boolean** | `true`/`false`, strings `"true"`/`"false"` |
| **DateTime** | ISO 8601 date strings, Unix timestamps |
| **Object** | Objects and arrays |

### Rejected events

When an event fails schema validation, PostHog drops it and logs an [ingestion warning](/docs/data/ingestion-warnings) of type `schema_validation_failed`. The warning includes the event name, the distinct ID, and details about which properties failed validation and why (missing or type mismatch).

You can monitor rejected events in **Data Management** > **Ingestion Warnings**.

<CalloutBox icon="IconWarning" title="Enable enforcement carefully" type="caution">

Enabling enforcement on high-volume events can cause data loss if your instrumentation doesn't match the schema. Start with lower-volume events and monitor [ingestion warnings](/docs/data/ingestion-warnings) before rolling out enforcement more broadly.
...
- **Enable enforcement gradually** - Start with lower-volume events and monitor [ingestion warnings](/docs/data/ingestion-warnings) before enabling enforcement on high-traffic events
