...
Once you subscribe to group analytics, billing applies to **all identified events across every project in your organization**, not just events with group properties attached. This is because group analytics enables infrastructure that processes all identified events to support group-level analysis.
...
- The add-on is subscribed per organization, so it applies to every project in that organization. You can't limit it to a single project, even if only one of your projects defines group types.
