...
**People and groups** are connected by events, not by a membership list. Because of this, one person can be connected to more than one group of the same type. See [how people relate to groups](#how-people-relate-to-groups).
...
PostHog doesn't store group membership on the person or on the individual group. The connection lives on the event: each event records the individual groups it belongs to, and PostHog builds the relationship from those events.

```mermaid
flowchart LR
    P["<strong>Person</strong><br/>jane@example.com"]
    E1["<strong>Event</strong><br/>document_created"]
    E2["<strong>Event</strong><br/>document_created"]
    G1["<strong>Group</strong><br/>company: Acme Corp"]
    G2["<strong>Group</strong><br/>company: Tech Solutions Inc."]
...
- An event links to a maximum of one individual group per group type, as the section above shows.
- A person links to as many individual groups as their events point to, including more than one group of the same type. A consultant who works with two companies is an example. Neither company is the primary or current one, because there is no membership list to hold that.

To record this, set the group the user works in, then set it again when the user changes:
