...
To add a breakdown, click on `+ Add breakdown` in the group filters section and search for your desired property. We support a huge range of breakdown properties:
...
| Organization properties| Properties stored on an account / organization profile (requires [group analytics](/docs/product-analytics/group-analytics)) |
| Cohorts                | Events performed by a pre-defined group/list of users                                   |
| HogQL                  | Custom breakdown using SQL (see our [advanced breakdowns tutorial](/tutorials/hogql-breakdowns)) |

Once you've selected a property, you should see your graph split into multiple lines, each one corresponding to a distinct value of that property.
...
Breakdowns only support a single property at a time, unless you create a breakdown using HogQL. Read [Using HogQL for advanced breakdowns](/tutorials/hogql-breakdowns) for more on using HogQL in breakdowns.
...
When you create a break down using a cohort, an 'Add cohort' button will appear where you can add more cohorts to your breakdown.
...
If your property can take the values `20, 21, 22, ..., 40` and you split it up into `10` bins, it will break your events up into ranges of `20 - 22`, `22 - 24`, etc.
