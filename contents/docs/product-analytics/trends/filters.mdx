...
Trends support two different [types of filters](#types-of-filters):

1. **Inline filters**, which only apply to a specific series in your insight.
2. **Filter groups**, which are global filters that apply to the whole insight.
...
Click on the settings icon to customize the filters that are used.