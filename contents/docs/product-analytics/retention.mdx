...
- Filter your retention insights using filter groups, such as event properties (country, device, etc.), person properties (job title, company name, etc.), [extended person properties](/docs/data-warehouse/join#person-joins) from data warehouse joins, feature flags (users who have a flag enabled), and existing [cohorts](/docs/data/cohorts) (pre-defined groups of users).
...
You can break down your retention numbers and graph on event, person, [extended person properties](/docs/data-warehouse/join#person-joins), group properties, and more. For example, breaking down the retention numbers by **Country Name** can help to see which countries have higher retention for the feature.
