...
Next set the time interval that aligns with the natural frequency of your product by updating the "grouped by" setting. Here were' looking at "Weekly" active as in this hypothetical scenario, we expect people to be sending messages every week, but depending on how people use your product, you might want to change the group by setting to by "Hour", "Day" or "Month". You can also set how far back you would like to consider this data over.
...
You can also click on the Option button and select "Show values on series" to dispaly values on the chart for More details: 
