...
Once values are visible, you can also enable **Show percentages on series** to see each segment's share of total activity for that time interval. For example, `580 (42%)` means that segment accounts for 42% of all activity in that band.
