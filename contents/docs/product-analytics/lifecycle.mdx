...
The lifecycle insight visualizes new, returning, resurrecting, and dormant users. You can use it with both PostHog events and [data warehouse](/docs/data-warehouse) tables.
...
Instead of a PostHog event, you can select a [data warehouse](/docs/data-warehouse) table as your lifecycle series. This enables analyzing lifecycle patterns for any entity in your data warehouse, not just PostHog users.

When you select a data warehouse table, you must configure three fields:

- **Timestamp** – A timestamp field representing when the activity occurred.
- **Created at** – A timestamp field representing when the entity was first created. This is used to determine "new" vs "returning" status.
- **Aggregation target** – A field identifying the entity you want to track (e.g., customer ID, order ID).

#### Aggregation type

The default aggregation is by user. However, you can also aggregate events by [groups](/docs/product-analytics/group-analytics) if you have any set up. When using a data warehouse table, you can also select **Custom entities** as the aggregation type for entities that can't be mapped to a person or group (such as orders or subscriptions). When using custom entity aggregation, the person modal is not available when clicking on the chart.
...
## Limitations when using data warehouse tables

When using data warehouse tables in lifecycle insights, the following features are not available:

- Test account filters
- Sampling

Property filters must use properties from the selected data warehouse table.
