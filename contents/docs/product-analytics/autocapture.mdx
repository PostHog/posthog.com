...
> **Note:** Rage click detection is a proximity-and-timing heuristic – it doesn't know whether the clicks were productive. Legitimate patterns like double-click-to-select, rapid quantity steppers, or fast pagination can trip it once they hit the third click within the window. To cut down on these false positives, PostHog ignores common navigation elements by default (via `content_ignorelist`), and you can exclude specific elements with the `.ph-no-rageclick` class or a [`css_selector_ignorelist`](/docs/libraries/js/config#configuring-rageclick). We recommend confirming rage clicks against [session replays](/docs/session-replay) before treating them as a hard frustration signal.
