...
Dead click detection automatically ignores clicks inside interactive elements like `a`, `button`, `form`, `input`, `select`, `textarea`, and `label`. Clicking an icon inside a button or a span inside a download link won't trigger a dead click, even if the inner element doesn't cause a page change.

This filtering matches how autocapture attributes clicks to interactive ancestors, reducing false positives from common UI patterns like icon buttons and wrapped links.

### Excluding elements from dead click detection

Some elements don't cause visible page changes by design – download buttons, file picker triggers, or print dialogs, for example. You can exclude these from dead click detection using the `.ph-no-deadclick` CSS class:

```html
<button class="ph-no-deadclick">Download PDF</button>
```

Elements with `.ph-no-capture` are also excluded from dead click detection by default.

The exclusion check walks the DOM tree, so adding the class to a parent element excludes all its children:

```html
<div class="ph-no-deadclick">
    <button>Download</button>  <!-- excluded -->
    <button>Print</button>     <!-- excluded -->
</div>
```

For more control, use the `css_selector_ignorelist` option:

```js-web
posthog.init('<ph_project_token>', {
    api_host: '<ph_client_api_host>',
    capture_dead_clicks: {
        css_selector_ignorelist: ['.my-download-button', '[data-no-deadclick]']
    }
})
```

Setting a custom `css_selector_ignorelist` overrides the defaults. An empty array (`[]`) disables all exclusions.
