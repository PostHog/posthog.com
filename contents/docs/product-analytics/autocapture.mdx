...
The PostHog [iOS SDK](/docs/libraries/ios) can capture the following interaction events when `captureElementInteractions` is enabled:
...
Autocaptured interaction events display as the autocaptured interaction type in the [activity tab](https://app.posthog.com/activity/explore), such as `touched ...` or `value_changed ...`.
