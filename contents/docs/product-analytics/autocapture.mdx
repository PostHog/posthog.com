...
A dead click (or slow click) is a click which isn't followed by a change to the page. They indicate that users are expecting to be able to interact with something but cannot. 

You can collect dead clicks with the Web SDK by enabling them in PostHog settings. Or by setting your config
