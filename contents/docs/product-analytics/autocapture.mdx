...
Autocaptured events (and client-side custom events) have a number of properties that are automatically added to them. These are distinguished by `$` prefix in their name, the PostHog logo next to them in the event explorer, and the verified event logo in the [data management tab](https://app.posthog.com/data-management/properties). These properties can be hidden in the event explore by checking the "Hide PostHog properties" box.
