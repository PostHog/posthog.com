...
Conversely, this means that autocapture is only enabled if neither of these options are disabled.

> Disabling autocapture **will not disable [session recording](/docs/session-replay/manual)**. You can disable session recording using `disable_session_recording` or by turning it off in your project's settings.
