...
You can also disable autocapture in your [project settings](https://app.posthog.com/project/settings), or by setting `autocapture: false` in the [config](/docs/libraries/js#config). If _one_ of these is disabled, autocapture is disabled.
...
For security reasons and to prevent sensitive data being captured, PostHog is conservative regarding `input` tags. We specifically only collect the `name`, `id`, and `class` attributes from `input` tags. If you need to collect more data from inputs, you should use [custom events](/tutorials/event-tracking-guide).
