...
export const ImgClipboardAuocaptureLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/docs/autocapture/clipboard-activity-light.png";
export const ImgClipboardAuocaptureDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/docs/autocapture/clipboard-activity-dark.png";
...
| [Navigation](/docs/product-analytics/autocapture#navigation-and-lifecycle-event-autocapture) | Captures pageviews, pageleaves, and screen views                                                                                |
...
You can disable interaction autocapture in two ways:
...
> **Note:** Disabling **interaction** autocapture does not disable **navigation** autocapture (pageviews and pageleaves). See [navigation autocapture](#navigation-and-lifecycle-event-autocapture) for how to disable those separately.
...
const MyForm = () => {
  ...
  return (
    <form data-ph-capture-attribute-name={productName}>
      <input
        name="product-name"
...
A rage click is captured when a user clicks three times in rapid succession in roughly the same spot. Specifically, PostHog fires a `$rageclick` event once it sees three clicks that are each within **30 pixels** and **1 second** of the previous one. All three thresholds (click count, pixel distance, and timeout) are configurable – see [Configuring rageclick](/docs/libraries/js/config#configuring-rageclick).

The event is captured as a `$rageclick` event. You can use it to identify opportunities to improve your UI, as it often points to where users are frustrated with your product.

<details>
<summary>Reducing rage click false positives</summary>

Rage click detection is a proximity-and-timing heuristic – it doesn't know whether the clicks were productive, so legitimate patterns like double-click-to-select in a text field, rapid quantity steppers, or fast pagination can trip it once they hit the third click within the window.

With the latest SDK [`defaults`](/docs/libraries/js/config), PostHog automatically ignores the most common culprits: navigation controls (`next`, `previous`, `>`, `<`), quantity steppers (`+`, `-`), and repeated clicks on text-selection surfaces (inputs, textareas, and `contenteditable` elements). You can tune this with [`content_ignorelist` and `ignore_text_selection`](/docs/libraries/js/config#configuring-rageclick), or exclude specific elements with the `.ph-no-rageclick` class or a `css_selector_ignorelist`.

We recommend confirming rage clicks against [session replays](/docs/session-replay) before treating them as a hard frustration signal.
...
The PostHog [iOS SDK](/docs/libraries/ios) can capture UIKit interaction events when `captureElementInteractions` is enabled. Autocaptured interaction events display as the autocaptured interaction type in the [activity tab](https://app.posthog.com/activity/explore), such as `touched ...` or `value_changed ...`.

See the [iOS autocapture documentation](/docs/libraries/ios/configuration#autocapture-configuration) for supported events, configuration options, and privacy controls.
...
You can disable `$pageview` and `$pageleave` autocapture by setting `capture_pageview: false` and `capture_pageleave: false` in the [SDK config](/docs/libraries/js/config):
...
To exclude an element from dead click capture without disabling autocapture, session replay, or heatmaps for that element, add the `.ph-no-deadclick` class:

```html
<button class="ph-no-deadclick">Repeatedly clicked control</button>
```

For custom selectors, pass `capture_dead_clicks.css_selector_ignorelist` in your JavaScript web config:

```js-web
posthog.init('<ph_project_token>', {
    api_host: '<ph_client_api_host>',
    capture_dead_clicks: {
        css_selector_ignorelist: ['.ph-no-deadclick', '.ph-no-capture', '[data-no-dead-clicks]'],
    },
})
```

Providing a custom ignore list replaces the defaults, so include `.ph-no-capture` if you still want it to suppress dead click capture.
...
Autocaptured events (and client-side custom events) have many default properties. These are distinguished by `$` prefix in their name, the PostHog logo next to them in the activity tab, and the verified event logo. You can find them in [PostHog](https://app.posthog.com/data-management/properties) or in [the references](/docs/references/posthog-js).
...
- `$device_type` is set by `posthog-js` from the browser's User Agent, so it describes the *web* client. Possible values are `Desktop`, `Mobile`, `Tablet`, `Console`, and `Wearable` — `Mobile` means a mobile **web browser**, not a native app. Native mobile app events come through the iOS, Android, React Native, and Flutter SDKs, which don't populate `$device_type` this way; distinguish them using `$lib` or `$os`.
