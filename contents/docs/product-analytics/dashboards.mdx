...
Button tiles add clickable links to your dashboard for quick navigation to related dashboards, external tools, or other areas of PostHog.

To add one:

1. Click **Add** and select **Button**.
2. Enter a URL – either an internal path (like `/dashboards`) or an external URL (like `https://example.com`).
3. Enter the button text.
4. Choose placement (left or right) and style (primary or secondary).
5. Optionally enable **Transparent background** to blend the tile with the dashboard.
6. Click **Save**.
...
<<<<<<< HEAD
Internal links open within PostHog. External links open in a new tab.
