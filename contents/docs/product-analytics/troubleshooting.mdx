...
Another common reason for missing events is that certain plugins have been configured to drop certain or all events. To debug events follow these steps in the following order:

1. Make sure the Filter Out plugin is correctly configured.
2. Try to temporarily disable the Filter Out plugin.
3. Try to temporarily disable all of the other enabled plugins.

### 3. There is a bug the library.
