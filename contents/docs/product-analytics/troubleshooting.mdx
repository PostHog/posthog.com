...
PostHog dedicates huge amounts of infrastructure to running your insight and queries. We also use a specialised database called an OLAP database (Clickhouse in our case) which is optimized for running analytics queries quickly.
...
### 2. Reduce the time period you're filtering on

A smaller time period means less data to process. You can always adjust the time period later if you need to.
...
We recommend using properties "Use person properties from the time of the event," but "Use person IDs and person properties from the time of the event" is even faster if needed. The tradeoff is that the latter mode creates inaccurate funnels and unique user counts.

You can find more details about how these mode works in our doc on [how querying works](/docs/how-posthog-works/queries#filtering-on-person-properties).
