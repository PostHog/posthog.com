---
title: Product Analytics troubleshooting
---

This page covers troubleshooting for Product Analytics and event capture. For setup, see the [installation guides](/docs/product-analytics/installation).
...
```Web
posthog.init('<ph_project_token>', { api_host: '<https://webhook.site/843c9506-b845-4ce4-af46-d04022279393>', defaults: '<ph_posthog_js_defaults>' }) // Replace with the URL you copied from webhook.site
```
...
**2. Your events go to a different project**

The capture endpoint accepts any valid project token and returns a success response. If you use the token of another project, PostHog ingests your events into that project. If your requests succeed but your project stays empty, compare the token in your [initialization code](/docs/product-analytics/installation) with the token in the ["Project variables" section of your project settings](https://app.posthog.com/settings/project#variables).

**3. PostHog is adblocked**
...
**4. An app has been configured incorrectly**
...
**5. There is a bug in the PostHog library**
...
**6. Events are not ingested after being received by PostHog**
...
#### Automated browsers are filtered

The bot filter also drops every capture when the browser looks automated. That means `navigator.webdriver` is true, or the user agent or `navigator.userAgentData.brands` contains `HeadlessChrome`. Playwright, Puppeteer, and Selenium all match by default. The confusing part is that everything else still works. The SDK loads, feature flag requests go out, and `identify()` persists to storage, but event requests never leave the page. If your end-to-end tests see no events in PostHog, check this first.

To confirm, load your page with `?__posthog_debug=true` and look for `[PostHog.js]` console lines mentioning a "likely bot". To test with an automated browser anyway, override all three signals before any page script runs:

- set a real Chrome user agent on the browser context
