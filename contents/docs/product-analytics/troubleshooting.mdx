...
#### Automated browsers are filtered too — and it looks like the SDK is working

The bot filter also drops every capture when the browser looks automated: `navigator.webdriver` is true, or the user agent or `navigator.userAgentData.brands` contains `HeadlessChrome`. This catches Playwright, Puppeteer, and Selenium runs by default. The confusing part is that everything *else* still works — the SDK loads, feature flag requests go out, `identify()` persists to storage — while event requests never leave the page. If your end-to-end tests see no events in PostHog, this is the first thing to check.

To confirm, load your page with `?__posthog_debug=true` and look for `[PostHog.js]` console lines mentioning a "likely bot". To test with an automated browser anyway, override all three signals: set a real Chrome user agent on the browser context, and override `navigator.webdriver` and `navigator.userAgentData` before any page script runs.
