...
- **PostHog branding** - Show or hide PostHog branding on the shared dashboard
- **Show insight details** - Include extra details like who created each insight and applied filters
- **Theme** - Choose between System (default), Light, or Dark mode for the shared dashboard

Copy the public link to share with anyone. This link and dashboard won't require authentication to view. 
...
Shared and embedded dashboards auto-refresh every 30 minutes while the page is open. The dashboard header shows the refresh interval. Auto-refresh pauses when the browser tab is inactive and resumes when the tab becomes visible again.
