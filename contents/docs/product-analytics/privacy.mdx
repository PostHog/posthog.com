...
You can sanitize properties on the client side by setting the `sanitize_properties` [config](/docs/libraries/js#config) option. This is a function that enables you to modify the properties before they are sent to PostHog. For example:
...
Additionally, you can set [allow or ignore lists](/docs/product-analytics/autocapture#reducing-events-with-an-allow-and-ignorelist) in your PostHog config to prevent specific elements from being captured.
