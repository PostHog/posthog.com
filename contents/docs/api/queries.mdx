...
Since the query endpoint accesses PostHog's data warehouse directly, it's important to write performant queries. We aren't able to optimize these queries for you like we would with other products. Here is some advice:
...
To do this, write your query in the [SQL editor](https://us.posthog.com/sql), click **Materialize**, then **Save and materialize**, and give it a name without spaces (I choose `mat_event_count`). You can also schedule to update the view at a specific interval.
...
Reading a large table like `events` or `persons` more than once in the same query multiplies the work ClickHouse has to do (more I/O, more CPU, more memory). For example, this query is inefficient:
