...
### Public and confidential CIMD clients

A CIMD client can never hold a client secret, because there is no registration step in which PostHog could deliver one. `token_endpoint_auth_method` therefore takes one of two values:

- `"none"` (the default if you omit it) – a public client, identified by its `client_id` and relying on PKCE.
- `"private_key_jwt"` – a confidential client. Also publish a `jwks_uri` pointing at an HTTPS URL that serves your public keys as a JWK Set, and sign a short-lived assertion ([RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523)) per request. Nothing secret is ever transmitted, and you rotate keys yourself by publishing a new one.
...
Because the document is re-read on each refresh, you can move between the two at any time by editing it. Your `client_id` does not change.

Confidential client authentication is used by the [provisioning API](/docs/integrate/provisioning#choosing-an-authentication-method), where some endpoints accept confidential clients only.
...
Open a [support request](https://us.posthog.com/#panel=support%3Asupport%3Alogin%3A%3Atrue) in the app, choosing **Authentication** as the topic, and include:
