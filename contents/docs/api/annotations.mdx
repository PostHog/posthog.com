...
| `deleted` | `boolean` | Filters annotations based on a `deleted` property. `true` will return only deleted annotations, and `false` will return only non-deleted annotations. Ommitting this parameter will return annotations of both types. | ❌ |
