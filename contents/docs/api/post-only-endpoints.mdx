...
The `historical_migration` field is optional and must be set to `true` when running imports from other systems: this will ensure that events are processed in order without triggering our spike detection systems. For realtime events, it can be left out or set to `false`.
