...
You can optionally pass in an object containing person or group properties to override server values. This is how posthog-js handles [bootstrapping](/docs/feature-flags/bootstrapping-and-local-evaluation), which can be useful if you want to make feature flag decisions without waiting for these changes to be ingested and available on PostHog's servers.
...
curl -v -L --header "Content-Type: application/json" -d '  {
    ...
    "group_properties": {"<groupProp1>":"<groupVal1>"}
