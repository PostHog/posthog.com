...
> **Note:** The `$identify` event works differently from the `identify()` method in the [JavaScript SDK](/docs/libraries/js#identifying-users). This event updates the person properties, while the JavaScript `identify()` method connects an anonymous user and a distinct ID.
...
For backward compatibility, PostHog captures events with [person profiles](/docs/data/persons) by default. To capture events without them, add the `$process_person_profile` property to the event and set it to `false`.
