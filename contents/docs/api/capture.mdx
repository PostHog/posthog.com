...
For backward compatibility, PostHog captures events with [person profiles](/docs/data/persons) by default. To remove them, add the `$process_person_profile` property to the event and set it to `false`.
