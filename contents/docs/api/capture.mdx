...
For backward compatibility, PostHog captures identified events by default. To capture anonymous events, add the `$process_person_profile` property to the event and set it to `false`.
