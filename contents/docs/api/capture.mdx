...
Put `distinct_id` at the top level of the event. PostHog also reads it from `properties.distinct_id` when the top level does not have it, and the top level wins if both are set. This applies to every event in this reference, including `$groupidentify`, `$identify`, and `$create_alias`. Use the top level unless your client can only send properties.
