...
**Meaning:** A `200: OK` response means we have successfully received the payload, it is in the correct format, and the project API key (`api_key`) is valid. It **does not** imply that events are valid and will be ingested. As mentioned under [invalid events](/docs/api/capture#invalid-events), certain event validation errors may cause an event not to be ingested.
