...
- The events values endpoint (`/api/projects/{project_id}/events/values`) has stricter rate limits with a personal API key: `60/minute` and `300/hour`. Requests must include the `event_name` parameter. For queries without event filters, use the [`query`](/docs/api/queries) endpoint instead.
