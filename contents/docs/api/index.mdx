...
When you build OAuth integrations with PostHog, the token endpoint (`/oauth/token/`) returns region metadata that helps your application determine the correct API endpoints:
...
| Field | Description |
|-------|-------------|
| `posthog_region` | The PostHog cloud region: `us` for US Cloud or `eu` for EU Cloud |
| `posthog_base_url` | The base URL for API requests (e.g., `https://us.posthog.com` or `https://eu.posthog.com`) |

Use `posthog_base_url` as the base for all API calls to ensure requests go to the correct region.

> These fields are only included for US and EU cloud deployments. Self-hosted instances don't include region metadata since they use a single, known endpoint.

The OAuth authorization server metadata endpoint (`/.well-known/oauth-authorization-server`) also includes `posthog_region` for cloud deployments. This lets clients discover the region before starting the OAuth flow.
