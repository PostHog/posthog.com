...
- Listing session recordings and fetching their snapshots have extra limits on top of the analytics limits above, and those depend on your plan, so a higher sustained limit is a plan question rather than something to work around. These limits only affect reading recordings through the API. Recordings keep being captured either way.
...
- For the `/invites` and `/invites/bulk` endpoints, you can send `50` invites per hour and `200` per day, with up to 20 invites per bulk request. This also applies to invites sent from the PostHog UI.

- Public POST-only endpoints such as event capture (`/e`, `/i/v0/e`) and feature flag evaluation (`/flags`) have no request-level rate limits. Event capture still returns `200` when your project is over its billing quota, and names the limited resources in the `quota_limited` field of the response body. It also applies a [per-distinct-ID ingestion protection](/docs/how-posthog-works/ingestion-pipeline#rate-limiting-high-volume-distinct-ids): if a single distinct ID sends a high, sustained volume of events (roughly 5,000 per minute), those events are still accepted with a `200` response but are processed without strict ordering and without person profile updates for the duration of the spike. PostHog logs an [ingestion warning](/docs/data/ingestion-warnings) when this happens.

Each limit is counted **per personal API key**. Two keys get two separate budgets, so a script that exhausts one key's limit doesn't throttle requests made with a different key. Requests that aren't authenticated with a personal API key are counted per project instead.

This means the shape of your key setup decides how your traffic shares limits. If you're building an integration that covers several projects, create a personal API key per project so their traffic doesn't pile into one budget. If you deliberately want a set of scripts to share a budget, give them the same key.

> Aside from the plan-based session recording limits above, we are not offering higher limits than these at this time, but you may wish to try our [endpoints product](/docs/endpoints), which offers query customization and higher limits. Alternatively, you may be able to use our [batch exports product](/docs/cdp/batch-exports) to pull the data that you need from our events or persons tables on a faster cadence.
...
**Meaning:** A `200: OK` response means we have successfully received the payload, it is in the correct format, and the project token (`token`) is valid. It **does not** imply that events are valid and will be ingested. As mentioned in [invalid events](/docs/api/capture#invalid-events), certain event validation errors may cause an event not to be ingested.
...
- **Project secret API keys** (`phs_`): The key is automatically rolled and project admins and owners are notified via email.
- **Team secret tokens** (legacy, also `phs_`): Project admins and owners are notified via email so they can rotate the token.
