...
2. **Private `GET`, `POST`, `PATCH`, `DELETE` endpoints** are used for [querying](/docs/api/queries), creating, updating, or deleting nearly all data in PostHog. They give the same access as if you were logged into your PostHog instance, but require authentication with a personal API key or project secret API key.
...
Project secret API keys provide project-scoped access to the API without being tied to a specific user. They are prefixed with `phs_` and have explicit scopes that control what resources they can access.

Project secret API keys are ideal for:

- Server-side integrations
- CI/CD pipelines
- Shared automation that shouldn't depend on a specific user's access

**Creating a project secret API key:**

1. Go to [your project settings](https://us.posthog.com/settings/project)
2. Navigate to the **Project Secret API Keys** section
3. Click **Create key**, add a label, and select the scopes you need
4. Copy the key (it's only shown once)

**Authenticating with a project secret API key:**
...
> **Note:** Project secret API keys have limited scopes compared to personal API keys. Check the available scopes when creating your key to ensure it has access to the endpoints you need.
...
- **Project secret API keys** (`phs_`): Project admins and owners are notified via email so they can rotate the key.
