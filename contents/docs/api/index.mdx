...
- For public POST-only endpoints like event capture (`/i/v0/e`) and feature flag evaluation (`/flags`), there are no request-level rate limits and these endpoints never return `429`. Event capture does apply a [per-distinct-ID ingestion protection](/docs/how-posthog-works/ingestion-pipeline#rate-limiting-high-volume-distinct-ids): if a single distinct ID sends a high, sustained volume of events (roughly 5,000 per minute), those events are still accepted with a `200` response but are processed without strict ordering and without person profile updates for the duration of the spike.
