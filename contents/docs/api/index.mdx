...
- For [session recording snapshot](/docs/session-replay/snapshot-api) endpoints, rate limits are **tier-based**: Free tier (`30/minute`, `150/hour`), Paid tier (`90/minute`, `500/hour`), Enterprise tier (`120/minute`, `600/hour`). See the [snapshot API docs](/docs/session-replay/snapshot-api#rate-limiting) for details.
