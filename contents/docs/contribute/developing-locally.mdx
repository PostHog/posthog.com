...
If you are using a Mac with Apple Silicon, the command above will fail due to `grpcio`. To fix it, run this instead:
...
With `grpcio` installed, you don't need to specify any flags the next time you `pip install`.

6. If you want to fully test all our features, you'll need to install a few dependencies for SAML to run properly. If you're on macOS run the command below, otherwise check out the official [xmlsec repo](https://github.com/mehcode/python-xmlsec) for more details.
