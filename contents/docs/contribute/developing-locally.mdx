...
>    ⚠️ **Important:** Do not use <code>release-*</code>  patterns in your branches unless pushing a release. These branches have get special handling by our CI for releases.
...
With [PyCharm's](https://posthog.com/handbook/engineering/beginners-guide/developer-setup#alternative-pycharm) built in support for Django, it's fairly easy to setup debugging in the backend. This is especially useful when you want to trace and debug a network request made from the client all the way back to the server. You can set breakpoints and step through code to see exactly what the backend is doing with your request.
