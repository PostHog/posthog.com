...
make it is secure to threat actors. As every installation is unique, we will provide
in our documentation some general guidelines on how to make it secure but it is then
up to each administrator to make sure all the best practices are followed correctly.

<Disclaimer />

* make sure your Kubernetes cluster is provisioned using industries best practices
