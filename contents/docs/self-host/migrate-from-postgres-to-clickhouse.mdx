...
> This doc is still in **Beta**. If you're attempting this migration, feel free to ask questions and provide feedback via the [PostHog Communty Slack workspace](/slack) or [a GitHub issue](https://github.com/PostHog/posthog.com/issues).

**PostHog backed by Postgres is now deprecated**. We will continue to provide support to Postgres backed installs but we strongly encourage you to migrate to PostHog backed by ClickHouse for [vastly superior performance](docs/self-host/postgres-vs-clickhouse), as well as to receive new features and continued support in the future.


## Requirements

- You should have a clean [ClickHouse-backed PostHog instance up and running](docs/self-host).
- Your old and new instances should both be running **the same version of PostHog**.
- Your new PostHog instance should have no ingested events. We recommend using a fresh and unused installation.
- The minimum PostHog version for migrating historical events is `1.29.0`. 
...
```shell
psql -h your-pgbouncer-host -p 6543 -d posthog -U postgres < tmp/export.sql
```
...
As a final step, let's check that our pods are all healthy. Run the following:
...
If any of your pods has a status of `Pending`, wait a while and run the command again to check if this changes.

If, after a while, any of your pods has a status other than `Running`, you should run an upgrade of the cluster, like so:

```shell
helm upgrade -f values.yaml -n posthog
```

Once everything's listed as `Running`, you can now start using your new instance as if it was your old! 

However, if you want to import your old events as well, make sure to read the following section before sending events to the new instance.

## Exporting events

PostHog Events Migration Plugin coming soon. 
