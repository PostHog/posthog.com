...
> **This page covers our free, open-source Docker compose deployment**, which is available under a MIT license [without guarantee](/docs/self-host/open-source/disclaimer). We continue to develop this version, but some premium features are only available on PostHog Cloud. We do not provide support for this "Hobby" version. 
>
> We [no longer support paid, open-source deployments](/blog/sunsetting-helm-support-posthog) and it is **no longer possible to buy licenses for self-hosted versions** - we instead recommend [migrating to PostHog Cloud](/docs/migrate/migrate-to-cloud). 
