...
<CalloutBox icon="IconStarFilled" title="[BETA] PostHog Self Host TUI" type="fyi">

We're exploring a new version for our self-hosting deployment via a TUI binary. This is a trial and will change or might be removed based on your feedback.
...
If you wanna run this programmatically, you can turn CI mode on and run it with an extra `--domain` flag to specify where you're hosting it on.
...
5k recordings, 1500 survey responses,
...
Check out the [PostHog Docker Hub repository](https://hub.docker.com/r/posthog/posthog) for a list of all available tags.
...
### Disk space and container logs

Docker log rotation is automatically configured for all containers with a maximum size of 50MB per log file and a maximum of three files per container. This prevents container logs – especially from ClickHouse – from filling up your disk.

If you're running low on disk space, check the size of your container logs:

```bash
sudo du -sh /var/lib/docker/containers/*/*-json.log
```

To manually clear container logs:
