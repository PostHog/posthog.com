...
> **In a rush?** All actions are prefixed with a light bulb emoji. Search for 💡 on this page to quickly find all listed mitigation actions.
...
Given backpressure is essentially the difference between events processed by the Django server and those processed by the plugin server, backpressure can increase if we're receiving more events on the Django end _or_ processing fewer events on the plugin server end (or both at the same time).
...
### 💡 Action: Scale the plugin server
...
> **Recap:** A given team's plugin instance is called a _plugin config_ and these are stored on the Postgres table `posthog_pluginconfig`. When looking for issues with "plugins" we are in fact looking for problematic plugin configs.  
...
Is a plugin config triggering unusually long jobs?

**Potential mitigation** 

[Disable the problematic plugin config](#-action-disable-the-problematic-plugin-config)

### 💡 Action: Disable the problematic plugin config

To disable a specific plugin config, access the Django shell and run the following:

```python
plugin_config = PluginConfig.objects.get(id=<plugin_config_id_here>)
plugin_config.enabled = False
plugin_config.save()
```

If you're unsure about how to connect to the Django shell, follow [this guide for ECS](handbook/engineering/production-access) or [this guide for Helm chart deployments](docs/self-host/deploy/troubleshooting).
