...
```
Compare the numbers of the **Helm chart version** (in the format `posthog-{major}.{minor}.{patch}` - for example, `posthog-19.15.1`) when running the commands above. If the upgrade is for a major version, **check the [upgrade notes](/docs/self-host/deploy/upgrade-notes) before moving forward**.
