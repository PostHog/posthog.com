...
```
Compare the numbers for the chart version (in the format `posthog-{major}.{minor}.{patch}` - for example, `posthog-18.2.3`) when running the commands above. If the upgrade is for a major version, **check the [upgrade notes](/docs/self-host/deploy/upgrade-notes) before moving forward**.
