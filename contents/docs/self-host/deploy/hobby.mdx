...
To upgrade PostHog, you can run `./posthog/bin/upgrade`. This will cause a few minutes of downtime. 

If `./posthog/bin/upgrade` can't be found, run `cd posthog && git pull origin master && cd ../` first.
