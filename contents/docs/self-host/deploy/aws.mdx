...
Note that AWS by default has `allowVolumeExpansion` set to `false`, in the default EKS cluster this can be changed with `kubectl patch sc gp2 -p '{"allowVolumeExpansion": true}'`, see more info in [eks user guide for storage classes](https://docs.aws.amazon.com/eks/latest/userguide/storage-classes.html).
