...
If you are not overriding the `clickhouse.serviceType` **there's nothing you need to do**. If you are overriding the `clickhouse.serviceType` to either `LoadBalancer` or `NodePort` please remind to keep your installation secure by:
