---
title: Content Security Policy tracking
sidebar: Docs
showTitle: true
---
...
```http
Content-Security-Policy: <your existing rules here>; report-uri <ph_client_api_host>/report/?token=<ph_project_token>; report-to posthog
Reporting-Endpoints: posthog="<ph_client_api_host>/report/?token=<ph_project_token>"
```
...
```http
Content-Security-Policy-Report-Only: <your initial rules here>; report-uri <ph_client_api_host>/report/?token=<ph_project_token>; report-to posthog
Reporting-Endpoints: posthog="<ph_client_api_host>/report/?token=<ph_project_token>"
```
...
app.use(
  helmet.contentSecurityPolicy({
    ...
    directives: {
      defaultSrc: ["'self'"],
      reportUri : ['<ph_client_api_host>/report/?token=<ph_project_token>'],
      reportTo  : ['posthog'],
    },
  })
);
app.use((req, res, next) => {
  res.setHeader(
    'Reporting-Endpoints',
    'posthog="<ph_client_api_host>/report/?token=<ph_project_token>"'
  );
  next();
});
```
...
Rails.application.config.content_security_policy do |policy|
  ...
  policy.report_uri  "<ph_client_api_host>/report/?token=<ph_project_token>"
...
```http
<ph_client_api_host>/report/?token=<ph_project_token>&sample_rate=0.05
```
...
```http
<ph_client_api_host>/report/?token=<ph_project_token>&v=1
```
...
```http
<ph_client_api_host>/report/?token=<ph_project_token>&distinct_id=REPLACE_THIS&session_id=REPLACE_THIS
```
