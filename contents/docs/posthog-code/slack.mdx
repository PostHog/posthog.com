...
When a task opens a PR, PostHog Code can watch CI and re-run the agent to fix failed checks. Toggle this mid-run by replying in the task thread:
...
Turning babysitting on resets the CI repetition counter, so the agent gets a fresh cap of follow-up attempts. If the task hasn't opened a PR yet, the command does nothing.

The default for new tasks comes from your [user settings](/docs/posthog-code/settings#pr-babysitting). You can override it per-task at creation time or flip the in-app toggle once a PR exists.
