...
PostHog Code tracks its own LLM spend through PostHog [AI observability](/docs/ai-observability), so you can see exactly where your tokens go. The breakdown covers spend by product, tool, and model over the last 30 days, along with tips on where to optimize.
...
<ProductScreenshot
    ...
    classes="rounded"
    alt="Token spend in PostHog Code broken down by product, tool, and model"
/>
