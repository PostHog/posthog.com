...
You can send each comment on its own and have the agent address them one at a time, or stack several up and hand them over together. To batch your comments, check **Add to review** before submitting each one, then **Send to agent** when you're ready. Adding comments to a batch works like leaving review comments on a GitHub PR: the agent picks up all of them in a single follow-up when you submit the review, instead of reacting to each one as you write it.

<ProductVideo
    videoLight="https://res.cloudinary.com/dmukukwp6/video/upload/batch_review_comments_a1ec602809.mp4"
    alt="Send review comments to the agent"
