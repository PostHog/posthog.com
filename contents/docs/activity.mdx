...
3. Switching between relative and absolute time. Click the column header menu on the **Time** column and select **Show absolute time** to see full timestamps instead of relative times like "2 minutes ago". For custom formatting, add an SQL expression column like `formatDateTime(toTimeZone($timestamp, 'America/Chicago' ), '%b %d, %H:%i:%s')`.
