...
5. **Sessions:** Shows a list of sessions with customizable columns. You can add person properties (like email or name) as columns, filter by person properties, and sort by any column. For sessions where the user hasn't been identified, the distinct ID is shown as a fallback.
