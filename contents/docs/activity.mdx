...
When debugging issues from logs, error reports, or monitoring tools, you often need to find events around a specific point in time. Use the "Jump to timestamp" shortcut to quickly navigate to events within a time window.

Press `J` then `T` to open the jump dialog, or search for "Jump to timestamp" in the [command palette](/docs/cmd-k). Enter a timestamp in ISO 8601, unix seconds/milliseconds, or MM/DD/YYYY format, select a time window, and you'll be taken directly to the filtered event list.
