...
| **SLA** | Deadline for responding to the ticket (set via [Workflows](/docs/workflows)) |
...
- **SLA** - Filter by SLA state (breached, at risk, on track)
...
SLAs help you track response time commitments. Set them via [workflow automation](/docs/support/workflows) using the **Update ticket** action.

### SLA states

Tickets with SLAs show a color-coded deadline indicator:

| State | Meaning |
|-------|---------|
| **On track** (green) | More than 1 hour remaining |
| **At risk** (orange) | Less than 1 hour remaining |
| **Breached** (red) | Deadline has passed |

### Filtering by SLA

Use the **SLA** filter dropdown to show tickets by state:

- **All SLA states** – All tickets regardless of SLA
- **Breached** – Tickets past their deadline
- **At risk** – Tickets within 1 hour of their deadline
- **On track** – Tickets with more than 1 hour remaining

### Sorting by SLA

Click the **SLA** column header to sort by deadline. Tickets closest to their deadline appear first.
