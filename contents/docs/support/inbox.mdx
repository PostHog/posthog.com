...
- **Search** by ticket number (with or without `#`, e.g. `#1234` or `1234`), customer name, email, or message content. If a search returns no results and filters are active, a button appears to rerun the search with all filters cleared.
