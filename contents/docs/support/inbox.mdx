...
| **SLA** | Due date for ticket resolution. Displays time remaining or overdue status |
...
- **SLA** - Filter by SLA status (`breached`, `at-risk`, `on-track`)
...
SLA (Service Level Agreement) tracking helps you monitor response time commitments for support tickets. When an SLA is set on a ticket, the inbox displays a countdown to the deadline.

### SLA states

Tickets with an SLA are categorized into three states:

| State | Meaning |
|-------|---------|
| **On track** | More than one hour remains before the deadline |
| **At risk** | Less than one hour remains before the deadline |
| **Breached** | The deadline has passed |

The SLA column in the ticket list shows relative time (e.g., "in 2 hours") color-coded by state – green for on track, yellow for at risk, and red for breached.

### Setting SLAs

SLAs can be set:

- Manually by updating a ticket's SLA field
- Automatically via [Workflows](/docs/workflows/workflow-builder) using the **Update ticket** action
- Programmatically through the external ticket API
...
Messages can be marked as private. Private messages:
