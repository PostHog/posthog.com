...
- **Filter** by status, priority, [channel](/docs/support/concepts/channels), SLA state, tags, or assignee (including a dynamic **Me** option that resolves to whoever is viewing)
...
Assign a ticket to a user or a role from the **Assignee** dropdown in the sidebar. Assignment is manual, with no automatic routing, and each ticket has one assignee.

When filtering by assignee, select **Me (current user)** to create views that dynamically resolve to whoever is signed in. Unlike filtering by a specific user, the **Me** filter makes saved views shareable – a "My tickets" view shows each viewer their own tickets. You can combine **Me** with other values like **Unassigned** to see tickets assigned to you or waiting to be claimed.

Add free-form tags to categorize and filter tickets, and snooze a ticket until a set time when you're waiting on something external.
...
- Use saved views to switch between common filter combinations. Filter to **Me (current user)** to create a shareable "My tickets" view that works for everyone on the team.
