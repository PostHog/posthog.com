...
| **Snoozed until**     | Datetime when the ticket automatically reopens (if snoozed) |
...
| `on_hold`  | Paused for external reason or snoozed until a specific time |
| `resolved` | Issue resolved, conversation closed                         |

## Snoozing tickets

Snoozing puts a ticket on hold until a specific date and time. When you snooze a ticket, the status automatically changes to `on_hold`. When the snooze period expires, the ticket automatically reopens with the `open` status.

To snooze a ticket:

1. Open a ticket
2. Click the **Snooze** date picker in the sidebar
3. Select a future date and time

To clear a snooze and immediately reopen the ticket, click the **Snooze** date picker and clear the value.
...
- **Snoozed** – Filter by whether tickets are currently snoozed
...
6. **Snooze tickets** that need follow-up later instead of leaving them open
