...
You can select multiple tickets and update their status in bulk from the ticket list.

Use the checkboxes in the leftmost column to select individual tickets. Hold **Shift** and click a checkbox to select a range of tickets at once. Use the checkbox in the column header to select or deselect all tickets on the current page.

Once you've selected tickets, use the **Mark as** dropdown in the toolbar to change their status. If the selected tickets have different statuses, the dropdown displays "mixed" as the current value. Only tickets that don't already have the target status are updated.
