---
title: Support JavaScript API reference
sidebar: Docs
showTitle: true
---

The `posthog.conversations` JavaScript API gives you full programmatic control over PostHog Support. Use it to build a custom support interface or integrate support into your existing UI.

`posthog.conversations` is the namespace for Support's SDK methods. "Conversations" is the code-level name for support tickets and their messages, so you'll see it in method names, captured events, and settings – the product itself is Support.

## Check availability

Before calling other methods, check that the API is available:
...
- Support is enabled in your project settings
...
**Parameters:**
- `options` (optional) – Object with any of:
  - `status` (string) – Filter by status: `new`, `open`, `pending`, `on_hold`, or `resolved`
