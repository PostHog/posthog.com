...
| `$conversation_ticket_assigned` | Ticket was assigned to a team member or AI. Includes `assignee_type` and `assignee_id` properties. |
| `$conversation_message_sent` | Team member sent a message on a ticket. Includes `message_id`, `message_content`, `author_type`, and `author_id` properties. |
| `$conversation_message_received` | Customer sent a message on a ticket. Includes `message_id`, `message_content`, `author_type`, `customer_name`, and `customer_email` properties. |
