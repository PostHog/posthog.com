...
The JavaScript API is available at `posthog.conversations` and provides full programmatic control over conversations. Use it to build custom support experiences or integrate with your existing UI.
...
// Get messages after a specific timestamp (for polling/pagination)
const response = await posthog.conversations.getMessages(
  'ticket-uuid',
...
Messages are associated with the widget session ID. The user maintains access to their conversation across page loads, even without logging in.
...
First, disable the widget in **Settings > Conversations** by setting `widgetEnabled` to false.

Then use the API to build your own interface:
...
// Load existing messages if there's an active ticket
...
// Poll for new messages from your team
setInterval(async () => {
...
The conversations module automatically captures these events for your analytics:
...
These events work with the rest of PostHog. You can create funnels to measure support engagement or use cohorts to target users who have contacted support.
...
> **Note:** Calling `posthog.reset()` will clear the widget session ID, meaning the user will no longer have access to their previous tickets.
...
- Per-user throttling based on widget session ID
