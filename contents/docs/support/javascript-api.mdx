...
When using [identity verification](#identity-verification), you don't need to pass `userTraits` – the `identity_distinct_id` and `identity_hash` parameters handle access control and ticket lookup instead.
...
const response = await posthog.conversations.getMessages("ticket-uuid", "2024-01-15T10:30:00Z");
```

When using [identity verification](#identity-verification), ticket access is verified using the signed `identity_distinct_id` and `identity_hash` parameters instead of the widget session ID.
...
By default, tickets are scoped to the current browser's widget session ID. When using [identity verification](#identity-verification), tickets are returned for all browsers and devices associated with the verified `distinct_id`.
...
Identity verification uses HMAC-based signing to cryptographically verify a user's identity. Unlike basic [user identification](#user-identification), which associates traits like name and email with a session, identity verification lets users access their tickets across browsers and devices without needing the [email recovery flow](#recover-tickets-across-browsers).

When identity verification is enabled, the widget API uses the verified `distinct_id` for ticket lookup instead of the browser-specific `widget_session_id`.

### How it works

1. Your server computes an HMAC-SHA256 hash of the user's `distinct_id` using your project's secret API token
2. The client passes `identity_distinct_id` and `identity_hash` with each API call
3. PostHog verifies the hash server-side and uses the `distinct_id` for ticket access control

### Server-side hash generation

Compute the identity hash on your server – never expose your secret API token in client-side code.
...
def generate_identity_hash(distinct_id: str, secret_api_token: str) -> str:
    return hmac.new(
...
function generateIdentityHash(distinctId, secretApiToken) {
...
Fetch the hash from your server, then pass it to any conversations API call:

```javascript
// Fetch the identity hash from your server
const response = await fetch("/api/support-identity", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ userId: currentUser.id }),
});
const { identityHash } = await response.json();

// Pass identity verification to conversations API
await posthog.conversations.sendMessage("Hello!", {
  identityDistinctId: currentUser.id,
  identityHash: identityHash,
});
```

Identity verification parameters are supported on all widget API methods including `sendMessage()`, `getTickets()`, `getMessages()`, and `markAsRead()`.

<CalloutBox icon="IconWarning" title="Keep your secret API token secure" type="caution">

The identity hash must be generated server-side. Never expose your secret API token in client-side code. If your token is compromised, rotate it in your project settings.
...
If you've set up [identity verification](#identity-verification), you don't need this recovery flow – tickets are automatically accessible across browsers and devices.
...
- Invalid ticket IDs
- Server errors
- Identity verification failures (403 status)
...
All methods that communicate with the widget API (`sendMessage`, `getMessages`, `markAsRead`, `getTickets`) also accept `identity_distinct_id` and `identity_hash` parameters for [identity verification](#identity-verification).
