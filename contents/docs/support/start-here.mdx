...
Four ways to create tickets from Slack:
...
4. **Ticket nudges** – prompts users in non-support channels to open a ticket
...
You can connect up to 10 email addresses per project (e.g. `support@`, `billing@`, `sales@`). Domains are shared across all projects in your organization – see the [email channel docs](/docs/support/email) for details.
...
Migrating from another help desk? [Import historical tickets](/docs/support/imports) into the unified inbox — [Zendesk Support](/docs/support/imports/zendesk) available now (beta).

<OSButton variant="primary" asLink to="/docs/support/imports">
...
Connect your GitHub repositories to turn GitHub issues into support tickets. Issues opened in monitored repos automatically create tickets, and your replies in PostHog are posted back as comments on the GitHub issue.

How it works:

1. **New issues** – Issues opened in monitored repos become tickets
2. **Comments** – Issue comments sync to ticket messages
3. **Status** – Closing or reopening issues updates ticket status
4. **Replies** – Team replies in PostHog are posted back to the GitHub issue

### How to connect

1. Install the PostHog GitHub App from **Settings > Integrations**
2. Go to **Support > Settings** and open the **GitHub** tab
3. Select the GitHub App installation to connect
4. Choose which repositories to monitor

<OSButton variant="primary" asLink to="/docs/support/github">
...
<QuestLogItem
    ...
    subtitle="Opt-in AI agent"
    icon="IconPiggyBank"
>

Support's core features — the chat widget, inbox, and integrations — are free.

The Support AI agent will be opt-in. Only teams who turn it on will be billed, and we'll give you plenty of notice before that happens.
