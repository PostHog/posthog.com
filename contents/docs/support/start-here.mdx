...
Turn GitHub issues into support tickets with two-way sync. Issues from selected repositories automatically create tickets, and your replies in PostHog are posted as comments on the GitHub issue.

1. **Connect your GitHub integration** – use an existing GitHub App integration
2. **Select repositories** – choose which repos to monitor for issues
3. **Automatic sync** – new issues create tickets, comments sync both ways

Issue status syncs automatically – closing an issue resolves the ticket, reopening it sets the ticket back to open.
