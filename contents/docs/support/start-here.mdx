...
- **Session Replay ID** - Links to the user's [session recording](/docs/session-replay) if replay is enabled
...
Connect your email to turn incoming emails into support tickets and reply directly from PostHog. Two-way sync means emails from customers create tickets, and your replies are sent back as emails.

To set up the email channel:

1. **Enter your support email** — the address customers contact you at (e.g. support@company.com)
2. **Set up forwarding** — forward incoming emails to the provided PostHog address in your email provider
3. **Verify your domain** — add DNS records (SPF/DKIM) so outbound replies don't land in spam
