...
Ticket nudges are on by default. You can disable them in **Support** > **Settings** by unchecking **Nudge users to open tickets**.
...
- Download and re-host image attachments (up to 4 MB per image). If a file can't be downloaded, it appears as a link to open in Slack.
...
- Images and files from Slack are downloaded and stored in PostHog. If a file can't be re-hosted, it appears as a link to the original in Slack.
...
<CalloutBox icon="IconWarning" title="Existing installations may need to reconnect" type="warning">

If you see a warning banner in **Support** > **Settings** about file syncing, reconnect SupportHog to your Slack workspace. This grants the permissions needed for image and file attachments to sync between Slack and PostHog.

Without reconnecting, files sent in Slack won't appear on tickets, and images sent from PostHog arrive as links instead of attachments.
