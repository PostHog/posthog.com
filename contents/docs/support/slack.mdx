...
React with the configured emoji (default `:ticket:`) on any message to create a ticket from it. If the message already has a thread with replies, those replies are also synced as ticket messages. If a ticket already exists for that thread, the reaction is ignored to prevent duplicates.
