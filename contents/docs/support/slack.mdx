...
| **Bot display name** | Override the name shown when the bot posts messages to Slack. | Bot's default Slack name |
| **Bot icon URL** | Override the avatar shown when the bot posts messages. Must be an HTTPS URL. | Bot's default Slack icon |
...
You can customize the display name and icon the SupportHog bot uses when posting messages to Slack. These fields are in the **Bot appearance** section of **Support > Settings**.

- Leave both fields blank to use the bot's default Slack name and icon.
- The icon URL must start with `https://`.
- The display name must be 200 characters or fewer.

These overrides only apply to bot-authored messages (confirmations, fallback replies). Messages posted on behalf of a team member still show that member's name.

> **Re-authorization required:** If SupportHog was connected before this feature was available, disconnect and reconnect the bot so it receives the additional `chat:write.customize` permission.
