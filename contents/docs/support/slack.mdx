...
The behavior depends on where you react within a thread:

- **Emoji on a thread root** – creates a ticket seeded from the root message and backfills all existing replies into the ticket
- **Emoji on a reply** – creates a ticket seeded from that specific reply and backfills only the replies posted after it
