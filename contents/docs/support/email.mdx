...
<CalloutBox icon="IconInfo" title="Domain ownership is organization-wide" type="fyi">

Email domains are scoped to your organization, not individual projects. Multiple projects in the same organization can use addresses from the same domain (e.g. `support@company.com` in one project and `billing@company.com` in another). Domain verification and DNS records are shared across all projects in your organization. A domain can only be claimed by one organization at a time.
