...
| `channel_source` | string         | `widget`, `email`, `slack`, or `teams` |
...
Set different SLA deadlines based on where a ticket came from – Slack or Teams (internal) versus the widget (customer-facing).
...
You can use the same pattern for Teams tickets by checking `event.properties.channel_source = "teams"`.
