...
| `sla_due_at`        | string or null | ISO 8601 deadline in force when the reply was sent, or null if no SLA is set                  |
| `sla_active`        | boolean        | Whether an SLA deadline was set on the ticket                                                 |
| `sla_breached`      | boolean        | Whether the reply was sent after the SLA deadline                                             |
| `sla_delta_seconds` | int or null    | Seconds from the deadline (positive = past due, negative = time remaining), or null if no SLA |
