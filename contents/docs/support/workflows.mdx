...
| `snoozed_until`         | string or null    | ISO 8601 datetime when the ticket will automatically reopen, or null |
...
| `snoozed_until` | string   | No       | ISO 8601 datetime to snooze the ticket until. Set to `null` to clear. |
...
<CalloutBox icon="IconInfo" title="Snooze auto-updates ticket status" type="fyi">

When you set `snoozed_until`, the ticket status automatically changes to `on_hold`. When the snooze period expires, the status automatically changes back to `open`. These automatic transitions only apply if you don't explicitly set a `status` in the same update.
...
### 12. Snooze low-priority tickets

Automatically snooze low-priority tickets for 24 hours. The ticket status changes to `on_hold` and automatically reopens when the snooze period expires.

**Trigger:** New ticket created

**Flow:**

1. **If** `event.properties.priority` equals `low`
   - **Update ticket:**

```
snoozed_until = 24 hours from now
```
