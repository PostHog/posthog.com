...
- \*_Event description_:\* Ticket status changes (e.g. new → pending → resolved). You can filter on `new_status` to trigger only for specific transitions.
...
| `sla_business_hours` | object   | No       | Working hours for SLA calculation. See [business hours](#business-hours). |
...
### 3. Set SLA with business hours

Count only business hours toward the SLA deadline instead of calendar time.
...
This sets a 4 business-hour SLA that only counts time during Monday–Friday, 9am–5pm Eastern. If a ticket is created on Thursday at 4pm, the deadline resolves to Friday at 12pm (1 hour Thursday + 3 hours Friday).

### 4. Auto-assign new tickets to a role
...
### 5. Auto-assign based on channel
...
### 6. Move ticket to Open when team replies
...
### 7. Move ticket to Pending after team reply
...
### 8. Reopen ticket when customer replies
...
### 9. Auto-tag tickets by customer email domain
...
### 10. Escalate high-priority tickets
...
### 11. Tag tickets moving to Pending
...
### 12. Full SLA Workflow with priority and channel
...
### Calendar hours (default)

By default, SLA deadlines use calendar hours – every minute counts regardless of day or time.
...
Set `sla_business_hours` to count only working hours toward the SLA. The SLA clock pauses outside the configured window and resumes when the next business period starts.

`sla_business_hours` accepts an object with these keys:

| Key        | Type                            | Description                                                                                                              |
| ---------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `days`     | array of strings                | Lowercase weekday names to include, e.g. `["monday", "tuesday", "wednesday", "thursday", "friday"]`                      |
| `time`     | `"any"` or `["HH:MM", "HH:MM"]` | `"any"` for the full day, or a start/end pair like `["09:00", "17:00"]`. End must be after start (no overnight windows). |
| `timezone` | string                          | IANA timezone, e.g. `"America/New_York"`, `"Europe/London"`, `"UTC"`. Defaults to `"UTC"`.                               |

When `sla_unit` is `day` with a custom time window, one day equals one window length. For example, with a 9am–5pm window, `1` day equals 8 business hours.

If `sla_business_hours` is omitted or covers all seven days with `time` set to `"any"`, the SLA falls back to calendar hours.
