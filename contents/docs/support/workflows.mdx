...
```
Ticket event → Workflow trigger matches → Hog function executes → Ticket updated
```
...
When ticket or message state changes, these events are emitted and can be used as workflow triggers:

| Event | When it fires |
|-------|---------------|
| `$conversation_ticket_created` | A customer creates a new ticket |
| `$conversation_ticket_status_changed` | Ticket status changes (e.g. new → pending → resolved) |
| `$conversation_ticket_priority_changed` | Ticket priority is set or changed |
| `$conversation_ticket_assigned` | Ticket is assigned or unassigned |
| `$conversation_message_sent` | A team member sends a reply |
| `$conversation_message_received` | A customer sends a message |
...
- You can filter on `new_status` to trigger only for specific transitions.
...
**Output fields** (available as `{variables.ticket.*}`):
...
### Conditions on event properties (no Get ticket needed)
...
### Conditions on ticket data (after Get ticket)
...
1. **If** `event.properties.channel_source` equals `slack`
   - **Update ticket:** SLA = 24 hours
2. **Else**
   - **Update ticket:** SLA = 4 hours
...
**Trigger:** Event trigger on `$conversation_ticket_priority_changed`

**Flow:**

1. **If** `event.properties.new_priority` equals `high`
   - **Update ticket:** SLA = 1 hour
2. **Else if** `event.properties.new_priority` equals `medium`
   - **Update ticket:** SLA = 4 hours
3. **Else**
   - **Update ticket:** SLA = 24 hours
...
1. **Update ticket:** `assignee` = Support role
...
1. **If** `event.properties.channel_source` equals `slack`
   - **Update ticket:** `assignee` = Engineering role
2. **Else**
   - **Update ticket:** `assignee` = Support role
...
1. **Update ticket:** `status` = `pending`
...
1. **If** `event.properties.customer_email` contains `@bigcorp.com`
   - **Update ticket:** `tags` = `["enterprise", "priority"]`
2. **Else if** `event.properties.customer_email` contains `@startup.io`
   - **Update ticket:** `tags` = `["startup"]`
...
1. **If** `event.properties.priority` equals `high`
   - **Update ticket:** `sla_amount` = `30`, `sla_unit` = `minute`, `assignee` = Senior Support role
   - **Update ticket:** `tags` = `["escalated"]`
...
1. **Update ticket:** `tags` = `["awaiting-customer"]`
...
2. **If** `event.properties.channel_source` equals `slack`
   - **Update ticket:** `priority` = `low`, `sla_amount` = `24`, `sla_unit` = `hour`, `assignee` = Engineering role
3. **Else if** `event.properties.customer_email` contains `@enterprise.com`
   - **Update ticket:** `priority` = `high`, `sla_amount` = `1`, `sla_unit` = `hour`, `assignee` = Senior Support role, `tags` = `["enterprise", "vip"]`
4. **Else**
   - **Update ticket:** `priority` = `medium`, `sla_amount` = `4`, `sla_unit` = `hour`, `assignee` = Support role
