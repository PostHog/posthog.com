...
| `sla`                   | string or null    | SLA deadline, or null if none                                      |
| `snoozed_until`         | string or null    | When the ticket will be unsnoozed                                  |
| `assignee`              | object or null    | Assigned user or role with `id`, `type`, `user`, and `role` fields |
| `url`                   | string or null    | URL where the customer was when creating the ticket                |
| `slack_channel_id`      | string or null    | Slack channel ID (for Slack tickets)                               |
| `slack_thread_ts`       | string or null    | Slack thread timestamp (for Slack tickets)                         |
| `slack_team_id`         | string or null    | Slack team ID (for Slack tickets)                                  |
| `email_subject`         | string or null    | Email subject line (for email tickets)                             |
| `email_from`            | string or null    | Sender's email address (for email tickets)                         |
| `email_to`              | string or null    | Destination email address (for email tickets)                      |
| `cc_participants`       | array             | CC email addresses (for email tickets)                             |
