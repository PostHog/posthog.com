...
All events include these base properties: `ticket_id`, `ticket_number`, `channel_source`, `status`, and `priority`. Staff and system actions also include `actor_type`, `actor_id`, and `actor_email` properties to identify who performed the action.
...
| `actor_type`  | string         | Who performed the action: `user`, `system`, `external`, or `customer` |
| `actor_id`    | int or null    | PostHog user ID if the actor is a staff member                        |
| `actor_email` | string or null | Email of the actor if available                                       |
...
| `actor_type`      | string         | Always `user` for team replies          |
| `actor_id`        | int or null    | PostHog user ID of the sender           |
| `actor_email`     | string or null | Email of the sender                     |
