...
| `tags`       | tags     | No       | Replace all tags on the ticket. Each tag supports variable templating (e.g. `{event.properties.region}`). |
...
Tag values support variable templating, so you can use expressions like `{variables.zendesk_ticketid}` or `{event.properties.region}` as tag names. The variables are resolved when the workflow runs.
...
```
tags = ["enterprise", "priority", "{event.properties.region}"]
```
