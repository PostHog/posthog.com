...
| `tags`       | tags     | No       | Tags to apply to the ticket.                                |
| `tags_mode`  | choice   | No       | How tags are applied: `add` (default), `set`, or `remove`.  |

<CalloutBox icon="IconInfo" title="Tag modes" type="fyi">

- **Add** (default) – Adds the specified tags to any existing tags. Safe when multiple workflows tag the same ticket.
- **Set** – Replaces all existing tags with the specified tags.
- **Remove** – Removes only the specified tags from the ticket.
...
- **Tag modes** – Use `tags_mode` to control how tags are applied: `add` (default) preserves existing tags, `set` replaces all tags, and `remove` deletes specific tags.
