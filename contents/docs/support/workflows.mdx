...
| `priority`       | string or null | `low`, `medium`, `high`, `critical`, or null |
...
| `priority`   | choice   | No       | `low`, `medium`, `high`, `critical`                         |
...
IF event.properties.new_priority = critical
   SET SLA = 15 minutes
ELSE IF event.properties.new_priority = high
   SET SLA = 1 hour
...
### 9. Escalate critical-priority tickets

Immediately set a tight SLA and assign to senior support when a critical-priority ticket is created.

**Trigger:** New ticket created

**Flow:**

1. **If** `event.properties.priority` equals `critical`
   - **Update ticket:**
