...
If `domains` is empty or not set, the widget shows on all domains. However, email-based [ticket recovery](#recover-tickets-across-browsers) is disabled until at least one domain is configured.
...
<CalloutBox icon="IconWarning" title="Authorized domains required" type="caution">

Email-based ticket recovery requires at least one [authorized domain](#domain-restrictions) to be configured. This ensures recovery links only point to a site you control. For logged-in users, consider using [identity verification](#identity-verification) instead – tickets persist across browsers and devices automatically without relying on email recovery.
