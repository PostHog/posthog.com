...
Use the [JavaScript API](/docs/support/javascript-api) to programmatically show or hide the widget:
...
If your app has logged-in users, identity verification works better than [email-based recovery](/docs/support/widget#recover-tickets-across-browsers). You sign the user's distinct ID on your server, and tickets persist across browsers and devices automatically – no user action required.

<CalloutBox icon="IconInfo" title="setIdentity vs identify" type="fyi">
...
1. Your server computes an HMAC-SHA256 hash of the user's `distinct_id` using your **Secret API key** from Support settings
...
Copy the key from the **Secret API key** section of [**Support** > **Settings**](https://app.posthog.com/support/settings). If the section is empty, use the button next to it to generate a key.

Several PostHog keys start with `phs_`, so the prefix does not tell you which key you have.

| Key | Where you find it | Signs identity hashes? |
|-----|-------------------|------------------------|
| **Secret API key** (`phs_`) | Support > Settings > Secret API key | Yes |
| **Feature flags secure API key** (`phs_`) | Feature flag settings, [local evaluation](/docs/feature-flags/local-evaluation) | Yes, it is the same key under an older name |
| **Project secret API key** (`phs_`) | [Project secret API keys](/docs/api/project-secret-api-keys) in project settings | No |
| **Personal API key** (`phx_`) | [Personal API keys](/docs/api/personal-api-keys) in user settings | No |
| **Project token** (`phc_`) | Project settings, the token you pass to `posthog.init()` | No |

If you sign the hash with a key that does not work, the widget API answers `403 Forbidden` and gives no detail about the cause. The response is the same for a wrong key, a bad signature, and a project with no key, so that the API tells anonymous callers nothing about your setup. If `GET /widget/tickets` or `POST /widget/message` answers `403`, check your key first.

When you rotate the key, PostHog keeps the previous key as a backup, and hashes signed with the backup keep working. Delete the backup key after all your servers use the new key.

### Server-side implementation

Generate the identity hash on your backend. Never expose the secret API key in client-side code.
...
def compute_identity_hash(distinct_id: str, secret_api_key: str) -> str:
    return hmac.new(
        secret_api_key.encode(),
        distinct_id.encode(),
        hashlib.sha256,
    ).hexdigest()
```
...
function computeIdentityHash(distinctId, secretApiKey) {
  return crypto
...
def compute_identity_hash(distinct_id, secret_api_key)
  OpenSSL::HMAC.hexdigest('sha256', secret_api_key, distinct_id)
end
```
...
func ComputeIdentityHash(distinctID, secretAPIKey string) string {
    h := hmac.New(sha256.New, []byte(secretAPIKey))
...
```php
function computeIdentityHash(string $distinctId, string $secretApiKey): string {
    return hash_hmac('sha256', $distinctId, $secretApiKey);
}
```
...
<CalloutBox icon="IconWarning" title="Keep your secret API key secure" type="caution">

Your secret API key must never be exposed in client-side code. Always compute the identity hash on your server and pass only the hash to the frontend.
...
If your project has no secret API key, requests that carry identity fields get `403 Forbidden`. Generate a key in Support settings. Requests without identity fields keep using the browser session.
...
<CalloutBox icon="IconLightBulb" title="Use Workflows for advanced notifications" type="fyi">
