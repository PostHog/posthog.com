---
title: OpenClaw LLM Observability installation
...
The PostHog plugin initializes automatically on startup. Once users send messages through any connected channel (Telegram, Slack, Discord, or WebChat), LLM Observability events are captured and sent to PostHog.
...
- **`$ai_span`** — Each tool execution, including tool name, input parameters, output result, duration, and parent generation ([learn more](/docs/llm-observability/spans)).
- **`$ai_trace`** — Completed message cycles with aggregated token totals and latency ([learn more](/docs/llm-observability/traces)).
