---
title: Pi Coding Agent LLM Observability installation
...
- **`$ai_generation`** — Every LLM call, including model, provider, token usage, cost, latency, and input/output messages (in [OpenAI chat format](/docs/llm-observability/generations)).
- **`$ai_span`** — Each tool execution (read, write, edit, bash, etc.), including tool name, input parameters, output result, and duration ([learn more](/docs/llm-observability/spans)).
- **`$ai_trace`** — Completed agent runs with aggregated token totals and latency ([learn more](/docs/llm-observability/traces)).
