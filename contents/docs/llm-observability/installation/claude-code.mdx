---
title: Claude Code LLM Observability installation
...
1. Go to the [LLM Observability](https://us.posthog.com/llm-observability) tab in PostHog.
...
- **`$ai_generation`** — Every LLM call, including model, provider, token usage (input, output, cache read, cache creation), stop reason, and input/output messages (in [OpenAI chat format](/docs/llm-observability/generations)).
- **`$ai_span`** — Each tool execution (Bash, Read, Write, Edit, Grep, Glob, MCP tools, etc.), including tool name, input parameters, output result, duration, and error info ([learn more](/docs/llm-observability/spans)).
- **`$ai_trace`** — Completed sessions (or prompts, depending on grouping mode) with aggregated token totals and latency ([learn more](/docs/llm-observability/traces)).

## Next steps

Now that you're capturing Claude Code sessions, continue with the resources below to learn what else LLM Observability enables within the PostHog platform.
...
| [Analyze LLM performance](/docs/llm-observability/dashboard) | Learn how to create dashboards to analyze LLM performance. |
