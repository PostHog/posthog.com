...
The **LLM Observability** > [**Generations** tab](https://app.posthog.com/llm-observability/generations) displays a list of generations, along with a preview of key autocaptured properties. You can filter and search for generations by various properties.
...
When calling LLMs with our [SDK wrappers](/docs/llm-observability/installation), you can also enrich the `$ai_generation` event with your own [custom properties](/docs/llm-observability/custom-properties) and PostHog attributes like groups and distinct IDs for identified users.
...
Generations are nested under [spans](/docs/llm-observability/spans) and [traces](/docs/llm-observability/traces). 
...
When a generation includes tool calls (function calls), PostHog automatically extracts them and displays them as tags on the generation. You can see aggregated tool usage across all your generations in the [Tools](/docs/llm-observability/tools) tab.

## Evaluating generations

You can automatically assess the quality of your generations using [evaluations](/docs/llm-observability/evaluations). Evaluations use an LLM-as-a-judge approach to score outputs based on criteria like relevance, helpfulness, or safety.

## Sentiment classification

PostHog can classify the sentiment of user messages in your generations as negative, neutral, or positive. Sentiment is computed on-demand using a local model when you view a [trace](/docs/llm-observability/traces) — no data is sent to third-party services. See [Sentiment classification](/docs/llm-observability/sentiment) for more details.
