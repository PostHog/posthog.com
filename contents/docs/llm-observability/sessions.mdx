...
Sessions group multiple [traces](/docs/llm-observability/traces) together using the `$ai_session_id` property.
...
- **Trace** (`$ai_trace_id`): Groups related AI events (generations, spans, embeddings) together. Required for all LLM Observability events.
...
Set the `$ai_session_id` property when capturing LLM events using the `posthogProperties` (JavaScript) or `posthog_properties` (Python) parameter in PostHog's LLM Observability SDKs. Use any identifier that makes sense for grouping related traces in your application.
...
When viewing a [trace](/docs/llm-observability/traces), you can see which session it belongs to, helping you understand the broader context of related traces.
