---
title: LLM Observability basics
---

import OSButton from 'components/OSButton'

This page covers how your LLM calls become analytics in PostHog and defines key concepts for LLM Observability.

## How LLM calls become events

PostHog's LLM Observability works by wrapping your existing LLM provider's SDK to capture requests and responses. Your API calls still go directly to your provider, but the wrapper extracts metadata from each call and sends it to PostHog as an event.
...
You can also [set custom pricing](/docs/llm-observability/calculating-costs) if you have negotiated rates or use unsupported models.
...
|------|------------|---------|
| [Session](/docs/llm-observability/sessions) | Groups multiple traces together | A user's conversation thread |
| [Trace](/docs/llm-observability/traces) | Contains generations and spans for a single request | One chatbot message and response |
| [Span](/docs/llm-observability/spans) | Tracks an operation within a trace | A retrieval step or function call |
| [Generation](/docs/llm-observability/generations) | An LLM call, tracked as `$ai_generation` events | Sending a prompt to Claude |
| [Embedding](/docs/llm-observability/embeddings) | Converts text into vectors | Vectorizing documents for RAG |
