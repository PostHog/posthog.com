...
![Signup Funnel conversion with PostHog](../../images/tutorials/supabase-nextjs-signup-funnel/signup-funnel-conversion-posthog.png)
...
1. A [self-hosted instance of PostHog](/signup/self-host) or [sign up for PostHog Cloud](https://app.posthog.com/signup)
...
PostHog JS has an [autocapture](/docs/integrate/client/js#autocapture) feature that automatically captures browser events (this can be disabled). However, it won't capture navigation events in Next.js where the window doesn't reload, so we need to add some custom code to capture navigations.
...
To add the second step, click either of the **Add funnel step** buttons. Change the event to **Autocapture** since the event we're looking to make use of was one [autocaptured](/docs/integrate/client/js#autocapture) by the PostHog JS SDK. Then, set a filter. When you click the filter icon this time, select the **Elements** tab and select the **Text** property. 
