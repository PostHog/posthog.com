...
The landing page of the app is currently the authentication page. So, let's move that to `/auth` path instead. To do this, simply rename `pages/index.js` to `pages/auth.js`.

The `/auth` page defaults to showing a sign in view. Since we are focusing on signup we need to change to default view. To do that, open `pages/auth.js` and update the default authentication view to be `sign_up` as follows:

```js
const [authView, setAuthView] = useState('sign_up')
```

Finally, `pages/profile.js` file refers to the "/" path within a `<Link />` component so also update that reference.

### Update the auth redirect location
