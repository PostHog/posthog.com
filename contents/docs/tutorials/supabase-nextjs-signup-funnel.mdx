...
1. User lands on the main website landing page which has two CTAs (call-to-actions) of **Sign Up**. One in the header and one in the landing page hero.
...
4. User successfully signs up.

### Move the Auth functionality

The landing page of the current app is currently the authentication page. So, let's move that to `/auth` path instead.

To do this, simply rename `pages/index.js` to `pages/auth.js`. The `pages/profile.js` file refers to the "/" path within a `<Link />` component so also update that reference.
