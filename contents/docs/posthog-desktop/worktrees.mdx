---
title: PostHog Desktop worktrees
---

<CalloutBox icon="IconFlask" title="Worktrees are in beta" type="action">

You may experience unexpected behavior with worktrees in complex development environments. For running parallel agents, we recommend using [cloud tasks](/docs/posthog-desktop/cloud-runs) instead.
...
For setup scripts that run when a worktree is created (installing dependencies, running migrations, and so on), see [Local environments](/docs/posthog-desktop/environments#local-environments).
...
When you create a worktree, PostHog Desktop:
