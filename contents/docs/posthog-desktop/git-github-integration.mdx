...
Check that your branch has a remote configured and that `gh auth status` reports you as authenticated. The **Push** action never force-pushes, so if the remote has diverged, resolve that before you retry.
...
Check the task's branch on GitHub. The pull request can still exist even when its link does not appear in PostHog Desktop.
