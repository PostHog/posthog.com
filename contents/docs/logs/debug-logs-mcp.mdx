...
- **Search and filter logs** – Query by severity level, service name, date range, and property filters.
...
| Tool | Description |
|------|-------------|
| `logs-query` | Search logs with filters for severity level (trace, debug, info, warn, error, fatal), service name, date range, and structured attribute filters. Supports pagination for large result sets. |
