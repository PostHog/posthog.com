...
1. In the [logs view](https://app.posthog.com/logs), click on the log entry you're interested in to open the log details modal
2. In the modal header, click the **View Recording** button to open the session replay
3. Watch the user's interaction in context alongside the backend logs

The **View Recording** button only appears when the log entry has an associated session ID.
