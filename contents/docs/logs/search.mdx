...
<<<<<<< HEAD
- **Logs** – top-level log fields like `message`, `severity_level`, `trace_id`, and `span_id`
||||||| 89b548c6b
...
## Filter by severity, trace, or span

The **Logs** group in the filter picker exposes four top-level fields:

| Field | What it filters | Notes |
|-------|-----------------|-------|
| `message` | The log body | Full-text search with equals, contains, and regex operators |
| `severity_level` | Log severity | Dropdown with `trace`, `debug`, `info`, `warn`, `error`, `fatal` |
| `trace_id` | OpenTelemetry trace ID | Accepts hex (from trace URLs) or base64 |
| `span_id` | OpenTelemetry span ID | Accepts hex or base64 |

`severity_level`, `trace_id`, and `span_id` only support equals and not-equals operators.

**Correlate with traces** – If you're debugging a distributed request, paste the `trace_id` from a trace URL into the filter to find all logs emitted during that trace. Add a `span_id` filter to narrow to a specific operation.
...
1. Click the filter bar and pick a field. Logs, resource attributes, and attributes are grouped separately in the picker.
...
To search log bodies, pick **message** from the Logs group in the filter bar. Message supports three operators, each with a negated variant for exclusion:
