...
## Discover recurring log patterns

Pattern mining clusters your log messages into recurring templates, replacing variable parts (IDs, timestamps, IP addresses, numbers) with placeholder tokens like `<*>`, `<uuid>`, `<ip>`, and `<num>`.

For example, if your logs contain:
...
Pattern mining groups them into a single template: `User <*> not found`.

This is useful when you want to:

- **Find noisy log templates** — identify which patterns dominate your log volume.
- **Spot recurring errors** — surface error patterns you didn't know existed without manually searching.
- **Understand log distribution** — see how your log volume breaks down across different message types.

Each discovered pattern includes:

- How many times it occurred and what share of total volume it represents
- How many occurrences were errors
- When it was first and last seen
- Example raw log bodies that produced the pattern
- Which services generated it

For large time windows, patterns are mined from a random sample rather than every matching log. When this happens, you can use the reported sample size and total count to estimate full-window numbers.
