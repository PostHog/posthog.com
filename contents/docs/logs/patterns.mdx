...
- **Readable by agents.** Your coding agent can [mine and diff patterns over MCP](#use-patterns-with-ai-agents), so it starts an investigation with up to 200 ranked templates instead of burning its context window on raw lines.
...
Read the line counts before you trust a new template. When a window holds only a few lines, the miner cannot tell which parts of the message vary. It can then leave a timestamp in the template. The same log statement appears twice: once as new, and once as gone.
