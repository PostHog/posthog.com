...
- Send logs to `https://us.i.posthog.com/logs` (or your self-hosted endpoint)
...
const sdk = new NodeSDK({
  logRecordProcessor: new SimpleLogRecordProcessor(
    new OTLPLogExporter({
      url: 'https://us.i.posthog.com/logs',
      headers: {
...
const sdk = new NodeSDK({
  logRecordProcessor: new SimpleLogRecordProcessor(
    new OTLPLogExporter({
      url: `https://us.i.posthog.com/logs?token=${YOUR_PROJECT_TOKEN}`
    })
  )
});
```
...
otlp_exporter = OTLPLogExporter(
    endpoint="https://us.i.posthog.com/logs",
...
```python
otlp_exporter = OTLPLogExporter(
    endpoint=f"https://us.i.posthog.com/logs?token={YOUR_PROJECT_TOKEN}"
)
```
    ...
    .addLogRecordProcessor(
        BatchLogRecordProcessor.builder(
            OtlpHttpLogRecordExporter.builder()
                .setEndpoint("https://us.i.posthog.com/logs")
...
```java
OtlpHttpLogRecordExporter.builder()
    .setEndpoint("https://us.i.posthog.com/logs?token=" + YOUR_PROJECT_TOKEN)
    .build()
```
...
```go
otlploghttp.WithURLPath("/v1/logs?token=" + YOUR_PROJECT_TOKEN)
```
