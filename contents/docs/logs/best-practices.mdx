...
Six log lines, none of them useful in production at `INFO` level. They tell you what the code _does_ (you already know that, you wrote it), not what happened to a specific request.
...
Two concepts that separate useful logs from noise. You want both high cardinality _and_ high dimensionality. One wide event with 50 fields tells you more than 50 separate log lines with three fields each.
...
- **Secrets:** API keys, passwords, tokens, credit card numbers. If you log these by accident, you now have a security incident _and_ a logging problem.
...
Even with good practices, sensitive data can slip into logs accidentally. PostHog can automatically redact common PII patterns from your log payloads at ingestion time, before anything is stored.

To enable it, go to [**Settings** > **Logs** > **PII scrubbing**](https://app.posthog.com/settings/environment-logs#logs-pii-scrub) and turn on the toggle.

When enabled, the following patterns are detected and replaced with `{{REDACTED}}`:

- **Bearer tokens** – `Bearer <token>` headers
- **Stripe secret keys** – keys matching `sk_live_*` or `sk_test_*`
- **Email addresses**
- **Credit card numbers** – 13–19 digit runs that pass Luhn validation (with optional spaces or hyphens)
- **Sensitive attribute keys** – the entire value is redacted when the attribute key contains substrings like `password`, `secret`, `token`, `authorization`, `cookie`, `credential`, `apikey`, `api_key`, `access_token`, `refresh_token`, `bearer`, or `email`

For JSON log bodies, the scrubber recursively walks the structure and applies both key-based and pattern-based redaction.

<CalloutBox icon="IconWarning" title="PII scrubbing is lossy" type="caution">

Redaction replaces matched values with a fixed placeholder. Original values can't be recovered. This is not reversible hashing – once scrubbed, the data is gone.

</CalloutBox>

This feature catches common patterns but isn't exhaustive. Secrets that don't match these shapes, digit runs that fail Luhn validation, and values under keys outside the sensitive list only get generic pattern coverage. Treat PII scrubbing as a safety net, not a replacement for avoiding sensitive data in your logs in the first place.
