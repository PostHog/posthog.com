---
title: Migrating from a not working PostHog self-hosted instance
sidebar: Docs
showTitle: true
---
...
If you **can** login to your PostHog instance we highly recommend the [migrating between PostHog instances guide](/docs/migrate/migrate-between-posthog-instances)
...
2. Disable the geo-ip plugin of your new PostHog instance
3. Write a script to send them as new requests (likely easiest with our node or python library). Here's an example of how we do it with the [Replicator app](https://github.com/PostHog/posthog-plugin-replicator/blob/main/index.ts).
4. Re-enable the Geo-ip plugin
...
- Your old and new instances should both be running the **exact** same version of PostHog (minimum `1.30.0`). TODO: Understand if this minimum is required
...
### Option 1 - Migrating to PostHog Cloud

If you are migrating to PostHog cloud, you'll need to write a script to process this data and create the following resources on PostHog cloud. The [PostHog migrate code](https://github.com/PostHog/posthog-migrate-meta/blob/main/index.ts) which we use to migrate between two cloud instances will likely be useful.

### Option 2 - Migrating to another PostHog self-hosted instance

#### 2. Migrating to another PostHog instance
