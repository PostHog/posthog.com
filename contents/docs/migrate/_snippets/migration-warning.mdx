...
> 3. Raise an [in-app support request](http://us.posthog.com/home#supportModal) (Target Area: Data Management) detailing where you are sending events from and the total volume. For example, "*we are migrating 30M events from a self-hosted instance to EU Cloud using the migration scripts at 10k events per minute.*"
