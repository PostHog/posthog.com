...
The schema of Statsig's event data is similar to PostHog's schema, but it requires converting to work with the rest of PostHog's data. You can see details on Pendo's schema in [their docs](https://www.statsig.com/glossary/event-schema) and events and properties PostHog autocaptures in [our docs](/docs/product-analytics/autocapture#autocaptured-events).
...
- Event `timestamp` to a ISO 8601 timestamp
...
client_api_key = 'phc_67zPXoeBvPFCB66M'
