...
2. [Import data into PostHog](/docs/integrate/ingest-historic-data#importing-events) using PostHog's Python SDK or `batch` API with the `historical_migration` option set to `true`.
