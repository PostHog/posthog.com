...
> Looking to move or split event data **between projects on the same instance** instead (for example, consolidating several projects into one, or splitting one project's data by source)? See [moving or splitting event data between projects](/docs/migrate/split-or-merge-projects).
...
Before you start, disable transformations and filtering apps in the destination cloud project (e.g. GeoIP). Keeping these enabled may change the events you are migrating.
