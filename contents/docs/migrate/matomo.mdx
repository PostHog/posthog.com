...
Migrating data from Matomo is a three step process:
...
The schema of Matomo's exported event data is similar to PostHog's schema, but it does require conversion to work with the rest of PostHog's data. You can see details on Matomo's schema in [their docs](https://developer.matomo.org/guides/database-schema) and events and properties PostHog autocaptures in [our docs](/docs/product-analytics/autocapture#autocaptured-events).

The big difference is that Matomo structures their data around **visits** that contain one or more **actions**. Matomo actions are similar to PostHog's **events**.

To do this, you can go through each visit and convert it to PostHog's schema by doing the following:

1. Converting properties like `url` to `$current_url` and `operatingSystemVersion` to `$os_version`. 
