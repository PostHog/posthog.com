...
Each direct import job supports a maximum date range of one year. To migrate data spanning more than one year, run multiple jobs that cover smaller, consecutive ranges. For Amplitude migrations, the date range must also be at least 1 hour due to Amplitude API limitations.
