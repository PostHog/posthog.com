...
2. **Upload to S3**: Place your `.jsonl` or `.jsonl.gz` files in an S3 bucket (or S3-compatible storage)
...
<<<<<<< HEAD
  - Endpoint URL (optional, for S3-compatible storage only)
||||||| ce23260be
=======
...
S3 imports support S3-compatible storage providers like Cloudflare R2 and MinIO. To import from these providers, specify a custom **Endpoint URL** when setting up your import.

| Provider | Endpoint URL format | Region |
|----------|-------------------|---------|
| Cloudflare R2 | `https://<ACCOUNT_ID>.r2.cloudflarestorage.com` | Set to `auto` |
| MinIO | Your MinIO server URL (e.g., `https://minio.example.com`) | Your configured region |

Leave the **Endpoint URL** field empty when importing from standard AWS S3.
