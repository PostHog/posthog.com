...
- **Import events from Amplitude** - Import Amplitude events into PostHog. Enabled by default.
- **Generate identify events to link user IDs with device IDs** - Automatically generate `$identify` events whenever a new device ID is associated with a user ID. These identify events help maintain proper user identity linking in PostHog by connecting anonymous device IDs with known user IDs. Enabled by default.
- **Generate group identify events from group property changes** - Generate group identify events when group properties change. Disabled by default.
