...
We're using it to build our [CDP](/cdp) product, which you can follow on [GitHub](https://github.com/PostHog/posthog/issues/22833).

Hog was designed to be syntax-compatible with [HogQL](/docs/hogql), the SQL-like language we use to query data in PostHog. This means all HogQL expressions work as Hog code.

## Quickstart

Ready to try it?

When would I use Hog? What can I do with it?

- Start with a HogQL query: Product Analytics > New Insight > HogQL
- Then a Hog example: Product Analytics > New Insight > Hog
