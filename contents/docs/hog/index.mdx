...
Hog's standard library includes the following functions and will expand. To see the most up-to-date list, check the [Python VM's `stl/__init__.py` file](https://github.com/PostHog/posthog/blob/master/common/hogvm/python/stl/__init__.py).
...
- `tryDecodeURLComponent(arg: string): string`
...
- `formatDateTime(input: DateTime, format: string, zone?: string): string` - we use the [ClickHouse formatDateTime syntax](https://clickhouse.com/docs/en/sql-reference/functions/date-time-functions#formatdatetime).
