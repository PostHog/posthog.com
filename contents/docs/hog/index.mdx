...
When creating a destination, you’ll start with an input interface sheet. This allows for easy changes and maintenance to your destination. Each input’s property name is noted beside its title.

You can include additional inputs by clicking **Add input variable**.

Under the hood, the default webhook template looks like this:

```hog

let payload := {
// This is the object we'll use to structure our request
...
// When the request returns, execution will continue
// The response will be stored to the above constant, `res`
...
You could even add a new input field to your destination, which is convenient if you think you’ll want to change the value again in the future.
