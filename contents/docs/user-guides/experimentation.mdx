...
Experiments helps you test product changes to your app. You can test hypotheses about which product changes will lead to optimal results. You may have heard it called A/B testing before. Examples:
...
Furthermore, even if there's a lot of exposure, if, say, all variants are exposed to 10,000 participants, if the difference in conversion rate between the variants is less than 1%, it's hard to say whether one variant is truly better than the other. To be significant, there must be enough difference between the conversion rates.
