...
Path cleaning rules are an advanced feature that can be used to standardize the form of URLs within a Paths visualization. These rules consist of two parts: a regex which is used to match the various URLs you would like to combine, and an alias, which is the name given to any events that matched the regex.
In this sense, these path cleaning rules are an advanced for of [Wildcard groups](#wildcard-groups), which use regex to match instead of the simple `*` wildcard.
