...
PostHog currently supports the SSO providers listed below. All providers are available and readily configured on PostHog Cloud. This section contains instructions on how to configure each provider in your self-hosted instance. All the providers in this section (i.e. everything except SAML) is **configured instance-based**. SAML is configured domain-based (see [SAML](#saml) below).
