...
The first touchpoint means that `Firefox` will be the breakdown value.
...
For the example above, when step one is chosen, the value will be Chrome, otherwise Firefox.
...
1. `$pageview` with `browser = Firefox`
2. `user signed up` with `browser = Firefox`
