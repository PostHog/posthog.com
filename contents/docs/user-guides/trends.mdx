...
By default, the total number of events (Total count) that occured within a specific date range will be shown. If you'd like to see other metrics, such as 'Unique users' or 'DAU', check out the [event and property aggregation section](event-and-property-aggregation).
