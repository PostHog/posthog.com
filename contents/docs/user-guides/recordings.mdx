...
-   In particular, this feature will not work with <b>Segment</b>, as Segment's SDK does not collect and send this data. _If you use
    Segment, and want to use the Recordings feature, you should add the PostHog library too (but make sure to only send regular event data from one source)._
...
Billing for recordings is handled a bit differently than for regular events as a recording will include a lot of events. Billing is on a per recorded session basis at a price of $0.003 per recording, beyond a free tier of 10,000.
