...
## Persisting flag across authentication steps

You also have an option to persist flags across authentication steps.

Consider this case: An anonymous person comes to your website
and you show them a green call to action.

With usual feature flags, the flag value can change when someone logs in, or changes their identity. So, once they login, they may see a red call
to action instead.
...
However, with some businesses, like e-commerce, it's very common to browse things anonymously and login right before checking out. In cases like these
you can preserve the feature flag values by checking this checkbox.

![Persist feature flags](../../images/features/feature-flags/feature-flag-persist.png)

Note that this slows down the feature flag response a bit. Also note that this functionality is currently available for `posthog-js` only.
