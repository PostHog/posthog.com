...
Feature Flags allow you to safely deploy and roll back new features. It means you can deploy features and then slowly roll them out to your users. If something has gone wrong, you can roll back new features without having to re-deploy your application. Feature Flags can also help you control access to certain features in your product (e.g. only show paid features to users with an active subscription).
...
> **Looking to test changes to your product?** Check out our [Experimentation](/docs/user-guides/experimentation) feature.

## Learning resources

### Tutorial

For a comprehensive step-by-step tutorial on how to use Feature Flags, check out our [How to safely roll out new features](/docs/tutorials/feature-flags) tutorial.
...
Behind the scenes, every time a user loads a page we call an endpoint to get the feature flags that apply to that user. We store those flags as a cookie. This means that for most page views the feature flags will be available immediately, _except_ the very first time a user visits your site.
...
Feature Flags and Experimentation are different features and work for different use cases. Check out our [Experimentation](/docs/user-guides/experimentation) guide for more details.
