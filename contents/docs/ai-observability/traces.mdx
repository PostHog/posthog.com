...
Traces have shareable URLs that encode the trace ID, selected span, and a timestamp hint for fast loading. You can copy a trace link and send it to a teammate — when they open it, they see the same trace and span you were viewing.

To copy a trace URL, click the **Copy link** button in the trace drawer header or in the span row actions. Browser back and forward navigation also works when viewing traces, so you can move through traces you've opened without losing your place.
