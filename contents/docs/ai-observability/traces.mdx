...
If you also send OpenTelemetry spans to [Distributed tracing](/docs/distributed-tracing), you can set `$ai_trace_id` to the trace ID of the surrounding span and correlate the two products. See [Link traces to LLM analytics](/docs/distributed-tracing/link-llm-analytics).
