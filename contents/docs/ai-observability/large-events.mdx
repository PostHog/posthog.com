...
The dedicated endpoint only accepts events whose name starts with `$ai_`, and every event must include the `$ai_model` property. If a batch contains any other event, or an event without `$ai_model`, PostHog rejects the whole request with a `400` and records an [ingestion warning](/docs/data/ingestion-warnings#discarded-ai-events). Send other events to the regular endpoint, which has no `$ai_model` requirement.
...
Over [OpenTelemetry](#sending-over-opentelemetry), a lower **4MB** per-export cap also applies: an export over it is rejected with HTTP 413, while a span whose event exceeds 8MB is dropped without an application error.
