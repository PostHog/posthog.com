...
The dedicated endpoint only accepts events whose name starts with `$ai_`. Any other event in the batch is discarded with an [ingestion warning](/docs/data/ingestion-warnings#discarded-ai-events), and the rest of the batch is ingested. Send other events to the regular endpoint.
