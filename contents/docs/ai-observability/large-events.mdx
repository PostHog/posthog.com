...
The dedicated endpoint only accepts events whose name starts with `$ai_`. If a batch contains any other event, PostHog rejects the whole request with a `400` and records an [ingestion warning](/docs/data/ingestion-warnings#discarded-ai-events). Send other events to the regular endpoint.
