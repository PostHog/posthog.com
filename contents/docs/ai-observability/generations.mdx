...
import GenerationEventSnippet from "./_snippets/generation-event.mdx"
import NotableGenerationPropertiesSnippet from "./_snippets/notable-generation-properties.mdx"
...
<NotableGenerationPropertiesSnippet />

<<<<<<< HEAD:contents/docs/llm-analytics/generations.mdx
## Correlating with provider dashboards

PostHog captures provider-specific identifiers so you can jump from a generation event to the matching entry in your provider's logs:

- **`$ai_completion_id`** – The provider-assigned response ID (e.g., `chatcmpl-…` for OpenAI Chat Completions, `resp_…` for Responses API). Use this to look up the exact request in OpenAI's [Logs dashboard](https://platform.openai.com/logs).

- **`$ai_provider_metadata`** – Provider-specific fields that don't fit the shared schema. For OpenAI, this includes `system_fingerprint` (the backend configuration that served the request) and `request_id` (the `x-request-id` header, useful for support tickets).

These properties are captured automatically when using the [OpenAI](/docs/llm-analytics/installation/openai) or [Azure OpenAI](/docs/llm-analytics/installation/azure-openai) wrappers.
...
||||||| 199860d0e:contents/docs/llm-analytics/generations.mdx
When calling LLMs with our [SDK wrappers](/docs/llm-analytics/installation), you can also enrich the `$ai_generation` event with your own [custom properties](/docs/llm-analytics/custom-properties) and PostHog attributes like groups and distinct IDs for identified users.
=======
When calling LLMs with our [SDK wrappers](/docs/ai-observability/installation), you can also enrich the `$ai_generation` event with your own [custom properties](/docs/ai-observability/custom-properties) and PostHog attributes like groups and distinct IDs for identified users.
>>>>>>> master:contents/docs/ai-observability/generations.mdx
...
Generations are nested under [spans](/docs/ai-observability/spans) and [traces](/docs/ai-observability/traces). 
...
When a generation includes tool calls (function calls), PostHog automatically extracts them and displays them as tags on the generation. You can see aggregated tool usage across all your generations in the [Tools](/docs/ai-observability/tools) tab.

## Evaluating generations

You can automatically assess the quality of your generations using [evaluations](/docs/ai-evals). Evaluations use an LLM-as-a-judge approach to score outputs based on criteria like relevance, helpfulness, or safety.

## Sentiment classification

PostHog can classify the sentiment of user messages in your generations as negative, neutral, or positive. Sentiment is computed on-demand using a local model when you view a [trace](/docs/ai-observability/traces) — no data is sent to third-party services. See [Sentiment classification](/docs/ai-observability/sentiment) for more details.
