...
> **Note:** `opencode-posthog` is a community-maintained plugin, not an official PostHog package.
...
- **`$ai_generation`**: Every LLM call, including model, provider, token usage, cost, stop reason, and message content in [OpenAI chat format](/docs/ai-observability/generations).
- **`$ai_span`**: Each tool execution, including tool name, duration, input and output state, and any error information ([learn more](/docs/ai-observability/spans)).
- **`$ai_trace`**: Completed user prompts with total latency, accumulated token counts, and final input and output state ([learn more](/docs/ai-observability/traces)).
