...
- **`$ai_generation`** — Every LLM call, including model, provider, token usage, cost, latency, and input/output messages (in [OpenAI chat format](/docs/ai-observability/generations)).
- **`$ai_span`** — Each tool execution (read, write, edit, bash, etc.), including tool name, input parameters, output result, and duration ([learn more](/docs/ai-observability/spans)).
- **`$ai_trace`** — Completed agent runs with aggregated token totals and latency ([learn more](/docs/ai-observability/traces)).
