...
posthog.init('<ph_project_token>', {
  ...
  tracing_headers: ['api.your-app.com']  // Your backend hostname(s)
})
```

Use hostnames only, without the protocol or path. This automatically adds `X-POSTHOG-SESSION-ID`, `X-POSTHOG-DISTINCT-ID`, and `X-POSTHOG-WINDOW-ID` headers to all `fetch` and `XMLHttpRequest` calls to the specified hostnames.
