...
1. A [personal API key](https://app.posthog.com/settings/user-api-keys) with `query:read` permissions
2. Your project ID (found in [project settings](https://app.posthog.com/settings/project#variables))
...
curl -H "Authorization: Bearer <ph_project_api_key>" \
  "<ph_app_host>/api/projects/<your_project_id>/web_analytics/overview?date_from=2024-01-01&date_to=2024-01-31"
```
...
url = f"<ph_app_host>/api/projects/{project_id}/web_analytics/overview"
headers = {
...
const response = await fetch(
  `<ph_app_host>/api/projects/${projectId}/web_analytics/overview?date_from=2024-01-01&date_to=2024-01-31`,
  {
    headers: {
...
```shell
curl -H "Authorization: Bearer <ph_project_api_key>" \
  "<ph_app_host>/api/projects/<your_project_id>/web_analytics/breakdown?date_from=2024-01-01&date_to=2024-01-31&breakdown_by=Page&limit=2"
```
...
url = f"<ph_app_host>/api/projects/{project_id}/web_analytics/breakdown"
headers = {
    "Authorization": f"Bearer {api_key}"
}
params = {
...
const response = await fetch(
  `<ph_app_host>/api/projects/${projectId}/web_analytics/breakdown?${params}`,
  {
    headers: {
      "Authorization": `Bearer ${apiKey}`
    }
...
```json
{
  "next": "<ph_app_host>/api/projects/<your_project_id>/web_analytics/breakdown?offset=2&limit=2",
  "results": [
    {
...
base_url = f"<ph_app_host>/api/projects/{project_id}/web_analytics"
...
(async () => {
  ...
  const baseUrl = `<ph_app_host>/api/projects/${projectId}/web_analytics`;
