...
The two tabs count activity differently:

- **Unique users** - Uses session-start attribution. Each session counts toward the hour bucket when it started. If a session starts at 11pm and continues past midnight, the user only appears in the 11pm bucket. This matches how visitors are counted in the overview tiles at the top of this dashboard.

- **Total pageviews** - Counts every pageview in the bucket where it occurred. A session spanning 11pm to 1am contributes pageviews to each hour bucket where they happened.

Because of this, unique users may appear lower than you'd expect compared to total pageviews, especially for sessions that span multiple hours.
