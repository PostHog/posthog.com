...
## Drilling down into tables

Clicking a row in the channels, UTMs, geography, or devices tables filters the dashboard by that value and automatically navigates to the next level of detail.

The drill-down chains work like this:

| Category | Drill-down path |
|----------|-----------------|
...
For example, clicking a channel like "Organic Search" filters by that channel and switches to the **Referring Domain** tab. Clicking a referring domain then switches to **Referring URL**.

At the end of a chain (like Referring URL, UTM Term, City, or Viewport), clicking a row applies the filter without switching tabs.

Clicking an already-filtered row removes the filter and stays on the current tab.
