...
Like other dashboards in PostHog, the web analytics dashboard is filterable. This means you can filter for data with certain event or person property values. Options include browser, path name, device type, country, UTMs, and [cohorts](/docs/data/cohorts). Just click the **Add filter** button next to the date range at the top of the dashboard.

> **Note:** Cohort filtering is available in the Filters V2 beta.

This enables you to dive into specific stats for regions, parts of the site, specific marketing campaigns, and specific groups of users.
