...
If your site uses multiple subdomains or domains, you can enable the **Include host** toggle to show the full host and path (e.g., `example.com/about`) instead of just the path (`/about`). This helps distinguish between the same path on different hosts. Note that click-to-filter is disabled when this toggle is active.
