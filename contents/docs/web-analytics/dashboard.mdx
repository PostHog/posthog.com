...
You can export your web analytics data as CSV files for external analysis or sharing.

To export all dashboard tiles at once, click the side menu and select **Export all as CSV (.zip)**. This downloads a ZIP archive containing a separate CSV file for each tile on the dashboard. For tiles with multiple tabs (like channels, referrers, and UTMs), only the currently active tab is included in the export.

You can also export individual tiles by clicking the **...** overflow menu on any tile and selecting **Download CSV**.

If some tiles are still loading when you trigger an export, you'll see a warning that the export may be incomplete.
