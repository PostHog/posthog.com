...
You can zoom into trend graphs by clicking or dragging:

**Click to zoom** – Click any data point to zoom in. The zoom level depends on your current view:

- **Monthly view** – zooms to that month, grouped by day
- **Weekly view** – zooms to that week, grouped by day
- **Daily view** – zooms to that day, grouped by hour

**Drag to zoom** – Drag across the graph to select a specific time range. The granularity adjusts automatically based on the range you select.

Click **Reset zoom** to return to your original view.
