...
Clicking a row in any of these tables filters the dashboard and automatically navigates to a more detailed breakdown. See [drill-down navigation](#drill-down-navigation) for details.
...
Clicking a country, region, or city filters the dashboard and navigates to a more detailed geographic breakdown (Country → Region → City).
...
When you click a row in Web Analytics tables, the dashboard automatically navigates to a more detailed breakdown. This creates drill-down chains that let you explore your data hierarchically:

- **Sources:** Channel → Referring Domain → Referring URL
- **UTMs:** Source → Medium → Campaign → Content → Term
- **Geography:** Country → Region → City
- **Devices:** Device Type → Browser → OS → Viewport

For example, clicking a channel like "Organic Search" filters the dashboard and switches to the Referring Domain tab, where you can then click a domain to drill down to specific Referring URLs.

End-of-chain tabs (Referring URL, UTM Term, City, Viewport) apply the filter without switching tabs since there's no deeper level to explore.

> **Note:** Drill-down navigation only triggers when adding a filter. Clicking to remove an existing filter keeps you on the current tab.
