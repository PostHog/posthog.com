...
### Including the host in path breakdowns

By default, path breakdowns only show the pathname (e.g., `/landing`). If you have multiple hosts or subdomains sending data to the same project, enable the **Include host** toggle to show the full host + path (e.g., `example.com/landing`). This makes it easier to distinguish between the same path on different domains.

> **Note:** This feature requires the `web-analytics-include-host` [feature flag](/docs/feature-flags) to be enabled.

When this toggle is enabled, clicking a row no longer filters the dashboard since the combined host + path value doesn't map to a single filter.
