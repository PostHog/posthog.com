...
### Choosing a distinct ID

Server requests don't carry a PostHog cookie, so the ingestion source has to derive a `distinct_id` for each `$http_log` event. The strategy you pick matters for two reasons:

- **Person profiles and cost.** Backend events are [identified](/docs/data/anonymous-vs-identified-events) by default, so every new `distinct_id` creates a person profile and is billed on the person-profiles line. A high-cardinality strategy – a fresh ID per request, or a hash that rotates daily – can create a large number of profiles, which adds cost and slows person-joined queries. If you only need aggregate traffic and bot counts, capture these as [anonymous events](/docs/data/anonymous-vs-identified-events#how-to-capture-anonymous-events) by setting `$process_person_profile` to `false`. Bot detection still works, since it reads event properties, not the person profile.
- **Unique counts and retention.** A daily-rotating ID counts the same crawler as a new visitor every day, which inflates unique counts and flattens retention. A stable per-client ID – for example a hash of IP, host, and user agent – keeps one identity per client.

The Vercel logs source exposes a **Distinct ID strategy** setting (rotating salt, fixed salt, raw IP, or a custom template) so you can balance privacy, unique-count accuracy, and the number of profiles created.
