...
Server requests don't carry a PostHog cookie, so the ingestion source derives a `distinct_id` for each `$http_log` event and decides whether that ID becomes a person profile. The Vercel logs source has two settings for this:

- **Person processing.** Backend events are [identified](/docs/data/anonymous-vs-identified-events) by default, which creates a person profile per `distinct_id` and bills on the person-profiles line. For high-cardinality log traffic that adds cost and slows person-joined queries. The source defaults to [anonymous](/docs/data/anonymous-vs-identified-events#how-to-capture-anonymous-events) (`$process_person_profile` set to `false`), so no person profiles are created – cheaper, faster to query, and bot detection still works since it reads event properties, not the profile. Choose **identified** only if you need person-level stitching.
- **Distinct ID strategy.** Once events are anonymous, the distinct ID no longer affects cost (no profiles either way) – it only affects unique-visitor counts. The default, **fixed salt**, is a hash of IP, host, and user agent, so the same client keeps one stable ID. Don't collapse everything onto a single shared ID, which would count all traffic as one visitor. Rotating salt adds the day to the hash for extra privacy, at the cost of inflated unique counts; raw IP and a custom template are also available.
