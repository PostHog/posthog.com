...
PostHog classifies traffic by user agent so you can tell humans apart from bots, crawlers, and automation directly in your queries. For bots from operators that publish their crawler IP ranges (currently Google, OpenAI, Perplexity, Microsoft, Apple, and Ahrefs), classification also checks the client IP as a fallback when the user agent doesn't match a known pattern. It categorizes each request – so you can welcome, measure, or exclude AI agents, search crawlers, and automation independently, rather than treating every bot the same. The classification runs in [SQL](/docs/sql), so it works anywhere HogQL does – the SQL editor, insights, trends, and Web Analytics breakdowns.
...
- **IP coverage** – IP range detection currently covers published crawler ranges from Google, OpenAI, Perplexity, Microsoft (Bing), Apple, and Ahrefs. Bots from operators that don't publish their IP ranges won't be caught by the IP check.
