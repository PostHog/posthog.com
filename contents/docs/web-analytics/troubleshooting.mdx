...
You can filter by traffic type properties in insights, dashboards, and Web Analytics to analyze bot traffic or exclude it:

- **Is bot** - Whether the event came from a bot, crawler, or automation tool
- **Traffic type** - Regular, Bot, AI Agent, or Automation
- **Traffic category** - More specific categories like llm_crawler, search_crawler, seo_crawler, or regular
- **Bot name** - The detected bot or crawler (e.g., Googlebot, ChatGPT, Claude)

To filter out bot traffic from a trend insight, add a filter where **Traffic type** does not equal "Bot".
