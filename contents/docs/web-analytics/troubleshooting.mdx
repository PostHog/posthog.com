---
title: Web Analytics troubleshooting
sidebar: Docs
showTitle: true
---
...
This page covers troubleshooting for Web Analytics. For setup, see the [installation guides](/docs/web-analytics/installation).
...
All good web analytics dashboards need to include a world map. Ours includes it too. If you can't see it, you need to enable the [GeoIP transformation](https://app.posthog.com/data-management/transformations?search=GeoIP) in Data pipelines.

## Do stats include bots and crawlers?

By default, no. The PostHog JavaScript SDK blocks known bots and crawlers client-side, so they never send events – you can see the [list of blocked user agents](https://github.com/PostHog/posthog-js/blob/main/packages/browser/src/utils/blocked-uas.ts) (pull requests welcome). If you want to capture that traffic instead of blocking it, set the `opt_out_useragent_filter` [config option](/docs/libraries/js/config) to `true` – PostHog then sets `$browser_type` to either `bot` or `browser` instead of dropping bot events entirely.
...
While PostHog blocks known bots from analytics by default, you can use [bot and traffic detection](/docs/web-analytics/bot-detection) functions to analyze bot traffic patterns or build custom filtering logic. These functions are available anywhere HogQL runs, including the [SQL editor](/docs/data-warehouse/sql), trends, and custom queries.

| Function                                         | Returns                                                            |
| ------------------------------------------------ | ------------------------------------------------------------------ |
| `isLikelyBot(properties.$raw_user_agent)`        | `true` if the user agent matches bot or automation patterns        |
| `getTrafficType(properties.$raw_user_agent)`     | One of 'AI Agent', 'Bot', 'Automation', or 'Regular'               |
| `getTrafficCategory(properties.$raw_user_agent)` | Subcategory like 'ai_crawler', 'ai_search', 'search_crawler', etc. |
| `getBotName(properties.$raw_user_agent)`         | Specific bot name like 'Googlebot' or 'ChatGPT'                    |
| `getBotOperator(properties.$raw_user_agent)`     | Operator or company name like 'Google' or 'OpenAI'                 |
| `getBotType(properties.$raw_user_agent)`         | Bot category or empty string for regular traffic                   |

For example, to see which bots are visiting your site:
...
SELECT
    ...
    count() AS hits
FROM events
WHERE isLikelyBot(properties.$raw_user_agent)
GROUP BY bot, traffic_type
ORDER BY hits DESC
```

For the full list of supported HogQL functions, see the [ClickHouse functions](/docs/sql/clickhouse-functions) reference.

## Why is my pageview/user count different on PostHog than my other analytics tool?

**1. Ad blockers**

_Affects: visitor count, session count, pageview count_
...
Our client-side bot detection code is available on [GitHub](https://github.com/PostHog/posthog-js/blob/main/packages/browser/src/utils/blocked-uas.ts) (pull requests welcome!) and we automatically prevent detected bots from sending events.
...
For tracking users across different domains or websites, for example a marketing site at `marketing.com` and an app at `app.com`, or in some cases between subdomains, you might need to set up [cross-domain tracking](/tutorials/cross-domain-tracking) to ensure that sessions are correctly attributed. This typically means you need to pass the user's `distinct_id` and `session_id` between domains but check the guide for complete instructions.

This ensures that:

- Campaign attribution is preserved across domain transitions
- User journeys are tracked accurately from first touch to conversion
- Analytics data from both sites is associated with the same user profile
...
Some Web Analytics tiles show a small database-with-a-lightning-bolt icon in their top-right corner. It means the tile loaded from a pre-computed state: we calculated and stored the result ahead of time, rather than running a live query against all your events when you opened the tile.

Pre-computed results have a TTL and are re-computed on a regular cadence, so the data you see should be fresh. They should be very close to a live query, but aren't guaranteed to be exact.

The trade-off is deliberate: on larger datasets, reading a pre-computed result is much faster than scanning all your events on every load. That speed gain is what makes the small loss of precision worthwhile.

This is currently rolling out and is in test. We pre-compute up to 90 days of data, and we don't use it for very large time ranges yet – those tiles continue to run live. To force every eligible tile to query live data, open the settings menu in the Web Analytics dashboard and turn off **Allow precompute**.

## Solved community questions

<SolvedQuestions topicLabel="Web Analytics" />
