...
Some Web Analytics tiles show a small database-with-a-lightning-bolt icon in their top-right corner. It means the tile's data is pre-computed: we calculated it and had it ready ahead of time, rather than running a live query when you opened the tile.
...
The trade-off is deliberate: on larger datasets, reading a pre-computed roll-up is much faster than scanning your raw events on every load. That speed gain is what makes the small loss of precision worthwhile.
