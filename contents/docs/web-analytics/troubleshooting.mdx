...
Some Web Analytics tiles show a small icon in their top-right corner – a lightning bolt, or a database with a lightning bolt. It means the tile's data is pre-computed: we calculated it and had it ready ahead of time, rather than running a live query when you opened the tile.
