...
Clicking a row in these tables filters the dashboard and automatically navigates to more detailed breakdowns. For example, clicking a channel switches to the Referring Domain tab, and clicking a domain switches to Referring URLs.
...
Clicking a row in the device or geography tables filters the dashboard and navigates to more detailed breakdowns (Device Type → Browser → OS → Viewport for devices, Country → Region → City for geography).
