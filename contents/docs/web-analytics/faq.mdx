...
The simple answer: PostHog consumes resources (CPU) and execution time to load the resources that make it work. This isn't without cost, but PageSpeed Insights makes this cost seem larger than it actually is. 
...
We load most of these assets from an external `.js` file **asynchronously**. This means it doesn't block other assets, such as the ones required to render the page. In PageSpeed terms,  This minimizes the impact on First Contentful Paint (FCP) and Largest Contentful Paint (LCP). We also serve these assets from a CDN to speed them up.
...
3. Although we don't recommend it, you could lazy load PostHog by having it trigger on scroll or a timer. This can miss initial events, start session replays later, and delay other features. 
