...
## How do I set up attribution tracking across multiple websites?

If you're running campaigns or tracking users across different domains (for example, a marketing website at `example.com` and an app at `app.example.com` or even a completely different domain), you'll need to ensure proper attribution setup.

### Cross-domain tracking for attribution

PostHog automatically handles cross-subdomain tracking when using first-party cookies. For tracking across different domains, you need to pass the user's `distinct_id` and `session_id` between domains. See our [cross-domain tracking tutorial](/tutorials/cross-domain-tracking) for detailed implementation steps.

This ensures that:
- Campaign attribution is preserved across domain transitions
- Session replays continue across domains
