...
## How do I preserve UTM parameters across domains?

When linking between domains, make sure to include your UTM parameters in the URL. For example:

```
https://app.yoursite.com/?utm_source=linkedin&utm_medium=paid-social&utm_campaign=winter-2025
```

PostHog automatically captures and stores UTM parameters (`utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, and `utm_content`) as person properties on first visit, which means they persist throughout the user's journey even if the URL parameters are removed.
