...
For tracking users across different websites (for example, a marketing site at `marketing.com` and an app at `app.com`), PostHog automatically handles cross-subdomain tracking when using first-party cookies. For tracking across completely different domains, you need to pass the user's `distinct_id` and `session_id` between domains. See our [cross-domain tracking tutorial](/tutorials/cross-domain-tracking) for detailed implementation steps.
