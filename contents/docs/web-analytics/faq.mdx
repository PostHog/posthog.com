...
## Why are my campaign parameters lost from in-app browsers?

Links opened from mobile apps (like email apps, messaging apps, or social media in-app browsers) often lose attribution data when redirecting to the default browser. This is especially common with:

- **iOS Mail app and Messages**: Strip ad-specific parameters like `gclid` and `fbclid`
- **Social media in-app browsers**: May not preserve UTM parameters when opening links externally
- **iOS 17+ Link Tracking Protection**: Can strip tracking parameters from URLs in Safari and other apps

**Solutions:**
1. Use UTM parameters instead of platform-specific tracking parameters (like `gclid` or `fbclid`) as they're less likely to be stripped
2. Implement [deep links](https://developer.apple.com/documentation/xcode/supporting-universal-links-in-your-app) on iOS and [app links](https://developer.android.com/training/app-links) on Android to maintain tracking when redirecting to mobile apps
3. For LinkedIn ads specifically, use their [dynamic UTM parameters](https://learn.microsoft.com/en-us/linkedin/marketing/integrations/ads-reporting/dynamic-utm-tracking) feature to automatically tag campaigns at the campaign level
4. Consider using a reverse proxy to prevent tracking blockers from stripping parameters
