...
## Why are my UTM parameters lost in redirects?

If you're using server-side redirects (like 301 or 302 redirects), ensure that your redirect preserves URL parameters. A redirect from:

```
example.com/promo?utm_source=linkedin
```

Should redirect to:

```
example.com/landing?utm_source=linkedin
```

Not just to `example.com/landing` (which loses the parameter).
