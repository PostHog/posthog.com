...
1. **Use consistent naming**: Always use lowercase (e.g., `linkedin`, not `LinkedIn`) to prevent data fragmentation
2. **Never use UTMs for internal links**: Only use UTM parameters when linking to your site from external sources
3. **Standardize your format**: Use hyphens for multi-word values (e.g., `winter-2025` not `winter_2025` or `Winter 2025`)
4. **Set up a naming convention**: Document your UTM naming standards and share them with your team to ensure consistency across campaigns
