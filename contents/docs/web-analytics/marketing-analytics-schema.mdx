...
| Column | Description |
|--------|-------------|
| `Name` | Name of the campaign, ad group, or ad (depending on your selected [drill-down level](/docs/web-analytics/marketing-analytics#performance-breakdown)). At campaign level, matched against `utm_campaign` in your conversion events if matching field is `Campaign name`. |
| `Source` | Ad platform identifier (e.g., `google`, `meta`, `linkedin`). Matched against `utm_source` in your conversion events. |
| `ID` | Platform-specific identifier for the campaign, ad group, or ad. At campaign level, matched against `utm_campaign` in your conversion events if matching field is `Campaign ID`. |
