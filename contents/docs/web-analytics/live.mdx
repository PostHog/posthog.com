...
- **Session Replay** – when [Session Replay](/docs/session-replay) is enabled, each person row shows a play button with the number of recordings from the last 30 minutes. Click it to jump to that person's session recordings.
...
- [Session Replay](/docs/session-replay) – watch real user sessions to debug issues
