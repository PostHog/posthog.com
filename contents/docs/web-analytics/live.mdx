...
- **Recording** – a link to watch the session replay (if [session recording](/docs/session-replay) is enabled)
...
When you click on a person in the feed, you can see details about their current session. If they have session recordings available from the last 30 minutes, you'll see a button showing the recording count that links directly to their recordings.
