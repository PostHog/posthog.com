...
In the context of building a successful product, this data is invaluable. Think of your product as a ship ⛵ out in the middle of the ocean in the dead of night 🌃. There are plenty of ways to make it back to land, but there's no guarantee you'll get there. You'll need to use all the data available to you to get a sense of direction and forge ahead, lest ye be torn asunder. 🌊
...
Next, let's take a look at some practical examples of what to do with all that web analytics data.
...
Based on the analytics, maybe it's time to double down on that feature that 80% of your users are there for? Or maybe it's time to shutter that one feature that only 5% care about when it's generating 90% of your support issues (more on this later)?
...
If you live in San Francisco, it's safe to assume all your users live there too, right? Oh weird, it turns out half of the traffic is coming from Paris? Would it be considered gauche to put a billboard on the Louvre Pyramid? Or maybe it's time to add a French translation?

#### 🔄 Retention: Are visitors coming back?

It looks like 100% of the people visiting for the first time are visiting for the first time (it's a relief that objective mathematical reality works here). And one week later, maybe 5% of visitors are coming back. That's alright it makes sense that only a subset are going to come back. But what if, after two weeks, it's a precipitous drop down to 1%? Maybe you could re-engage users with a timely email after two weeks?
