...
PostHog converts currency values for the entire team to the base currency you set in [your project settings](https://app.posthog.com/settings/project#base-currency) before displaying them. This applies to marketing analytics as well as [insight visualizations](/docs/product-analytics/insights) like trends, funnels, and tables. If we can't properly detect your currency from the source data, we'll assume it's in the base currency as well.
