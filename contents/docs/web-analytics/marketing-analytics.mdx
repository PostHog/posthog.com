...
- **Google Ads**: [Setup guide](/docs/cdp/sources/google-ads) - requires `campaign` and `campaign_stats` tables
- **LinkedIn Ads**: [Setup guide](/docs/cdp/sources/linkedin-ads) - requires `campaigns` and `campaign_stats` tables  
- **Meta Ads**: [Setup guide](/docs/cdp/sources/meta-ads) - requires `campaigns` and `campaign_stats` tables
- **TikTok Ads**: [Setup guide](/docs/cdp/sources/tiktok-ads) - requires `campaigns` and `campaign_report` tables
- **Reddit Ads**: [Setup guide](/docs/cdp/sources/reddit-ads) - requires `campaigns` and `campaign_report` tables
...
You can connect external data warehouse sources (currently BigQuery) to display marketing analytics. You'll need to map the required columns for each table to enable the functionality.
