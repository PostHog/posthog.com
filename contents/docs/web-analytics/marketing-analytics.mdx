...
The breakdown table shows performance metrics at different levels of your ad hierarchy. Use the drill-down selector to choose your level of detail:

- **Campaign** (default): View performance aggregated by campaign
- **Ad group**: Break down performance by ad group within campaigns
- **Ad**: See metrics for individual ads

<CalloutBox icon="IconInfo" title="Additional tables required for deeper drill-down" type="fyi">

Ad group and ad-level drill-down require additional tables beyond the base campaign tables. If these aren't syncing, you'll see a banner linking to your source sync settings. See the [native sources table](#native-sources) for the required tables at each level.

</CalloutBox>

The breakdown table provides detailed performance metrics:

- **Name**: Identifier for the campaign, ad group, or ad (depending on drill-down level)
...
| Platform | Setup Guide | Campaign Tables | Ad Group Tables | Ad Tables |
|----------|------------|-----------------|-----------------|-----------|
| Google Ads | [Setup guide](/docs/cdp/sources/google-ads) | `campaign`, `campaign_overview_stats` | `ad_group`, `ad_group_stats` | `ad`, `ad_stats` |
| LinkedIn Ads | [Setup guide](/docs/cdp/sources/linkedin-ads) | `campaign_groups`, `campaign_group_stats` | `campaigns`, `campaign_stats` | `creatives`, `creative_stats` |
| Meta Ads | [Setup guide](/docs/cdp/sources/meta-ads) | `campaigns`, `campaign_stats` | `adsets`, `adset_stats` | `ads`, `ad_stats` |
| Pinterest Ads | [Setup guide](/docs/cdp/sources/pinterest-ads) | `campaigns`, `campaign_analytics` | `ad_groups`, `ad_group_analytics` | `ads`, `ad_analytics` |
| TikTok Ads | [Setup guide](/docs/cdp/sources/tiktok-ads) | `campaigns`, `campaign_report` | `ad_groups`, `ad_group_report` | `ads`, `ad_report` |
| Reddit Ads | [Setup guide](/docs/cdp/sources/reddit-ads) | `campaigns`, `campaign_report` | `ad_groups`, `ad_group_report` | `ads`, `ad_report` |
| Bing Ads | [Setup guide](/docs/cdp/sources/bing-ads) | `campaigns`, `campaign_performance_report` | `ad_group_performance_report` | `ad_performance_report` |
| Snapchat Ads | [Setup guide](/docs/cdp/sources/snapchat-ads) | `campaigns`, `campaign_stats_daily` | `ad_squads`, `ad_squad_stats_daily` | `ads`, `ad_stats_daily` |

Each native source automatically handles data formatting at the campaign level. To use ad group or ad-level drill-down, sync the additional tables shown above. The configuration interface shows which tables are syncing and which still need to be enabled.
