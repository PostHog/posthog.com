...
The **Marketing analytics** feature allows you to track marketing information, costs and performance from multiple advertising platforms in your [marketing analytics dashboard](https://app.posthog.com/web/marketing).
...
The marketing costs chart displays your total advertising spend aggregated across all configured data sources:
...
- **Source**: Ad platform name (e.g., google, shuld match to utm_source value)
...
The simplest way to add marketing data is to [add a Google Ads source](https://us.posthog.com/project/2/pipeline/new/source?kind=googleads) by following [this guide](https://posthog.com/docs/cdp/sources/google-ads).
...
Configure external data warehouse sources to display marketing analytics. You'll need to map the required columns for each table to enable the functionality. Currently only BigQuery tables are supported.
...
Configure self-managed sources from S3, GCS, Cloudflare R2, and Azure. You'll need to map the required columns for each table to enable the functionality.
...
Please refer to the [Data Warehouse docs](/data-warehouse) for more information on configuring external data sources.
