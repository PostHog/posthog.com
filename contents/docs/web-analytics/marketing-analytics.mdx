...
The breakdown table shows performance metrics for your marketing data. You can switch between three aggregation levels using the segmented button above the table:

- **Channel** - groups data by channel type derived from your sources using web analytics classification (e.g., Paid Search, Organic Social, Direct)
- **Source** - groups data by normalized source name (e.g., `google`, `facebook`)
- **Campaign** - groups data by individual campaign, including campaign name, ID, and source (default)

Each level displays the following metrics:

- **Total cost** - total spend in your base currency
