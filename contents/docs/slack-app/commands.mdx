...
For repo selection, the bot checks routing rules first. If nothing matches and you have multiple repos, it opens the in-thread picker. If you only have one repo connected, the task runs against it directly. The `project` commands only matter if your Slack workspace is connected to multiple PostHog projects - see [Pick a project](/docs/slack-app/setup#pick-a-project-multi-integration-workspaces).
