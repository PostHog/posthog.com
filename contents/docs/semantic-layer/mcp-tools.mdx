...
| Tool | What it does | Confirmation | Scopes |
|------|--------------|--------------|--------|
| `data-catalog-certification-propose` | Proposes a trust mark on a warehouse table or view. Address the target by id or by name – an ambiguous name returns the candidate ids to pick from. Pass `proposed_status` (`certified` by default, or `deprecated`) to say whether the proposal asks to trust the source or to avoid it. | – | `data_catalog:write` |
