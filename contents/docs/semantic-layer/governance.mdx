...
Every promoting or vetoing action requires you to reply with the literal word "confirm" before it executes. Each ships as a `-prepare` / `-execute` [tool pair](/docs/semantic-layer/mcp-tools#how-confirmations-work): `-prepare` returns a signed confirmation hash, and `-execute` runs only once you've typed the word.
