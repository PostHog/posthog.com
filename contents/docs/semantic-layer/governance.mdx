...
1. **Agents propose as they go.** When an agent derives a number that gets reused, it offers to save the derivation as a proposed metric. When it notices a stale table it proposes deprecating it, and when it spots an obvious join it proposes the relationship with evidence.
