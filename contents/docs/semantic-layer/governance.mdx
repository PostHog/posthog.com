...
Every promoting or vetoing action requires the user to reply with the literal word "confirm" before it executes. Each ships as a `-prepare` / `-execute` [tool pair](/docs/semantic-layer/mcp-tools#how-confirmations-work): `-prepare` returns a signed confirmation hash, and `-execute` runs only once you've typed the word.

- `data-catalog-metric-approve-prepare` / `-execute` – additionally blocked while the metric is drifted
- `data-catalog-certification-certify-prepare` / `-execute`
- `data-catalog-certification-deprecate-prepare` / `-execute`
- `data-catalog-relationship-accept-prepare` / `-execute`
- `data-catalog-relationship-reject-prepare` / `-execute`
...
Practical setup: the API key you hand your MCP server or CI agent gets `data_catalog` only – it can build and use the catalog but never bless anything. Your own key carries both scopes, so approvals happen through you. (`data-catalog-metric-run` also needs `query:read`, and `data-catalog-relationship-accept-prepare` / `-execute` needs `warehouse_view:write` to create the join. `data-catalog-metrics-refresh-from-insight-create` needs `insight:read` to re-read the source insight.)
