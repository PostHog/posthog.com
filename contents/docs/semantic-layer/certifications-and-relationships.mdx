...
The flow mirrors the rest of the semantic layer: anyone proposes with [`data-catalog-certification-propose`](/docs/semantic-layer/mcp-tools#certification-tools) (targets can be addressed by ID or by name – an ambiguous name returns candidates to pick from), and a human resolves it with `data-catalog-certification-certify-prepare` / `-execute` or `data-catalog-certification-deprecate-prepare` / `-execute`, both of which require typing "confirm". Revoking a certification deletes it outright, with the change activity-logged.
...
Acceptance and rejection both go through a human with a typed "confirm" ([`data-catalog-relationship-accept-prepare` / `-execute`](/docs/semantic-layer/mcp-tools#relationship-tools) and `data-catalog-relationship-reject-prepare` / `-execute`).
