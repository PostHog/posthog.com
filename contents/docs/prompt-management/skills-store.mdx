...
1. Navigate to **Prompt Management** > **Skills**
2. Click **New skill**
...
Share a direct link to any bundled file in a skill. When someone opens the link, the file viewer expands, loads the content, and scrolls it into view.

To copy a link, click the link icon next to any file in the skill's file list. If you're viewing a historical version, the URL includes the version number so recipients see the same version.

The URL uses a `?file=` query parameter with the file path:

```text
https://app.posthog.com/prompt-management/skills/hog-release-notes?file=references/changelog.md
```

Add `version` to link to a file in a specific skill version:

```text
https://app.posthog.com/llm-analytics/skills/hog-release-notes?file=scripts/format.py&version=2
```
