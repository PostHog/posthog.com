...
| `prompt-get`    | Get a prompt by name, including full content. Optionally pass `label` or `version` to fetch a specific version |
...
Labels are movable pointers that map a human-readable name (like `production` or `staging`) to a specific prompt version. Unlike version numbers, which are fixed when a version is published, a label can be moved to a different version at any time — so your code always fetches the version the label points to without a deploy.

### How labels work

- A label points to exactly one version of a prompt
- Moving a label to a new version takes effect immediately for all consumers
- Each prompt can have up to 50 labels
- Label names use lowercase letters, numbers, dots, hyphens, and underscores
- `latest` is reserved and numbers-only names are not allowed

### Set a label

1. Open a prompt and select a version from the **Version history** sidebar
2. Click **Set label** and enter a label name (e.g. `production`)
3. If the label already exists on another version, it is moved to the selected version

You can also set labels through the MCP `prompt-label-set` tool or the API.

### Delete a label

Remove a label when it is no longer needed. Any code fetching the prompt by that label will receive a 404 after deletion.

You can delete labels through the MCP `prompt-label-delete` tool or the API.
...
# Or fetch by label (movable pointer to a version)
result = prompts.get(
    'support-system-prompt',
    with_metadata=True,
    label='production',
...
To track which prompts and versions are used in which generations, include the `$ai_prompt_name`, `$ai_prompt_version`, and `$ai_prompt_label` properties when capturing:
...
response = client.chat.completions.create(
    ...
    posthog_properties={
        ...
        "$ai_prompt_version": 2,
        "$ai_prompt_label": "production"  # If fetched by label
    }
)
```
...
- Filter generations by prompt name, version, or label
- View related traces from the prompt detail page
- See which specific version and label was used in each trace
