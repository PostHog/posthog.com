...
Prompt label changes are tracked in [activity logs](/docs/settings/activity-logs). When a label is created, moved to a different version, or deleted, the change is recorded with:

- Who made the change
- Which label (shown as `prompt-name: label-name`, e.g. `support-system-prompt: production`)
- The from/to version numbers for moves (e.g. v2 → v6)

No-op label changes (pointing a label at the version it already targets) are not logged.

Archiving a prompt deletes its labels, and each label removal is individually recorded in the activity log.
