...
| `skill-rename`      | Rename a skill – keeps its version history, bundled files, and owners             |
...
allowed-tools: mcp__posthog__skill-list, mcp__posthog__skill-get,
  ...
  mcp__posthog__skill-update, mcp__posthog__skill-rename,
  mcp__posthog__skill-file-create, mcp__posthog__skill-file-delete,
  mcp__posthog__skill-file-rename, mcp__posthog__skill-duplicate
---
...
Use `skill-rename` to change the name of a skill. The skill keeps its version history, bundled files, and owners – only the name changes:

```json
// skill-rename
{
  "skill_name": "hog-release-notes",
  "new_name": "posthog-release-notes"
}
```

A rename takes only the two names – it does not accept `base_version`. The name is the slug agents call with `skill-get`, and the directory name in a zip export, so a rename changes both at the same time. Update any bridge skill, `allowed_tools` list, or other skill that refers to the old name.

Two rules apply to the new name: it must be unique in the project, and the `signals-scout-` and `review-hog-` prefixes are reserved for PostHog-managed skills that key their own settings on the skill name.
