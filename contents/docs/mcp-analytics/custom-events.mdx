...
Pass an `eventProperties` callback to attach extra properties to automatically captured MCP events. The callback runs per request, so values can depend on the current call (request id, transport, headers, env, region, deploy SHA, etc).
...
Return constants from `eventProperties` to add the same values to captured MCP events. This is similar to `posthog.register(...)` in other SDKs. The callback runs for each event, so values can vary by request.
