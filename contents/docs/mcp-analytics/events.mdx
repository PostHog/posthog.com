...
| `$mcp_llm_model` | string | The model from recognized client metadata or an SDK-injected `llm_model` argument. Available in both SDKs when model capture is enabled (the default since TypeScript 0.17.0 and Python 7.56.0). Missing, blank, and `unknown` values are omitted. Both sources are unverified. |
