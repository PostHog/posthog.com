...
| `$mcp_custom` | A call to `capture` with no custom `event` name | Your `properties`, plus `$mcp_resource_name`, `$mcp_parameters`, `$mcp_response` if you passed them. A custom `event` name sends an event of that name verbatim instead. |
