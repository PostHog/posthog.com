...
| `mcp_custom` | A call to `publishCustomEvent` with no recognized `eventType` | Whatever you passed in `tags` and `properties` |
| `mcp_<custom>` | A `publishCustomEvent` with a custom `eventType` — slashes are normalized to underscores | Whatever you passed |
| `posthog_identify` | `identify()` returns a new identity for a session | `$set` populated from `userName` and `userData` |
...
Set on `posthog_identify` events when `identify()` returns a user.
