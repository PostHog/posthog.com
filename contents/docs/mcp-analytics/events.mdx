...
| _(your event name)_ | A call to `capture(server, { event, properties })` | Sent under the verbatim `event` name you pass (a customer event, not `$`-prefixed), with your `properties` merged in. See [Custom events](/docs/mcp-analytics/custom-events). |
...
| (any) | Top-level keys of `UserIdentity.userData` are spread onto `$set` (e.g. return `userData: { email }` to set a person's email) |
