...
| `$exception` | Sibling event whenever a tool errors | `$exception_list`, `$exception_level`, plus the same `$mcp_*` context as the main event |
...
Present on `$exception` events emitted alongside any failed tool call. The SDK reuses `@posthog/core`'s error-tracking parser, so these are the same `$exception_list` properties every other PostHog SDK emits — they slot straight into [Error tracking](/docs/error-tracking).
