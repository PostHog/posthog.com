...
Both SDKs capture resource events. The event enum also reserves `$mcp_prompts_list` and `$mcp_prompt_get`, but the server wrappers don't emit them yet; prompt payloads pass through unchanged.
