...
The SDK supports any TypeScript MCP server built on `@modelcontextprotocol/sdk`. The fastest way to get set up is the wizard, which installs the package and wires up `instrument()` for you:
