...
## Quick install with the wizard

The fastest way to get set up is the [PostHog wizard](/docs/ai-engineering/ai-wizard), which installs the package, adds your `posthog-node` client, and wires up the `instrument()` call for you:
