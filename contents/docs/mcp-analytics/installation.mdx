...
Starting with `@posthog/mcp` 0.17.0 and Python `posthog` 7.56.0, model capture and conversation IDs are enabled by default, alongside intent and exception capture. [Missing-capability reporting](/docs/mcp-analytics/missing-capability) and agent feedback remain opt-in.
...
For finer, per-conversation grouping, [`enableConversationId`](/docs/mcp-analytics/conversation-id)
is enabled by default. The SDK adds a `conversation_id` argument and returns a handle in eligible
tool results. The tool schema asks the agent to reuse it on later calls. Correlation is best-effort:
some clients ignore the handle or treat tool output as untrusted content.
...
Model capture is enabled by default in both SDKs. The SDK reads recognized client metadata first, then falls back to the agent's `llm_model` argument. It captures `$mcp_llm_model` with `$mcp_llm_model_source` set to `"client_metadata"` or `"self_reported"`.

For compatible tool schemas, the TypeScript SDK advertises `llm_model` as required. Python does the same on official high-level adapters and custom dispatchers, but advertises it as optional on raw low-level servers and standalone FastMCP. Dispatch never enforces the injected field.

The SDK removes arguments it can confirm it injected before your handler runs. A fresh low-level instance that hasn't served `tools/list` can read `llm_model`, but leaves arguments untouched. This can capture an application-owned `llm_model` value until the instance learns the tool's schema. High-level adapters use the registered schema to preserve application-owned arguments.

Both sources are unverified client input. Use them to compare tool quality, latency, and errors by model, not for billing or security decisions. Missing, blank, or `unknown` values are omitted. Reasoning effort isn't captured.

To disable model capture and conversation IDs, pass `{ captureModel: false, enableConversationId: false }` in TypeScript or `MCPAnalyticsOptions(capture_model=False, enable_conversation_id=False)` in Python. Existing explicit opt-outs remain in effect after upgrading.

Model capture works on both protocol revisions. See [Python](#python) for the standalone FastMCP limitation and [MCP SDK v2](/docs/mcp-analytics/sdk-v2#mcp-apps) for current MCP Apps coverage.
...
With `@posthog/mcp` 0.17.0 or later, `instrumentMutator(posthog)` enables model capture and conversation IDs, including on fresh low-level server instances created per request. Conversation correlation still requires the agent to echo the handle.
...
instrument(server, posthog, MCPAnalyticsOptions(
    ...
    enable_conversation_id=True,  # stitch calls across reconnects (default)
...
| `enable_conversation_id` | `bool` | `True` | Inject an optional `conversation_id` argument to stitch calls when the agent echoes the handle. |
...
Python supports [model capture](#capture-the-calling-model), enabled by default starting with `posthog` 7.56.0. On standalone FastMCP with MCP SDK 1.x, middleware that can change tool listing or dispatch disables `llm_model` injection. This includes pass-through middleware that overrides those hooks. Model capture from recognized client metadata remains enabled.
...
| `captureModel` | `boolean \| { description: string }` | `true` | Capture the model from recognized client metadata or an SDK-injected `llm_model` argument. Enabled by default since `@posthog/mcp` 0.17.0. |
...
TypeScript also captures resource discovery and reads. Resource bodies pass through unchanged and aren't captured. See the [event reference](/docs/mcp-analytics/events) for SDK coverage. Prompt requests don't emit automatic analytics yet.
