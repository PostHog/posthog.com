...
You bring your own [`posthog-node`](/docs/libraries/node) client (the same pattern as [`@posthog/ai`](/docs/ai-engineering)) and pass it to `instrument()`. You own its lifecycle — call `posthog.shutdown()` or `posthog.flush()` yourself. If you don't pass a `posthog` client, the SDK installs the wrapper but captures nothing — that's how you turn capture off.
...
`instrument()` accepts these options. Pass a `posthog` client to actually send events; without one the wrapper installs but captures nothing.

| Option | Type | Default | What it does |
|---|---|---|---|
| `posthog` | `PostHog` (from `posthog-node`) | — | Your own `posthog-node` client. The SDK sends events through it but does not own its lifecycle — you call `posthog.flush()` / `posthog.shutdown()`. Omit it to disable capture. |
