...
`instrument()` needs the MCP SDK at runtime, but you already have it — you built your server with `mcp` or `fastmcp`, so it's treated as a peer dependency rather than bundled. Both official `mcp` majors are supported (`mcp>=1.26,<3`) and detected at runtime. jlowin's standalone `fastmcp` package is also supported. (`PostHogMCP` for custom dispatchers needs nothing beyond `posthog`.)
...
On Python's standalone FastMCP with MCP SDK 1.x, middleware that overrides tool-listing or dispatch hooks disables `llm_model` injection, even if the middleware only passes requests through. Capture from recognized client metadata still works.
