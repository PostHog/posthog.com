...
- **Model capture** — preparation helpers support it in both SDKs, enabled by default in TypeScript 0.17.0 and Python 7.56.0. Use `prepareToolList()` to advertise the argument, then `prepareToolCall()` to extract it. Pass its `llmModel` and `llmModelSource` to `captureToolCall()`.
