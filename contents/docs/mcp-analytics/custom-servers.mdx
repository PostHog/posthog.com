...
| Field | Maps to | Notes |
|---|---|---|
| `distinctId` | `distinct_id` | Supplying it enables person processing so `setProperties` lands on a real person. Omit it for anonymous traffic — events are sent with `$process_person_profile: false`. |
...
| `setProperties` | Person properties | Set on the person profile (`{ name, email, plan }`), same as the `properties` you'd pass to `identify`. Not stored on the event. |
