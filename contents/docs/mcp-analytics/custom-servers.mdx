...
`PostHog::MCP::Client` ships in `posthog-ruby` and is **experimental**: method signatures may change in a minor release. It needs nothing beyond `posthog-ruby` – no `mcp` gem.

</CalloutBox>

The Ruby SDK ships the same custom-dispatcher path as `PostHog::MCP::Client`, a subclass of the [`posthog-ruby`](/docs/libraries/ruby) client. Method names are snake_case and arguments are keywords:

```ruby
require "posthog/mcp"

posthog = PostHog::MCP::Client.new(api_key: "phc_your_project_api_key", host: "https://us.i.posthog.com")

# Advertise your tools with the injected `context` intent argument (and, optionally,
# the get_more_tools virtual tool). Tools are `tools/list` entries (Hashes).
tools = posthog.prepare_tool_list(my_tools, report_missing: true)
...
def handle_tools_call(request, name, arguments)
  ...
  # Pass these on every capture — see "Attributing the caller" above:
  common = {
    distinct_id: user_id,
    session_id: mcp_session_id,
    client_user_agent: request.get_header("HTTP_USER_AGENT"),
    vendor_client: request.get_header("HTTP_X_ANTHROPIC_CLIENT"),
    groups: { organization: org_id }
  }
  ...
  begin
    ...
    posthog.capture_tool_call(
      ...
      is_error: true,
      error: e, # → $mcp_error_message, $mcp_error_type, and the $exception sibling
      **common
    )
    raise
  end
...
`PostHog::MCP.encode_session_id` and `PostHog::MCP.decode_session_id` implement the `Mcp-Session-Id` token for your HTTP layer, or add `PostHog::MCP::RackMiddleware` to a Rack stack to mint and decode it for you. `PostHog::MCP::Client` is a `posthog-ruby` client, so flush it yourself with `posthog.flush` or `posthog.shutdown`.
