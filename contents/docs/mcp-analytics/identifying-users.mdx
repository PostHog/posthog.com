...
2. Emits a `posthog_identify` event the first time the identity is observed (or whenever it changes for that session), with the `$set` properties populated from `userName` and `userData`.
...
When `identify` eventually returns a user, the SDK emits `posthog_identify` with `$anon_distinct_id` set to the prior session id. PostHog's identity-merging logic then attributes the anonymous events to the identified user. From that point on, events for that session go out under `userId` directly.
