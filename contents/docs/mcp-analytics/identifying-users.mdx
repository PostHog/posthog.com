...
instrument(server, {
  ...
  identify: async (request, extra) => {
    ...
    return {
      ...
      userData: {
        email: user.email,              // becomes $set.email
...
The `UserIdentity` fields map to PostHog as follows:

- `userId` → the event's `distinct_id`.
- `userName` → `$set.name`.
- Every key of `userData` is spread into `$set` — so to set a person's email, return `userData: { email }`.
...
4. Caches the identity in a small LRU keyed by session id, so unchanged identities are silently deduped.
