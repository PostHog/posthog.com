...
By default, every MCP event is attributed to an SDK-generated session id (`ses_…`). On `2025-11-25`, that normally follows the protocol session. On the handshake-free `2026-07-28` revision, each request gets a new session unless [conversation IDs](/docs/mcp-analytics/conversation-id) are enabled. Either way, you can't yet say "Alice from Acme is calling this tool 200 times a day" — because the SDK doesn't know who Alice is.
