...
instrument(server, posthog, {
  identify: async (request, extra) => {
    ...
    return {
      ...
      properties: {                     // become person properties
        name: user.name,
        email: user.email,
        plan: user.plan,
...
- `properties` → written verbatim to the person profile (so to set a person's name or email, put them here, e.g. `properties: { name, email }`). Query them as [person properties](/docs/product-analytics/person-properties) — they aren't stored on the event.
...
2. Emits a `$identify` event the first time the identity is observed (or whenever it changes for that session), setting person properties from `properties`.
