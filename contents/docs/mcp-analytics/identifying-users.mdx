...
By default, every MCP event is attributed to an SDK-generated session id (`ses_…`). On `2025-11-25`, that normally follows the protocol session. On the handshake-free `2026-07-28` revision, [conversation IDs](/docs/mcp-analytics/conversation-id) are enabled by default, but calls share a session only when the agent echoes the handle. Either way, you can't yet say "Alice from Acme is calling this tool 200 times a day" — because the SDK doesn't know who Alice is.
...
`identify` is a sync or async callback that returns a `UserIdentity` or `null`. It runs on each request. The SDK caches resolved identities per session to avoid duplicate `$identify` events; it doesn't skip callback execution.
