...
`track(server, options)` patches your MCP server's request handlers. When the agent calls a tool, the SDK:
...
3. Sends it to PostHog through `posthog-node`, batched and flushed automatically.
...
- **[Custom events and metadata](/docs/mcp-analytics/custom-events)** — `publishCustomEvent`, `eventTags`, `eventProperties`
...
The SDK source lives at [github.com/PostHog/mcp-analytics](https://github.com/PostHog/mcp-analytics). Issues, PRs, and feedback are welcome. We started from a duplicated copy of the MIT-licensed [MCPcat TypeScript SDK](https://github.com/MCPCat/mcpcat-typescript-sdk) — the event schema, identity model, and feature surface have since diverged.
