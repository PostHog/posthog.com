...
The pipeline is identical in Python — same sanitizer, same truncation caps, same `$exception` fan-out — and `before_send` is the same final say. Exception messages are sanitized and capped at 2048 characters.
