...
Model capture and conversation IDs are enabled by default in `@posthog/mcp` 0.17.0 and Python `posthog` 7.56.0. To disable them, pass `{ captureModel: false, enableConversationId: false }` or `MCPAnalyticsOptions(capture_model=False, enable_conversation_id=False)`, respectively. Conversation IDs add a visible JSON text block to eligible tool results.
