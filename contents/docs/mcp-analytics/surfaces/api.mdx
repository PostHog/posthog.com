...
Use the API to pull MCP Analytics data into your own dashboards, reports, or workflows. Reading requires the `mcp_analytics:read` scope; submitting feedback or missing capabilities, generating session summaries, and recomputing clusters require `mcp_analytics:write`.
