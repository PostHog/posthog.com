...
Model capture and conversation IDs are enabled by default in `@posthog/mcp` 0.17.0 or later. The low-level `Server` works the same way. If you previously passed `server.server` to get past the old compatibility check, pass `server` with the same PostHog client instead.
...
[Sessions on `2026-07-28`](#sessions-on-2026-07-28) work as described below. Python `posthog` 7.56.0 enables model capture and conversation IDs by default. Both PostHog SDKs derive the same session id from the same echoed conversation handle.
...
- **[Conversation IDs](/docs/mcp-analytics/conversation-id)** — enabled by default in `@posthog/mcp` 0.17.0 and Python `posthog` 7.56.0. The SDK injects a `conversation_id` parameter and returns a handle in eligible tool results. Calls share a `$session_id` when the agent echoes that handle. Clients that ignore it can still produce a separate session per request.
