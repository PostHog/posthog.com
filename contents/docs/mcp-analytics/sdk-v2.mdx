...
The official Ruby MCP SDK has a single major that serves both revisions: the `initialize` handshake for `2025-11-25` and earlier, and the per-request `_meta` envelope for `2026-07-28`. The experimental [Ruby SDK](/docs/mcp-analytics/installation#ruby) handles both with the same call:
