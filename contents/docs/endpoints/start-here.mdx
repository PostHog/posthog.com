...
2. If you created an insight, click on the three dots button and navigate to the `create endpoint` option
<ProductScreenshot
...
```
You can also find your list of endpoints by navigating to the endpoints product page:
<ProductScreenshot
...
1. **Your personal API key**: With the required `endpoint:read` scope. Create one in [your user settings](https://us.posthog.com/settings/user-api-keys#personal-api-keys)
2. **Your endpoint name**: The name you chose when creating the endpoint (which you can also find in the endpoints project page)
...
- **Cache responses**: Endpoints return cached data when available, and you should materialize your endpoint when possible
