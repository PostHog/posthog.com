...
<CalloutBox icon="IconWarning" type="warning" title="Deprecated parameter">

The `filters_override` parameter is deprecated for insight-based endpoints and is not supported for HogQL endpoints. Requests using `filters_override` return a deprecation warning in the `X-PostHog-Warn` response header. Use `variables` instead for all new endpoints. See the [variables documentation](/docs/endpoints/variables) for migration guidance.
