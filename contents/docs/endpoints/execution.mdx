...
HogQL endpoints support pagination using `limit` and `offset` parameters. Insight-based endpoints (like Trends or Funnels) don't support `offset`.
...
When using pagination, the response includes additional fields:
