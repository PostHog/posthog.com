...
To manage Endpoints with the CLI, your personal API key needs the `endpoint:write` scope. If your endpoints use variables, you also need the `insight_variable:read` or `insight_variable:write` scope, depending on whether you intend to update endpoints using the CLI. You can create or update your API key in [Personal API Keys](https://app.posthog.com/settings/user-api-keys).
...
materialization:
  enabled: true
  schedule: "1hour" # Options: 15min, 30min, 1hour, 6hour, 12hour, 24hour, 7day
```
