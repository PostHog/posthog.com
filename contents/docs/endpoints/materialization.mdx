...
PostHog runs the first materialization immediately. After that, it refreshes on a schedule determined by your [data freshness](/docs/endpoints/caching#data-freshness) setting.

## Sync schedule

The materialization sync schedule is controlled by the **Data freshness** setting in the **Configuration** tab. PostHog automatically determines when to refresh materialized data based on the value you choose:
...
| 1 day (default) | Every 24 hours   |
| 7 days          | Every 7 days     |

Shorter data freshness values mean more frequent syncs, which keeps your data fresher but consumes more compute. Choose a value that balances freshness with cost for your use case.

For non-materialized endpoints, data freshness controls how long query results are cached. See [Caching](/docs/endpoints/caching) for more details.
