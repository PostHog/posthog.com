...
When using range variables on timestamp columns, certain aggregate functions cannot be re-aggregated and are incompatible:

- `avg` / `avgWeighted`
- `uniq` / `uniqExact` / `countDistinct`

Only re-aggregatable functions like `count`, `sum`, `min`, and `max` (and their combinator variants like `countIf`, `sumIf`) work with timestamp range variables.
