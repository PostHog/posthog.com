...
PostHog automatically calculates the cost of each evaluation run using the judge model, provider, and token counts. These costs are included in your total AI cost tracking alongside generation and embedding costs. See [Calculating LLM costs](/docs/llm-analytics/calculating-costs) for details on how costs are determined.
