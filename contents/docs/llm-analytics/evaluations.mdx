...
You can optionally filter which generations get evaluated using [property filters](/docs/llm-analytics/custom-properties), including both event properties and person properties. For example, only evaluate generations from production, from a specific model, or above a certain cost threshold. You can also use person properties to exclude internal users by email domain or filter by user role.
...
4. Configure the evaluation:
   ...
   - **[Property filters](/docs/llm-analytics/custom-properties)** (optional): Filter by event properties (model, environment, cost) or person properties (email domain, user role) to narrow which generations to evaluate
