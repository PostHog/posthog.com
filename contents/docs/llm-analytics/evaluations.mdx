...
Evaluation results also appear as color-coded badges directly in the [trace](/docs/llm-analytics/traces) detail panel header for generation events, so you can see pass/fail status at a glance without switching tabs. Hover over a badge for details including the eval name, timestamp, run count, and reasoning. Click a badge to jump to the **Evaluations** tab.
