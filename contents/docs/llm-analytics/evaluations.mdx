...
Evaluations use an LLM to run the judge. Your first 100 evaluation runs are on us so you can try the feature right away. After that, add your own OpenAI or OpenRouter API key in [project settings](https://app.posthog.com/llm-analytics/settings) to keep running evaluations.
