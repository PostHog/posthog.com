...
The playground lets you test prompts before publishing. Open any prompt, make changes, and save them back.

### Opening a prompt in the playground

1. Navigate to **LLM analytics** > **Prompts**
2. Open a prompt
3. Click **Open in Playground**

From here you can test the prompt with different messages, models, and settings.

### Saving from the playground

Click the save icon next to the system prompt to access these options:

- **Save to [prompt name]** – Publishes a new version of the linked prompt with your current system prompt
- **Unlink from source** – Disconnects the playground from the prompt so you can experiment freely
- **Save as new prompt** – Creates a new prompt from the current playground state
- **Load prompt** – Opens the prompts list to load a different prompt

You can also open the playground from a generation's trace view or conversation display to test the exact prompt used.
