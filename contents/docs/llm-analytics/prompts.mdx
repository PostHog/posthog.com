...
When viewing or editing a prompt, click the markdown icon next to the **Prompt** label to toggle markdown rendering. This formats your prompt text with headings, lists, bold, and other markdown elements. The default view is plain text.
...
The MCP server provides five prompt management tools:
...
| `prompt-duplicate` | Duplicate an existing prompt to a new name |

This enables teams to manage prompts programmatically from agent workflows without using the web UI.

## Duplicating prompts

Since prompt names are immutable after creation, use the duplicate feature to copy an existing prompt to a new name. This is useful when you want to:

- Create a variation of an existing prompt
- Fork a prompt for A/B testing
- Preserve the original while iterating on a copy

### Duplicate from the UI

1. Navigate to **LLM analytics** > **Prompts**
2. Find the prompt you want to duplicate
3. Click the three-dot menu and select **Duplicate**
4. Enter a new name (defaults to `{original-name}-copy`)
5. Click **Duplicate**

The duplicate creates a new prompt at version 1 with the content from the latest version of the source prompt. The original prompt remains unchanged.

### Duplicate via MCP

Use the `prompt-duplicate` tool to copy a prompt programmatically:

**Prompt:** "Duplicate the 'support-assistant' prompt to 'support-assistant-v2'"

The agent duplicates the latest version's content to a new prompt with the specified name.

### Naming rules for duplicates

- New names must follow the same rules as creating prompts: only letters, numbers, hyphens, and underscores
- The name must be unique among active prompts in your project
- You can reuse a name from an archived prompt
- The reserved name `new` cannot be used
