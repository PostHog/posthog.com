...
- **Project API key (`phc_...`)** – Used as `token` query param so prompt reads are resolved deterministically to the right project
- **PostHog SDK** – Install the Python or JavaScript SDK with the AI package

When you initialize `Prompts` with a PostHog client, the SDK uses the client's project API key automatically. If you initialize `Prompts` directly, you must pass both keys.
