...
- **[Personal API key](/docs/api#personal-api-keys) (`phx_...`)** – Used as Bearer auth for prompt fetches. Prompts may contain sensitive business logic, system instructions, or proprietary context, so PostHog requires authenticated access to read prompt content. The personal API key provides this authentication and creates an audit trail of who accessed prompts.
- **Project token (`phc_...`)** – Used as `token` query param to resolve prompt reads to the correct project. The project token identifies your project but doesn't authenticate requests on its own.
...
### Secure key handling for teams

- Store personal API keys in environment variables or a secrets manager – never commit them to source control.
- Create separate personal API keys per environment (development, staging, production) for better audit trails.
- Each team member who needs to test prompts locally should create their own [personal API key](https://app.posthog.com/settings/user-api-keys) rather than sharing one.
