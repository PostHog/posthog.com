...
- **Versioning** – Every change creates an immutable version you can view, restore, or fetch specific versions programmatically
...
Every time you save a prompt, PostHog creates a new immutable version. This gives you a complete history of changes and lets you fetch specific versions in your code.

- **Immutable versions** – Each save creates a new version (v1, v2, v3, etc.)
- **Latest version** – By default, SDK fetches return the latest version
- **Version history** – View all versions and who created them from the prompt detail page
- **Rollback** – To revert to an older prompt, copy its content into a new version

### Fetching a specific version

By default, the SDK fetches the latest version of a prompt. To pin your code to a specific version (for example, to keep production stable while testing changes), pass the `version` parameter:

<MultiLanguage>

```python
# Fetch the latest version (default)
template = prompts.get('support-system-prompt')

# Fetch a specific version
template = prompts.get('support-system-prompt', version=2)
```
...
// Fetch a specific version
const template = await prompts.get('support-system-prompt', { version: 2 })
```
...
=========
=======
>>>>>>> origin/promptless/llm-prompt-versioning-docs
...
Archiving a prompt soft-deletes all its versions. Archived prompts don't appear in the prompt list and can't be fetched via the SDK.

To archive a prompt:

1. Go to **LLM analytics** > **Prompts**
2. Click the prompt you want to archive
3. Click **Archive** and confirm

After archiving, you can create a new prompt with the same name – it starts fresh at version 1.
