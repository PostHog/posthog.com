...
1. **Automatic header injection** - the PostHog SDK automatically adds headers to your API requests
2. **Manual approach** - explicitly pass the session ID in your request body

### Option 1: Automatic header injection (recommended)

Configure the PostHog SDK to automatically inject session headers into requests to your backend:
...
posthog.init('<ph_project_token>', {
  ...
  addTracingHeaders: ['api.your-app.com']  // Your backend hostname(s)
})
```

```typescript file=React Native
new PostHog('<ph_project_token>', {
...
This automatically adds `X-POSTHOG-SESSION-ID` and `X-POSTHOG-DISTINCT-ID` headers to `fetch` requests matching the specified hostnames. On web, `XMLHttpRequest` calls are also patched.
