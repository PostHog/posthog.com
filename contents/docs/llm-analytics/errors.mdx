...
Click the trend icon on any error row to open an insight (in a new tab) showing that error's occurrences over time.

The table toolbar includes an **Error trends** button that opens a trends insight showing all normalized errors broken down over time across all AI event types (generations, spans, traces, and embeddings).
