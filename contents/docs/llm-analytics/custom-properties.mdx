...
### 4. Session and conversation tracking

Track multiple conversations or interactions that occur during a single user session using `$ai_session_id`. This is useful for grouping separate conversations that happen during the same app visit or workflow:
...
// User starts a new conversation in the same session
...
This enables you to:
- Track how many conversations users have per session
- Analyze conversation patterns within user sessions
- Calculate total cost and usage per session across multiple conversations
- Filter traces in the dashboard by session ID to see all related conversations
