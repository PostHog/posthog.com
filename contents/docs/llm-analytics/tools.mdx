...
Each tool row has icons next to the tool name that open insights in a new tab:

- **Trend** – Time series chart of the tool's usage over time
- **Combinations** – Bar chart of the most common tool call combinations that include this tool
- **Paths** – Tool call sequences starting from that tool, showing how tools are chained together in your AI workflows

The table toolbar has buttons for aggregate analysis across all tools:

- **Tool trends** – Trends insight with a line per tool (top 10 by usage) for comparing usage over time
- **Tool co-occurrence** – Heatmap showing how often pairs of tools are called together in the same generation
