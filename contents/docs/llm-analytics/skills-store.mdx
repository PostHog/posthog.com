...
| `skill-update`    | Publish a new version — full `body`, incremental `edits`, or `file_edits` for bundled files |
...
### Use incremental file edits

For bundled files, use the `file_edits` parameter to patch specific files without replacing the entire `files` array:
...
Each entry targets one existing file by path and applies sequential find/replace edits. Files not listed in `file_edits` carry forward unchanged.

Like body edits, each edit must match exactly once in the target file. If the `old` text matches zero times or more than once, the update fails with an error indicating which file and edit index failed.

You can combine `file_edits` with body `edits` in the same request. You can use either `files` (full replacement) or `file_edits` (incremental), but not both. `file_edits` only patches existing files — to add, remove, or rename files, use the `files` parameter.
