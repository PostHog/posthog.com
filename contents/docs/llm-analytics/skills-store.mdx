...
| `skill-update`    | Publish a new version — provide full `body` or incremental `edits`       |
...
Compare two skill versions side-by-side to see what changed. Available for skills with two or more versions.

To compare versions:

1. Open a skill and click **Compare versions** next to **Skill body**
...
You can also click the compare icon on any version in the **Version history** sidebar to compare it with the currently selected version. The comparison target is highlighted with a **Comparing** tag.

Unchanged regions collapse automatically. Click **Compare versions** again to exit the diff view.

This is useful when an agent has updated a skill — you can review exactly what changed before the rest of your team picks it up.
