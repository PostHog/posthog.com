...
You can filter traces in the [traces tab](https://app.posthog.com/llm-analytics/traces) by `trace_id`, `span_id`, and `duration` in addition to span attributes.

- **trace_id** and **span_id** - Support equals and not equals operators
- **duration** - Supports equals, not equals, greater than, greater than or equal, less than, and less than or equal operators

When you apply a filter, PostHog searches all spans across traces — not just root spans. It then returns the full trace (root span and sub-spans) for every trace that contains a matching span. Spans that match your filter criteria are visually highlighted in the flame chart, while non-matching spans are dimmed.
