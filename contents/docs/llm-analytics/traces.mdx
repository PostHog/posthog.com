...
When [evaluations](/docs/llm-analytics/evaluations) have run on a trace's generations, color-coded badges appear in the trace detail panel header so you can see pass/fail status at a glance. Each badge reflects the latest result for a given evaluation:

- **True** (green) – the evaluation passed
- **False** (red) – the evaluation failed
- **Error** (red) – the evaluation encountered an error
- **Running** (blue) – the evaluation is still in progress
- **N/A** (gray) – the evaluation was not applicable

Hover over a badge to see the evaluation name, when it ran, how many times it has run, and the reasoning behind the result. Click a badge to navigate to the **Evaluations** tab for full details.
