...
Initialize PostHog with your project API key and host from [your project settings](https://us.posthog.com/settings/project), then pass the Vercel AI OpenAI client and the PostHog client to the `withTracing` wrapper.
...
Now, when you use the Vercel AI SDK, PostHog automatically captures `$ai_generation` properties. These include:
