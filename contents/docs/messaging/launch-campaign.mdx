...
After setting up your [messaging channel](/docs/messaging/channels), you can launch your first campaign. Campaigns are workflows that deliver messages to your users based your configured logic.
...
<Step title="Add an action" badge="required">

Now that we have a trigger that can initiate the campaign workflow, we need to add an action to send a message to the users who trigger the campaign.
...
Drag an **Email** action beneath the trigger, into the campaign. Then, click on the email block to configure it.
