...
Your data pipelines is an add-on to your analytics events based on events ingested so you're just paying a percentage on top of each event's base cost. It doesn’t matter how many real-time or batch destinations you have e.g. you pay the same price if you send an event to 1 destination or 10 destinations. You get unlimited exports on the same data as we don't charge you each time you export.
