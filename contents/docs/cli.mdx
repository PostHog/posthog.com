...
The `AGENTS.md` snippet tells agents to load `posthog-cli api --agent-help` into context, use `posthog-cli api` for PostHog tasks, and follow the search, inspect, schema, and call workflow. Skills add task-specific instructions for common PostHog workflows.

The snippet is installed as a `<posthog>...</posthog>` block. Rerunning the install replaces the block in place, so reinstalling after a CLI update refreshes outdated instructions without duplicating them.
...
To set up your coding agent globally instead of per repo, use the [PostHog wizard](/docs/ai-engineering/ai-wizard). It detects which agents you have installed and writes the same snippet into the agent's global instructions file, such as `~/.claude/CLAUDE.md` for Claude Code or `~/.codex/AGENTS.md` for Codex:
...
Agents only use `posthog-cli api` if their instructions tell them to. Install the steering snippet into the repo's instructions file, or globally for your agent with the wizard:
...
Skills are installed per repo into `.agents/skills/`. If a skill isn't there, install it:
