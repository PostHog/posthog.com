...
export const ReuseFeatureFlagLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/experiments/reuse-feature-flag-for-experiment-light-mode.png";
export const ReuseFeatureFlagDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/experiments/reuse-feature-flag-for-experiment-dark-mode.png";
...
## Why can't I freeze my experiment's exposure?

The [freeze exposure](/docs/experiments/managing-lifecycle#freezing-exposure) action has several requirements. The action is hidden or disabled when:

- **The experiment isn't running** – Draft experiments haven't started collecting exposures, ended experiments are already complete, and paused experiments need to be resumed first.
- **The experiment is group-aggregated** – It targets groups (like companies or organizations) instead of persons, and the person-based snapshot cohort can't freeze group-level enrollment.
- **The experiment has a holdout or early access conditions** – Both are evaluated before release conditions, so new users would continue entering through those paths.
- **Exposure is already frozen** – Unfreeze first if you need to rebuild the snapshot.

A freeze can also be rejected after you confirm the dialog, once PostHog has scanned the exposure data:

- **Too many exposed users** – The scan is bounded by a cap on the number of exposed users and a timeout, so very large experiments are rejected rather than frozen.
- **Anonymous users** – If more than about 5% of exposed users have no person profile (anonymous or personless traffic), freezing is rejected because those users would silently lose their variant.
- **No exposures yet** – There's no one to snapshot.

These causes are structural, so retrying won't help. Freezing also takes longer the more exposed users your experiment has. A slow freeze isn't a failed one, so wait for the confirmation dialog to finish.

## Does freezing affect local evaluation SDKs?

Frozen experiments use static cohorts to restrict enrollment. Since static cohorts can't be evaluated locally by SDKs using [local evaluation](/docs/feature-flags/local-evaluation), the flag falls back to server-side evaluation via `/decide`. If your SDK is configured with `only_evaluate_locally`, it returns the flag's default value instead. This is standard behavior for any flag using static cohorts, not specific to freezing.
...
### My experiment metric shows "Retry N of M"

When a metric displays a "Retry N of M" tag, PostHog is automatically retrying the calculation after a transient failure. The tag replaces the usual "Recalculating" indicator and shows a countdown to the next attempt. Hovering the tag opens a popover with the full details.

Reasons a retry can happen:

- **Rate limited**: The ClickHouse cluster is under heavy load and rate-limited the query.
- **Timeout**: The calculation took too long to complete.
- **Out of memory**: The query required more memory than was available.
- **Server error**: Something unexpected went wrong while computing the metric.

PostHog retries automatically with increasing delays between attempts. The chart cell for the affected metric shows the reason, the retry counter, and a countdown to the next attempt. The status bar at the top of the results also shows how many metrics are retrying.

Metrics that already have results continue displaying them while a recalculation is retried. Only metrics waiting for their first result show the retry panel in place of the chart.

**What to do:**

- **Wait.** Most retries succeed within a few minutes.
- **Check again later.** If retries keep failing, the underlying query may be too expensive for the current cluster load.
- **Contact support.** If a metric consistently fails after all retry attempts, [contact support](https://app.posthog.com/home#supportModal=bug%3Aexperiments) for help.
