...
The `$feature_flag_called` event that tracks [exposures](/docs/experiments/exposures) is deduplicated by the SDK — once emitted for a given flag and value, it won't fire again unless `identify` (with a new distinct ID) or `reset` is called. This means that if a user checked the flag **before** your experiment launched, the event was already cached and won't re-emit when the experiment starts. Returning users who don't trigger `identify` or `reset` will appear to have no exposure.
...
When enabled, the deduplication cache resets with each new session, so `$feature_flag_called` is re-emitted for every flag checked in that session. This ensures that experiments launched after a flag was already in use will still capture exposures for returning users.
