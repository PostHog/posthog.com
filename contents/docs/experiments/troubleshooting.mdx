...
The [freeze exposure](/docs/experiments/managing-lifecycle#freezing-exposure) action has several requirements. If the action is unavailable or returns an error, check these common causes:
...
- **Holdouts and early access conditions** – If your experiment is in a holdout or the feature flag has early access conditions, freezing is rejected. These conditions are evaluated before release conditions, so new users would continue entering through those paths.
- **Too many exposed users** – The exposure scan is bounded by a cap on the number of exposed users and a timeout, so very large experiments are rejected rather than frozen.
- **Anonymous users** – If more than a small share of your exposed users don't have a person profile (anonymous or personless traffic), freezing is rejected. The snapshot cohort requires person profiles, so anonymous users would silently lose their variant.
- **Already frozen** – An experiment can only be frozen once. If you need to modify the freeze, unfreeze first, then freeze again.

These requirements are structural, so retrying a rejected freeze won't help. Also note that freezing runs synchronously and takes longer the more exposed users your experiment has – a slow freeze isn't a failed one, so wait for the confirmation dialog to finish.
