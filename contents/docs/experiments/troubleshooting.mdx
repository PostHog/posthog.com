...
The [freeze exposure](/docs/experiments/lifecycle) action has several requirements. If the action is unavailable or returns an error, check these common causes:

- **Experiment must be running** - You can only freeze a running experiment. Draft experiments haven't started collecting exposures, ended experiments are already complete, and paused experiments need to be resumed first.
- **Group-aggregated experiments aren't supported** - Experiments that target groups (like companies or organizations) instead of persons can't be frozen. The snapshot cohort is person-based and can't freeze group-level enrollment.
- **Holdouts and early-access conditions** - If your experiment is in a holdout or the feature flag has early-access conditions, freezing is rejected. These conditions are evaluated before release groups, so new users would continue entering through those paths.
- **Too many exposed users** - Experiments with more than 100,000 exposed users can't be frozen in a single request. This limit ensures the freeze operation completes reliably.
- **Anonymous users** - If more than 5% of your exposed users don't have a person profile (anonymous or personless traffic), freezing is rejected. The snapshot cohort requires person profiles, so anonymous users would lose their variant.
- **Already frozen** - An experiment can only be frozen once. If you need to modify the freeze, unfreeze first, then freeze again.
