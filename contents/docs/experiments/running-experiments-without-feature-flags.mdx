...
If feature flag support is not yet available in your PostHog SDK, but you still want to leverage PostHog's feature flag infrastructure, use the PostHog [API](/docs/api) to call the [`decide`](/docs/api/decide) endpoint. This enables you to calculate the feature flag value for a given user.
