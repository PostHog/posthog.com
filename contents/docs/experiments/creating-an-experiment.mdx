...
Participants are automatically split equally between variants. For experiments with exactly two variants, adjusting one variant's rollout percentage automatically adjusts the other to maintain a 100% total. For three or more variants, you need to manually adjust each variant's percentage. If you want to assign specific users to a specific variant, see the section below on [distribution and release conditions](#distribution-and-release-conditions).
