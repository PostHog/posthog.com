...
The **Traffic preview** shows a visual bar chart of how traffic is distributed across your variants based on your split percentages and rollout percentage. Each variant displays as a color-coded section with its letter label (A, B, C, etc.) and the percentage of total traffic it receives. A hatched pattern indicates users excluded from the experiment. This helps you understand the combined effect of your variant split and rollout settings – for example, a 50% rollout with a 50/50 split results in each variant receiving 25% of total traffic.
