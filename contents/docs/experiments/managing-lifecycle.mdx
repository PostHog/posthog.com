...
After you've determined a winning variant, click the **End experiment** button on the experiment page. This lets you choose a variant to keep and end the experiment.

### Choosing a release mode

When you select a variant to ship, you choose how to release it:

- **Roll out to the experiment population** (default) – Serves the winning variant only to users who match the feature flag's existing release conditions. Per-user variant overrides continue to apply. Use this to preserve your targeting rules.

- **Roll out to all users** – Adds a release condition that serves the winning variant to 100% of users, overriding existing release conditions and per-user variant overrides. Use this when you want everyone to see the variant immediately.
...
| **Ending**    | Modified to roll out chosen variant (see [release mode](#choosing-a-release-mode)) | Chosen variant    | Continues (chosen variant only) | Fixed    | No             |
