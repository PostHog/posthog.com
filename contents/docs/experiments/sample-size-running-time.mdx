...
Before running an experiment, you'll want to know how long it needs to run to get reliable results. The running time calculator helps you estimate this based on your traffic and the size of effect you want to detect. You can use it both while planning a draft experiment and while one is running.

## Finding the calculator

The running time indicator appears on your experiment page. The display varies depending on your experiment's state:

- **Draft experiments**: Shows "Estimated duration" (displays "Not calculated" until you configure it)
- **Running experiments**: Shows "Remaining time" (displays "Pending" until enough data is collected)
...
The calculator offers two modes: **Automatic** and **Manual**. For running experiments, Automatic mode is the default and recommended option. For draft experiments, only Manual mode is available.
...
> **Note:** Automatic mode is only available after launching your experiment. For draft experiments, this option is disabled with the tooltip "Available after launching experiment."
...
Manual mode is the only option available for draft experiments and lets you estimate running time before launching. It's also useful for running experiments when you want to plan ahead or override the automatic calculations. Use it when you know your approximate traffic numbers and baseline metrics.
...
1. **Use automatic mode for running experiments**: Real data is more reliable than estimates. Once your experiment has been running for a day with 100+ exposures, automatic mode gives you the most accurate projections. For draft experiments, use manual mode to plan your experiment duration before launching.
