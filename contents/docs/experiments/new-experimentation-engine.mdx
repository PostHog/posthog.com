...
- If you select “average revenue,” we first compute each user’s average (total revenue / number of events), then average those values across users. (NB: not yet supported, but coming soon!)
...
Cap metric values at specified percentile thresholds by setting lower and upper bounds. This is especially useful for metrics with skewed distributions, like revenue or total event counts.
...
Limit metric events to only those that occur within a specified time window after a user’s first exposure to the experiment.
