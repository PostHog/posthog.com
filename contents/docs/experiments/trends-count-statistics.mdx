...
Trends metrics for count-based data use Bayesian statistics with a Gamma-Poisson model to evaluate the **win probabilities** and **credible intervals**. [Read the statistics primer for an overview](/docs/experiments/statistics-primer) if you haven't already.
