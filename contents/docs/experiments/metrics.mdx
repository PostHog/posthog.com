...
### Only count measurable participants

When you set a conversion or retention window, you can choose to include all participants or only those whose window has fully elapsed. This is controlled by the **Only count measurable participants** setting.

When enabled, PostHog excludes participants whose conversion or retention window hasn't elapsed yet. For example, with a 7-day conversion window, a user exposed today won't appear in the analysis until 7 days have passed – even if they've already converted. For retention metrics with a 7-day retention window, the same logic applies: users must have had enough time to reach the retention window before being included.

This is useful when you want everyone to have the same amount of time to convert or return before being counted. Without it, recently exposed users who haven't had time to complete their window could skew your results.

To enable this setting:

1. Go to your experiment's **Settings** tab
2. Find the **Only count measurable participants** toggle

You can also set a default for all new experiments in your environment settings under **Experiments > Default conversion window filter**. This default can be overridden per experiment.
