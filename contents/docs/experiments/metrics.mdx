...
Use this to track metrics like total event count, summed values (e.g., revenue), or percentiles. Here's how it works:

1. We aggregate the chosen value for each user using your selected method (count, sum, average, min, max, or a percentile like median, p75, p90, p95, or p99).
...
- If you choose **average revenue**, we first compute each user's average (total revenue / number of events), then average those values across users.
- If you choose **median revenue**, we calculate the median revenue per user, then take the mean of those medians across users. This is useful when you want to reduce the impact of outliers in per-user calculations.
...
Both the numerator and denominator support all aggregation methods (count, sum, average, min, max, percentiles, unique values). You can even use the same event with different properties for each part of the ratio, such as dividing total revenue by total quantity sold from purchase events.
