...
CUPED (Controlled-experiment Using Pre-Experiment Data) reduces variance in experiment results by using historical data about your users. It accounts for each user's baseline behavior before the experiment started, helping you detect significant effects faster without needing more traffic.

### How it works

CUPED analyzes each user's pre-experiment metric values and uses them as a covariate adjustment. If a user historically has high revenue, CUPED factors that in when measuring the experiment's impact on their revenue. This reduces noise from natural user-to-user variation, making it easier to isolate the effect of your change.

### When to use CUPED

CUPED works best when:
- Your metric has high natural variance between users
- Users have historical data for the metric you're measuring
- You want to reach statistical significance faster without increasing sample size

CUPED supports **mean** and **funnel** metrics.

### Lookback window

The lookback window controls how many days of pre-experiment data CUPED uses. Longer windows capture more historical behavior but require users to have been active before the experiment started. The default is 14 days.

You can configure the lookback window at two levels:

**Per experiment** – Set a custom lookback window (1-30 days) in the experiment's CUPED settings.

**Team default** – Set a default for all new experiments in **Experiments > Settings**. This applies when an experiment doesn't specify its own value.

PostHog resolves the lookback window in this order:
1. Experiment-level setting (if set)
2. Team default setting (if configured)
3. Default of 14 days

### Enabling CUPED

You can enable CUPED at two levels:

**Per experiment** – Toggle CUPED on or off in the experiment's statistics settings. This controls whether CUPED applies to supported metrics for that specific experiment.

**Team default** – Set a default for all new experiments in **Experiments > Settings**. When enabled, new experiments use CUPED by default on supported metrics. You can still override this on individual experiments.

PostHog resolves whether CUPED is enabled in this order:
1. Experiment-level CUPED setting (if explicitly set)
2. Team default setting
3. Off (disabled)
