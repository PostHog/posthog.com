...
Breakdowns let you segment experiment results by property values, so you can see how different groups of users respond to each variant.

Breakdowns are available for all metric types: funnel, mean, ratio, and retention. You can break down results by both event properties and person properties.

For example, you might break down results by a person's `plan` property to see if paid users respond differently to a variant than free users, or by an event's `browser` property to check for platform-specific effects.

To add a breakdown, click the **Add breakdown** button on any metric and select a property from the event or person property lists.
