...
Outlier handling is available for **mean** and **ratio** metrics. For ratio metrics, the numerator and denominator are capped independently—each with its own percentile bounds computed separately across the pooled experiment population. This allows you to, for example, cap outlier revenue values in the numerator while leaving the order count denominator uncapped.
