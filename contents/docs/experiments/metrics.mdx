...
By default, a mean metric reports the average value per user. Sometimes you instead want to know what percentage of users reached a milestone – like 100 words dictated, 2,000 events fired, or a cumulative amount of spend. Set a **threshold** to measure this.

When you set a threshold, each user's summed or counted value collapses to a binary outcome: they either reached the threshold or they didn't. PostHog then reports the percentage of exposed users who crossed it, analyzing the metric as a proportion in the same way it analyzes funnel conversion rates.

Thresholds are only available for **sum** and **total count** math, where the value per user is an accumulation. A few things to keep in mind:

- A threshold must be a positive number. Clearing it (or setting it to 0) removes the threshold and reverts the metric to a standard mean.
- You can't combine a threshold with outlier handling. Winsorization caps continuous outliers, which has no meaning once the value is binary.
- CUPED variance reduction is disabled for threshold metrics, since it doesn't apply to a binary outcome.
