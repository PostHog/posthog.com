...
See exactly what users experienced in each variant with [session recordings](/docs/session-replay) tied to your experiment results. Every experiment has a **Recordings** tab that shows the sessions of the people PostHog exposed to the test, and the player starts at the moment they entered it. Filter the list by variant, or by whether the session completed your metric, to understand why users behaved differently. See [watching session recordings](/docs/experiments/analyzing-results#watching-session-recordings).

### <IconToggle className="text-seagreen w-7 -mt-1 inline-block"/> Feature Flags

Use [feature flags](/docs/feature-flags) to target specific user segments, implement gradual rollouts, or run experiments on [groups](/docs/product-analytics/group-analytics) instead of individuals. You can also use [holdout groups](/docs/experiments/holdouts) to measure long-term effects of your changes by excluding a subset of users from all experiments.

### <IconFlask className="text-red w-7 -mt-1 inline-block"/> Advanced experimentation

- **Statistical approaches** – Choose between [Bayesian](/docs/experiments/statistics-bayesian) (default) or [frequentist](/docs/experiments/statistics-frequentist) analysis depending on your needs
- **Multiple variants** – Test more than two variants simultaneously by adding up to nine test variants alongside a control
- **[Outlier handling](/docs/experiments/metrics#outlier-handling)** – Cap metric data at specified percentile thresholds to prevent extreme values from distorting results
- **[Best practices](/docs/experiments/best-practices)** – Predetermine test duration, use launch checklists, and calculate the sample size needed to detect meaningful changes
