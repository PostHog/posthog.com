...
## Why do my experiment's exposure counts not match insights?

Currently, insights aren't able to account for users that have been exposed to `$multiple` variants of your feature flag. We are actively working on improving this, however in the meantime exposures as measured by the experiment metrics are more accurate.

If you are wondering why you see `$multiple` exposures, this most commonly happens when a feature flag is evaluated for a user before and after sign in, but they are assigned a different variant. To fix this, you could consider either:
...
You're also able to select how [`$multiple` exposures are handled](https://posthog.com/docs/experiments/exposures#handling-multiple-exposures) in your experiment results.