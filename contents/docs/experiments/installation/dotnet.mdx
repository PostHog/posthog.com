...
For this tutorial, let's report a conversion event when a user clicks a CTA. In server SDKs like .NET, this can be called when a specific route is visited or reported from your client side app.
...
var variant = flags.GetFlag("your-experiment-key")?.VariantKey;
