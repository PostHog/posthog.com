...
For experiments that have started and have results, you can use [PostHog AI](/docs/posthog-ai/session-summaries) to automatically analyze user behavior patterns across session recordings.

Click the **Summarize session replays** button on your experiment page to open PostHog AI. It analyzes recordings from each variant and identifies behavior patterns, helping you understand how users interact differently with each variant without manually watching recordings.
