...
<CalloutBox icon="IconFlask" title="Beta" type="info">

This feature requires the `experiment-significance-alerts` feature flag to be enabled.

</CalloutBox>

Set up notifications to get alerted when an experiment metric reaches statistical significance.

To set up significance notifications:

1. Go to your experiment and click the **Settings** tab.
2. In the **Notifications** section, click **New notification**.
3. Choose a destination:
   - **Slack** – Post to a Slack channel when a metric reaches significance. You may need to connect your Slack workspace first.
   - **HTTP Webhook** – Send a webhook to a custom URL.
4. Configure the destination settings and click **Create & enable**.

Each notification includes:

- **Winning variant**: The variant that won
- **Metric name**: The metric that reached significance
- **Relative change**: The percentage change vs control (e.g., +78%)
- **Chance to win**: Statistical probability the variant is winning (e.g., 99%)
- **Goal direction**: Whether the goal is to increase or decrease the metric
- **View experiment link**: Direct link to the experiment in PostHog
