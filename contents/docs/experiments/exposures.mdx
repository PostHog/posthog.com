...
The `$feature_flag_called` event is deduplicated by the SDK — once emitted for a given flag and value, it won't fire again unless `identify` (with a new distinct ID) or `reset` is called. This means if a user checked the feature flag **before** your experiment launched, the event was already cached and won't be re-emitted when the experiment starts. As a result, returning users may not appear as exposed.

To fix this, enable `advanced_feature_flags_dedup_per_session` in your [PostHog config](/docs/libraries/js/config#advanced-configuration):
