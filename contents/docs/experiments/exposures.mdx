...
1. **Check your feature flag implementation**: Verify the flag is being evaluated correctly across all code paths. Ensure you're calling `identify()` before evaluating flags (for frontend SDKs).
...
3. **Check for ad-blockers or network issues**: These can prevent feature flag calls from reaching PostHog for certain users, skewing your distribution.
