...
The `system.batch_exports` table contains batch export configurations from your project. Use this table to query export settings, schedules, and status. For more on batch exports, see the [batch exports docs](/docs/cdp/batch-exports).
...
| `deleted`         | Soft-delete flag (1 = deleted, 0 = active)                            |
...
| `last_updated_at` | When the batch export was last modified                               |
...
> **Note:** Filter with `deleted = 0` to exclude soft-deleted exports. Filter with `paused = 0` to find actively running exports. Destination secrets are not exposed through this table.

### `system.batch_export_backfills`

The `system.batch_export_backfills` table tracks [backfill](/docs/cdp/batch-exports#backfills-exporting-historical-data) jobs for batch exports. A backfill is a one-time historical data export for a specific time range.
...
| `status`             | Current status: `Starting`, `Running`, `Completed`, `Failed`, `Cancelled` |
| `created_at`         | When the backfill was created                                   |
| `finished_at`        | When the backfill completed                                     |
| `last_updated_at`    | When the backfill was last modified                             |
