...
The `system.accounts` table contains customer analytics account data from your project. This table requires `customer_analytics` access.
...
| `billing_id`         | Billing ID (extracted from properties)                     |
...
> **Note:** The external ID fields (`stripe_customer_id`, `hubspot_deal_id`, `billing_id`, `sfdc_id`, `zendesk_id`) are extracted from the `properties` JSON column for convenience. You can also access the raw `properties` JSON directly.
