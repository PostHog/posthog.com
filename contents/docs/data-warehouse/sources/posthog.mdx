...
Much of your PostHog data is available in the data warehouse by default. This includes data like [events](/docs/data/events), [persons](/docs/data/persons), [sessions](/docs/data/sessions), [groups](/docs/product-analytics/group-analytics), [ai_events](/docs/llm-analytics), and the [query log](/docs/data/query-log).
...
- [LLM analytics](/docs/llm-analytics) captures events like `$ai_generation`, `$ai_span`, `$ai_embedding` with properties like `$ai_model`, `$ai_input_tokens`, and `$ai_total_cost_usd`. For advanced queries, use the dedicated `ai_events` table.
...
The `ai_events` table provides queryable access to [LLM analytics](/docs/llm-analytics) data. It's optimized for analyzing AI/LLM usage including model performance, token consumption, costs, and errors.

Some of the columns in the `ai_events` table are:

| Column                | Description                                            |
| --------------------- | ------------------------------------------------------ |
| `uuid`                | Unique ID of the event                                 |
| `event`               | Name of the event (e.g., `$ai_generation`, `$ai_span`) |
...
| `input`               | Input payload (JSON)                                   |
| `output`              | Output payload (JSON)                                  |
| `tools`               | Tool definitions (JSON)                                |

The `ai_events` table supports person joins, so you can access person properties using `person.properties` just like with the `events` table.
