...
The `system.activity_logs` table provides an audit trail of user and system actions across PostHog entities like feature flags, insights, dashboards, and experiments.

> **Note:** Only team-scoped activity logs are visible. Organization-level logs (e.g., membership changes) are not included because they are not associated with a specific team.
...
| `scope`      | Entity type (e.g., `FeatureFlag`, `Insight`, `Dashboard`)  |
| `detail`     | JSON containing structured details about the change        |
| `created_at` | When the activity occurred                                 |

The `detail` field contains a JSON object with information about what changed, including the entity name, field-level changes with before/after values, and the type of change.

Common scopes include: `FeatureFlag`, `Insight`, `Dashboard`, `Experiment`, `Cohort`, `Survey`, `Notebook`, `Action`, `HogFunction`, `Person`, `Replay`, `BatchExport`, `Team`, `Project`, `ErrorTrackingIssue`, `EarlyAccessFeature`, `Annotation`, and `AlertConfiguration`.
