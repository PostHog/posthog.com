...
The `system.dashboard_tiles` table contains metadata about dashboard tiles, including layouts, colors, and linked content.
...
| `insight_id`             | ID of the linked insight (null if tile contains text or button)            |
| `text_id`                | ID of linked text content (null if tile contains insight or button)        |
| `button_tile_id`         | ID of linked button content (null if tile contains insight or text)        |
| `layouts`                | JSON containing tile position and size for each breakpoint                 |
| `color`                  | Custom color applied to the tile                                           |
| `show_description`       | Whether the insight description is shown (1 = yes, 0 = no)                 |
| `transparent_background` | Whether the tile background is transparent (1 = yes, 0 = no)               |
| `filters_overrides`      | JSON containing dashboard-level filter overrides applied to this tile      |
