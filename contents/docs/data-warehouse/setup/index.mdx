...
When choosing incremental append replication, you must select a field to identify new data. This is often something like a `created_at` timestamp, or an autoincrementing ID. Not all fields are suitable to be used to identify new and updated data, and so we only support the following types as replication keys:
