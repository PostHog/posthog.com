...
When viewing a saved SQL insight that uses `{filters}`, you can adjust the date range and add property filters directly on the insight page without opening the SQL editor. Changes re-run the query immediately but are not saved until you explicitly save the insight. Clicking **Edit** carries any unsaved filter changes into the SQL editor where you can save them.
