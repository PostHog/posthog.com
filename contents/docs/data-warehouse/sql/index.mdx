...
You can use `VALUES` to create inline tables directly in your query without needing to reference an existing table. This is useful for creating lookup tables, test data, or mapping values.
...
FROM (VALUES
    ...
    (2, 'inactive'),
    (3, 'pending')
) AS status_lookup(id, status_name)
```

If you don't provide column names, they default to `col0`, `col1`, etc.

You can also join `VALUES` expressions with other tables. For example, to map event names to categories:
...
JOIN (VALUES
    ('$pageview', 'engagement'),
    ('$screen', 'engagement'),
    ('$autocapture', 'interaction'),
    ('$rageclick', 'frustration')
) AS category_map(event_name, category) ON events.event = category_map.event_name
GROUP BY events.event, category_map.category
...
Use `WITH FILL` after an `ORDER BY` column to fill gaps in time series data. This is useful when you have missing data points (like days with no events) and want to include them in your results.

The `WITH FILL` clause supports three optional parameters:

- `FROM` - The starting value for filling (defaults to the minimum value in the column)
- `TO` - The ending value for filling (defaults to the maximum value in the column)
- `STEP` - The increment between filled values (defaults to 1)
...
This query returns a row for each day in the last seven days, even if no events occurred on that day.

#### INTERPOLATE

Use `INTERPOLATE` alongside `WITH FILL` to control how non-ordered columns are filled for newly inserted rows. By default, filled rows use default values (0 for numbers, empty string for strings). With `INTERPOLATE`, you can define custom fill expressions or specify that a column should keep its default.

`INTERPOLATE` can be used bare (to interpolate all columns with defaults) or with explicit column expressions:
