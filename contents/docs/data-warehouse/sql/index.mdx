...
Use `LIMIT` to restrict the number of rows returned by the query. It specifies the maximum number of rows the query should retrieve. By default, PostHog sets it at `2000`.
...
> **Note:** SQL insights default to a `LIMIT 2000`. If you're adding the insight to a dashboard or using longer date ranges, you can adjust this limit with your own `LIMIT` clause.
