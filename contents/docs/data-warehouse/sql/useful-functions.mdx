...
If you want to access dashboard or insight date filters, you can use the [variables](/docs/data-warehouse/sql/variables) `filters.dateRange.from` and `filters.dateRange.to`.
...
SELECT
DISTINCT properties.$lib_version
FROM events
...
Use `getSurveyResponse()` to retrieve survey response values from event properties. It takes a zero-based question index, an optional question ID, and an optional boolean for multiple choice responses.

Use `uniqueSurveySubmissionsFilter()` in a `WHERE` clause to deduplicate survey submissions. It takes a survey ID and optional start/end timestamps.

```sql
SELECT
    getSurveyResponse(0) as first_response,
    getSurveyResponse(1, 'question_abc') as second_response
FROM events
WHERE event = 'survey sent'
    AND uniqueSurveySubmissionsFilter('survey_abc123')
```

### Currency conversion

Use `convertCurrency()` to convert an amount from one currency to another using exchange rates. It takes a source currency code, target currency code, amount, and an optional timestamp for historical rates.

```sql
SELECT
    convertCurrency('USD', 'EUR', 100) as euros_now,
    convertCurrency('USD', 'GBP', properties.revenue, timestamp) as gbp_at_event_time
FROM events
...
PostHog includes helper functions used internally for [channel type](/docs/data/channel-type) classification. You can also call them directly:

- `lookupDomainType(domain)` - Returns the domain category (e.g., search, social, shopping)
- `lookupPaidSourceType(source)` - Returns the paid channel type for a traffic source
- `lookupPaidMediumType(medium)` - Returns the paid channel type for a medium
- `lookupOrganicSourceType(source)` - Returns the organic channel type for a traffic source
- `lookupOrganicMediumType(medium)` - Returns the organic channel type for a medium

```sql
SELECT
    lookupDomainType(properties.$initial_referring_domain) as domain_type,
    lookupPaidSourceType(properties.$initial_utm_source) as paid_source_type
FROM events
WHERE event = '$pageview'
```
...
Use `embedText()` to generate a vector embedding for a given text string. This is useful for semantic search and AI applications. It takes a text string and an optional model name.

```sql
SELECT
    embedText('user clicked the buy button') as embedding
FROM (SELECT 1)
```

### CSP reports

Use `explainCSPReport()` to format Content Security Policy violation reports into a human-readable explanation.

```sql
SELECT
    explainCSPReport(properties) as csp_explanation
FROM events
WHERE event = '$csp_report'
```
