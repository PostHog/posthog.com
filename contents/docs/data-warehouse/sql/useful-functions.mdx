...
URLs are automatically clickable in the table visualization (not **Results**), but you can also set a custom link using an `<a>` tag.

```sql
select 
    properties.$pathname,
    <a href={concat('https://posthog.com', properties.$pathname, '')} target='_blank'>{'Link'}</a> as link
from events
