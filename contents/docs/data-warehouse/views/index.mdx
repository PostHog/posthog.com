...
Query views are created directly inside SQL insights. If the query has valid view characteristics, the **Save as view** button is enabled. When clicked, you are prompted to give the view a name which can then be referenced in other queries. You can also check **Materialize this view** in the dialog to [materialize](/docs/data-warehouse/views/materialize) the view immediately with a daily sync. You can adjust the sync frequency later.
...
Click on any view from the [**Models** page](https://app.posthog.com/models) to open its detail page. The detail page includes:

- **Details** – Metadata about the model including its type, created date, and when it was last refreshed.
- **Query** – A SQL editor displaying the saved query. You can edit and run the query directly from this page.
- **Lineage** – An interactive visualization showing upstream and downstream dependencies between models. Click **Fullscreen** to expand the graph, or click on any connected node to navigate to its detail page.
