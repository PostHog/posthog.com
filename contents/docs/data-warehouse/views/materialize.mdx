...
If a materialized view reads from another materialized view, PostHog refreshes them in dependency order, so a view never reads a half-written table.

This also means an upstream view refreshes at least as often as anything downstream of it. Setting a view to refresh every 15 minutes pulls everything it reads from onto that cadence too.
...
The **Materialization** panel lists every run of the view with its status, row count, and any error. Use it to confirm a view is refreshing on schedule and to read the error when one isn't.

To test new data before PostHog publishes it, [block materialization on failing data quality checks](/docs/data-warehouse/data-quality/block-materialization).

If runs are failing or the data looks out of date, see [materialization troubleshooting](/docs/data-warehouse/views/materialize-troubleshooting).
