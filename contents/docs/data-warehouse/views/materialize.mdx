...
To materialize a view, go to the [SQL editor](https://app.posthog.com/sql), select the **Materialization** tab below the query, click **Save and materialize**, and give it a name without spaces. You can then query the view like any other table.
...
## Viewing materialization run logs

Each materialization run captures detailed logs that show execution progress, timing, and any errors that occurred. To view them, expand any row in the **Materialization Runs** table by clicking on it.

Logs are useful for troubleshooting failed materializations or understanding what happened during a specific run. They include information-level messages about query execution and data processing, along with any warnings or errors encountered.
