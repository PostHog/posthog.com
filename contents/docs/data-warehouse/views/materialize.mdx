...
## Managing materialization via API

You can programmatically manage materialization using the [PostHog API](/docs/api) with a [personal API key](/docs/api/overview#personal-api-keys). Your key needs the `warehouse_view:write` scope.

Two endpoints are available:

- **`POST /api/projects/{project_id}/warehouse_saved_queries/{id}/materialize/`** - Enables materialization for a saved query with a 24-hour sync frequency.
- **`POST /api/projects/{project_id}/warehouse_saved_queries/{id}/revert_materialization/`** - Reverts materialization, deleting the materialized table and its schedule.

These endpoints are rate limited to five requests per hour per saved query.
