...
After [linking your source](/docs/cdp/sources), you can use its data in [trend](/docs/product-analytics/trends) and [lifecycle](/docs/product-analytics/lifecycle) insights like you would event or person data.
...
You can use data warehouse tables in lifecycle insights to analyze new, returning, resurrecting, and dormant patterns for any entity.

When using a data warehouse table in a lifecycle insight, you must configure three fields:

| Field | Description |
|-------|-------------|
| **Timestamp** | When the activity occurred. |
| **Created at** | When the entity was first created. Used to determine "new" vs "returning" status. |
| **Aggregation target** | The field identifying the entity to track. |

This enables lifecycle analysis beyond just users. For example, you can track orders, subscriptions, or any entity with activity timestamps and creation dates.

> **Note:** Filters, test account filters, and sampling are not available when using data warehouse tables in lifecycle insights.

For more on interpreting lifecycle charts, see the [lifecycle insight docs](/docs/product-analytics/lifecycle).
