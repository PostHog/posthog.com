...
## Data warehouse funnels

When building [funnels](/docs/product-analytics/funnels) using data warehouse tables, there are a few differences from standard event-based funnels:

- **Per-step aggregation** - For funnels where all steps are data warehouse tables, the global "Aggregating by" dropdown is disabled. Instead, configure the **Aggregation target** field within each individual step's settings.

- **Correlation analysis unavailable** - [Correlation analysis](/docs/product-analytics/correlation) is not available for funnels containing data warehouse sources.
