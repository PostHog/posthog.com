...
When you select **Auto**, PostHog picks the best visualization for your query results:

- **Line chart**: Data with a date/datetime column, a numeric column, and multiple rows
- **Big number**: A single numeric value
- **2D heatmap**: Two string columns and a numeric column
- **Bar chart**: Numeric values without a date/datetime column
- **Table**: Everything else

The dropdown shows "Auto (Line chart)" or similar so you can see which type was selected. Chart types that require a date/datetime column (like line and area charts) are disabled when your data doesn't include one.
