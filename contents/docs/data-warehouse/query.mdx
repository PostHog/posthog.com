...
The SQL editor supports writing multiple queries in a single tab, separated by semicolons. When you click **Run** or press <kbd>Cmd</kbd>+<kbd>Enter</kbd> (Mac) / <kbd>Ctrl</kbd>+<kbd>Enter</kbd> (Windows/Linux), only the query under your cursor executes. The active query is highlighted with a blue background so you can see which one runs next.

For example, if your editor contains:
...
Placing your cursor on the second line and clicking **Run** executes only `SELECT count(*) FROM persons`.

If you manually select text in the editor, pressing <kbd>Cmd</kbd>+<kbd>Enter</kbd> runs the selected text instead.

### Running subqueries

Press <kbd>Cmd</kbd>+<kbd>Shift</kbd>+<kbd>Enter</kbd> (Mac) / <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>Enter</kbd> (Windows/Linux) to run the innermost subquery at your cursor position. This is useful for testing nested queries independently.

For example, with the query:

```sql
SELECT * FROM (SELECT id, event FROM events WHERE event = 'pageview') LIMIT 10
```

Placing your cursor inside the inner `SELECT` and pressing <kbd>Cmd</kbd>+<kbd>Shift</kbd>+<kbd>Enter</kbd> runs only `SELECT id, event FROM events WHERE event = 'pageview'`.

Subqueries that may fail when run standalone — such as those referencing CTEs defined elsewhere — show a wavy yellow underline with a tooltip explaining the issue on hover.

## SQL visualizations

SQL queries have multiple visualization options including:

- Auto (default) - automatically selects the best chart type based on your query results
- Table
- Big number
- Line chart
- Bar chart
...
> **Note:** Line chart and area chart require a date or datetime column in the query results.
...
In the **Series** tab, you can format numerical columns with different number styles:

- **None** - Display raw values
- **Number** - Standard number formatting
- **Short Number** - Compact format for large numbers (e.g., 1,200 displays as "1.2K", 1,500,000 as "1.5M")
- **Percentage** - Display values as percentages

You can also set a prefix, suffix, and decimal places for each series.
...
## Raw query mode for external connections

When querying external data sources, PostHog normally translates your SQL through HogQL. If you run into syntax that HogQL doesn't yet support, you can enable **Send raw query** mode to bypass HogQL translation and send your query directly to the external connection.

To enable raw query mode:

1. Select an external connection in the SQL editor.
2. Click the **settings gear** icon in the editor toolbar.
3. Toggle **Send raw query** on.

Your query is sent directly to the external database without any HogQL translation.

<CalloutBox icon="IconInfo" title="When to use raw query mode" type="fyi">

Use this as an escape hatch when HogQL translation doesn't support the SQL syntax you need. Your query may be logged to help improve HogQL support.
