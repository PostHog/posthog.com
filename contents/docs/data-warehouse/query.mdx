...
For chart types (line, bar, stacked bar, and area), the **Display** tab includes a **Show nulls as zero** toggle. When enabled, missing or null data points display as zero values. When disabled (the default), null values are hidden from the chart, creating gaps in the visualization.
