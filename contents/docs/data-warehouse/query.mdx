...
These metrics are helpful for optimizing queries, especially when working with large datasets or complex joins. As you develop and change your query, keep an eye on what the impact of your changes is on the metrics. Seeing a spike in bytes read might prompt you to use a more selective filter.
