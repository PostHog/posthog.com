...
## Querying direct connections (beta)
...
If you've linked a [Postgres source](/docs/data-warehouse/sources/postgres) with the **Query directly** access method, you can run SQL queries directly against that database from the SQL editor.

Switch between connections using the dropdown at the top of the database panel:

- **PostHog (ClickHouse)** – Query your synced data warehouse (default)
- **Your Postgres connections** – Query directly against linked Postgres databases

When you select a direct connection:

- The database tree shows tables from that Postgres source
- Queries run live against your Postgres database
- Results can't be joined with PostHog data

To add a new Postgres direct connection, select **+ Add postgres direct connection** from the dropdown.
