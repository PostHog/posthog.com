...
The SQL editor keeps a log of your recent queries. Click **Query log** in the toolbar to view your query history from the last 7 days.

The log shows:

- **Query**: The SQL text (hover to see the full query if truncated)
- **Time**: When you ran the query
- **Result rows**: Number of rows returned
- **Duration**: How long the query took
- **Status**: Whether it succeeded or failed

Click any column header to sort. To reload a query into the editor, click the arrow button next to it.
