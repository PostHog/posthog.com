...
To create a query, go to the [SQL editor](https://app.posthog.com/sql). Here you can see and search the schema of all available sources and [PostHog tables](/docs/data-warehouse/sources/posthog) as well as saved views.
...
## Querying external databases directly

In addition to querying data synced into the PostHog warehouse, you can run live SQL queries against external [PostgreSQL](/docs/cdp/sources/postgres) databases directly from the SQL editor.

When [linking a PostgreSQL source](/docs/cdp/sources/postgres), choose **Query directly** as the access method. This creates a live connection that appears in the SQL editor's connection selector, letting you switch between querying PostHog data and querying your PostgreSQL database.

<CalloutBox icon="IconInfo" title="Direct query limitations" type="fyi">

Direct query sources run queries live against your external database. Data from these sources can't be joined with PostHog data (events, persons, etc.). If you need to combine external data with PostHog data, use the **Sync to warehouse** access method instead.
