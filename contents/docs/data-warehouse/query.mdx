...
1. Create a [new SQL insight](https://us.posthog.com/insights/new).
2. Choose a table to query like `stripe_charge` or `hubspot_contacts`
...
You can also use an `JOIN` such as `INNER JOIN` or `LEFT JOIN` to combine data. For example, to get a count of events for your Stripe customers you can `INNER JOIN` on `distinct_id` and `email` like this:
