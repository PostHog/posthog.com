...
The SQL editor sidebar has a connection selector to choose which database to query:

- **PostHog (ClickHouse)** – The default. Query your PostHog warehouse data, including events, persons, sessions, and synced external sources.

- **Direct database connections** – Query databases like Postgres directly without syncing data into PostHog. These appear after you [link a Postgres source](/docs/cdp/sources/postgres) with the **Query directly** access method.

When you select a direct database connection, the sidebar displays that database's schema instead of the PostHog warehouse schema. You can also enable **Send raw query** to bypass HogQL translation and send queries directly to the connected database using its native SQL dialect.

<CalloutBox icon="IconWarning" title="Direct query limitations" type="caution">

Data from direct query sources cannot be joined with PostHog data. If you need to combine external database data with PostHog events or persons, use the **Sync to warehouse** access method instead.
