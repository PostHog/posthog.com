...
A saved view in a data modeling directed acyclic graph (DAG) runs its checks when the DAG executes that view. PostHog runs the DAG in dependency order, then starts the applicable checks for each view that ran.

A view outside a DAG has no automatic execution trigger. Run its checks manually.

Disabling a check prevents it from running after source syncs, materializations, DAG runs, schedules, and manual **Run all checks** actions.
