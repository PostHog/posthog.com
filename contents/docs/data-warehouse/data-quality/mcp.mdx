...
For a personal API key, include `query:read` and the matching subject scope. Use `warehouse_objects:read` or `warehouse_objects:write` for warehouse subjects, and `data_catalog:read` or `data_catalog:write` for metrics.
...
Call `data-quality-check-run` with the returned check `id`. The tool returns a suite run before the individual check finishes. Poll `data-quality-check-results` with the check ID until the result is `passed`, `failed`, `errored`, or `skipped`.
...
Call `data-quality-check-delete` to soft-delete a check. The definition no longer runs, but its past rows remain in `system.information_schema.data_quality_check_runs`. A deleted check releases its name and assertion so you can create them again as a new check.
...
A subject gets its schedule when you add its first check. Check the current settings on the subject before updating them. If an update returns an uncertain result, reload the subject's schedule before you try again.
...
This table is bounded to the newest 500 readable runs. A deleted check's history remains, with `check_id` set to null. Use `data-quality-check-results` when you need the compiled query from a specific check run.
