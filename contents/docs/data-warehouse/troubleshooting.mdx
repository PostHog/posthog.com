...
## Why is my BigQuery sync failing with a custom quota error?

If your BigQuery sync fails with a message like "Your BigQuery project hit a custom usage quota set by your administrator," it means the sync exceeded a custom cost control on your Google Cloud project, such as a `QueryUsagePerDay` daily spend cap. This is a non-retryable error – PostHog stops the sync instead of retrying indefinitely, since the quota only resets on Google's daily schedule.

To fix this:

1. In the [Google Cloud Console](https://console.cloud.google.com/), raise the custom cost-control quota for your project under **IAM & Admin > Quotas & System Limits**, or remove the cap.
2. Or, reduce how much data you're syncing so it stays under the quota.
