...
## Why is my BigQuery sync failing with "Billing has not been enabled"?

If your BigQuery sync fails with a message like "Billing has not been enabled for this project," it means your Google Cloud project is running in BigQuery sandbox mode, which doesn't allow the query jobs PostHog needs to import data. This is a non-retryable error – PostHog stops the sync instead of retrying indefinitely.

To fix this:

1. Go to the [Google Cloud billing console](https://console.cloud.google.com/billing) and enable billing for your project.
2. In PostHog, go to [**Data pipelines > Sources**](https://app.posthog.com/data-management/sources) and select your BigQuery source.
3. Re-run the sync.
