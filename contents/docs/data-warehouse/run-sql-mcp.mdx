...
| `query-validate`                     | Validate a HogQL query without executing it. Returns structured errors with character positions, warnings, and the list of tables referenced. No ClickHouse cost.                                       |
| `hogql-schema`                       | List all tables and columns available to HogQL queries. Returns every queryable table (PostHog, system, warehouse, views) with field names and types. Call this before writing queries from scratch.    |
