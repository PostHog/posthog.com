...
- **Query external databases directly** – Run queries against Postgres or DuckDB connections without syncing data into PostHog
...
| Tool                                 | Description                                                                                                                                                                                                     |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `query-run`                          | Run a HogQL query and return results. Supports any valid HogQL syntax including joins, CTEs, subqueries, and window functions. Accepts an optional `connectionId` to query Postgres or DuckDB sources directly. |
| `query-validate`                     | Dry-run a HogQL query to check for errors without executing it. Returns structured errors, warnings, and referenced tables. Accepts an optional `connectionId` to validate against an external source.          |
| `hogql-schema`                       | List all tables and columns available to HogQL queries. Accepts an optional `connectionId` to return the schema of a Postgres or DuckDB source instead of the ClickHouse catalog.                               |
...
The `query-run`, `query-validate`, and `hogql-schema` tools accept an optional `connectionId` parameter. When set, queries run against that external data source (such as a Postgres or DuckDB direct-query connection) instead of ClickHouse.

To find available connection IDs, use the `external-data-sources-list` tool.
...
For querying external data sources directly:

- `List my external data sources so I can find connection IDs.`
- `Show me the schema of my Postgres connection.`
- `Run a query against my DuckDB source to count all rows in the users table.`
- `Validate this query against my Postgres connection before running it.`
