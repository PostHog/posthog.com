...
| `external-data-sources-preview-resource` | Preview sample rows for one resource of a [Custom REST source](/docs/cdp/sources/custom-rest-source) to verify `data_selector`, `primary_key`, and `cursor_path` against live data before creating the source. Only Custom source type is supported. |
...
- `Preview sample rows from my Custom REST source to verify the data_selector is correct.`
