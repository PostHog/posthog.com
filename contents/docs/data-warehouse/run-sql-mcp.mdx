---
title: Run SQL, manage views, sync sources, and manage SQL variables with MCP
---

The [PostHog MCP server](/docs/model-context-protocol) lets your AI coding agent run [HogQL](/docs/hogql) queries, manage [Data Warehouse](/docs/data-warehouse) views, sync import sources, manage SQL variables, and explore your schema directly from your code editor. Query events, create and materialize views, configure Postgres/Stripe/HubSpot imports, manage reusable SQL variables, and work with warehouse tables – without switching to the PostHog app.
...
- **Manage SQL variables** – Create, update, and delete reusable SQL variables for HogQL queries
...
## SQL variable management tools

These tools let you manage [SQL variables](/docs/endpoints/variables) – reusable placeholders you reference in HogQL queries using the `{variables.code_name}` syntax. You can also query the `system.insight_variables` table to list existing variables.

| Tool                   | Description                                                                                                                                                             |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sql-variables-create` | Create a reusable SQL variable for HogQL queries. Provide a name and type. The `code_name` is generated from the name and referenced in SQL as `{variables.code_name}`. |
| `sql-variables-update` | Update an existing SQL variable by ID. Change the name, type, default value, or allowed values for List variables.                                                      |
| `sql-variables-delete` | Delete a SQL variable by ID. Queries referencing this variable by `code_name` may stop working after deletion.                                                          |
...
For SQL variable management:

- `Create a SQL variable called "customer_id" with type String and default value "acme-corp".`
- `Update my "region" SQL variable to have default value "US".`
- `Delete the SQL variable with ID "019db4bb-7a2b-0000-3539-34b85883a064".`
- `Query system.insight_variables to list all my SQL variables.`
