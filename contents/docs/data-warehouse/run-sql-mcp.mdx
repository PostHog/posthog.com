...
- **Validate queries before running** – Check HogQL syntax and catch column typos without ClickHouse cost
...
- **Query Postgres/DuckDB directly** – Run queries against direct-query data warehouse connections without going through ClickHouse
...
| Tool                                 | Description                                                                                                                                                                                             |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `query-run`                          | Run a HogQL query and return results. Supports any valid HogQL syntax including joins, CTEs, subqueries, and window functions. Pass `connectionId` to query a Postgres or DuckDB source directly.       |
| `query-validate`                     | Validate a HogQL query without executing it. Returns structured errors with character positions, warnings, and the list of tables referenced. Pass `connectionId` to validate against a direct-query source. |
| `hogql-schema`                       | List all tables and columns available to HogQL queries. Returns every queryable table (PostHog, system, warehouse, views) with field names and types. Pass `connectionId` to get the schema of a direct-query source. |
...
Postgres and DuckDB data warehouse connections support direct querying, which lets you run queries against the source database without going through ClickHouse. This is useful when you need real-time data from your external database or want to use native SQL syntax.

To query a direct-query source:

1. Use `external-data-sources-list` to find the `connectionId` of your Postgres or DuckDB source
2. Pass that `connectionId` to `query-run`, `query-validate`, or `hogql-schema`

When you pass a `connectionId`, `hogql-schema` returns the source's native schema and `query-validate` checks your query against that schema before execution.
...
For schema discovery and query validation:

- `Show me all tables and columns available for HogQL queries.`
- `What fields are on the events table?`
- `Validate this HogQL query before running it: SELECT user_id FROM events`
- `Check if my query has any errors: SELECT * FROM event WHERE timestamp > now() - INTERVAL 1 DAY`

For direct Postgres/DuckDB querying:

- `List my data warehouse sources and show which ones support direct queries.`
- `Show me the schema for my Postgres data warehouse connection.`
- `Run this query directly against my Postgres source: SELECT * FROM customers LIMIT 10`
- `Validate this query against my DuckDB connection before running it.`
