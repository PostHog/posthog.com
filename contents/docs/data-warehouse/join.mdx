...
Person joins are a special type of table joins. They are joins on the `persons` table in PostHog. When you join external data on this table, we enable you to use it like a person filter in insights. Extended person properties from these joins can be used in [Trends](/docs/product-analytics/trends), [Funnels](/docs/product-analytics/funnels), and [Retention](/docs/product-analytics/retention) insights.
