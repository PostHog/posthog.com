...
On some sources and tables, you'll be given the option to choose the sync method for that table. The options are incremental replication or full table replication. 

### Incremental

With incremental replication, you only sync new or updated data, this reduces the total amount of rows synced and reduces how long it takes to sync. When choosing incremental replication, you'll have to select a field to identify new or updated data, this is often something like a `updated_at` or `created_at` timestamp, or an autoincrementing id.

Not all fields are suitable to be used to identify new and updated data, and so we only support the following types to be used as replication key:
...
The one downside to incremental syncing is that deletions of data won't be synced to your PostHog data warehouse - you'll need to use full table refreshes for this

### Full table

This will reload the whole table on every sync. This option is great for tables with common data deletions, and for tables without an incrementing field (such as a `updated_at` timestamp).
