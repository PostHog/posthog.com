...
It effectively lists how often each table is referenced. This means it also includes views and other queries that reference tables, so it's not a perfect one-to-one-map, but it might be able to help you identify unused or underused tables and disable syncs for them.