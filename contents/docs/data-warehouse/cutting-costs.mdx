...
PostHog offers sync options from every 5 minutes to every 12 hours to monthly. For incremental and full table syncs, increasing the sync frequency can reduce the number of rows synced.
...
Beyond just looking at [the source synced](https://us.posthog.com/pipeline/sources), you can use PostHog's `query_log` table. This contains details on all queries run in your project. You can use this to see what tables are (or are not) being queried and how often.
...
SELECT 
  IF(position(tok,'.') > 0, arrayElement(splitByChar('.', tok), length(splitByChar('.', tok))), tok) AS table_after_from,
  count() AS hits
FROM tokens
WHERE tok != ''
...
This query gets the most queried tables after the `FROM` keyword in the query (and it's a bit complex, we know). This means it also includes views and other queries that reference tables, but it might be able to help you identify unused or underused tables and disable syncs for them.