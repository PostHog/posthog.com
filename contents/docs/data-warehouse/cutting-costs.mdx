...
PostHog offers sync options from every 15 minutes to every 12 hours to monthly. For incremental and full table syncs, decreasing the sync frequency can reduce the number of rows synced.
...
Beyond just looking at [the source synced](https://app.posthog.com/data-management/sources), you can use PostHog's [`query_log` table](/docs/data/query-log). This contains details on all queries run in your project. You can use this to see what tables are (or are not) being queried and how often.
