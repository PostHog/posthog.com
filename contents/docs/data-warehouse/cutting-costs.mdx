...
Beyond just looking at [the source synced](https://app.posthog.com/pipeline/sources), you can use PostHog's [`query_log` table](/docs/data/query-log). This contains details on all queries run in your project. You can use this to see what tables are (or are not) being queried and how often.
