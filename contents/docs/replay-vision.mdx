...
The key idea is that Replay Vision actually _watches the video_ of each recording alongside its events. This lets it pick up on visual and behavioral cues – hesitation, confusion, where a user's attention goes – that aren't captured by clicks and pageviews alone. Instead of manually reviewing hundreds of replays to spot a pattern, you describe what matters once and let it run continuously.
...
3. Write a prompt describing what the scanner should look for. Be concrete and avoid speculation – e.g. _"Return true if someone appears stuck on a page – scrolling without engaging, hovering over elements with no clear CTA, or abandoning shortly after arriving."_
...
7. Save. The scanner is enabled by default. You can toggle a scanner on or off at any time from its overview page using the **Enabled**/**Disabled** switch in the page header.
...
Each scanner's overview page has an **Enabled**/**Disabled** toggle in the header. Flip it to pause or resume a scanner without opening the full edit flow. Disabling a scanner stops it from running against new recordings; re-enabling it resumes automatic observation. Toggling requires Editor-level access.
