...
An evaluation only grades generations captured after you create it. To apply one to data you already have, run a **backfill** from the **Backfills** tab on the evaluation.

### How backfills work

A backfill runs the evaluation over a time range you pick, using the same conditions and sampling rate as the live evaluation. It skips anything the evaluation has already graded, so running one over a range the evaluation was already covering finds nothing to do.
...
3. Adjust the conditions if you want a narrower scope than the live evaluation uses
4. Check the count above the **Start backfill** button, which shows how many generations would be evaluated
...
Turn on **Include generations that already have a result** to grade generations a second time. Use it after changing a prompt or a code check, when you want the new version applied to data the old version already scored. Both results are kept, so you can compare them.

### Reading a backfill's progress

The **Covered** column counts the generations in range that hold a result. A finished backfill usually reads as fully covered, because the evaluation grades new data on its own while the backfill works through older data.

Expand a row to see the range, the conditions it used, and how many generations it started, skipped, or left without a result.
...
Each generation a backfill evaluates counts as one AI Observability event, the same as a live evaluation, and LLM judge backfills use your provider API key. The confirmation dialog shows how many generations will be evaluated before you start, so check that number on a wide range.
