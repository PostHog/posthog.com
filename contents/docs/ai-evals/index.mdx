...
The MCP server provides these evaluation management tools:

| Tool                       | Description                                                             |
| -------------------------- | ----------------------------------------------------------------------- |
| `llma-evaluation-list`     | List evaluations, with an optional search term and enabled filter       |
| `llma-evaluation-get`      | Get one evaluation by UUID                                              |
| `llma-evaluation-create`   | Create an LLM-as-a-judge, code-based (Hog), or sentiment evaluation     |
| `llma-evaluation-update`   | Update the configuration, name, or enabled status of an evaluation      |
| `llma-evaluation-delete`   | Soft-delete an evaluation. Past results stay                            |
| `llma-evaluation-run`      | Run an evaluation on one generation event. The run is asynchronous      |
| `llma-evaluation-test-hog` | Test Hog code against recent data before you save it to an evaluation   |

For the full list, including the tools for evaluation reports and directories, see [Use AI Observability over PostHog MCP](/docs/ai-observability/surfaces/mcp). That list is generated from the MCP tool catalog at build time, so it stays current.
