...
Evaluations automatically assess your LLM [generations](/docs/ai-observability/generations) as they're captured. Most evaluations return a pass/fail result with reasoning, while sentiment evaluations classify how users feel in their messages. PostHog supports three types of evaluations:
...
- **Sentiment analysis** – Classifies the sentiment of user messages on each matching generation as positive, neutral, or negative using a local ML model. Returns a sentiment label and score instead of a pass/fail result, with no prompt or code to write. Free to run with no LLM cost.
...
Unlike LLM-as-a-judge and code-based evaluations, they don't return a pass/fail verdict – each run produces a sentiment label and a confidence score. For more on how PostHog classifies sentiment, see [Sentiment classification](/docs/ai-observability/sentiment).
...
Because sentiment is classified automatically, there's no prompt or code to write. You only choose which generations to evaluate, using the same sampling rate and property filters as the other evaluation types, so you can scope them to production traffic, specific models, or particular user segments.

Sentiment evaluations don't use an LLM judge, so they don't require a provider API key. They also don't support N/A results, success rates, or evaluation reports, since those apply only to pass/fail evaluations.
...
| `evaluation-create` | Create LLM judge, Hog code, or sentiment evaluations           |
