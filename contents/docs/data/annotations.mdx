...
Annotations appear by default on time-series charts. You can toggle them on or off without editing the full query.

To toggle annotations from an insight card, click the **⋯** menu and select **Show annotations** or **Hide annotations**. This option appears for:

- Trends insights with time-series display types (line, area, bar charts)
- Funnels with the historical trends visualization

You can also toggle annotations while editing an insight. Open the chart settings panel, then check or uncheck **Show annotations** in the display options.
