...
To delete an action, go to the [**Actions** tab in Data Management](https://app.posthog.com/data-management/actions), click the **more** (⋯) menu next to the action, and select **Delete action**. You can also delete an action from its detail page.

If the action is referenced by other resources – such as insights, experiments, cohorts, or destinations – PostHog shows a warning dialog before deleting. The dialog tells you how many resources reference the action and that deleting it may break them. You can choose to **Delete anyway** or **Cancel**.
