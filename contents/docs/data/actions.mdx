...
In PostHog, for example, we track custom events like `insight created`, `insight analyzed`, and `dashboard created`. These are all different ways to interacting with product analytics.

We then combine these and similar events into an action called `Product Analytics Interactions`, which gives us an aggregate view of how much people are using product analytics.

Actions are also useful for tracking conversions on your website. Our website homepage, for example, has several call to actions (CTAs) to signup to PostHog. We combine all these events into a single `Homepage CTA` action, so we can track all clicks from the signup page in a trends insight.

## How to create an action

You can create new actions by either: 

1. Clicking the "New action" button in the 'Actions' tab in [Data Management](https://app.posthog.com/data-management/events).

2. Using the [PostHog Toolbar](/docs/toolbar) to tag the elements that are relevant – see our step-by-step tutorial on [creating actions using the toolbar](https://posthog.com/tutorials/how-to-capture-events-the-easy-way).
...
The easiest way to select them is [using the toolbar](https://posthog.com/tutorials/how-to-capture-events-the-easy-way), but you can also set them manually by providing something to identify the element, like a selector.
