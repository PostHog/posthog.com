...
Actions are also useful for tracking conversions on your website. Our website homepage, for example, has several call to actions (CTAs) to sign up to PostHog. We combine all these events into a single `Homepage CTA` action, so we can track all clicks from the signup page in a trends insight and run A/B tests with this action as a goal.

## How to create an action

You can create new actions by either:

1. Clicking the "New action" button in the "Actions" tab in [Data Management](https://app.posthog.com/data-management/actions).
...
- **matches exactly** - the screen name matches one of the specified values exactly (supports multiple screen names)
...
## Copying an action to another project

You can copy an action to another project in the same organization. This option shows only when your organization has more than one project.

1. Open the action you want to copy.
