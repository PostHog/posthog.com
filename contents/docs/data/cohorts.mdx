...
You list of cohorts are accessible from the People page 
...
1. Go to [insights](https://app.posthog.com/insights) and open any insight that's aggregated by users – i.e. `unique users`, `weekly active user`, etc. Click a data point on the visualization to view the [persons](/docs/data/persons) represented in the underlying data.
...
1. Navigate to the [Persons](https://app.posthog.com/persons) page within the sidebar, then click on the [Cohorts tab](https://app.posthog.com/cohorts):
...
### Static cohorts
Static cohorts are created from insights, or by uploading a CSV of users. 
...
| **Did not complete a sequecne of events or actions** | Did not complete `user signed up` in the last 5 days followed by `watched video` within 1 day of the initial event |
