...
You can select `days`, `weeks`, `months` or `years` time ranges here. For "Completed event" and "Completed event multiple times" criteria, you can also specify a date range with both a start and end date. For example, "Completed `purchase` between 30 days ago and 7 days ago" targets users who performed the event within that specific window, excluding more recent activity.
...
You can select `days`, `weeks`, `months` or `years` time ranges here. For "Completed an event for first time," you can also specify a date range with both a start and end date, or use absolute dates.
