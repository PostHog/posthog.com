...
Cohorts enable you to easily create a list of users who have something in common, such as completing an [action](/docs/data/actions) or having the same [property](/docs/product-analytics/user-properties).
...
> **Note:** Cohorts rely on [user properties](/docs/product-analytics/user-properties), so you need to capture events with [person profiles](/docs/data/persons/#people-and-person-profiles) to create and use them. 
...
Once you've captured [events with person profiles](/docs/data/persons#capturing-person-profiles), there are two ways to create a new cohort:
...
### Can you use a dynamic behavioral cohort as a feature flag target?
 ...
 Not yet, but we are working on [rewriting cohort calculations in HogQL](/teams/product-analytics#goals) which will unlock your ability to do this. 
