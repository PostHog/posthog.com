...
Cohorts enable you to easily create a list of users who have something in common. You can use the cohorts to view a list of users who all did the same action or have the same property.
...
- Groups aggregate events based on entities, such as organizations or companies, and do not necessarily have to be connected to a user. They enable you to analyze trends, insights, and dashboards at an entity-level, as opposed to a user-level.
...
You can create cohorts using data from insights such as trends, [funnels](/product-analytics/funnels), [user paths](/docs/product-analytics/paths) and [lifecycle](/docs/product-analytics/lifecycle).

**Step 1:** Go to [insights](https://app.posthog.com/insights) and open any insight. Click on any data point to view persons represented in the underlying data.
...
1. Either using [`posthog.identify`](/docs/getting-started/identify-users) and setting a unique ID for your team members, or
