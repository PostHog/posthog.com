...
Dynamic (SQL) cohorts require the `cohorts-allow-custom-hogql` feature flag. Contact support if you'd like early access.

</CalloutBox>

Dynamic (SQL) cohorts use a [HogQL](/docs/sql) query to define membership. This is useful for complex behavioral conditions that can't be expressed with the standard filter builder, like:

- Users who completed event A but not event B in the last 30 days
- Users with more than five pageviews grouped by person
- Complex combinations of event sequences and properties

To create a Dynamic (SQL) cohort:

1. Go to **People** > **Cohorts** and click **New cohort**.
2. Select **Dynamic (SQL) · Define with HogQL** from the type dropdown.
3. Write a HogQL query that returns person IDs in the SQL editor.
4. Click **Update query** or press **Cmd+Enter** to preview matching persons.
5. Click **Save** to create the cohort.

Your query must return one of these columns: `person_id`, `actor_id`, `id`, or `distinct_id`. When querying from the events table, use `person.id AS person_id` to get the person identifier.

Here's an example query that creates a cohort of users who viewed a page more than five times in the last 30 days:
...
Like other dynamic cohorts, SQL cohorts are recalculated automatically once every 24 hours.
