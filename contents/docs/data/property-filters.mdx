...
<CalloutBox icon="IconInfo" title="Feature flag limitations" type="fyi">

When using property filters in [Feature Flags](/docs/feature-flags), some numeric operators have limitations:
...
> **Note:** Standard semver format (e.g., `1.2.3`) works best. Two-component versions like `3.10` and zero-padded versions like `3.08` are also supported — they are automatically normalized (to `3.10.0` and `3.8.0` respectively) during server-side (remote) flag evaluation. Local evaluation in server-side SDKs, and cohort/insight analytics computed in ClickHouse, still require standard MAJOR.MINOR.PATCH semver. Genuinely invalid strings (e.g., `not-a-version`) won't match any semver condition.
