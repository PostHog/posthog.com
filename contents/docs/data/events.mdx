...
1. An `event` name like `$pageview` or `query completed`. Events starting with `$` are PostHog defaults. Events starting with `$ai_` are [AI Observability events](/docs/ai-observability/basics), which are stored and billed differently.
