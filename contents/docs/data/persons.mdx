...
Using people in PostHog requires capturing [identified events](/docs/data/anonymous-vs-identified-events). When you capture your first identified event for a user, it creates a **person profile** for them. Then, any future events captured will be attributed to them. You can also [set properties](/docs/product-analytics/person-properties) for the user.
