...
## Enabling person last seen tracking

PostHog doesn't track when people were last active by default. To enable it, go to [**Settings**](https://app.posthog.com/settings/project-product-analytics#person-properties) > **Project** > **Person properties** and toggle on **Person last seen tracking**. This requires admin access.

Once enabled, PostHog records when each person was last active (updated hourly) and shows it in the **Last seen** column of the People list.
...
> **Note:** The **Last seen** column only appears if you've [enabled person last seen tracking](#enabling-person-last-seen-tracking) in your project settings.
