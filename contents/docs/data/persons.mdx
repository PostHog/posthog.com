...
By default, PostHog captures events with person profiles. To change this in the JavaScript Web configuration, change the `process_person` config option with one of the three options:
...
Other SDKs and the API can pass the `$process_person` property on events.
