...
Events with person processing create or update an associated person profile (see above). This allows for the following features:
...
- Tracking initial UTM tags and referrers across anonymous and identified users
...
When sending events from other SDKs, you can pass a `$process_person` property. There are two options:

- `$process_person: true` _(default)_ - we will process persons data for this event
- `$process_person: false` - we won't process persons data for this event
