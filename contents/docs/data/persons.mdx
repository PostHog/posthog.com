...
A single real-world user can end up with multiple person profiles in PostHog. This typically happens when someone uses different devices or browsers before being [identified](/docs/product-analytics/identify) — each device generates a separate anonymous distinct ID, which creates a separate profile.

This means that counts based on person profiles (such as cohort sizes or feature flag targeting) may be higher than your actual number of unique users.

To merge duplicate profiles, see [how to merge users](/docs/product-analytics/identify#how-to-merge-users). To prevent duplicates, call [`identify`](/docs/product-analytics/identify) as soon as a user logs in on each device.
