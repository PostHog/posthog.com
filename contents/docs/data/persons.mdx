...
For backward compatibility and to make sure we don't drop person information accidentally, processing events with persons is the default for configs without any option set. We also "force upgrade" anonymous users created before person profiles lanched.
...
Other SDKs and the API can pass the `$process_person_profile` property on events. Again, person profiles are enabled by default, but can be disabled by setting the property to `false`.
