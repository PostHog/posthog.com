...
### Skipping last seen updates

Every event updates the person's **Last seen** timestamp by default, including events from backend enrichment jobs or automated processes.

To prevent this, set `$update_person_last_seen_at` to `false`:
...
This is useful for backend processes that update person properties but don't represent actual user activity.
