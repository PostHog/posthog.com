...
These warnings apply to [AI Observability](/docs/llm-analytics) events.

| Warning | Cause |
| --- | --- |
| Discarded an AI event with an unsupported event name or no `$ai_model` | The event name didn't start with `$ai_`, or the required `$ai_model` property was missing. |
| Discarded a non-AI event sent to the LLM analytics capture endpoint | An event whose name doesn't start with `$ai_` was sent to the LLM analytics endpoint (`/i/v1/ai/events`). Only that event was discarded. The rest of the batch was ingested. Send events without an `$ai_` name to the standard capture endpoint instead. |
