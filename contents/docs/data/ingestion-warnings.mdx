...
- [Distinct ID was truncated](#distinct-id-was-truncated)
...
## Distinct ID was truncated

Legacy capture endpoints automatically shorten any `distinct_id` that exceeds 200 characters. When this happens, PostHog ingests the event with a truncated `distinct_id`. This warning means your event was stored under a shortened ID that differs from what you originally sent.

This typically happens when a JWT token, serialized JSON payload, or other long string is accidentally passed as the `distinct_id` instead of a proper user identifier. The truncation can cause person data to fragment because events land under an ID you never intended to use.

To fix this, ensure you're passing a stable, short identifier (such as a database ID or email) as the `distinct_id`. See [identifying users](/docs/product-analytics/identify) for best practices.

> **Note:** The newer v1 capture endpoint rejects oversized distinct IDs outright (`distinct_id_too_large`) rather than truncating them. This warning only applies to legacy capture endpoints.
