...
This warning means PostHog's rate limiter detected an unusually high volume of events for a specific `distinct_id` and disabled [person profile](/docs/data/persons) processing for those events. The events are still ingested, but person properties won't update for them, which can cause missing person data.

This is typically caused by an instrumentation bug such as a loop repeatedly sending events with the same `distinct_id`. Review your event capture code to ensure you're not sending events in a tight loop or from a background process that retries indefinitely.
