...
- Aggregate and filter events by event properties e.g. URL, geographic location, utm source, etc.
- Create insights like [trends](/docs/data/trends), [funnels](/docs/data/funnels), [SQL insights](/docs/product-analytics/sql) and more.
...
### What happens to previous and future events when an anonymous user is identified? 

All future events are associated with their person profile and are captured and billed as identified events.

All past events remain anonymous and are not changed.

### Can identified events still be "anonymous"?

Yes, identified events can still be "anonymous" in the sense that they don't need to contain personal information such as name, email, phone number, etc. A person profile is still created for them, but it doesn't have to include any personal information.

### Can I specify some events to be identified, and keep other events anonymous for the same users?

Not if you're already identified them. Once a user is identified, all _future_ events for that user are associated with their person profile and are captured as identified events.

### How do I revert back to anonymous events after identifying them?
...
Since identified events are associated with a person profile, querying them requires using resource intensive table JOINs.

On the other hands, anonymous events are stored in a single table and are more efficient to query.

### Why does PostHog capture identified events by default?

This is due to legacy reasons. Previously, all events were captured as identified events. When we added anonymous events, we didn't want to change the default behavior since that could cause issues with existing users.
