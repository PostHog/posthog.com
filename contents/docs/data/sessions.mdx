...
Within one session, PostHog adds a `$window_id` property to each event. This property identifies the tab or window that sent the event. Session replay uses it to stitch every tab into one recording, and to show you which tab each moment came from. Read [recordings across multiple tabs and windows](/docs/session-replay/how-to-watch-recordings#recordings-across-multiple-tabs-and-windows).
...
You can query all of these from the `sessions` table using [SQL insights](/docs/data-warehouse/sql).
