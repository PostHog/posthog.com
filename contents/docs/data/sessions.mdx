...
If you enable tracing headers within the JavaScript SDK, PostHog adds session, window, and distinct ID headers to `fetch` and `XMLHttpRequest` calls to your backend. You can enable tracing headers by adding the `tracing_headers` option to your `posthog.init` call:
