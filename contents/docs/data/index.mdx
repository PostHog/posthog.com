...
There are very few cases where it would be good to manually provide an event UUID. PostHog generates these for you, and that should be sufficient in almost all cases. The valid use cases for sending your own UUID are:

- unreliable connections (transparently implemented in our web and mobile SDKs)
- an event source that does not guarantee idempotency (webhook, kafka), and only if a [valid UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier) is available: not all 128-bit words are valid UUIDs.

Beyond the above officially supported cases, we do not support supplying a UUID for deduplication or event mutation. There are many potential unintended consequences, and we cannot support recovery in cases where data is lost or corrupted as a result of this process.
