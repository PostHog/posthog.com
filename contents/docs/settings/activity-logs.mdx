...
Organization admins and owners can view activity across all projects from a single view. This helps you audit changes across your organization without switching between projects.

To access organization-wide activity logs:

1. Go to **Settings** → **Project** → **Activity logs**
2. Click the **Project** / **Organization** toggle in the top right and select **Organization**

In organization view:

- A **Project** column shows which project each activity belongs to (or "Organization-wide" for org-level events)
- A **Project** filter lets you narrow results to specific projects using multi-select
- Deep-link to the organization view with `?view=organization` in the URL, and filter by project with `&team_ids=123,456`

> **Note:** Export isn't available in organization view yet. Switch to project view to export activity logs.
...
Activity log notifications are powered by [data pipelines](/docs/cdp). When an activity is logged, PostHog emits an internal `$activity_log_entry_created` event that triggers the notification destinations you configure from the activity logs settings page.

> **Note**: `$activity_log_entry_created` is an internal-only event. It does **not** flow into your project's regular event stream, so you cannot query it from the `events` table, use it in product analytics insights, dashboards, or HogQL, or filter on it from standard [data pipeline destinations](/docs/cdp). It exists solely to drive activity log notifications configured from this settings page. If you need activity log data outside of notifications, use the [activity log API](#api-support), [HogQL on `system.activity_logs`](#query-with-hogql), or [export](#export-options) instead.
...
The project-scoped endpoints return activity logs for the current project. If **Show organization activity** is enabled in project settings, organization-level events are also included.

The organization-scoped endpoint returns activity across all projects in the organization. It supports a `team_ids` filter to narrow results to specific projects and is restricted to organization admins and owners. Export is not yet available on this endpoint.
