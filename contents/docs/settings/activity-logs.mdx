...
| `GET /api/organizations/<id>/advanced_activity_logs/`       | Organization-wide activity logs (admins and owners only)        |
||||||| ed7372840
| Endpoint                                             | Description                                               |
| ---------------------------------------------------- | --------------------------------------------------------- |
| `GET /api/projects/@current/activity_log/`           | Basic activity log retrieval with simple filtering        |
...
The organization-scoped endpoint returns activity across all projects in the organization. It supports a `team_ids` filter to narrow results to specific projects. Export is not yet available on this endpoint.
||||||| ed7372840
