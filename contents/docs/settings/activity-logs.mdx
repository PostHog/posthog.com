...
| IP address | The client IP address captured at request time. Shows `—` for system or background job activity.                                                      |
...
**IP address**
Filter by the client IP address that made the request. Enter exact IPv4 or IPv6 addresses, or use wildcards to match ranges (e.g., `203.0.113.*` matches all addresses in the 203.0.113.x subnet). This helps when investigating activity from specific locations or spotting unusual access patterns.
