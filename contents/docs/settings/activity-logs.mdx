...
| IP address | The IP address of the client that initiated the action. System-generated actions display "—"                                                          |
...
- Export is not available in this view – use the project view for exports
...
**IP address**
Filter by the client IP address that initiated the action. Use this to investigate actions originating from specific IP addresses or to identify suspicious activity from unknown locations. Wildcards are supported (e.g., `203.0.113.*`). System-generated actions don't have IP addresses and won't appear when filtering by IP.

## Exporting and retention

### Retention periods

<FeatureAvailability availability={_frontmatter.availability.features.activityLogRetention} />

Activity logs are retained for a period that your [platform package](/platform-packages) sets:

| Platform package | Retention period |
| --- | --- |
| None (Free or Pay-as-you-go) | Activity logs are not available |
| Boost | 7 days |
| Scale | 2 months |
| Enterprise | 60 months |

Enterprise plans can also agree a custom retention window. If you are on a plan that is no longer for sale, your retention period can be different. Open the platform package section of your [billing settings](https://us.posthog.com/organization/billing) to see the period that applies to your organization.
