...
PostHog's access control system manages permissions at three levels: organization, project, and resource.

## Data and property access

Resource access controls who can open dashboards, flags, and other product surfaces. Separate controls restrict the data itself:

- **[Warehouse access control](/docs/settings/warehouse-access-control)** – Restrict who can query warehouse tables and views, including through [PostHog AI](/docs/posthog-ai) and the API.
- **[Hiding sensitive properties](/docs/settings/hiding-sensitive-properties)** – Hide event and person properties from filters and queries. Requires the [Enterprise plan](/platform-packages).
...
<CalloutBox icon="IconWarning" title="Native tables, replay, and profiles are not restricted yet" type="caution">

Resource access controls limit who can open product surfaces in the PostHog UI and API. They don't restrict querying PostHog's native tables (`events`, `sessions`, `persons`, `groups`), viewing session replays, or opening person and group profiles.

[Warehouse tables and views](/docs/settings/warehouse-access-control) are restricted at query time, including through [PostHog AI](/docs/posthog-ai) and the API. You can also [hide event and person properties](/docs/settings/hiding-sensitive-properties).
...
Restrict properties such as `email` from a vendor role. See [hiding sensitive properties](/docs/settings/hiding-sensitive-properties#hide-pii-from-vendors).
