...
export const resourceAccessControlLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/q_auto,f_auto/resource_access_control_light_638d035213.png";
export const resourceAccessControlDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/q_auto,f_auto/resource_access_control_dark_fd4a9382dd.png";
export const objectAccessControlLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/q_auto,f_auto/object_access_control_light_5c740b9e38.png";
export const objectAccessControlDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/q_auto,f_auto/object_access_control_dark_03278b80ea.png";
...
Some of these resource types are still in beta, so they only show in the list if your project has the related product enabled.

Resource access controls limit who can open product surfaces in the UI or execute the corresponding MCP tools.

<CalloutBox icon="IconWarning" title="Native tables aren't restricted yet" type="caution">

Queries to the core PostHog tables (`events`, `sessions`, `persons`, `groups`) through the SQL editor or MCP still work. To prevent queries from returning sensitive event or person properties (for example, `email`), configure [property access control](/docs/settings/property-access-control).
...
When you connect external sources to PostHog through the [Data Warehouse](/docs/data-warehouse), you can configure who can query the data synced into PostHog. Apply access controls to all sources or to a specific one, table, or view. See [warehouse access control](/docs/settings/warehouse-access-control) for details.

## Property access control

Event or person data may contain sensitive properties (for example, email addresses). To prevent someone from exporting your entire user base, configure [property access control](/docs/settings/property-access-control).

When a property is restricted, explicit reads return `NULL`, and results that include all properties omit it. This applies in both the SQL editor and MCP.
