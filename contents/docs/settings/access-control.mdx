...
#### 4. Effective access levels

These read-only endpoints return computed effective access levels that consider defaults, role overrides, and organization admin status. Each entry includes the saved override (`access_level`), the computed maximum (`effective_access_level`), and the inherited floor from defaults or roles (`inherited_access_level` with `inherited_access_level_reason`).

- `GET /api/projects/@current/access_control_defaults` – Project and resource default access levels
- `GET /api/projects/@current/access_control_roles` – Effective access per role
- `GET /api/projects/@current/access_control_members` – Effective access per member
...
#### Getting access control defaults

**Request**: `GET /api/projects/@current/access_control_defaults`

**Response**:
```json
{
  "can_edit": true,
  "available_project_levels": ["none", "member", "admin"],
  "available_resource_levels": ["none", "viewer", "editor", "manager"],
  "project_access_level": "member",
  "resource_access_levels": {
    "dashboard": { "access_level": "viewer", "minimum": "none", "maximum": "manager" },
    "feature_flag": { "access_level": null, "minimum": "none", "maximum": "manager" }
  }
}
```

#### Getting effective access for roles

**Request**: `GET /api/projects/@current/access_control_roles`
...
- `access_level` – The explicitly saved override, or `null` if none is set
- `effective_access_level` – The computed maximum of the override and inherited level
- `inherited_access_level` – The floor from project defaults
- `inherited_access_level_reason` – Why the inherited level applies (e.g., `project_default`)

#### Getting effective access for members

**Request**: `GET /api/projects/@current/access_control_members`
...
```json
{
  ...
  "results": [
    {
      "organization_membership_id": "018e0d4f-...",
      "user": {
        "uuid": "...",
...
Each member entry includes the same fields as role entries. The `inherited_access_level_reason` can be one of:
- `project_default` – Inherited from the project's default access level
- `role_override` – Inherited from a role assignment
- `organization_admin` – Organization admins automatically receive the highest access level
