...
- **Querying data** – any project member can query events and other data via SQL/HogQL, including through the [PostHog AI assistant](/docs/posthog-ai) and the API. Data warehouse tables synced from external sources respect access controls from both the table and the source (see [Data warehouse table access fallback](#data-warehouse-table-access-fallback)).
...
Each project can have up to 1,000 objects per resource type with individual access controls. For example, you can set custom access controls on up to 1,000 dashboards, 1,000 insights, and so on. If you need a higher limit, contact support.
...
3. **Fallback parent object permissions** - For resources linked to a parent (e.g., a synced warehouse table linked to its source), permissions set on the parent object
4. **Fallback parent resource type permissions** - Access set for all resources of the parent type (e.g., all data warehouse sources)
5. **Default object permissions** - Default access that applies to a resource object (e.g., Insight Y has view access by default)

**Note**: Resource creators and organization admins always have full access, regardless of these precedence rules.

### Data warehouse table access fallback

Data warehouse tables synced from external sources (such as Stripe, Postgres, or HubSpot connectors) have a parent relationship with their source. When no specific table-level or "Data warehouse tables & views" resource-level rule applies, PostHog **falls back to the source's access control** — both object-level rules on that source and the "Data warehouse sources" resource-level setting.

This means restricting a source to "No access" now effectively prevents access to all tables synced from that source, unless a more specific table-level rule overrides it.

**Self-managed tables** (such as S3 buckets, file uploads, or direct database connections) do **not** fall back to source permissions, since they have no associated source. These tables are governed only by table-level and "Data warehouse tables & views" resource-level rules.

> **Example:** If you set the Stripe source to "Viewer" access and don't configure any specific table rules, all tables synced from Stripe inherit "Viewer" access. However, if you set a specific table (like `stripe_customers`) to "Editor", that table uses "Editor" access while other Stripe tables remain "Viewer".
