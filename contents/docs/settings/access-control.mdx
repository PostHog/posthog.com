...
#### 1. Object-specific access controls
**Endpoint**: `GET/PUT /{resource}/{id}/access_controls`

This endpoint allows you to view and manage access controls for a specific resource object.

Example endpoints:
- `/api/projects/@current/dashboards/{dashboard_id}/access_controls`
- `/api/projects/@current/notebooks/{notebook_id}/access_controls`
- `/api/projects/@current/insights/{insight_id}/access_controls`
- `/api/projects/@current/feature_flags/{flag_id}/access_controls`

#### 2. Resource access controls
**Endpoint**: `GET/PUT /api/projects/@current/resource_access_controls`

This endpoint manages project-wide access rules for all resources of a given type. These settings apply when no specific resource-level access control is set.
...
The API uses different permission levels depending on the resource type:
...
#### Resource objects (dashboards, insights, notebooks, feature flags)
- `none` – No access
- `viewer` – Read-only access
- `editor` – Read and write access (default)
- `manager` – Full management access including permission management
...
{
  "access_level": "editor",
  "organization_member": "member-uuid"  // Use either organization_member OR role
}
```
...
To manage access controls via the API:
- **For resource-specific changes**: You must be the resource creator, an organization admin, or have manager-level access to the resource
- **For global/project-wide changes**: You must be an organization admin
- **For read operations**: Any project member with appropriate API key scopes can view access controls

### Example workflows

#### Grant a team member access to a specific dashboard

```bash
curl -X PUT https://app.posthog.com/api/projects/@current/dashboards/{dashboard_id}/access_controls \
...
#### Set role-based access for all notebooks

```bash
curl -X PUT https://app.posthog.com/api/projects/@current/resource_access_controls \
  -H "Authorization: Bearer {your-api-key}" \
  -H "Content-Type: application/json" \
  -d '{
    "resource": "notebook",
    "role": "analysts-role-uuid",
    "access_level": "editor"
  }'
```
...
```bash
curl -X PUT https://app.posthog.com/api/projects/@current/insights/{insight_id}/access_controls \
  -H "Authorization: Bearer {your-api-key}" \
  -H "Content-Type: application/json" \
  -d '{
    "organization_member": "member-uuid",
...
- You cannot set multiple access levels for the same member/role on a single resource
- Project admins always have full access and cannot be restricted via the API
