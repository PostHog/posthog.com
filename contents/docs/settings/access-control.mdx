...
- **Querying data** – any project member can query events, data warehouse tables, and other data via SQL/HogQL, including through the [PostHog AI assistant](/docs/posthog-ai) and the API. For example, a user with "No access" to data warehouse sources can't open the SQL editor, but can still ask PostHog AI to query those tables on their behalf.
  ...
  > **Note:** When you query system tables that have access restrictions (e.g., dashboards or insights), PostHog displays a warning indicating "Results may exclude [resources] you don't have access to." This warning appears as a banner in the [SQL editor](/docs/product-analytics/sql), and is also included in [MCP](/docs/model-context-protocol) and API query responses. The warning indicates you have restrictions on the resource type — it doesn't necessarily mean rows were excluded from your specific query.
