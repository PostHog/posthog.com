...
Any organization member can create invites for roles at or below their own level. Admins and Owners can restrict this in organization settings so only admins can create and view invites. See [access control](/docs/settings/access-control) for details on invite permissions.

Invites are valid **for 3 days** after creation and **only for the specified email**. Emails can be resent by removing and re-adding the invitee.
