...
You can still query data for your app or marketing website separately if needed. A way to do this is [filtering](/docs/product-analytics/trends/filters) by the `host` property, for example setting `host` to `posthog.com`. Alternatively, you could set [super properties](/docs/libraries/js/usage#super-properties) and then filter by them.
...
- [Embedded analytics](/docs/data/embedded-analytics-projects) products that need to isolate each customer's data

### Don't create a project per customer

If you run a multi-tenant app, don't split tenants across projects. All queries are project-specific, so a project per customer makes it impossible to analyze your customer base as a whole. Every new signup also means provisioning a project, token, and set of dashboards.

Keep all tenants in one production project and separate them by attaching the tenant to your events. Use [group analytics](/docs/product-analytics/group-analytics) when you need to aggregate by tenant, and see [multi-tenant apps](/docs/product-analytics/multi-tenant-apps) for the full walkthrough.
