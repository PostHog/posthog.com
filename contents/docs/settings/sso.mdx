...
To add an Authentication domain, go to your [Organization settings](https://app.posthog.com/settings/organization-authentication).
...
When SSO enforcement is enabled on a domain, users with email addresses on that domain can't change their email address. This prevents bypassing SSO requirements by switching to an unmanaged domain.

Similarly, users can't change their email to an address on an SSO-enforced domain. This prevents lockout scenarios where a user moves to a domain that requires SSO but has no way to satisfy the enforced login method.

Additionally, when any user successfully verifies a new email address, all connected social authentication providers (Google, GitHub, GitLab) are automatically disconnected. This ensures the old email's social login can no longer access the account. Users need to reconnect these providers with their new email address if they wish to use social login.
