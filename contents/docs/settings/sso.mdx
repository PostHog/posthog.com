...
The authentication domains table shows the status of each feature per domain, including **SAML** and **SCIM** columns. These columns indicate whether the feature is enabled, not yet set up, or requires a plan upgrade or domain verification.
...
1. In PostHog, navigate to Organization settings and go to [Authentication domains](https://app.posthog.com/settings/organization-authentication). The **SCIM** column in the table shows the current status for each domain – "SCIM enabled", "SCIM not set up", "Upgrade to enable SCIM", or "Verify domain to enable".
