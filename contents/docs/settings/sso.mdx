...
3. Check **Enable API integration** and enter:
   ...
   Set the **Unique identifier field for users** to a unique attribute for provisioned users. A common choice is `userName` mapped to the user's email address, but any unique identifier works. If not set, PostHog defaults to the user's email.
