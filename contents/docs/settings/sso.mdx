...
> If you are self-hosting, please be sure to [update PostHog](/docs/sdk-doctor) to version 1.35.0+. Before this version, SAML used to work instance-based with environment variables and this behavior is fully deprecated.
...
SCIM lets you automatically sync users and their roles from your identity provider (IdP) into PostHog. This is useful for automatically creating accounts and assigning roles when someone joins your organization, and disabling accounts when they leave. 

> SCIM is currently in beta and available on request for customers on the Enterprise plan. [Contact us](https://posthog.com/support) to enable it for your organization.

### Prerequisites

Before setting up SCIM, you need:

1. A verified authentication domain in PostHog
2. SAML SSO configured and working for your domain
3. An Identity Provider that supports SCIM 2.0 (e.g., Okta, OneLogin, Azure AD)

### Setting up SCIM

1. In PostHog, navigate to [Organization settings](https://us.posthog.com/settings/organization-authentication) and go to Authentication domains.
...
5. Configure role mapping in your IdP. Map your IdP roles to PostHog roles:
    ...
    - Admin
    - Member
    - (Other custom roles if applicable)

6. Assign users and roles to the PostHog application in your IdP, verify that they appear in PostHog.

### How SCIM works

- When you assign a user to PostHog in your IdP, they're automatically created in PostHog with the mapped role.
- Changes to user details or roles in your IdP automatically sync to PostHog.
- When you remove a user from PostHog in your IdP, their PostHog account is deactivated.
- If a user already exists in PostHog with the same email, SCIM updates their details and role to match your IdP.
- If roles already exist in PostHog, they are matched by name (case sensitive).

### Example: Okta

1. In Okta admin, go to your PostHog application (the same one used for SAML).
...
5. In the "Provisioning" settings, enable:
    - Create Users
    - Update User Attributes
    - Deactivate Users

6. Go to the "Assignments" tab and configure role mapping under "Provisioning to App".

7. Assign users to the PostHog application. They'll be automatically provisioned in PostHog.

### Example: OneLogin

1. In OneLogin admin, go to your PostHog application.

2. Navigate to the "Provisioning" tab.

3. Check "Enable provisioning" and enter:
    - **SCIM Base URL** from PostHog
    - **SCIM Bearer Token** from PostHog

4. Under "Provisioning Options", enable:
    - Create user
    - Delete user
    - Update user

5. Configure role mappings in the "Parameters" section to map OneLogin roles to PostHog roles.

6. Save the configuration and assign users to the application.
