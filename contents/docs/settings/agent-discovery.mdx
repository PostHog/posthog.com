...
- [ID-JAG (XAA)](/docs/settings/authentication/id-jag) for agent registration via identity assertion (Enterprise plan, beta)
...
- `identity_endpoint` is the token endpoint where an agent exchanges an identity assertion for an access token. PostHog accepts the [ID-JAG](/docs/settings/authentication/id-jag) assertion type via the JWT-bearer grant. See the [ID-JAG docs](/docs/settings/authentication/id-jag) for the exchange itself.
