...
#### Autocapture
The ability to capture user behavior (events, pageviews) automatically, without having to implement dedicated instrumentation. PostHog enables you to [autocapture](/docs/data/autocapture) data by using our [JavaScript web library or snippet](/docs/libraries/js/usage#capturing-events).
...
#### Context 🦔
Context is everything your agents need to know about your product, tied together in one place: events, recordings, errors, and logs from PostHog, plus other business data like Slack, code, Notion, and support tickets. It's the fuel that powers [self-driving](/manual/glossary#self-driving). [Find out more about context](/docs/self-driving/context).
...
#### HogQL 🦔
PostHog's SQL dialect, built on top of ClickHouse SQL. HogQL lets you query your PostHog data directly using SQL commands, with additional shortcuts for accessing event and person properties. It powers the [SQL insight](/docs/data-warehouse/sql) and is used throughout PostHog's query interface. [Find out more about HogQL](/docs/sql).
...
#### Self-driving 🦔
Self-driving is PostHog's overall positioning: PostHog makes your product self-driving. It pairs the full [context](/manual/glossary#context) of your product with agents that find problems and opportunities and ship the fix – reactively (prompted, e.g. via Slack) or proactively (e.g. via the inbox). [Find out more about self-driving](/docs/self-driving).
