...
To start adding content to notebooks, start typing. There are four main ways to add content to notebooks:
...
4. **Pasting markdown.** Copy raw markdown source text and paste it into a notebook. Block-level markdown like headings, lists, blockquotes, fenced code blocks, horizontal rules, and tables is automatically detected and rendered as formatted content. This only applies to plain-text pastes — if the clipboard already contains rich HTML (e.g. copied from a rendered preview), the default paste behavior is used instead.
...
You can also paste markdown source directly into a notebook and it's automatically rendered. Detection looks for block-level signals like headings, lists, blockquotes, and code fences. Inline-only markdown (like `**bold**` in the middle of prose) isn't auto-detected to avoid unexpected reformatting.
