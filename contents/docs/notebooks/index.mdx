...
You can add [experiments](/docs/experiments) to notebooks using the slash command `/experiment` or clicking the **+** icon and selecting **Experiment**. This opens a modal where you can search, browse, and select from your existing experiments.

The modal displays each experiment's name, status, creator, and creation date. Experiments already in the notebook are highlighted so you can easily see what's been added. Click an experiment to add it to the notebook, or click a selected experiment to remove it.
