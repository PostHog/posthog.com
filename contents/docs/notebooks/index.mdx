...
You can add HogQL query nodes to notebooks to run SQL queries directly against your PostHog data. Add a HogQL node using the slash command `/hogql` or by clicking the `+` icon and selecting **SQL (HogQL)**.

HogQL nodes support:

- **Python variable placeholders**: Define variables in a Python node, then reference them in HogQL queries using `{variable_name}` syntax. This enables dynamic queries based on computed values.

- **Composed queries**: Combine results from multiple HogQL queries by referencing other HogQL nodes as subqueries.

- **Return variables**: Each HogQL node outputs results to a variable (default: `hogql_df`) that you can use in subsequent Python or HogQL nodes.

You can also call `hogql_execute()` directly from Python nodes for programmatic query execution:

```python
result = hogql_execute("SELECT event, count() FROM events GROUP BY event")
df = result.to_df()  # Convert to pandas DataFrame
```
