...
You can add an experiment by pasting an experiment URL into your notebook.

Once you add an experiment, you can see key information at a glance:

- **Collapsed** - Shows the experiment name, status (draft, running, or complete), and a results tag indicating whether results are significant.
- **Expanded (launched)** - Shows duration, exposures with a sparkline chart, number of variants, winning variant with probability, and a compact results table comparing each variant's performance.
- **Expanded (draft)** - Shows variant count and metric count.
- **Expanded (legacy)** - Shows a warning banner directing you to the full experiment view.
