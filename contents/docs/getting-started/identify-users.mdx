...
There's no gotcha's with calling `identify` multiple times for the same user **as long as you continue to pass the same `distinct_id`**, so feel free to call it multiple times througout the course of a session.
...
### What is the difference betwen `$set` and `$set_once`?
