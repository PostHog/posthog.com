...
PostHog allows you to identify your users with an ID of your choice, which enables PostHog to associate events with specific users, track them on different platforms, and connect events from before and after users log in for the first time.
...
Leveraging the device ID works very well for keeping track of users across a single session but becomes far less effective as time goes on. More importantly, it only works when a user accesses your app from a single device, which is rarely the case.
For these reasons, we will use the `identify` method to associate events with a logged-in user rather than simply the device they are using.
...
Calling `identify` will do two things:
...
Tracking properties on users becomes incredibly crucial when we start creating insights, which we'll cover in the next guide.

In PostHog, we can include special `$set` and `$set_once` properties on events to set properties for whichever user is sending the event.
