...
> Want to follow along? Check out the [Getting started](https://github.com/PostHog/getting-started) repo for sample apps!
...
There are a number of reasons why it's often preferrable to send certain custom events from the server-side:
...
2. **C(R)UD events**: This is a broad class of events, but generally speaking whenever you receieve an API request to create or update a specific item within your application, it's useful to send these to PostHog. It's also often important to include context about the request itself in the event (latency, errors, any properties passed in the request payload, etc.)
