...
<details>
  <summary>Configuring single-page apps</summary>

  Because PostHog relies on page loads to capture pageviews, it doesn't capture all navigation for [single-page app](/tutorials/single-page-app-pageviews) frameworks like [Next.js](/docs/libraries/next-js), [React Router](/docs/libraries/react-router), or [Vue](/docs/libraries/vue-js). Instead, you need to set the `capture_pageview` configuration option to `history_change`. This uses the history API to capture pageviews whenever the user navigates within your app.
</details>
