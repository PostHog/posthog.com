...
We recommend setting up a reverse proxy so that events are less likely to be intercepted by tracking blockers. We have our [own managed reverse proxy service available for free](/docs/advanced/proxy/managed-reverse-proxy), which routes through our infrastructure and makes setting up your proxy easy. 
