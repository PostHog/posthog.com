...
Because every dependent flag references the same base flag, they all activate for the *same* 10% of users – one consistent beta cohort across every feature. This is a result of how PostHog assigns rollouts: a flag's percentage rollout comes from a deterministic hash of `{flag_key}.{distinct_id}`. The `beta_program` base flag hashes each user to a stable position and enables the flag for the 10% below the threshold. Since each dependent flag rolls out at 100% to users where `beta_program` equals `true`, they all inherit that same 10%.

If you instead rolled out each feature to 10% on its own, you'd get a *different* 10% for each one. The flag key is part of the hash, so `new_dashboard.{distinct_id}` and `new_feature.{distinct_id}` bucket the same user to different positions – they might be in one flag's rollout but not another's. Gating everything on a shared base flag is what ensures a single group of users sees all your beta features.
