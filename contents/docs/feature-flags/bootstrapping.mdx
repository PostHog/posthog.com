...
1. When receiving a frontend request, fetch all the feature flags for the user on the backend by calling [`getAllFlags()`](/docs/feature-flags/adding-feature-flag-code#fetching-all-flags-for-a-user).
...
> **Note:** The only case where it doesn't make sense to include `distinctID` is when your code only runs for [identified users](/docs/product-analytics/identify) **and** you call `identify` immediately on load. 
