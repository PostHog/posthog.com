...
The feature-flags-loaded callback (`onFeatureFlags`) fires as soon as bootstrapped flags are applied, so startup logic can read them immediately. Mobile SDKs don't support the `sessionID` bootstrap option.
