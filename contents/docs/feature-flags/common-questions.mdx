...
## How can I reduce the amount I pay for feature flags?
...
If your bill is high because of too many local evaluation requests, here's how to fix it:

When you use local evaluation, you aren't charged for evaluating flags per user (as long as you only evaluate locally), but for requests for flag definitions. Every PostHog instance you initialize creates a poller that sends requests to us every 30 seconds by default.
...
To fix this, you can set the `pollInterval`, or the equivalent parameter in your SDK of choice to 300 seconds instead of 30 seconds. The ramifications here are that whenever you make an update to a feature flag using the PostHog UI, it takes 300 seconds, instead of 30, for that change to roll to all your servers.

> **Note:** Do not use local evaluation in an edge / lambda environment, because this initializes a PostHog instance on every call, which can raise your bill drastically. It's best to use regular flag evaluation here.
