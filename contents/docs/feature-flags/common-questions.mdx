...
In this case, when making your `getFeatureFlag()` call, you can manually include in these properties in the method arguments.
...
Evaluating feature flags requires making a request to PostHog. However, you can [evaluating feature flags locally](/docs/feature-flags/bootstrapping-and-local-evaluation#server-side-local-evaluation) instead – without having to make a request to PostHog.

## How can I reduce the cost of feature flags?

If your feature flag usage or costs are too high, there a few steps you can take to reduce them:
