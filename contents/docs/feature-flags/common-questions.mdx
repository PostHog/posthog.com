...
The usage you see for feature flags is for auditing and debugging how your flags are being used (and as a result, can be easily switched off). Further, they're not sent every time, just when the flag value changes for a person.
