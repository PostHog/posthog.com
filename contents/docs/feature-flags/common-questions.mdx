...
- Ensure you are capturing [person profiles](/docs/data/persons).
...
The usage you see for feature flags is for auditing and debugging how your flags are being used (and as a result, can be easily switched off). By default, these events are sent whenever you call `getFeatureFlag()` or `isFeatureEnabled()`. Further, they're not sent every time, just when the flag value changes for a given person. 

This is different from requests made to our servers to compute flags, which is what feature flag billable requests are. Thus, generally, there will be no relation between the no. of feature flag called events and the number you see in billing.
