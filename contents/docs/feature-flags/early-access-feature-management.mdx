...
export const CreateNewFeatureFlagLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/feature-flags/create-new-feature-flag-light-mode.png";
export const CreateNewFeatureFlagDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/feature-flags/create-new-feature-flag-dark-mode.png";
export const SiteAppDark =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/feature-flags/site-app-light-mode.png";
export const SiteAppLight =
  "https://res.cloudinary.com/dmukukwp6/image/upload/posthog.com/contents/images/features/feature-flags/site-app-dark-mode.png";
...
> **Important:** Early access feature opt-in is an _overriding condition_. When a user opts in or out, this overrides any other release conditions on the feature flag. Only when a user has not explicitly opted in or out will the flag's regular release conditions apply.

## Access control

Early access features support [resource-level access control](/docs/settings/access-control), enabling you to control who can view and edit early access features in your project.

The available access levels are:

- **Viewer** – Can view early access features but cannot create, edit, or delete them
- **Editor** – Can create, edit, and delete early access features
- **Manager** – Can do everything an editor can, plus manage access controls for specific features

Since early access features are linked to feature flags, users also need appropriate feature flag permissions to perform actions that modify the linked flag (such as creating a feature, promoting to an active stage, or deleting a feature with a linked flag).

Access controls can be configured in the **Access control** sidebar when viewing an early access feature, or at the project level under [Settings](https://app.posthog.com/settings/access-control).
...
## Copying a feature to another project

You can copy an early access feature to another project in the same organization. This option shows only when your organization has more than one project.

1. Open the early access feature you want to copy.
1. Open the actions sidebar.
1. Click **Copy to another project**.
1. Select the destination project.
1. Under **Dependencies**, choose what to do with the connected feature flag. **Copy** creates a new flag in the destination project. **Use existing** links the feature to a flag that is already there.
1. Click **Copy**.
...
You can fully customize your early access management experience using `getEarlyAccessFeatures` and `updateEarlyAccessFeatureEnrollment` from PostHog's [JavaScript Web library](/docs/libraries/js/usage#early-access-feature-management).
