...
- **Concept**: Features that are not yet available for testing. These are added to the Coming Soon tab, so users can register interest in advance. Users can opt in to register their interest, but the feature flag is not enabled – they won't see the feature until you move it to Alpha, Beta, or GA.
...
* The feature flag is only enabled for users in the **Alpha**, **Beta**, and **GA** stages. Users who opt in during the **Concept** stage register their interest but don't get the feature flag enabled until you promote the feature to an active stage.
