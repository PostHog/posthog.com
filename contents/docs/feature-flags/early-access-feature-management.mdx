...
> *Note:* The list of available rarly access features is cached per browser load, so if you have a browser open and create a new one, you wouldn't see it in your list until you refresh. However, if you want the latest early access features always available, `getEarlyAccessFeatures` takes a second force_reload parameter which makes a network request to get the list. You generally don't want this because it can slow down opening the features list, and the cached value persists only until browser reload, so it refreshes automatically on browser refresh.
