...
To ramp a rollout up over time, schedule one **Add condition** change per phase, each with a higher rollout percentage: 25%, then 50%, then 100%. The earlier conditions stay on the flag, but they do not hold the ramp back. [A pass on any condition set is a pass](/docs/feature-flags/creating-feature-flags#release-conditions), so the highest percentage decides the result, and rollout buckets are shared across sets, so nobody loses access part way through.

This only ramps a rollout up. A lower percentage added later has no effect, and you have to delete the earlier conditions yourself once the flag reaches 100%. It also needs [early exit](/docs/feature-flags/creating-feature-flags#stop-evaluation-at-first-matching-condition-set) turned off, because that makes the first condition decide the result for everybody.
