...
Feature flags accumulate, and every active flag still costs money even if no code explicitly checks it. The web SDK evaluates all active flags by default on load, and server-side calls to `getAllFlags()` include them too. Flags that were fully rolled out, linked to completed experiments, or simply forgotten keep generating costs while adding clutter to your codebase. Cleaning them up reduces your bill, removes tech debt, and makes your remaining flags easier to reason about.
...
<CalloutBox icon="IconWarning" title="Always clean up code first, then disable the flag in PostHog" type="caution">

A 100% rollout doesn't mean the flag is safe to disable. If your code still checks it, disabling it in PostHog turns the feature off for everyone. Remove the flag from your code first.
...
Hit the feature flags endpoint with the `active=STALE` filter:

```bash
curl -s "https://us.posthog.com/api/projects/<PROJECT_ID>/feature_flags/?active=STALE&limit=100" \
  -H "Authorization: Bearer <PERSONAL_API_KEY>" | python3 -m json.tool
```

Replace `us.posthog.com` with `eu.posthog.com` or your self-hosted URL. Get your personal API key from [project settings](https://us.posthog.com/settings/project#personal-api-keys).

The response includes each flag's key, rollout configuration, last evaluation date, and experiment associations.

## Removing flags from code
...
Paste this prompt into any AI coding agent (Claude Code, Cursor, Copilot, etc.) and adapt the flag list:
...
```bash
curl -X PATCH "https://us.posthog.com/api/projects/<PROJECT_ID>/feature_flags/<FLAG_ID>/" \
  -H "Authorization: Bearer <PERSONAL_API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{"active": false}'
```

To disable multiple flags at once, use a script:
...
disable_flag() {
  local flag_id="$1" flag_key="$2"
  echo "Disabling: $flag_key (id: $flag_id)"
...
disable_flag "12345" "new-checkout"
disable_flag "12346" "beta-dashboard-v2"

echo "Done."
```

## Automating cleanup with CI
