---
title: Evaluation tags (user-specified flag evaluation environment constraints)
sidebar: Docs
showTitle: true
availability:
  free: partial
...
2. Click the bolt icon <IconBolt className="inline w-4 h-4" /> to configure evaluation constraints
...
> **Note:** Evaluation tags are different from [evaluation runtime](/docs/feature-flags/creating-feature-flags#evaluation-runtime). Runtime filters by SDK type (client vs server), while evaluation tags provide fine-grained environment constraints that you define. For practical examples of using both together, see [How to use evaluation runtimes and tags together](/tutorials/evaluation-runtimes-and-tags).
