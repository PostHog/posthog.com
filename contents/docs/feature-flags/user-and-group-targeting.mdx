...
|------|-------------------|----------------|
| **User** (default) | The user's `distinct_id` | In-app features for logged-in users |
...
2. For a user-targeted condition, it hashes the user's `distinct_id` to determine rollout inclusion.
...
|-------------------|--------------------------|
| **User** | Person properties, cohorts, other feature flags |
...
The group condition is checked first. If the user's company has `industry` set to `tech`, the flag is enabled. If the company doesn't match the filter, PostHog falls through to the user condition and includes 25% of all users. The same fallthrough happens if the SDK call didn't include a company group at all — the group condition is skipped and the user condition is evaluated instead.
