...
| `bucketing_identifier`         | Identifier used to bucket users: `distinct_id` (user ID, the default) or `device_id`. Using `device_id` is incompatible with `ensure_experience_continuity=true`. See [device bucketing](/docs/feature-flags/device-bucketing).                                                        |
