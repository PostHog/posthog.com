...
## What happens if no device ID is available

If a flag is configured for device bucketing but no device ID is available:

| Scenario | Behavior |
|---|---|
| **JavaScript web** | The SDK normally includes `$device_id` automatically. The main exception is [cookieless mode](/docs/libraries/js#cookieless-tracking), where PostHog omits it and falls back to `distinct_id` bucketing. |
| **Node.js / Python** | Local evaluation cannot resolve a device-bucketed flag without a device ID. The SDK falls back to server evaluation when possible. If you force local-only evaluation, it returns no result (`undefined` in Node.js, `None` in Python). |
