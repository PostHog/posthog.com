...
By default, release conditions use **Properties** targeting with each condition set to **Users**. That means PostHog uses `distinct_id` to decide which rollout bucket or variant someone gets.
...
When you need consistency during login, device bucketing is the simpler choice than [persisting feature flags across authentication steps](/docs/feature-flags/creating-feature-flags#persisting-feature-flags-across-authentication-steps-optional). It avoids the tradeoffs of that approach, which disables local evaluation and bootstrapping, and adds extra server-side work.
...
When creating or editing a feature flag, select **Device** under **Release conditions** instead of the default **Properties** mode.
...
```python
flags = posthog.evaluate_flags("user_123", device_id=device_id)
...
with posthog.new_context():
    posthog.set_context_device_id(device_id)
    flags = posthog.evaluate_flags("user_123")
    variant = flags.get_flag("signup-flow-experiment")
```
