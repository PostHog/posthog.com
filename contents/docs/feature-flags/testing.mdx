...
## Method 1: Use the Testing tab

The **Testing** tab on any feature flag page lets you test how the flag evaluates for a specific person. This is useful for debugging why a flag matched or didn't match.

To use the Testing tab:

1. Go to your feature flag.
2. Click the **Testing** tab.
3. Search for and select a person from your PostHog instance.
4. Optionally, set a historical timestamp to evaluate the flag as it existed at that time.
5. Optionally, provide groups JSON for group-based feature flags.
6. Click **Test evaluation**.

The results show:

- **Flag result** – The value returned (`true`, `false`, or variant name).
- **Condition analysis** – A breakdown of each condition showing whether it matched, why, and the rollout percentage.
- **Person properties** – The properties used in the evaluation, with indicators showing which properties the flag conditions checked.

This helps you understand why a person receives a particular flag value without modifying any release conditions.

## Method 2: Assign a user a specific flag value
...
## Method 3: Use `posthog.featureFlags.overrideFeatureFlags()`
...
## Method 4: Use the PostHog toolbar
