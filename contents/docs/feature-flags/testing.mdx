...
## Method 4: Use the test evaluation tab

Every feature flag has a **Test** tab where you can evaluate how the flag resolves for a specific person. To use it:

1. Go to your feature flag and click the **Test** tab.
2. Search for and select a person.
3. Optionally configure group properties or a historical timestamp.
...
When the selected person has more than one merged distinct ID, the test tab evaluates all of them at once instead of requiring you to test each one individually. Click **Test all N distinct IDs** and a results table shows how each ID evaluates side by side.

If the IDs produce different results, a warning banner flags the divergence. Click any row to expand that ID's full condition analysis and person properties. A single failing ID is isolated in its own row and doesn't affect the rest of the table.
