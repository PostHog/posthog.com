...
By default, the web SDK sends only initial session properties with flag requests:
...
Mobile SDKs (React Native, iOS, and Android) include common device and app properties in every feature flag evaluation request. This behavior is controlled by the `setDefaultPersonProperties` configuration option, which defaults to `true`.
...
Server-side SDKs disable GeoIP by default because PostHog derives GeoIP properties from the request's IP address, which for server-side requests is your server's IP, not your user's. You can enable GeoIP with `disable_geoip=False`, but since there's no way to pass the user's IP for GeoIP resolution, it will resolve to your server's location.
