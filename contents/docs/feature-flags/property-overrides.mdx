...
On mobile SDKs, this automatically triggers a feature flag reload. For Flutter, pass `reloadFeatureFlags: false` to reset without reloading. On web, flags are not automatically reloaded. Call `reloadFeatureFlags()` manually if needed.
...
1. [Create a feature flag in PostHog](/docs/feature-flags/creating-feature-flags)
2. Add a filter condition using `$app_version` with a [semver operator](/docs/data/property-filters#semver-operators) – for example, `$app_version >= (semver) 2.0.0`
...
const flagValue = flags.getFlag('feature-flag-key')
```
