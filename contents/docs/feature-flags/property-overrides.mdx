...
1. **Properties are set elsewhere**: Properties might be set from different parts of your application (like a backend service or another client), and your frontend doesn't know if they've been processed. Additionally, when multiple clients set the same property, the value in PostHog can also change unpredictably as updates arrive from different sources.

2. **Values are transient or context-specific**: You may need to evaluate flags based on values that shouldn't be stored on the person record, such as session-specific context, derived values, or data you'd prefer not to persist.

3. **Testing and debugging**: You may want to test flag behavior with specific property values without modifying the actual user's properties.

Property overrides bypass the server-stored properties and instead send the properties directly with the flag evaluation request, ensuring consistent and immediate results.

## Automatic overrides in client-side SDKs

Client-side SDKs automatically cache properties from `identify()` and `group()` calls for use in flag evaluation. You don't need to do anything special for these to work. Mobile SDKs also include common device and app properties by default. The methods below give you additional control when needed.
...
Mobile SDKs (React Native, iOS, and Android) can automatically include common device and app properties in every feature flag evaluation request. This ensures flags targeting properties like app version or OS work correctly without waiting for server-side processing.

This behavior is controlled by the `setDefaultPersonProperties` configuration option, which defaults to `true`.

### Properties included

When enabled, the following properties are automatically sent with flag requests:
...
| `$os_name` | Operating system name (`macOS`, `Android`) |
...
### Use case: Targeting by app version

With `setDefaultPersonProperties` enabled, you can create feature flags that target specific app versions without any additional code:
...
This works immediately because the SDK sends the app version with every flag request.
