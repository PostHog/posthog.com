...
<CalloutBox icon="IconWarning" title="Unused flags still incur charges" type="caution">
  ...
  To stop charges for unused flags, you must **disable, delete, or archive** them in the PostHog UI – simply removing them from your code is not enough. If you disable/delete/archive a flag but keep it in your code, you'll still see `$feature_flag_called` events (since we track flag usage), but the flag won't be evaluated and will always return `false`.
