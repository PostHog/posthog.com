...
1. Set `advanced_disable_feature_flags_on_first_load` to `true`. This prevents the SDK from automatically requesting feature flags during its initial initialization. Note that other SDK methods that may happen during page load (like `posthog.identify()`) will still trigger flag requests when called. This is useful when you're immediately calling `identify()` anyway, since that requests flags on its own.
