...
<CalloutBox icon="IconInfo" title="Environment detection patterns" type="info">

Common patterns for detecting test environments:
- **JavaScript**: `process.env.NODE_ENV === 'test'` or `process.env.CI === 'true'`
- **Android**: `BuildConfig.DEBUG` or `System.getProperty("CI") != null`
- **iOS**: `ProcessInfo.processInfo.environment["CI"] != nil`
- **Flutter**: `Platform.environment['CI'] != null`

Wrap your PostHog initialization in these checks to automatically use the "quiet" configuration in test/CI builds.
...
<CalloutBox icon="IconInfo" title="Tip" type="info">

If you identify test or CI environments making unexpected requests, see [Configuring test and CI environments](#configuring-test-and-ci-environments) above for SDK-specific settings to minimize costs.
