...
Forgotten environments, like old demos, test apps, or staging servers, can silently rack up costs. Even if they don’t send events, they may still be polling and making `/decide` calls for feature flags, session replay, and surveys.
