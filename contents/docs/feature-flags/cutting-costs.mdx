...
To quickly clean up multiple unused flags, use **bulk delete** from the feature flags list – filter by status (like STALE), select the flags you want to remove, and delete them in one action.
