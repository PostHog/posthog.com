...
- [Deploying a reverse proxy](/docs/advanced/proxy) for your PostHog requests
- Using the [`flags_api_host`](/docs/libraries/js/config) configuration option to separate feature flag requests from analytics requests
...
We are also experimenting with client-side feature flag assignments. Check out our [fast feature flag minisite](https://posthog-fast-feature-flags.vercel.app/) to learn more.
...
## Why can't I use a cohort with behavioral or lifecycle filters in my feature flag?

Behavioral and lifecycle cohorts are [dynamic cohorts](/docs/data/cohorts#dynamic-cohorts) created using an event or action filter (i.e., the options under the "behavioral" or "lifecycle" subheadings)
...
<Caption>
Behavioral and lifecycle cohorts are dynamic cohorts created using the options under the "behavioral" or "lifecycle" subheadings.
</Caption>
...
[Realtime cohorts](/docs/data/realtime-cohorts) remove this limit for cohorts built from simple event criteria, by keeping track of who is in the cohort as events arrive instead of querying at evaluation time. They're in early beta and rolling out gradually, so they aren't available in most projects yet, and sequences and lifecycle criteria still aren't supported.

### How do I create a flag that targets users who completed an event?

Although you can't use behavioral or lifecycle cohorts, you still might want to target users who completed an event.

One option to do this is to create a dynamic behavioral or lifecycle cohort and then duplicate it as a static cohort (using the button on the top right of the cohort page).
...
If you plan to call the flag immediately after the event, you will likely need to [set the property for the flag manually](/docs/libraries/js/usage#overriding-server-properties) with `setPersonPropertiesForFlags()`.
...
PostHog enforces per-team limits on feature flags to protect against memory issues during flag evaluation and caching. If you hit a limit, you'll see one of these errors:

- **Flag count limit** – Each team can have a maximum of 2,000 active (non-deleted) feature flags. This limit is only enforced when creating new flags, not when updating existing ones.
- **Filter size limit** – A single flag's filter configuration can't exceed 512KB. This limit is enforced on both create and update.

To free up capacity, [delete unused feature flags](https://app.posthog.com/feature_flags) that are no longer needed.

Surveys, web experiments, and product tours also count toward the flag limit because they create backing feature flags. If you're close to the limit, check for flags created by these features as well.

Self-hosted users can adjust these limits using the `MAX_FEATURE_FLAGS_PER_TEAM` and `MAX_FEATURE_FLAG_FILTER_SIZE_BYTES` environment variables.

## Why am I getting an error about the 'in' operator for properties?

When creating or updating a feature flag, you may see an error like:

`"The 'in' operator is only valid for cohort properties, not 'person' properties."`

This occurs because the `in` and `not_in` operators are reserved for **cohort properties only**. They cannot be used with person properties, group properties, or other property types in feature flag release conditions.
...
- You are evaluating the flag with all the information and arguments it needs. For example, in Python, call `posthog.evaluate_flags()` with the distinct ID, then pass the flag key to `flags.is_enabled()` or `flags.get_flag()`.
...
For how often a flag is called and which values it returns, open the flag's [**Usage** tab](/docs/feature-flags/surfaces/web-app#see-how-a-flag-is-used). For what your flags cost you, use the billing usage template.
