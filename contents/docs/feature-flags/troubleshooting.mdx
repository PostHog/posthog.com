...
| `= (semver)` | Matches when the version is exactly equal to the specified value |
| `≠ (semver)` | Matches when the version is not equal to the specified value |
| `> (semver)` | Matches when the version is greater than the specified value |
| `>= (semver)` | Matches when the version is greater than or equal to the specified value |
| `< (semver)` | Matches when the version is less than the specified value |
| `<= (semver)` | Matches when the version is less than or equal to the specified value |
| `~ (semver)` | Matches versions in the tilde range (e.g., `~1.2.3` matches `>=1.2.3, <1.3.0`) |
| `^ (semver)` | Matches versions in the caret range (e.g., `^1.2.3` matches `>=1.2.3, <2.0.0`) |
| `* (semver)` | Matches versions in the wildcard range (e.g., `1.2.*` matches `>=1.2.0, <1.3.0`) |

Semver operators are useful for targeting users based on app or SDK versions. Learn more about [semver targeting](/docs/feature-flags/creating-feature-flags#semantic-version-semver-targeting).
