...
For person or group properties, use any of the [string, numeric, presence, or semver operators](/docs/data/property-filters). For example: `exact`, `contains`, `regex`, `is set`, or any of the [semver operators](/docs/data/property-filters#semver-operators) for version-based targeting.
