...
For person or group properties, use one of these operators:

| Operator | Description |
|----------|-------------|
| `exact` | Matches when the property value equals any value in the list |
| `is_not` | Matches when the property value does not equal the specified value(s) |
| `contains` / `does not contain` | Matches when the property value contains/doesn't contain the substring |
| `regex` / `does not match regex` | Matches when the property value matches/doesn't match a regular expression |
| `is set` / `is not set` | Matches when the property exists/doesn't exist |
