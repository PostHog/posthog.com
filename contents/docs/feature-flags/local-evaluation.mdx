...
Many properties that are avaiable on the frontend (eg browser, cookie info) can still be used for local flag evaluation, but you will need a way to patch them through. Additionally, if you plan to use geoIP / location-based flags, you will need to provide this info as well (we do this for you in the web SDK).
