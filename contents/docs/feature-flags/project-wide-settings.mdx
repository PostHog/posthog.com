...
## Evaluation context suggestions

Over time, your team may accumulate stale or mistyped [evaluation context](/docs/feature-flags/evaluation-contexts) names in the suggestion dropdown. This setting lets admins hide these names from the suggestion list without affecting flags that already use them.

To manage suggestions, go to [**Settings** > **Feature Flags** > **Evaluation context suggestions**](https://app.posthog.com/settings/project-feature-flags#feature-flag-evaluation-context-suggestions).

### How it works

- **Suggested contexts** – Appear as closable tags. Click the **X** to hide a context from suggestions.
- **Hidden contexts** – Appear in a separate list with a restore button. Click **Restore** to bring a context back.
- Hiding a context only removes it from the dropdown. Flags already using that context continue working normally.
- Hidden contexts stay hidden until you restore them from this settings page.

> **Note:** Managing evaluation context suggestions requires admin permissions.
