...
A feature flag payload is an additional pieces of information that is sent along to your app when a flag is matched for a user. The returned value can be any valid JSON type (object, array, number, string, Boolean or null).
...
The feature flag payload retrieval function is a separate function that can be called to obtain the payload associated with the flag