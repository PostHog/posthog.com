...
flowchart LR
    ...
    app -- "getFeatureFlag('flag-key', 'user-123')" --> api
    db -- "flag definitions" --> api
    db -- "person properties" --> api
    db -- "group properties" --> api
    db -- "cohort memberships" --> api
    api -- "true / false / variant" --> app
```

With local evaluation, you shift the work to your server. The SDK periodically fetches **flag definitions** from PostHog (the only network call). When you evaluate a flag, the SDK uses those cached definitions along with the **person and group properties that you provide in your code** — no network request at evaluation time:

```mermaid
flowchart LR
    subgraph posthog ["PostHog"]
        api["/local_evaluation API"]
        db[("Database")]
    end
    subgraph your-server ["Your server"]
        sdk["PostHog SDK"]
    ...
    db -- "flag definitions" --> api
    api -- "flag definitions\n(every 30s)" --> sdk
    app -- "getFeatureFlag('flag-key', 'user-123',\npersonProperties, groupProperties)" --> sdk
    sdk -- "true / false / variant" --> app
```

The key difference: with remote evaluation, PostHog already has all the data it needs. With local evaluation, **you are responsible for providing every property the flag's release conditions depend on**. If you forget to pass a property, the SDK can't evaluate the flag locally and will either fall back to a remote request or return `undefined` (depending on your configuration).
