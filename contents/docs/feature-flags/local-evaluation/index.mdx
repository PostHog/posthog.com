...
> **Tip:** You can also create and manage project secret API keys with custom labels and scopes in [Project settings](https://us.posthog.com/settings/project) under the "Project Secret API Keys" section. These keys work the same way for local evaluation as long as they have the `feature_flag:read` scope.
