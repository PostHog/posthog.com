...
5. Have ["Stop evaluation at first matching condition set"](/docs/feature-flags/creating-feature-flags#stop-evaluation-at-first-matching-condition-set) (early exit) enabled. Local evaluation always evaluates all condition groups, even when this setting is enabled. The setting only takes effect when flags are evaluated through the `/flags` endpoint.
