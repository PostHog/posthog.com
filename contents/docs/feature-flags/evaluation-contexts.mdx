...
Evaluation contexts are constraints that determine when a feature flag should be evaluated. They're configured in the PostHog UI in a dedicated **Evaluation Contexts** section on each feature flag – completely independent from tags. While tags are purely organizational, evaluation contexts actively filter which flags are returned during evaluation requests.
...
2. Add contexts that represent where the flag should evaluate (e.g., "production", "staging", "main-app", "marketing-site")
3. Contexts are independent from tags – adding or removing tags does not affect evaluation contexts
