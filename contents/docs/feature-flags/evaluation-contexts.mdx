...
Evaluation contexts control when a feature flag evaluates. You configure them in the PostHog UI in a dedicated **Evaluation Contexts** section on each feature flag. Unlike tags (which are purely organizational), evaluation contexts actively filter which flags are returned during evaluation requests.
...
- Flags without evaluation contexts continue to evaluate for all requests
...
2. Add contexts that represent where the flag should evaluate (e.g., "main-app", "marketing-site", "docs", "mobile") — autocomplete suggests your team's existing contexts
3. Click **Save**

Evaluation contexts are independent from tags. Use tags for organization and evaluation contexts to control where flags evaluate.

> **Note:** Each flag supports a maximum of 50 evaluation contexts, and each context name can be up to 255 characters.

> **Remember:** Setting evaluation contexts in PostHog is only half the setup. Your app also needs to declare its context via SDK configuration (Step 2).

### Step 2: Configure your SDKs

**This step is essential** – after adding evaluation contexts to your flags in PostHog, you must update your SDK configuration to declare which contexts your application represents. The SDK's `evaluation_contexts` parameter must match the contexts you've configured in the UI.
...
Consider these feature flags (with their evaluation contexts):
...
2. **Define naming conventions**: Create a consistent naming scheme for your contexts
...
6. **Monitor and adjust**: Track the impact and refine your context strategy
