...
Filtering therefore happens inside the SDK, so it only works on an SDK version that implements it. Check [SDK support](#sdk-support) before relying on it. On an older version, any flag your SDK can evaluate locally is evaluated without filtering, whether or not you use an option like the Node.js `strictLocalEvaluation`.
...
| **SDK parameter** | SDK option, such as `evaluation_contexts`, `evaluationContexts`, or `EvaluationContexts` | Automatic based on SDK type |
...
5. If you evaluate flags locally, confirm your SDK version filters locally. Node.js does this from 5.51.4+, and .NET doesn't filter locally yet. On earlier versions, evaluation contexts apply only to remote `/flags` requests, so every polled flag still evaluates. See [local evaluation](#local-evaluation).
...
5. **Update SDKs gradually**: Roll out [SDK changes](/docs/health-checks/sdk-health) with evaluation contexts per context
...
| SDK | Current option | Legacy option |
|-----|----------------|---------------|
| [JavaScript Web](/docs/libraries/js/config#evaluation_contexts) | `evaluation_contexts` in 1.331.0+ | `evaluation_environments` in 1.269.0+ |
| [React Native](/docs/libraries/react-native/) | `evaluationContexts` in 4.21.0+ | `evaluationEnvironments` in 4.10.0+ |
| [Node.js](/docs/libraries/node/) | `evaluationContexts` in 5.23.0+ | `evaluationEnvironments` in 5.10.0+ |
| [Android](/docs/libraries/android) | `evaluationContexts` in 3.29.1+ | `evaluationEnvironments` in 3.24.0+ |
| [iOS](/docs/libraries/ios) | `evaluationContexts` in 3.38.0+ | `evaluationEnvironments` in 3.33.0+ |
