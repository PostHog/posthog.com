...
Evaluation environments are currently in alpha release and require (at least) the boost add-on (they need the resource tagging feature to function correctly). This feature may not be available in your PostHog instance yet. Contact support if you'd like early access to this feature.
</CalloutBox>
...
5. **Update SDKs gradually**: Roll out SDK changes with `evaluation_environments` per application
