...
When you have many flags to clean up, use [bulk delete](/docs/feature-flags/creating-feature-flags#deleting-feature-flags-in-bulk) to select and delete multiple flags at once. You can filter flags by name or status, then use "select all matching" to select all flags that match your criteria.
