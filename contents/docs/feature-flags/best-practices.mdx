...
1. Identify the user on the frontend and use the same identified distinct ID when getting the flag on the backend.
```js
...
```
2. If you are unable to call identify on the frontend, and only have access to the anonymous distinct ID when it comes time to get the flag on the backend, then you can include the anonymous distinct ID as a person property override in the `getFeatureFlag` call.
```js
