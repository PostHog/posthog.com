...
PostHog enables you to bulk delete multiple flags at once from the feature flags list view. Filter by status (like STALE) to identify candidates, then select multiple flags using checkboxes. Use shift-click to select a range of flags quickly.

Flags with dependencies cannot be deleted until those dependencies are resolved. This includes flags linked to:

-   Active experiments
-   Early access features
-   Other flags that depend on them

After bulk deletion, PostHog provides a copyable AI prompt to help you remove flag references from your codebase using AI code editors like Cursor, Claude Code, or Copilot.
