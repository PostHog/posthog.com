...
Before you can evaluate a feature flag in your application code, you need to create it in PostHog. When you create a feature flag, you define the following behaviors:
- The key you use in your code to evaluate the feature flag
- In which [environments and runtimes](/tutorials/evaluation-runtimes-and-environments) the flag will be evaluated
- Whether the flag is a boolean, multivariate, or remote config flag
- The payload that will be returned when the flag is evaluated
- Release conditions that determine which users will see the flag

## Step 1. Create a new feature flag (required)

To create a new feature flag, navigate to the [feature flags tab](https://app.posthog.com/feature_flags) in your PostHog app and click **Create feature flag** button to create a new flag.
...
Continue here in the steps below to configure your feature flag's behaviors.

## Step 2. Feature flag key and description (required)
...
You will later [evaluate the feature flag](/docs/feature-flags/adding-feature-flag-code) in your code using expressions like `posthog.isFeatureEnabled('new-landing-page')` or `posthog.getFeatureFlag('betaFeature')`.

Use a descriptive key that reads well in your code and is easy to understand. You should also include a **description** to help you and your team understand the purpose of the feature flag.

## Step 3. Feature flag type (required)
...
|-----------|-------------|
| **Boolean flags** | These return `false` if the flag is disabled, `true` if the flag is enabled and has a matching release condition, and `null` or `undefined` if no release condition matches. |
| **Multiple variant flags** <br/> (multivariate flags) | Instead of indicating enabled vs. disabled, multivariate flags return a key – for example, `control` or `test`.<br/><br/>You can choose the rollout percentage for each variant key, where each is given a specific percentage of the total audience. Users will then be randomly assigned to each variant based on these percentages. |
| **Remote config flags** | While boolean and multivariate flags can be set up to serve different values to different users, remote config flags are meant to pass static configuration values to your applications at runtime. The config value is **always** passed in the flag's payload.<br/><br/>You can use remote config flags to tweak application configs on the fly without deploying code changes. |
...
## 4. Feature flag behavior (optional)
...
## Step 5. Evaluation runtime and environments (optional)
...
> By default, no tags are applied to a feature flag. This will only affect how your flags evaluation if you:
> - Configure tags for your feature flag
> - AND specify [`evaluation_environments`](/docs/feature-flags/evaluation-environments#step-2-configure-your-sdks) during capture.
...
## Migrating feature flags from another tool

If you can pull feature flag data from them, you can use [our API](/docs/api/feature-flags) to migrate them. We even wrote guides for doing this with [LaunchDarkly](/docs/migrate/launchdarkly) and [Statsig](/docs/migrate/statsig).
