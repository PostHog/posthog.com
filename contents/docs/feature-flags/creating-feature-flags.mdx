...
To create a new feature flag, navigate to the [feature flags tab](https://app.posthog.com/feature_flags) in PostHog and click the **New feature flag** button. A dropdown menu appears where you can select the type of flag you want to create – boolean, multivariate, or remote config. Selecting a type pre-configures the flag form for that type.
...
> **Tip:** You can create a draft [experiment](/docs/experiments) directly from a multivariate feature flag. Open your flag, go to the **Experiments** tab, and fill in the experiment name. The flag must have multiple variants including a `control` variant. You'll configure metrics after creating the draft.
