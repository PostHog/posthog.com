...
#### Stop evaluation if excluded by rollout
...
Enable **Stop evaluation if excluded by rollout** to change this. When enabled, if a user matches the targeting filters but falls outside the rollout percentage, evaluation stops immediately and returns `false`. PostHog won't check any remaining conditions.
