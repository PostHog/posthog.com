...
To create a new feature flag, navigate to the [feature flags tab](https://app.posthog.com/feature_flags) in PostHog and click the **New feature flag** dropdown button. Select a flag type from the menu:

- **Boolean flag** – A simple on/off toggle for feature releases
- **Multivariate flag** – Multiple variants with rollout percentages for A/B/n testing
- **Remote config** – Deliver configuration values to your app

Selecting a type pre-populates the form with appropriate defaults. You can also create an **Experiment** directly from this menu, or use **Ask AI** if you're not sure which type to use.
