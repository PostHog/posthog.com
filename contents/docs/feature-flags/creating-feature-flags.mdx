...
You can target users based on semantic version strings like `$app_version` or `$lib_version` using semver operators. These appear alongside regular string operators in the property filter dropdown, labeled with a `(semver)` suffix.
