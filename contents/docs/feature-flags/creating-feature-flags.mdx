...
> **Note:** Semver operators work on any string property, but they are most useful with version-formatted values. Common mobile version formats are supported: two-component versions like `3.10` are automatically treated as `3.10.0`, and zero-padded versions like `3.08` are treated as `3.8.0`. Pre-release and build suffixes (e.g., `-alpha.1`, `+build.7`) are preserved. Mobile SDKs automatically include `$app_version` – see [property overrides](/docs/feature-flags/property-overrides#default-properties-in-mobile-sdks) for details.
