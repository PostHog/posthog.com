...
This is the name of the key that you'll use in your code to fetch the feature flag.

### Description and tags 

Metadata to describe your feature flag in the PostHog app.

### Enable feature flag

As the name suggests, this determines whether your flag is enabled. Note that disabled flags return `undefined` or `null`.
...
Note that there are trade-offs to enabling this:

1. It slows down the feature flag response.
2. It disables local evaluation of the feature flag.
3. It disables bootstrapping the feature flag.
...
These return `true` or `false`. Optionally, you can also include a payload to be returned when the served value is true.
...
You can also choose the rollout percentage for each variant key, where each variant key can be given a specific percentage of the total audience. Users will then be randomly assigned to each variant based on these percentages.
