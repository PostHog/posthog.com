...
Evaluation environment tags are a special type of tag that constrains feature flag evaluation to specific environments. When evaluation environment are configured, the feature flag will only evaluate when the SDK provides matching evaluation environments in the `/flags` request.
...
To configure evaluation environments:
...
When using evaluation environment tags:
...
Learn more in the [evaluation environments docs](/docs/feature-flags/evaluation-environments)

> **Important:** After setting evaluation environment tags on your flags, you must update your SDK configuration to include the corresponding `evaluation_environments` parameter. Without this SDK update, your application won't know which environment it's deployed in, and the evaluation environment tags won't take effect.
...
> **Note:** Evaluation runtime is different from [evaluation environments](/docs/feature-flags/evaluation-environments). Runtime filtering is based on SDK type (client vs server), while evaluation environments provide fine-grained environment constraints that you define (e.g., "production", "staging", "app", "marketing-site").
