...
You can delete feature flags that are linked to experiments, as long as the experiment isn't currently running. Draft, stopped, and completed experiments don't block flag deletion – the experiment's historical results are preserved.

If you try to delete a flag linked to a running experiment, PostHog shows a warning and blocks the deletion. Stop the experiment first, then delete the flag.

- The experiment's historical data and results are preserved.
- The flag key is freed up so you can reuse it for a new flag.
- The experiment continues to reference the original flag internally.

If the flag is linked to a **running** experiment, PostHog shows a warning and blocks the deletion. Stop the experiment first, then delete the flag.

To re-launch an experiment whose flag was deleted, restore the flag first. You can do this from the flag's detail page.

<CalloutBox icon="IconInfo" title="Deletion is permanent for standalone flags" type="caution">

Deleting a feature flag that isn't linked to an experiment is permanent and cannot be undone. Flags linked to experiments are soft-deleted to preserve experiment history – you can restore them later if needed.
