...
To create a new feature flag, navigate to the [feature flags tab](https://app.posthog.com/feature_flags) in PostHog and click the **New** button. You can either:

- **Click the button directly** to create a default boolean flag
- **Use the dropdown menu** to select a specific flag type:
  - **Boolean flag** – Simple on/off toggle for feature releases
  - **Multivariate flag** – Multiple variants with rollout percentages (pre-configured with control/test at 50/50)
  - **Remote config** – Deliver configuration values to your app
  - **Experiment** – Opens the experiments UI to run A/B tests with statistical analysis

Selecting a type from the dropdown pre-configures the flag creation form with appropriate defaults.
