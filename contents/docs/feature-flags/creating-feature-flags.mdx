...
To update tags for multiple Feature Flags at once:

1. Go to the [Feature Flags tab](https://app.posthog.com/feature_flags) in PostHog.
2. Use the checkboxes to select the flags you want to update.
3. Click the **Update tags** button that appears in the toolbar.
4. Choose a tag operation:
   - **Add** – Add tags to all selected flags without removing existing tags
   - **Remove** – Remove specific tags from all selected flags
   - **Replace all** – Replace all existing tags on selected flags with a new set of tags
5. Select the tags to add, remove, or replace with, then click **Apply**.

Like bulk delete, tag updates support multi-page selection and filter-based selection.
