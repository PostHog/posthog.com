...
<CalloutBox icon="IconInfo" title="Feature flag limits" type="fyi">

Each team can have a maximum of 2,000 non-deleted feature flags. Individual flag filters are limited to 512KB. These limits are enforced when creating new flags. If you hit a limit, delete unused flags or simplify your filter conditions. See [troubleshooting](/docs/feature-flags/troubleshooting#why-cant-i-create-a-new-feature-flag) for more details.
...
- **Properties** (default) – Evaluates based on property filters. Within each condition set, use the **Target by** selector to choose between users and specific group types. See [user and group targeting](/docs/feature-flags/user-and-group-targeting) for more details.
...
> **Note:** Semver operators work on any string property, but they are most useful with version-formatted values. Common mobile version formats are supported during server-side (remote) flag evaluation: two-component versions like `3.10` are automatically normalized to `3.10.0`, and zero-padded versions like `3.08` are normalized to `3.8.0`. Pre-release and build suffixes (e.g., `-alpha.1`, `+build.7`) are preserved in the value, though build metadata doesn't affect matching. Mobile SDKs automatically include `$app_version` – see [property overrides](/docs/feature-flags/property-overrides#default-properties-in-mobile-sdks) for details.
...
- Supported SDKs must send the `evaluation_contexts` request parameter with matching contexts for the flag to evaluate (the legacy `evaluation_environments` request parameter is supported by some SDK versions)
...
### Copying Feature Flags to other projects in bulk

If you use multiple [projects](/docs/settings/projects) (for example, dev, staging, and production), you can copy several flags to other projects at once:
...
4. Select the destination projects and configure options.
5. Click **Copy** to start.

See [multi-project feature flags](/docs/feature-flags/multi-project-feature-flags) for full details on bulk copying, including how to use the Projects grid tab and what happens when target projects require approval.
