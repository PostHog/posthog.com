...
<CalloutBox icon="IconWarning" title="Not supported with local evaluation" type="caution">

This setting is not applied when a server-side SDK evaluates the flag with [local evaluation](/docs/feature-flags/local-evaluation). Local evaluation SDKs evaluate all condition sets regardless of this setting. It takes effect when the flag is evaluated through the `/flags` endpoint.
