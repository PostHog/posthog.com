...
4. **Incompatible with [anonymous events](/docs/data/anonymous-vs-identified-events):** Persistent flags are calculated based on person properties, which are only available for identified events.
