...
By default, when a user matches a condition's targeting filters but falls outside the rollout percentage, PostHog continues evaluating the remaining condition sets. This means they might still match a later condition.

Enable **Stop evaluation at first matching condition set** to change this. When enabled, if a user matches the targeting filters but falls outside the rollout percentage, evaluation stops immediately and returns `false`. PostHog won't check any remaining conditions.

Use this option when:
- You need deterministic "opt-out" behavior – matching a condition but missing the rollout shouldn't let a later condition enable the flag
- You want users who match specific targeting criteria to receive a consistent `false` rather than potentially matching a less specific condition below
- You're building layered rollouts where earlier conditions should definitively exclude certain users from any variant

<CalloutBox icon="IconWarning" title="Server-side only" type="caution">

This feature only works with server-side evaluation. Client-side and local evaluation SDKs evaluate all condition groups – early exit only takes effect when the flag is evaluated server-side.
