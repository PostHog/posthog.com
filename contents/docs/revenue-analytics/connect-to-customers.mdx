...
Once this is connected, revenue data will appear on person and group profile pages. You'll also get access to the `persons_revenue_analytics` and `groups_revenue_analytics` tables in the [data warehouse](https://app.posthog.com/data-warehouse), which map `person_id`/`group_key` to their all-time revenue.
