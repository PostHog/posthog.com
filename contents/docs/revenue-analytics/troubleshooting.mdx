...
If your revenue isn't appearing in your [managed views](/docs/revenue-analytics/managed-views), insights, or persons/groups profiles, there are a few common issues to check:
...
1. **Check your filters**: when querying your revenue in insights or the SQL editor, make sure your filters and date range include the entries you expect.

## Why isn't currency being converted in my Trends insight?

Currency conversion only happens automatically in the [managed views](/docs/revenue-analytics/managed-views). If you're using Product Analytics (Trends, Funnels, Experiments) with "Sum of Revenue", the raw values are summed without any currency conversion.
