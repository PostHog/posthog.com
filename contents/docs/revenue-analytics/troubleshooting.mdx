---
title: Revenue Analytics troubleshooting
sidebar: Docs
showTitle: true
---
...
This page covers troubleshooting for Revenue Analytics. For setup, see the [installation guides](/docs/revenue-analytics/start-here).
...
### Why isn't my revenue data appearing?

If your revenue data isn't showing up on person/group profiles or in managed views, check the following:

1. **Check the Revenue tab in Data management** - Verify your revenue events are being captured correctly by checking the [Revenue tab](https://app.posthog.com/data-management/revenue) in data management.
1. **Make sure Revenue tracking is enabled for your Data warehouse source** - Legacy data warehouse sources have revenue tracking disabled by default. Enable them in the [Revenue tab](https://app.posthog.com/data-management/revenue) in data management. Also, make sure they've finished syncing for the first time.
1. **Wait for managed views to sync** - Revenue properties on persons and groups are computed from [managed views](/docs/revenue-analytics/managed-views). After configuring your sources, wait for the sync job to complete.

## Why isn't currency being converted in my Trends insight?

Currency conversion happens automatically within the [managed views](/docs/revenue-analytics/managed-views) and on person/group revenue properties. If you're using Product Analytics (Trends, Funnels, Experiments) with "Sum of Revenue", the raw values are summed without any currency conversion.
