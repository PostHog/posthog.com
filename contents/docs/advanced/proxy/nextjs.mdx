...
Rewrites return PostHog's response headers unchanged, and `next.config.js` cannot delete a response header. If a cookie in the proxied response is the cause, you must move to a proxy that builds the response itself, such as a [Cloudflare worker](/docs/advanced/proxy/cloudflare).
