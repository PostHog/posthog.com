...
PostHog sets country, region, and city from the client IP address of the request it receives. It does not read platform geolocation headers, such as `x-vercel-ip-city` or `cloudfront-viewer-country`. If your proxy sends its own IP address, PostHog resolves every visitor to the location of one edge server. The location breakdown still looks correct, so this failure is silent.
