...
import ProxyCookies from "../_snippets/proxy-cookies.mdx"
...
PostHog sets country, region, and city from the client IP address of the request it receives. It does not read platform geolocation headers, such as `x-vercel-ip-city` or `cloudfront-viewer-country`. If your proxy sends its own IP address, PostHog resolves every visitor to the location of one edge server. The geo breakdown still looks correct, so this failure is silent.

To confirm, open a recent event in the [activity feed](https://us.posthog.com/activity/explore) and read its `$ip` property. If `$ip` is the same for visitors in different places, it is a proxy IP address.

To correct it:

1. Send the visitor's IP address in `x-forwarded-for`. PostHog reads this header first. See [client IP resolution](/docs/advanced/proxy/proxy-reference#client-ip-resolution) for the full order.
2. If a CDN or a load balancer is in front of your Next.js server, the IP address your Next.js server sees is the CDN edge, not the visitor. Read the visitor's IP address from the header that the CDN adds, then put that value in `x-forwarded-for`. An `x-forwarded-for` header that holds an edge IP address wins over every other header, so the visitor's IP address must be in it.
3. If you cannot set the header, set the `$ip` property on the event instead. PostHog keeps the `$ip` property that an event already has, and uses the request IP address only when the event has no `$ip`.
