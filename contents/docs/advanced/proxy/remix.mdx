...
Create a file `<ph_proxy_path>.$.tsx` in the routes folder. This will mean your path will be `/<ph_proxy_path>`. The `$` in the file name lets it match any sub-paths called by the SDK like `/<ph_proxy_path>/flags` or `/<ph_proxy_path>/i/v0/e`
