...
2. If a CDN or a load balancer is in front of the Railway service, the IP address the Railway service sees is the CDN edge, not the visitor. Read the visitor's IP address from the header that the CDN adds, then put that value in `x-forwarded-for`. An `x-forwarded-for` header that holds an edge IP address wins over every other header, so the visitor's IP address must be in it.
