...
**Why three location blocks?** PostHog uses separate domains for API requests and static assets/config. Your nginx config needs three location blocks:

- **`/static/`:** Routes to `us-assets.i.posthog.com` or `eu-assets.i.posthog.com` for the JavaScript SDK and other static files
- **`/array/`:** Routes to the same assets origin for SDK remote configuration (recording conditions, Feature flags, sampling rates)
...
- `proxy_set_header Cookie` and `proxy_set_header Authorization`: Clear the browser's cookies and auth headers before the request goes to PostHog.
