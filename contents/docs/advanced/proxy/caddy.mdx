...
The `header_up -Cookie` and `header_up -Authorization` directives remove the browser's cookies and auth headers before the request goes to PostHog. PostHog does not need them, because ingestion authenticates on the project token in the request body.
