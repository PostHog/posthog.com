...
Without `ui_host`, toolbar authentication redirects to your proxy URL instead of the PostHog app, causing OAuth to fail. When this happens, the toolbar displays a modal with instructions on how to set `ui_host` in your `posthog.init()` configuration.
