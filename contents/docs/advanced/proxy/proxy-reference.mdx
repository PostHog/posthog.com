...
- **Only the proxy path is blocked.** You see `403` responses on requests such as `/yourpath/static/recorder.js` or `/yourpath/e/`. Your firewall scores the proxied requests themselves. Add an exception for the proxy path, or stop proxying the SDK assets: remove the `/static` rule and set [`asset_host`](/docs/libraries/js/snippet-versioning#custom-asset-hosts-and-reverse-proxies) to `https://us-assets.i.posthog.com` or `https://eu-assets.i.posthog.com`. `asset_host` only covers `/static/*`, so `/array/<token>/config.js` still goes through your proxy.
...
To delete a cookie from the proxied response, you need a proxy that builds the response itself. nginx has `proxy_hide_header Set-Cookie`. Rewrite rules cannot do this.

The [Cloudflare worker](/docs/advanced/proxy/cloudflare) in our guide returns the upstream response unchanged. Responses from `fetch()` and the cache have immutable headers, so copy the response before you delete the header, in both `forwardRequest` and `retrieveAsset`:
