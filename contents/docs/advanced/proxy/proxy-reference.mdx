...
### Managed proxy stuck or erroring

If your [managed reverse proxy](/docs/advanced/proxy) is stuck in **issuing**, **erroring**, or **timed_out** state:

1. **Check your DNS settings** - Verify your CNAME record points to the correct PostHog proxy domain and that any DNS proxy features (like Cloudflare's orange cloud) are disabled.
2. **Retry the setup** - Go to your [organization proxy settings](https://app.posthog.com/settings/organization-proxy), click the menu icon next to the proxy, and select **Retry**. This restarts the provisioning process.
3. **Wait for propagation** - DNS changes can take up to 30 minutes to propagate. If you recently changed DNS settings, give it time before retrying.
