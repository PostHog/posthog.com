...
CloudFront's default settings don't forward the headers and query parameters PostHog needs to function. Without custom cache and origin request policies:
...
Leave cookies set to **None**. PostHog does not need them, because ingestion authenticates on the project token in the request body.
...
<Step title="Create cache behaviors for static assets and config">
...
<CalloutBox icon="IconWarning" title="Route /array/* to the assets origin" type="caution">
...
2. If a CDN or a load balancer is in front of CloudFront, the IP address CloudFront sees is the CDN edge, not the visitor. Read the visitor's IP address from the header that the CDN adds, then put that value in `x-forwarded-for`. An `x-forwarded-for` header that holds an edge IP address wins over every other header, so the visitor's IP address must be in it.
