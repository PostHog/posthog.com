...
> **Note:** If your proxy enters an **erroring** or **timed_out** state, you can retry the setup by clicking the menu icon next to the proxy in your [organization proxy settings](https://app.posthog.com/settings/organization-proxy) and selecting **Retry**. For other issues, see [troubleshooting](/docs/advanced/proxy/proxy-reference#troubleshooting).
