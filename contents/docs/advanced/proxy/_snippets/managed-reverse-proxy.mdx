...
### Does the managed reverse proxy forward cookies to PostHog?

Do not rely on the proxy to remove the `Cookie` header. Treat every cookie that is in scope for your proxy subdomain as forwarded to our ingestion service.

Ingestion authenticates on the project token in the request body, not on cookies. It does not read the `Cookie` header, and it does not add the header to your event data.

Your proxy runs on a subdomain of a domain you own. A cookie that you scope to your apex domain, such as `Domain=example.com`, is therefore in scope for that subdomain. In practice, the browser attaches these cookies to one request only:

- The web SDK sends its normal requests with `fetch` or `XHR`. Both are cross-origin and carry no cookies.
- The SDK flushes the last queued events when the page unloads. This flush uses [`navigator.sendBeacon`](https://w3c.github.io/beacon/), and the browser always sends a beacon with credentials. The SDK cannot turn this off.

To keep your cookies inside your own domain, do one of these:

- Scope your authentication cookies to the host that needs them, such as `Domain=app.example.com`, instead of to your apex domain.
- Set up a [self-hosted proxy](/docs/advanced/proxy) and delete the `Cookie` header in your own proxy configuration. The [Caddy](/docs/advanced/proxy/caddy) and [Node.js](/docs/advanced/proxy/node) guides show how.

The `persistence: "localStorage"` SDK option changes where PostHog keeps its own state. It does not stop the browser from sending your other cookies to the proxy subdomain.
