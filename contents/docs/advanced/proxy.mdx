...
It's available as part of our [Teams add-on](/addons#teams), which includes automatic provisioning, SSO and 2FA enforcement, priority support, and additional collaboration features.
...
- Make sure to [pass the proper `ui_host` parameter](/docs/libraries/js/config) when initializing PostHog, so that links to PostHog point to the correct host. This is required for tasks like authenticating the toolbar.
