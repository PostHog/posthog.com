...
This writes a TypeScript file per dashboard to `posthog/dashboards/`. Review with `git diff` before committing – existing files with the same name are overwritten. Pull currently covers dashboards only; the other resource types are coming soon and will share the same flow.
