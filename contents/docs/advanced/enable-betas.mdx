...
Opt-in betas are available to all PostHog Cloud users, as PostHog Cloud is automatically updated to run the latest version of PostHog. 

Betas are not available for Hobby deployments, or [sunset K8s deployments](/blog/sunsetting-helm-support-posthog). 

### How to join an opt-in beta

To opt in to a beta, select 'Feature Previews' from the account dropdown menu in the top right corner of the page. Then, toggle the betas you'd like to enable

![How to enable PostHog betas](./images/how-to-enable-betas.mp4)
