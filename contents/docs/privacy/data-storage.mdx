...
<<<<<<< HEAD
<CalloutBox type="warning" title="Cookieless server hash mode">

When [Cookieless server hash mode](/docs/privacy/data-collection#cookieless-tracking) is enabled, IP-based transformations like GeoIP enrichment and bot detection will **not** enrich events. In this mode, the IP address is hashed into the distinct ID and stripped before transformations run.
