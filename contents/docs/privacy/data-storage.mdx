...
| [Data retention](#data-retention)                                 | How long events stay queryable on each plan                      |
...
How long PostHog keeps your events depends on your plan. Retention is measured from the event's own timestamp, not from when it was ingested, so a historical import of events older than your window ages out immediately.

| Plan          | Events retention |
| ------------- | ---------------- |
| Free          | 1 year           |
...
These windows apply to organizations that subscribe to a paid plan on or after September 3, 2026. Organizations that were already on a paid plan before that date keep 7 years of retention for as long as they stay on a paid plan. Retention is set at the organization level, so every project in an organization shares the same window.

Events retention covers everything stored as an event: pageviews, autocapture, custom events, feature flag evaluation events, survey responses, and error tracking exceptions. It does not cover feature flag, experiment, or survey definitions, which stay until you delete them, and it does not delete person profiles while your organization is active. Session recordings have their own [retention settings](/docs/session-replay/recording-retention).

Events older than your window are hidden from queries. They are not deleted right away: we keep hidden events for at least six months, and upgrading to a plan with a longer window during that time restores access. After that, they can be permanently deleted. If you need the data for longer, upgrade your plan or set up a [batch export](/docs/cdp/batch-exports) to your own warehouse.
...
| `persons_deleted`                | integer | Number of person records deleted. Returns 0 if `keep_person` was set to true |
...
| `deletion_errors`                | array   | List of any persons that could not be deleted, each containing `person_uuid` |
