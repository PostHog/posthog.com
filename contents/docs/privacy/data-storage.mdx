...
<CalloutBox icon="IconWarning" title="Note" type="caution">
  Reusing `distinct_id` values after deletion is not supported. If you wish to delete bad data on some `distinct_id` values but keep that person's other `distinct_id` active, you should first split that person with the **Split IDs** button.
</CalloutBox>
...
A important part of protecting your users' privacy is to control who can access what data in PostHog. PostHog provides access control at 3 levels:
