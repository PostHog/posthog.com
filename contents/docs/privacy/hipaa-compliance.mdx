...
The [managed reverse proxy](/docs/advanced/proxy/managed-reverse-proxy) is **not** covered by BAAs and should not be used to process Protected Health Information (PHI). If you need a reverse proxy for HIPAA-compliant deployments, set up a [self-hosted proxy](/docs/advanced/proxy) on infrastructure you control instead.
