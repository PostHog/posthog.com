...
PostHog can be configured to do cookieless tracking in two different ways.

First is `cookieless_mode: "always"`. This:
...
Second is `cookieless_mode: "on_reject"`. This:
...
- Doesn't capture any events until after consent is either given or denied
...
Do not send data that could be used to identify users (e.g. setting an email address as a [Person property](/docs/product-analytics/person-properties) if they have not consented.
