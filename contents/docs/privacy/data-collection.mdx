...
If you require that certain data never reaches PostHog servers, you can use one of the tools below to block data from being captured.
...
If you have data that should never be captured, a safe way to start with PostHog is to mask all input and text, only selectively unmasking elements that you need to capture.
...
```js-web
{
    ...
    maskTextFn: (text, element) => {
        // only `<element data-capture="true">` will be captured
...
Defaulting to not capture in session replay makes unmasking elements a conscious choice and keeps you in control of what's captured.
