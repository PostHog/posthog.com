...
### Option 3: Backend or API event capture

If you're using a backend SDK or the API, you can still capture responses capturing a `survey sent` event the survey's ID, names, and any properties.
...
posthog.capture(
  ...
  {
    '$survey_id': 'survey_id',
    '$survey_response': 'This app is great!'
  }
)
```
...
const client = new PostHog(
  '<ph_project_api_key>',
  { host: '<ph_instance_address>' } // TIP: You can omit this line if using PostHog Cloud
)
