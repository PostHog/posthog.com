...
If you create a [popover survey](/docs/surveys/creating-surveys#presentation),  updating and targeting are handled automatically. When you create one in API mode, you need to add logic to fetch and display surveys yourself with the help of the JavaScript Web SDK or snippet.
...
Surveys are cached by default by the JavaScript SDK, so you can pass `true` to the `forceReload` parameter to force an API call to get the latest surveys.
