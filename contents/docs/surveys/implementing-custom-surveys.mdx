...
The main event you must capture to track survey results is `survey sent`. PostHog sends survey responses in two formats to maintain backward compatibility:
...
posthog.capture("survey sent", {
    ...
    $survey_response: survey_response, // Index-based (position 0)
    $survey_response_question_id: survey_response, // ID-based (using the question ID)
    
    // For additional questions:
    $survey_response_1: second_question_response, // Index-based (position 1)
    $survey_response_question_id_2: second_question_response // ID-based (using the question ID)
})
```
...
For multiple choice surveys, the response must be an array of values with the selected choices e.g., `$survey_response: ["response_1", "response_2"]`.

### Capturing multiple responses

If you have a survey with multiple questions, you can capture the responses to each question using both ID-based and index-based properties:
...
posthog.capture("survey sent", {
    ...
    $survey_response: first_question_response, // Index-based
    $survey_response_open_text_1: first_question_response, // ID-based
    
    // Second question responses
    $survey_response_1: second_question_response, // Index-based
    $survey_response_nps_rating_1: second_question_response, // ID-based
    
    // Third question responses
    $survey_response_2: third_question_response, // Index-based
    $survey_response_multiple_choice_1: third_question_response // ID-based
})
```

This dual approach ensures backward compatibility for existing implementations while providing more reliable question identification through question IDs. The question IDs are automatically generated based on the question type (e.g., `open_text_1`, `nps_rating_1`, `multiple_choice_1`) and are available in the survey response from the API.
