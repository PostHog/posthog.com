...
There are three other events you should capture to track the full lifecycle of a survey. They are `survey shown`, `survey dismissed`, and `survey abandoned`:
...
The `survey abandoned` event fires when a user answers at least one question in a multi-step survey but navigates away before completing it. This is different from `survey dismissed`, which fires when a user explicitly closes the survey.

Capturing all four events ensures you have a full implementation matching popover surveys and that your analysis is accurate in PostHog.
