...
> **Note:** With `ignoreConditions: false`, this checks the survey's display conditions, which can depend on feature flags. See [keeping matching surveys up to date](#keep-matching-surveys-up-to-date) if your survey is targeted with a flag.
...
### Keep matching surveys up to date

`getActiveMatchingSurveys()` reports what the SDK knows when you call it, and it calls your callback once. If your custom survey UI needs to react when eligibility changes later, subscribe with `onActiveMatchingSurveysChanged()` instead.
...
Call `unsubscribe()` when the component or view no longer needs survey updates.

The subscription sends its initial result when it is available, suppresses unchanged results, and stays active after recoverable load errors. A successful empty result has `context.isLoaded === true`. A load failure has `context.isLoaded === false` and an `error` value.
...
- Survey state changes such as session expiry, `posthog.reset()`, or marking a survey as seen.

For single-page apps, set `capture_pageview: "history_change"` to let route changes produce pageviews automatically. If your app does not capture pageviews automatically, capture `$pageview` after navigation when URL-based survey conditions must be re-evaluated.
...
If you use `displaySurvey()` with `ignoreConditions: false`, continue to wait for feature flags before calling it when its display conditions depend on flags. `onActiveMatchingSurveysChanged()` keeps custom API survey UIs in sync; it does not control `displaySurvey()`.

### Example: a dynamic feedback form

Use `onActiveMatchingSurveysChanged()` to keep the Next.js feedback survey in sync when matching conditions change:
...
export default function Feedback() {
  ...
  useEffect(() => {
    return posthog.onActiveMatchingSurveysChanged((surveys, context) => {
      if (context?.isLoaded === false) {
        console.error("Could not load surveys:", context.error)
        return
      }
