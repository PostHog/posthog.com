...
Surveys support translations for displaying survey content in multiple languages. You can add translations at two levels:

-   **Survey-level** - Translate the survey name, description, thank you message header, thank you message description, and close button text.
-   **Question-level** - Translate individual question text, description, button text, choices, and rating labels (`lowerBoundLabel`, `upperBoundLabel`).

Translations are stored as a mapping of language codes to translated fields. Language codes are flexible strings — you can use standard codes like `"es"` or `"es-MX"`, or custom keys like `"english"` or `"french"`.

When adding translated choices for a question, the translated choices array must have the same number of items as the original choices array.
