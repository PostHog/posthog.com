...
- **Device types targeting:** Show when device type either contains a string, exactly matches a string, matches a regex. The device type targeting is evaluated by parsing the [userAgent](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/userAgent) on the client-side, possible values are: Desktop, Mobile, Tablet, Console and Wearable. You can also target device types using *Person and group properties* (Latest or Initial Device type: `$device_type`), but these properties aren't immediately available on first page load for unidentified persons.
