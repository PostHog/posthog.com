...
[PostHog AI](/docs/posthog-ai), PostHog's AI-powered assistant, can help you create surveys quickly using natural language. PostHog AI understands different question types and can:
...
**In-app surveys** automatically link responses to users when you [identify users](/docs/product-analytics/identify) in your app.
...
You can build any survey from the question types above, but the most common ones have ready-made [templates](/templates?filter=type&value=survey) you can start from in the survey wizard. Popular options include [net promoter score (NPS)](/templates/nps-survey), [customer satisfaction (CSAT)](/templates/csat-survey), [product-market fit (PMF)](/templates/pmf-survey), and [customer effort score (CES)](/templates/ces-survey).

### Setting up a customer effort score (CES) survey

A [customer effort score (CES) survey](/templates/ces-survey) measures how easy it was for a user to complete a specific task. It's a strong predictor of loyalty and retention, and works especially well right after a key flow like onboarding, checkout, or contacting support.

To set one up:

1. Create a new survey and select the **Customer effort score (CES)** template (or start from a blank survey).
2. For the question, use a **Rating - number** question type with a **1–5 scale**. Phrase it around effort, for example _"How easy was it to [complete the task]?"_ where `1` is "very difficult" and `5` is "very easy".
3. Optionally, add a **Freeform text** follow-up question like _"What could we do to make this easier?"_ to capture qualitative feedback.
4. Under [display conditions](#display-conditions), target the survey so it appears right after the flow you want to measure — for example, use **User sends events** to trigger it after a key event, or **Selector matches** to show it after a specific action. Placing it immediately after the task keeps the effort fresh in the user's mind.
5. Launch the survey, then track your average score over time in [viewing results](/docs/surveys/viewing-results). A score of 4 or above is generally considered strong.

CES tells you how *easy* your product is to use, but not how much users *value* it — pair it with an [NPS survey](/templates/nps-survey) for the full picture.
...
- **The survey stops automatically when the last iteration ends.** A recurring survey is not open-ended. It runs for the number of repetitions times the cycle length, starting from the launch date. After the last iteration, PostHog sets an end date and stops showing the survey. In the example above, it runs for 90 days (3 iterations of 30 days) and then closes on its own. Removing the end date does not restart a finished survey. PostHog re-checks the schedule within a few hours and re-applies the end date.
  ...
  PostHog displays the auto-close date directly in the survey editor and overview. Before launch, it shows the total duration (e.g., "This survey will run for 90 days before it automatically closes"). Once running, it shows the exact end date (e.g., "This survey will automatically close on March 31, 2025").
...
## Copying a survey to another project

You can copy a survey to another project in the same organization. This option shows only when your organization has more than one project.
