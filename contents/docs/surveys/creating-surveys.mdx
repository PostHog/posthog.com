...
For surveys that are triggered by a specific event, you can configure the survey to repeat if the event is sent again. If you select the option `Every time the event is sent`, the survey will be shown again for each event.
...
For this option, the survey will be shown again every time the display conditions are met.

The main use case for this is for `Feedback button` surveys. This allows the survey to show up every time the user clicks the feedback button.

However, it can also be used in combination with the `Wait time` option. If you select this option, and also set a `Wait time` of 30 days, this means the survey will be shown for the user every time as long as they haven't seen any other surveys in the last 30 days.
