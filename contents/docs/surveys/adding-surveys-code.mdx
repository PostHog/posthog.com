...
> **Note:** Adding code is **not required** for [popover surveys](/docs/surveys/creating-surveys#display-mode).

If you're using PostHog's [survey API](/docs/surveys/creating-surveys#display-mode) to integrate a custom survey UI, you'll need to add your code before launching.

There are 2 steps:

## Step 1: Fetch your active surveys

To fetch the surveys which are currently enabled for your user, call `posthog.getActiveMatchingSurveys()`. 

Alternatively, if you wish to do survey targeting yourself, you can call `posthog.getSurveys()` to return a list of all surveys on PostHog.
...
You can capture user interactions with your surveys using the following 3 events:

```js-web
// 1. When a user has seen a survey
