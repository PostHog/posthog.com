...
There are a few reasons why your survey might not be showing up:
...
When surveys are enabled, postHog-js will fetch a `survey.js` script from the PostHog server. (This is not included in the default posthog-js to minimize the default bundle size)

Depending on your content security policy, this script may be blocked. You may need to add `https://app.posthog.com` (or your host URL if you are self hosted) to your list.
...
If you're running behind a proxy, the `/static/survey.js` endpoint may be blocked. You can read more about how to configure your proxy to allow this endpoint [here:](https://posthog.com/docs/self-host/configure/running-behind-proxy#public-endpoints)

### 3. Ad/tracking blockers
