...
Any team can turn on metrics: open [Metrics](https://app.posthog.com/metrics) in PostHog and select **Enable metrics** in the onboarding view. See [turn on metrics](/docs/metrics#turn-on-metrics) for the full flow. Setup details, including the ingestion endpoint, may change before general availability.
