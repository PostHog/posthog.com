...
There are three ways to get metrics into PostHog, and each takes a few minutes.
...
If your services already expose Prometheus `/metrics` endpoints, use the PostHog metrics agent to scrape them and forward to PostHog, with no code changes:
...
For per-platform setup, including the `posthog.metrics` API in [Node.js](/docs/metrics/installation/nodejs) and [Python](/docs/metrics/installation/python), and Prometheus scraping, see the [installation guides](/docs/metrics/installation).
