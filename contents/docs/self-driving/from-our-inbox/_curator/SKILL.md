...
1. **Real outcome.** The report is resolved, and one of these is true:
   ...
   - **No-code lane.** It has no implementation pull request, and its resolution note or work log names a concrete change someone made and checked: a scout's instructions published as a new version, a dashboard tile or insight added, an alert added or its threshold changed, a docs page updated. "Looked into it" or "no action needed" is not a change.
