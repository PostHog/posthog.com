...
They also compose. If a detector already exists, a scout can sit on top of it rather than replace it – the source keeps its deterministic coverage while the scout bundles related items, weighs who's affected, and flags what nobody actioned. An issue tracker splits cleanly this way. The GitHub Issues or Linear source catches each new issue as it arrives. A scout re-reads the open backlog and finds the issues whose state changed after someone filed them.
