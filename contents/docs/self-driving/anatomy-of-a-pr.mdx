...
When PR generation is on, an actionable report starts an implementation agent that builds the fix in a sandbox. If your team turns PR generation off in the self-driving settings, the actionable report stays in your inbox for review and manual follow-up.

In this example, PR generation is on, so the agent builds the fix:
