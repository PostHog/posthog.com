...
A report groups related [signals](/docs/self-driving/signals) into one item of work. Instead of triaging a noisy stream of findings, you get a single, framed problem with the evidence behind it.
...
A report is a starting point, not a verdict. Click **Ask AI** on a report to ask a question about what it found – who else is affected, whether the problem is new – or to tell the agent what to do next, like creating the alert the report recommends. The agent already has the report and its evidence, so you don't have to re-explain the problem. It works in a task you can follow, and you can keep the conversation going there.

The agent only acts on a report that still holds open work. If the report is resolved, or it already has a pull request, the agent answers your prompt instead of acting on it, so you never get a second fix for a problem that's already handled.

Some reports arrive with suggested prompts above the box. The [scout](/docs/self-driving/scouts) that wrote the report knows which threads it left open and what it recommends, so it offers those as a way in. A prompt is a question, like "Which other teams hit this error?", or an action, like "Create the alert the report recommends, then mark this report resolved". Click one to fill the box, then send it as written or edit it first – nothing is sent until you click **Ask AI**, so a wrong pick costs you nothing. Not every report suggests prompts, and you can always ignore them and write your own.
