...
<CalloutBox icon="IconInfo" title="When a scout pauses itself" type="fyi">

If a scout fails five runs in a row, PostHog pauses it automatically so a broken configuration does not keep consuming runs on its normal schedule. It retries once a day. A successful retry resumes the scout automatically; you can also enable the scout again after checking its configuration.
