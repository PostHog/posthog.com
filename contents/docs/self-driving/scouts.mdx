...
Records land in your project as `$scout_structured_output` events, with each field of the record flattened to an `output_<field>` property and a `subject` property naming the item that was judged. Because they're ordinary events, the metric works everywhere in PostHog with nothing to wire up: chart the share of items judged `critical` in an insight, break down by any field, set an [alert](/docs/alerts) on a shift, or query the raw records with [SQL](/docs/sql).
