...
When you dismiss, snooze, or restore a [report](/docs/self-driving/reports) and type a note explaining why, that note is the single most useful thing a scout can learn from: a human saying, in their own words, whether what it surfaced was worth surfacing. So it doesn't just sit on the report – it's also forwarded into the steering channel, where later runs read it by name.

The scout is told which way you judged it, so a restore teaches the opposite lesson from a dismissal. Bringing a report back into your inbox says filing it *was* right, and a note explaining why is how a scout learns to keep looking. Both halves of the feedback loop reach the fleet, not just the "stop doing this" half.

Resolving a report doesn't forward anything, because resolving says the report did its job rather than telling a scout anything about whether to file it again. That note stays on the report, which every scout searches before emitting. Forwarded notes expire after 30 days, so a steady stream of dismissals can't crowd out the steering you leave deliberately.

Forwarding needs the same access as leaving a scout note by hand, because scouts read note content verbatim. If you can't edit scouts, your note is still recorded on the report – it just doesn't enter the steering channel.

The practical upshot: triaging your inbox honestly is how scouts get quieter, and how they learn what to keep filing. You don't have to go and edit a scout to stop it repeating itself.
