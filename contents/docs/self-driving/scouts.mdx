...
By default, a scout runs on a rolling interval – once a set number of minutes have passed since its last run (every 24 hours by default, and never more often than every 30 minutes). If you'd rather it run at specific times, give it a five-field cron expression instead – like `30 9 * * *` for 9:30am every day, or `0 9 * * 1-5` for 9am on weekdays.

Cron schedules are evaluated in your project's timezone and follow daylight-saving changes, so a 9am schedule stays at 9am local year-round. When a scout has a cron schedule it takes precedence over the rolling interval, and its runs still have to be at least 30 minutes apart – the same floor as the interval.
