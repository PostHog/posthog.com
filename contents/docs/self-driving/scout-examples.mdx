...
A bigger troop isn't a better troop. Every enabled scout is a recurring scheduled agent run, so the setup wizard defaults to the general scout plus one or two specialists for the products you use most – copy that instinct when enabling more. And if error tracking or session replay are connected as [signal sources](/docs/self-driving/inbox/sources), leave their scouts off: the native sources already deliver the same findings to your inbox, so the scout only duplicates them.
