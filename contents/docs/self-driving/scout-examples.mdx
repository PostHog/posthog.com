...
| **Check every promise was kept** | Some failures return a 200: a user finishes a rewarded ad but the reward never lands, a payment succeeds but the webhook never arrives, an API call completes but with nothing in it. A scout can join each "promise" event to its expected fulfillment and flag when the unmatched share climbs above its baseline. |
...
| **Watch the physical world** | Anything that lands events in PostHog is watchable – including hardware. One team monitors a building access-control system's credential events, flagging deactivation surges and unexpected silence. If your product touches the real world, a scout can too. |
