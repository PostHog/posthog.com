...
| [PR follow-up](https://github.com/PostHog/posthog/blob/master/products/signals/skills/signals-scout-pr-follow-up/SKILL.md) | Recently merged pull requests, checked after deploy to verify their claimed effect and catch attributable side effects |
...
To see the fleet at work on a real product, read how PostHog helped [Juno](/customers/juno) find friction in its medication log.
...
You don't write a scout by hand. Head to your [inbox](https://app.posthog.com/inbox/scouts). On the top right, click **New scout**, then **Chat with an agent**. Tell the agent what you want watched. It checks your data and builds the scout with you. If you already know the instructions, schedule, and where reports go, pick **Fill in the form** instead. You can also ask any agent connected to the [PostHog MCP](/docs/model-context-protocol) to build one. A prompt like this works:
