...
You can combine these methods with your navigation to record only certain screens, or to pause recording on sensitive ones. For example, using `@react-navigation/native`, stop recording when a sensitive screen is focused and resume when they leave:
