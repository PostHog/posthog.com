...
Password inputs are always masked no matter your config. For React Native, this relies on the native view properties — your `<TextInput>` must use `secureTextEntry={true}` for the SDK to detect it as a password field. Similarly, fields like email are only auto-detected on iOS if you set the appropriate `textContentType` (e.g. `textContentType="emailAddress"`).
