...
> Requires PostHog Flutter SDK version >= [5.31.0](https://github.com/PostHog/posthog-flutter/releases).

By default, session replay only records your Flutter UI. When a fully native screen is presented over your app (a native paywall, a presented view controller, a native activity), the replay keeps showing the last Flutter frame until the user returns.
