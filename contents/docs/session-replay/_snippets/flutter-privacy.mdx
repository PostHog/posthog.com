...
Masking hides what's drawn on screen, but not where the user touches it: the recording still includes the position of each tap. On a PIN, passcode, or card-number keypad, those positions alone can reveal the value entered, so masking the keypad isn't enough. Stop session recording while such a screen is shown and start it again once the screen is dismissed. See the Flutter tab of [programmatic start and stop controls](/docs/session-replay/how-to-control-which-sessions-you-record#programmatic-start-and-stop-controls) for the methods and a widget example.
