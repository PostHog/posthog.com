...
For apps using async `URLSession` methods, PostHog provides wrapper functions that you can use to manually capture network logs. Pass an initialized SDK instance with `postHog: PostHogSDK.shared`; omitting it performs the request without capturing replay telemetry.
...
func fetchData(from url: URL) async throws -> Data {
    ...
    let (data, _) = try await URLSession.shared.postHogData(
        from: url,
