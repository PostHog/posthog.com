...
PostHog iOS SDK allows you to customize how log messages are processed and categorized by providing a custom `processConsoleOutput` handler. 
This handler receives raw console output and returns an array of processed log entries with their respective severity levels:

```ios_swift
let config = PostHogConfig(apiKey: <ph_project_api_key>, host: <ph_client_api_host>)

// Custom log processing and sanitization
config.sessionReplayConfig.captureLogsConfig.processConsoleOutput = { output in
...
By default, the SDK only captures error-level logs to minimize noise in session replays.
 You can adjust this using the `minimumLogLevel` setting:

```ios_swift
// Capture errors only (default)
config.sessionReplayConfig.captureLogsConfig.minimumLogLevel = .error

// Capture warnings and errors
config.sessionReplayConfig.captureLogsConfig.minimumLogLevel = .warn

// Capture all logs (most verbose)
config.sessionReplayConfig.captureLogsConfig.minimumLogLevel = .info
```

Only logs with a level greater than or equal to the `minimumLogLevel` will be captured. For example, if set to `.warn`, both **warning** and **error** logs will be captured, but **info** logs will be filtered out.
...
- Log filtering based on `minimumLogLevel` (default: `.error`). Only logs with equal or higher severity are captured. 
