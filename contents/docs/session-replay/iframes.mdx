...
- **There is no per-domain allowlist for iframe recording.** PostHog does not have a setting to whitelist specific third-party iframes (e.g. "record Typeform but nothing else"). Cross-origin iframe recording is an all-or-nothing config option that still requires PostHog to be installed inside the iframe.
