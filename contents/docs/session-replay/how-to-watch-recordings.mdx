...
4. Open the activity panel to see a list of all the events that happened during the recording. This also lets you see the network waterfall, page performance score, asset breakdown, console logs, and [backend logs](#viewing-backend-logs).
...
If you've set up [PostHog Logs](/docs/logs) and [linked them to Session Replay](/docs/logs/link-session-replay), you can view backend logs directly in the activity panel.

To view backend logs:

1. Open a session replay and click the activity panel on the right
2. Click **Logs** to filter to backend logs only
3. Use the mini-filters to narrow down by severity level (info, warn, error)

Each log entry shows severity level, timestamp, and message. Click a log entry to see details like service name, log attributes, and a link to view the full log in [Logs](/docs/logs).

Backend logs differ from console logs. Console logs capture client-side `console.log()` statements from the browser. Backend logs show server-side logs from your application, correlated with the user's session.
