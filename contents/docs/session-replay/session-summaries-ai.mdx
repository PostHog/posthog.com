...
<CalloutBox icon="IconInfo" title="PostHog Cloud only" type="fyi">

Session summaries are available on PostHog Cloud only. They are not supported on self-hosted deployments. This is separate from [PostHog AI](/docs/posthog-ai) (Max), which works on self-hosted instances with a configured `ANTHROPIC_API_KEY`.
