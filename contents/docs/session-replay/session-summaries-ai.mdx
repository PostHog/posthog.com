...
You can add product context to help PostHog AI generate more accurate summaries for your specific product. This context is injected into every single-session replay summary, helping the AI understand your custom events, product terminology, and intentional behaviors.

To configure product context, go to **Settings > Replay > AI product context**. Project admins can write up to 10,000 characters describing your product.

Good product context includes:

- **Custom event and feature names** – What key events mean in your product (e.g., "creating a workspace", "completing a sprint")
- **Product terminology** – Terms specific to your product that the AI should recognize
- **Intentional behaviors** – Things that look like errors but aren't (e.g., "Free users see an upgrade modal when they try to export – this is intentional")
- **Normal user patterns** – Behaviors that might look like confusion but are expected (e.g., "Users often switch between board view and list view rapidly when comparing tasks")
- **What success looks like** – How to recognize when a user has completed their goal
...
Product context only applies to single-session replay summaries generated from the replay page. It doesn't affect group summaries or summaries generated via Max.

## Add custom tags

PostHog AI automatically applies tags from a fixed taxonomy to each session summary. You can extend this with custom tags specific to your product or workflows.

Custom tags let you define a vocabulary that matches your domain. For example, an e-commerce checkout team might create tags like `cart_abandoned` or `payment_failed`, while a B2B tool might use `workspace_invite` or `first_dashboard_setup`.

To add custom tags, go to **Settings > Replay > AI**. Each tag needs:

- **Name** – A snake_case identifier (lowercase letters, numbers, and underscores). Maximum 60 characters.
- **Description** – Tell the AI when to apply this tag. Maximum 200 characters.

You can add up to 15 custom tags. The AI uses your descriptions to decide when each tag applies, so be specific about the behavior or outcome that should trigger the tag.

Custom tags appear separately from the fixed taxonomy in session summaries. Like product context, custom tags only apply to single-session replay summaries generated from the replay page.
