...
Session summaries are especially useful for understanding [Experiment](/docs/experiments) results. Metrics tell you _which_ variant won, but summaries tell you _why_.
...
## Customize with custom tags

By default, PostHog AI uses a fixed set of tags to categorize recordings. Custom tags let you define up to 15 product-specific tags (using `snake_case` names) so the AI can surface patterns relevant to your team – like `cart_abandoned` or `workspace_invite`.

For setup instructions, see [Customize summaries with custom tags](/docs/posthog-ai/session-summaries#customize-summaries-with-custom-tags).
