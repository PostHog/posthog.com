...
To watch recordings, you can either visit the '[Replay](https://app.posthog.com/replay)' page, or click on any data point in an insight and from the list of persons related to that data point. This is specially useful in [funnels](/docs/product-analytics/funnels), where you can drill down and watch recordings of users who converted or dropped off.

When watching replays, you can change the playback speed, as well as select the option 'skip inactivity' - this will skip parts of the recording where the user was inactive on the page.

// TODO add video of how to watch recording
...
## How to configure which sessions you record

If you want more granular controls on selecting which sessions to record, you can combine session recording with [feature flags](/docs/feature-flags). This enables you to control session recordings based on users with certain properties, actions, or events. You can also use feature flags to capture a percentage of all sessions.

To do this set `disable_session_recording: true` in the [JavaScript library config](/docs/integrate/client/js/#config). Then, when you'd like to start recording, call the method `posthog.startSessionRecording`. 

You can do this based on the value in a feature flag e.g.,:
...
Similarly, you can stop the recording at any point using:
