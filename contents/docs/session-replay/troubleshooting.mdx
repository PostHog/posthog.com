...
Authorized domains for recordings is deprecated, you should use URL triggers instead  [Read more: https://posthog.com/docs/session-replay/how-to-control-which-sessions-you-record#with-url-trigger-conditions](https://posthog.com/docs/session-replay/how-to-control-which-sessions-you-record#with-url-trigger-conditions)
...
Filtering options in session replays depend on [product analytics](/docs/product-analytics) events. 

To enable useful filtering options, we recommend at the very latest:
- Identifying users via `identify()` - to allow filtering by person properties
- Capturing `$pageview` events, either via [autocapture](/docs/product-analytics/autocapture) or manually - to allow filtering by page properties like URL
