...
You can also check using the [SDK Health](/docs/sdk-health) tool.
...
The **Enable recordings when URL matches** setting is only supported in the web SDK version `1.171.0` and above. If your recordings aren't being recorded as expected, even when the URL matches your regex, it's likely due to an [outdated SDK version](/docs/sdk-health).
