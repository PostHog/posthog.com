...
## Recording can't be played (missing snapshot)

If you see a "This recording can't be played" error, it means the initial full snapshot of the page never reached PostHog. Session Replay works by first capturing a complete snapshot of the DOM when recording starts, then recording only the changes (incremental snapshots) after that. Without the initial full snapshot, there's no baseline to play back.

This typically happens when:

- The browser is closed or navigated away before the first recording payload finishes uploading
- The browser goes offline during the upload
- The connection is slow – the initial snapshot is the largest payload, making it the most likely to be interrupted

This data is unrecoverable. The visual replay for the session can't be restored. However, other events from the session may still be available in [Product Analytics](/docs/product-analytics).

For very recent recordings (less than five minutes old), PostHog shows a "We're still working on it" message instead, since the data may still be in the ingestion pipeline. If the recording still can't be played after five minutes, the snapshot data didn't arrive.
