...
If the `crossorigin` attribute is not set on `link` elements a Chrome specific protection will prevent programatic access to stylesheet rules (e.g. `stylesheet.cssRules` throws an exception). This means that styles on your site render as expected but they cannot be captured by PostHog in order to recreate the session during playback.
