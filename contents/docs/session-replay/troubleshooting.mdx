...
It is not always possible to know if a recording has been captured in advance of trying to play it.

If an event is captured before your minimum recording duration setting there is no guarantee that the session persisted long enough to start recording.
