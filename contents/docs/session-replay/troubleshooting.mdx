...
## Internal and intranet applications

PostHog works for apps that run on a private network or behind a firewall, but two replay features need a network route to your app.

Product analytics, feature flags, surveys, and error tracking all work. Each SDK sends data to PostHog over an outbound HTTPS connection, so PostHog does not need to reach your app. Web SDKs connect from the user's browser, and server and mobile SDKs connect from their own runtimes. Each runtime that runs an SDK only needs outbound access to PostHog.

Two replay features are different because PostHog fetches assets from your app:

- **Replay assets that PostHog cannot inline.** PostHog captures most assets [inline](#ensure-your-website-permits-posthog-cors), but it loads the rest from your web server during playback. A viewer on your private network sees these assets. A viewer outside the network does not, because the browser cannot reach your server.
- **Server-side heatmap screenshots.** To render a [heatmap](/docs/toolbar/heatmaps) over a screenshot, PostHog fetches the page from its own servers. This fails for a private address, because PostHog cannot reach it. Adding [PostHog's outbound IPs](/docs/toolbar/heatmaps#viewing-heatmaps-in-posthog-beta) to your firewall does not help when the address is private.

When PostHog cannot render a heatmap screenshot, it [falls back to session recordings](/docs/toolbar/heatmaps#screenshot-or-iframe-heatmap-fails-to-load) that visited the same page. This shows heatmap data over a recording snapshot, so heatmaps still work for internal apps.
...
## Tabs are recorded as separate sessions

PostHog records all tabs of your site into one recording by default. Read [recordings across multiple tabs and windows](/docs/session-replay/how-to-watch-recordings#recordings-across-multiple-tabs-and-windows) for how this works. Check the causes below if each tab starts its own recording.

- **The tabs use a different browser, device, or browser profile.** PostHog cannot join these into one session. A normal window and an incognito window are also separate.
- **The tabs use different domains.** PostHog stores the session ID for one domain only. A user who moves from `example.com` to `example.org` starts a new session. Read the [session ID note](/docs/data/sessions#how-does-posthog-define-a-session) if you must join two domains yourself.
- **`persistence` is set to `sessionStorage` or `memory`.** Both values store the session ID for one tab only. Use the default value `localStorage+cookie` for cross-tab recording. Read [persistence](/docs/libraries/js/persistence).
- **`cross_subdomain_cookie` is set to `false` and the tabs use different subdomains.** Remove this setting, or set it to `true`. Read [configuration](/docs/libraries/js/config).
- **Your site runs on `herokuapp.com`, `vercel.app`, or `netlify.app`.** PostHog does not set a cross-subdomain cookie on these shared domains. Tabs on different subdomains get separate sessions. Use a custom domain to prevent this.
- **Your code calls `posthog.reset()`.** This call always starts a new session and a new recording.
- **The session reached a limit.** PostHog starts a new session after 30 minutes without activity, or after 24 hours. Read [how PostHog defines a session](/docs/data/sessions#how-does-posthog-define-a-session).
...
## Element looks frozen because of rapid DOM changes

Session replay records changes to the page DOM. The recorder limits how often it records attribute changes, such as `style` or `class` updates, on any single element. When an element's attributes change faster than that limit, for example during an SVG animation that updates styles every frame, the recorder drops the element's extra attribute changes until the rate slows down. Added nodes, removed nodes, and text changes are still recorded.

In playback, a throttled element can look frozen or out of date. To fix this:

- Avoid per-frame style or attribute animation on large elements and SVGs where you can.
- Exclude the element from capture with the `ph-no-capture` class, so the recorder skips it and the DOM changes inside it. Masking doesn't help here – it redacts the content but still records the element and its mutations. See [excluding elements from capture](/docs/session-replay/privacy#other-elements).
