...
If you use cohorts to filter sessions, or add cohort condition into the "Filter out internal and test users" setting - it can work incorrectly (or not work at all) while cohorts are calculating (to properly decide what people are in).
