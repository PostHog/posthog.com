...
The recorder applies a mutation ceiling while it captures. When a page passes the ceiling, the recorder throttles the extra DOM mutations to protect the browser from freezing. The rest of the session still records, but the throttled part can look incomplete, because some DOM changes were never captured.
