...
[Deploying a reverse proxy](https://posthog.com/docs/advanced/proxy) is the best way to avoid tracking blockers intercepting calls to PostHog.
...
When we capture data from your page we compress it and allow very large messages to be sent to our servers for storage. But even then sometimes we receive data from your page that is too large to be ingested.

This almost always means that playback won't be accurate. For example: you might notice missing styles or page navigation not being reflected in the playback.

The only way to fix this is to reduce the amount of data that you send to our service. [Learn how to restrict what you capture in our privacy docs](https://posthog.com/docs/session-replay/privacy)
