...
> **Important:** This API was never built for public use and is built around the needs of our UI. But an increasing number of people are building on top of it. That's cool and exciting but also means that it is not straight-forward to use. This documentation is to make life easier but this API will change with relatively little notice. We are building a batch export mechanism to remove the need to use this API for bulk copying of replay data.

## Overview

1. call the snapshots API with no parameters and receive a list of the sources in the recording
    * this is not idempotent and can change from second to second
3. call the snapshots API with each source one after the other
    * this call is idempotent, we never change this data once stored
4. post-process the data
    * session replay is write-heavy, we do not attempt to store or return vanilla rrweb-compatible JSON


> **Important:** extra important. For batch export it is safest to only export recordings that started more than 24 hours ago. We cut off sessions after 24 hours and so you know the recording is immutable at that point.

## The API (Version 1)

This is a little like hitting the homepage of a website to see what documents are available, and then right-clicking to open each one in a new tab
...
We (Pawel) have been working on a new version of ingestion pipeline that is way more efficient and uses way less resources. Last time we improved the ingestion pipeline and let our new costs settle we were able to reduce prices... watch this space
...
If you have any blob_v2 sources, you can ignore all other sources

And only make calls to snapshot s with `?source=blob_v2&blob_key={the blob_key}` for each `blob_v2` source

Because `blob_v2` files are flushed more frequently, there is no need to poll the realtime source. And instead you can poll the original snapshot API to check for new sources to load.
