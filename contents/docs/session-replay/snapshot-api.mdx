...
> **Important:** This API was never built for public use and is built around the needs of our UI. But an increasing number of people are building on top of it. That's cool and exciting but also means that it is not straight-forward to use. This documentation is to make life easy but this API will change with relatively little notice. We are building a batch export mechanism to remove the need to use this API for bulk copying of replay data.
