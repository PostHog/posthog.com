...
Rate limits apply only to requests authenticated with a [personal API key](/docs/api/overview#personal-api-key-recommended). Requests using session authentication (logged-in users in the PostHog app) or sharing tokens bypass rate limiting entirely.

Limits are determined by your organization's plan tier:

| Tier | Burst limit | Sustained limit |
|------|------------|----------------|
| Free | 30 requests/minute | 150 requests/hour |
| Paid (Scale, Teams) | 90 requests/minute | 500 requests/hour |
| Enterprise | 120 requests/minute | 600 requests/hour |

Rate limit tier results are cached for 12 hours per team. If there's an error determining your tier, the API falls back to free-tier rates.
