...
Canvas recording normally has no impact on your site, but it does need to capture the canvas to an image so can have performance impact on some sites.

You can override the frequency the image is captured, and the quality of the captured image when initialising PostHog on your site
...
posthog.init('<ph_project_api_key>', {
  ...
  session_recording: {
    captureCanvas: {
      canvasFps: 2, // default is 4, accepts a number between 0 and 12
      canvasQuality: "0.2", // default is 0.4, accepts a decimal number between 0 and 1 as a string
    }
  }
})
```
