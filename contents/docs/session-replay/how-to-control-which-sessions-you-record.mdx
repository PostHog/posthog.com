...
Trigger groups let you define multiple independent sets of recording triggers, each with its own sampling rate, minimum duration, and conditions. A recording starts if **any** of the trigger groups match (OR logic between groups).

To configure trigger groups, go to your [replay ingestion settings](https://app.posthog.com/settings/environment-replay#replay-triggers).

Each trigger group has the following options:

- **Name** – A descriptive label for the group (e.g., "Error Tracking", "Feature Testing")
- **Sample rate (%)** – The percentage of matching sessions to record (0–100)
- **Minimum duration** – How long a session must last before recording starts
- **Match type** – Whether **any** or **all** conditions within the group must match
- **Conditions** – One or more of the following:
  - **Event triggers** – Start recording when specific events are captured
  - **URL patterns** – Regex patterns that match the current page URL
  - **Feature flag** – Start recording when a specific flag is enabled

If a trigger group has no conditions, it matches all sessions (subject to its sample rate and minimum duration).

> **Note:** URL regex patterns are automatically wrapped with anchors (`^` and `$`) if you don't include them. For example, `https://example.com/` becomes `^https://example.com/$` to avoid unexpected matches.
