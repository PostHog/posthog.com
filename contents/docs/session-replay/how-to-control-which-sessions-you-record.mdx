...
> **NOTE:** Originally session recording url and event triggers matched when either matched, while linked flags and sampling only when both matched. This was confusing, so from version 1.238.0 of the web SDK you can choose which behavior you want. See "Combining conditions" below.
...
You'll capture 20% of any session on the check page that has an exception event.
