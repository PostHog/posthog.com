...
On mobile SDKs (iOS, Android, React Native, Flutter), `sessionReplay = true` puts PostHog in charge: [trigger groups](#trigger-groups), remote config, and session rotation can all start a recording, and a `stop` call only pauses the recording that's currently running. If your own app code decides who gets recorded (a feature flag you read yourself, consent state, a region check, a screen allow/deny list), set `sessionReplay = false` and drive `start`/`stop` yourself. In manual mode, a stop lasts until you call start again.
