...
<CalloutBox icon="IconWarning" title="Feature flag recording doesn't work with advanced_only_evaluate_survey_feature_flags" type="caution">

If you set [`advanced_only_evaluate_survey_feature_flags`](/docs/libraries/js/config#advanced-configuration) to `true`, feature flag-based session recording won't work. This setting causes the `/flags` endpoint to only return survey-related flags, and the response replaces any [bootstrapped](/docs/feature-flags/bootstrapping) flag values rather than merging with them. As a result, the linked recording flag has nothing to match against.

To work around this, use [`advanced_disable_feature_flags`](/docs/libraries/js/config#advanced-configuration) with [bootstrapping](/docs/feature-flags/bootstrapping) instead. This prevents the SDK from making feature flag requests, so your bootstrapped values (including the recording flag) are preserved. Note that this also disables [surveys](/docs/surveys), so this approach only works if you don't use surveys.
