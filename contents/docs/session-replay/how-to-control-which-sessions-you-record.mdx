...
In most cases, you can initialize the PostHog SDK with the default settings and use the trigger group conditions above to control what gets recorded. If your targeting logic lives on the server (an event, a linked feature flag, sampling), use trigger groups. If it lives in your own app code (a flag you read yourself, consent state, a region check), use [manual start/stop controls](#programmatic-start-and-stop-controls) instead. On mobile, that also means turning off automatic recording so PostHog doesn't start a session before you decide to.
...
<CalloutBox icon="IconInfo" title="On mobile, pick automatic or manual mode" type="fyi">

On mobile SDKs (iOS, Android, React Native, Flutter), `sessionReplay = true` puts PostHog in charge: [trigger groups](#trigger-groups), remote config, and session rotation can all start a recording, and a `stop` call only pauses the recording that's currently running. If your own app code decides who gets recorded (a feature flag you read yourself, consent state, a region check, a screen allow/deny list), set `sessionReplay = false` and drive `start`/`stop` yourself. Whether a stop lasts once you've started recording varies by platform. See the per-platform notes below.
