...
## With event or URL trigger conditions

You can opt to only start recordings once your user visits a certain page, or when an event is triggered.  
After the condition matches, the recording continues until their session ends.  
...
<ProductScreenshot
    imageLight="https://res.cloudinary.com/dmukukwp6/image/upload/replay_event_trigger_light_f1dc560667.png" 
    imageDark="https://res.cloudinary.com/dmukukwp6/image/upload/replay_event_trigger_dark_c8b2729731.png"
...
You can opt to only start recordings once a specific event happens. After the event is triggered, the recording continues even after they leave the matching page.  
The client keeps a short buffer in-memory, so you'll still be able to see how they have arrived at the page.
