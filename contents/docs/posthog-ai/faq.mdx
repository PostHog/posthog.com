...
PostHog AI runs entirely on third-party LLM providers (Anthropic, OpenAI, and Alphabet). We don't train or host our own models, so the environmental footprint of running PostHog AI largely tracks the sustainability commitments of those providers rather than PostHog's own infrastructure.

Where we can influence efficiency directly, we do: PostHog automatically selects the most efficient model capable of handling each AI feature, which avoids running larger, more energy-intensive models than a task requires.
