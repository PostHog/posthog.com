...
When you have a complex request that requires multiple steps, plan mode helps ensure the agent understands your goals correctly. The agent will:

1. Ask clarifying questions if needed
2. Create a plan document outlining the steps
3. Wait for your approval before executing

This prevents wasted effort on misunderstood requirements and gives you visibility into how the agent will approach your task.

## When to use plan mode

Plan mode is useful for:

- **Multi-step analyses** - "Why did conversion drop last week?" requires checking multiple metrics and correlating data
...
When you enter plan mode with a request, the agent may ask clarifying questions to understand your objective. For example:
...
The agent creates a plan notebook that outlines:
...
- **Create plan notebooks** - Document the planned approach
