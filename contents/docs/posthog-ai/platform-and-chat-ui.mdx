...
PostHog AI can call tools on third-party MCP servers that you've installed. This extends PostHog AI's capabilities beyond built-in tools to include external services like Linear, Notion, and other MCP-compatible servers.

To install MCP servers, go to [**Settings** > **MCP servers**](https://app.posthog.com/settings/mcp-servers). You can install recommended servers or add custom servers via URL using OAuth or API key authentication.

Once installed, PostHog AI automatically discovers and uses tools from these servers during your conversations.
