...
You can also access PostHog AI from the [command palette](/docs/cmd-k). Open it with `Cmd` + `K` (Mac) or `Ctrl` + `K` (Windows/Linux) and type a question – **Ask PostHog AI** appears as the first result. Press `Enter` to send your query directly to PostHog AI.
