...
<Caption>PostHog AI chat is always available in the sidebar</Caption>
...
This consent setting also controls access to AI-powered tools in the [PostHog MCP server](/docs/model-context-protocol). When AI data processing is disabled, tools that use LLMs internally (such as `query-generate-hogql-from-question` for natural language to HogQL conversion) are filtered out and not available to MCP clients.

When you enable AI data processing approval, these tools become available alongside PostHog AI chat.

## Data Warehouse AI features

This consent setting also enables AI-powered features in the [Data Warehouse](/docs/data-warehouse). For example, the [Custom REST source](/docs/cdp/sources/custom-rest-source) includes an AI manifest builder that drafts your source configuration from API documentation you provide. When AI data processing is disabled, these features aren't available.
