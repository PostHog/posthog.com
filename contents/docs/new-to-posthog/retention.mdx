...
Once more, we need to think about retention as a *precursor to revenue*. How can we measure that people are happy with us, and will be just as happy to pay for it?
...
What does retention mean for a social app like **Instagram**? If a user signs up and follows a bunch of people (activation), all of our effort is for nothing if they never come back. We'll have an empty room, no matter how many new rows are added to our users table. In this case, a user returning several times within a few days would represent strong retention.

Events that show up in your activation metric could also pull double-duty in a retention metric. In the **Uber** example, taking one ride is activation, while taking three rides in a month would demonstrate retention.
...
For each period after day 0, the table reports the percentage of users who performed the retention event.
