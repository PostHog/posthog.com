...
<blockquote class="warning-note">
    Warning! You can't call identify straight after an `.init` (as `.init` sends a page view, probably with the user's
    anonymous ID.
</blockquote>

To address the issue described above, you can call `.init` with a callback, inside which you can then call identify,
...
If a user is logged out, you probably want to call `reset()` to unset any `distinct_ids`.
