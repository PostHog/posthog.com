...
You must have admin or owner access to the PostHog organization to link it. Organizations already linked to Vercel won't appear in the dropdown. Billing stays with PostHog for linked accounts. When you link an existing account, PostHog automatically syncs your existing feature flags to Vercel and pushes the required environment variables.
