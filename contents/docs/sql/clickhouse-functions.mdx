...
In addition to native ClickHouse functions, HogQL provides PostHog-specific functions you can use anywhere HogQL runs – the SQL editor, HogQLQuery, Trends, and other query runners.

### Bot detection and traffic classification

These functions classify user agent strings to identify bots, crawlers, and AI agents. They use `multiMatchAnyIndex` for efficient single-pass pattern matching.

- `getTrafficType(user_agent)` – returns `'AI Agent'`, `'Bot'`, `'Automation'`, or `'Regular'`
- `getTrafficCategory(user_agent)` – returns a subcategory like `'ai_crawler'`, `'ai_search'`, `'ai_assistant'`, `'search_crawler'`, `'seo_crawler'`, `'regular'`, etc.
- `isLikelyBot(user_agent)` – returns `true` if the user agent matches bot or automation patterns, `false` otherwise
- `getBotType(user_agent)` – returns the bot category (e.g. `'ai_crawler'`, `'search_crawler'`) or empty string for regular traffic
- `getBotName(user_agent)` – returns the bot name like `'Googlebot'`, `'ChatGPT'`, etc. Empty string for regular traffic
- `getBotOperator(user_agent)` – returns the operator or company name like `'Google'`, `'OpenAI'`, `'Anthropic'`, etc. Empty string for regular traffic

**Example:** Break down events by traffic type:
