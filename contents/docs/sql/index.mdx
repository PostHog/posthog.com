...
You can use SQL insights within [notebooks](/docs/notebooks) and with external sources using the [data warehouse](/docs/data-warehouse). Notebooks also support dedicated HogQL nodes that integrate with Python variables and the `hogql_execute()` function for more advanced analysis workflows.
