...
> **Date literals parse in your project's timezone, not UTC.** `toDateTime('2026-07-17 06:42:00')` means 06:42 *project time*, so a UTC timestamp pasted into a literal can silently shift by hours — a recency filter built this way returns no rows and looks like missing data. For relative windows, prefer `now() - interval 20 minute`; when you need an absolute instant, pass the timezone explicitly: `toDateTime('2026-07-17 06:42:00', 'UTC')`.
