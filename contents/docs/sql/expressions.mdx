...
- `event`
- `elements_chain` (from [autocapture](/tutorials/sql-autocapture))
- `timestamp`
- `distinct_id`
- `person_id`
...
- Checking if a property or [autocapture element chain](/tutorials/sql-autocapture) contains a specific value or any of an array of values using `in` or `match`.
...
- Extracting the ID from [autocaptured elements](/tutorials/sql-autocapture) like `extract(elements_chain, '[:|"]attr__id="(.*?)"')`.