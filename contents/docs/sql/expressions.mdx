...
- Trends series
- Breakdowns
- [Retention](/docs/product-analytics/retention) breakdowns
...
- The [activity tab](https://app.posthog.com/events)
...
<CalloutBox icon="IconInfo" title="When to use bracket notation" type="fyi">

Dot notation only works for property keys that are valid HogQL identifiers – keys must start with a letter, underscore, or `$`, and can only contain letters, digits, underscores, and `$`.

Keys containing hyphens, dots, spaces, or starting with a digit **must** use bracket notation. For example, survey response properties like `$survey_response_0196e96d-7705-7d32` require bracket notation:

```sql
-- ❌ This fails (hyphens are parsed as subtraction)
SELECT properties.$survey_response_0196e96d-7705-7d32 FROM events

-- ✅ Use bracket notation instead
SELECT properties['$survey_response_0196e96d-7705-7d32'] FROM events
```

This applies to all JSON columns like `properties`, `person.properties`, and data warehouse table properties.
...
Types (and names) for the accessible data can be found in the [database](https://app.posthog.com/data-management/database) and [properties](https://app.posthog.com/data-management/properties) tabs in data management. They include:
