...
4. **Is the credit budget exhausted?** Check the spend tile on the Replay Vision home (the Usage tab breaks spend down per scanner). A scanner whose background sweep is paused by the budget will silently produce nothing until credits free up. See [quota and limits](/docs/replay-vision/quota-and-limits).
...
## Observations succeed but `$recording_observed` events are missing

If the Observations tab shows successful observations, but a query for `$recording_observed` returns no rows, an ingestion [transformation](/docs/cdp/transformations) is the most likely cause. A transformation that keeps only an allowlist of events discards `$recording_observed` unless you add it to the allowlist.

**What to do:** open the transformation in [data pipelines](https://app.posthog.com/data-management/transformations), add `$recording_observed` to the list of allowed events, and save. Events from before the change are not recovered, so only new observations become queryable.
...
A transient Gemini error – network hiccup, rate limit, model overloaded. PostHog retries the provider call automatically during the scan (up to three attempts). An observation only lands here once those retries are exhausted.

**What to do:** retry the observation from its detail page. If you're seeing a burst across many sessions, the upstream model is likely having availability issues – wait it out before triggering more on-demand work.
...
**What to do:** retry the observation from its detail page. The retry replaces the failed row with a fresh scan of the same recording. Interrupted observations don't spend credits. If the same session orphans repeatedly, [contact support](/questions) with the observation ID.
