...
You can retry any failed observation using the **Retry** button, which appears on the observation row in the scanner's observations table, on the observation detail page, and in the replay player's observations dock. Retrying deletes the failed observation and queues a fresh scan for the same session. The new attempt counts against your monthly [quota](/docs/replay-vision/quota-and-limits). There is no bulk retry – each failed observation must be retried individually.

### `provider_transient`

A transient Gemini error – network hiccup, rate limit, model overloaded.

**What to do:** retry the observation once the transient issue has passed. If you're seeing a burst of failures across many sessions, the upstream model is likely having availability issues – wait it out before retrying or triggering more on-demand work.
