...
1. **Look at which scanner is consuming the most quota.** The **Usage** tab shows per-scanner spend, including observation count, sampling rate, and share of total spend – scanners are ordered by cost, so the biggest consumers are at the top.
