...
During the closed beta, each organization gets **3,000 observations per calendar month**. Each observation costs a model-dependent number of credits – the default model costs 5 credits per observation, with cheaper and more capable options available. The counter resets at the start of each calendar month (UTC).
...
- **Choose a more economical model** – Gemini 3.5 Flash Lite costs 2 credits per observation vs. 5 for the default, stretching your budget further.
- **Combine these** – a tight filter on the sessions you care most about, plus Focused or Balanced coverage, plus a higher sampling rate, often yields better signal-per-observation than a loose filter with Comprehensive coverage and low sampling.
