...
Each scanner also has a built-in **daily digest** – a scheduled summary that synthesizes the scanner's recent observations into a single report. The digest appears in the scanner's **Summaries and alerts** tab alongside any user-created summaries and alerts, marked with a "Daily digest" chip. You can also trigger a summary immediately with the **Run now** button instead of waiting for the next scheduled run.
