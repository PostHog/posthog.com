...
Each scanner has an **Observations** tab listing every observation it's produced. The detail view for one observation shows its structured result, the model's reasoning (with clickable citations that jump the embedded player to the cited moment), and the prompt that produced it – frozen even if you've since edited the scanner.
...
Pending and running observations count against your monthly [quota](/docs/replay-vision/quota-and-limits); failed and ineligible ones don't.

## Re-running an observation

Each `(scanner, session)` pair can only be observed once. To re-run a scanner against a session it's already looked at, delete the existing observation first – the next on-demand trigger (or matching background sweep) will produce a fresh one. Deleting an observation also removes its `$recording_observed` event from your project.
