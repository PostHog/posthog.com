...
- **`failed`** – the detail view shows an error reason. Failed observations can be retried: click the **Retry** button on the observations table row, the observation detail page, or the replay dock. Retrying deletes the failed row and starts a fresh observation for the same session. The retry counts against your [quota](/docs/replay-vision/quota-and-limits). See [troubleshooting](/docs/replay-vision/troubleshooting#failed-observations) for what each failure kind means.
