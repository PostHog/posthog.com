...
{/* SCREENSHOT: observations-dock – the replay player with the observations dock expanded at the bottom: two observation cards from different scanners, the "Scan this recording" button visible in the dock header. */}
...
```sql
SELECT
    arrayJoin(properties.scanner_output_friction_points) AS friction_point,
    count() AS sessions
FROM events
WHERE event = '$recording_observed'
  AND properties.scanner_name = 'Session summary'
