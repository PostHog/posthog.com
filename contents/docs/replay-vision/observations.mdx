...
Succeeded, pending, and running observations spend [credits](/docs/replay-vision/quota-and-limits); failed and ineligible ones don't. A failed observation can be retried from its detail page (or from its card in the replay player), which replaces it with a fresh scan of the same recording.

## Querying observations as events

Every successful observation is captured as a `$recording_observed` event in your project, so you can build [insights](/docs/product-analytics/insights), [dashboards](/docs/product-analytics/dashboards), and [alerts](/docs/alerts) on top of Replay Vision output using PostHog's [SQL](/docs/data-warehouse/sql).

If your project uses a [transformation](/docs/cdp/transformations) that keeps only an allowlist of events, add `$recording_observed` to that allowlist. If you do not, PostHog discards these events at ingestion, and the queries below return nothing – the observations themselves are unaffected.
...
| `recording_distinct_id`          | Distinct ID of the person in the recorded session (the subject, not whoever triggered the scan)                                                     |
...
| `credits`                        | What this observation cost, in credits (1 credit = $0.01), frozen at the price when it ran                                                          |
...
Each event carries a `credits` property with the observation's cost (1 credit = $0.01), so spend is just another query. This is the same data behind the **Usage** tab on the Replay Vision home:

```sql
SELECT
    properties.scanner_name AS scanner,
    sum(toFloat64(properties.credits)) AS credits
FROM events
WHERE event = '$recording_observed'
  AND timestamp > now() - INTERVAL 30 DAY
GROUP BY scanner
