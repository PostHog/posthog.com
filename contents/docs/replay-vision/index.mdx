...
- **Want a regular write-up?** Put a [scout](/docs/replay-vision/scouts) on the scanner to read its findings on a schedule and report what's worth a look.
- **Want to be notified?** Add an [alert](/docs/replay-vision/alerts) that fires on matching observations or a metric threshold, and sends to Slack or a webhook.
...
<CalloutBox icon="IconChat" title="Chat with other Replay Vision users" type="fyi">

Other teams are pointing scanners at their own products. Join the [PostHog Discord](https://discord.gg/posthog) to ask what their scanners caught, post the prompts that work for you, and share feedback with the people who build it.
