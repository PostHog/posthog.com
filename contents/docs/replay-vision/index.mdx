...
{/* SCREENSHOT: replay-vision-overview-app — full Replay Vision landing page showing the scanner list (3–4 scanners with mixed types and statuses) and the per-month usage meter in the header. Both light and dark mode. */}
...
- **Observation** – one application of a scanner to a single recording. Each observation runs the scanner against that session and produces a structured result that's persisted and emitted as a queryable [`$recording_observed` event](/docs/replay-vision/querying-observations).
...
{/* SCREENSHOT: replay-vision-pipeline-diagram — illustration showing: recording → rendered video + events → Gemini → structured result → observation row + $recording_observed event. Light/dark variants if possible. */}
...
- **Already have observations?** Learn how to [query them as events](/docs/replay-vision/querying-observations) to build insights, dashboards, and alerts.
