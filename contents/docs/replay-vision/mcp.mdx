...
{/* SCREENSHOT: posthog-ai-mcp-prompt — an MCP-enabled editor (e.g. Cursor) showing the agent calling vision-scanners-scan-session in response to a natural-language prompt. */}
...
- **Audit a scanner** – list its recent observations and stats to decide whether the prompt or filters need tuning.

## The tools, by job

The MCP server ships a set of tools all prefixed `vision-` – the [overview](/docs/replay-vision) explains what scanners and observations are. The most useful groupings:
...
- `vision-scanners-update` / `vision-scanners-partial-update` – edit an existing one.
...
The call queues a Temporal workflow that takes a few minutes to finish. Poll `vision-scanners-observations-list` to find the result once it lands.
...
- `vision-scanners-observations-stats-retrieve` – aggregate stats for the scanner.
- `vision-scanners-stats-retrieve` – overall scanner-level stats.
...
See the [MCP server docs](/docs/model-context-protocol) for full setup instructions, and the [Replay Vision overview](/docs/replay-vision) for product concepts.
