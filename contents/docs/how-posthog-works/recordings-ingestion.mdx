...
Storing snapshot data in ClickHouse is not ideal. It's not a good fit for the data model and it's not a good fit for the workload. So, recordings storage is being moved to blob storage. We use [minio](https://min.io/product/kubernetes) for blob storage. 
...
After 90 days have passed the (very large) session_recording_events table is redundant and the system simplifies to:
