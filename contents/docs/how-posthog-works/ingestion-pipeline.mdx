...
### Rate limiting high-volume distinct IDs

To keep ingestion fast and reliable for everyone, the capture service applies a protection when a single distinct ID sends an unusually high, sustained volume of events within one project.
If a `(project, distinct ID)` stream exceeds roughly 5,000 events per minute sustained (measured over a rolling window), the capture service treats that stream specially for as long as the spike continues:

- Its events are rerouted to our overflow processing pipeline, which relaxes processing order guarantees for that distinct ID while the limit is active.
- [Person processing](#2-person-processing) is skipped for those events, so they do not create or update a person profile during the spike.

Events are still ingested and stored. Nothing is dropped, and the capture API still returns a normal `200` response, so there is no rate-limit error to handle in your SDK or integration.

PostHog logs an [ingestion warning](/docs/data/ingestion-warnings) each time this happens, so you can see which distinct IDs are affected.

This almost always affects distinct IDs that stand in for many users or machines rather than a single person, such as a shared server identifier (`backend`, `system`), an anonymous placeholder, or a bot.
The [identity resolution](/docs/product-analytics/identity-resolution#catch-all-distinct-ids) guide explains how to avoid it: give each real user a stable, unique distinct ID, and send server or system events with [person processing disabled](/docs/product-analytics/capture-events#how-to-capture-anonymous-events).
If you have a legitimate high-volume identifier that must keep strict ordering and person processing, [contact support](https://posthog.com/talk-to-a-human).
...
These arguments are instructions for updating person data — they aren't retained on the stored event, so you can't filter, break down, or query events by them. Query the resulting [person properties](/docs/product-analytics/person-properties) instead.
