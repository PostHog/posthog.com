...
We recommend sending a UUID with every request, and some of our libraries such as `posthog-js` will do this automatically. If you don't send UUIDs for every event, we wouldn't be able to know it's the same event and hence we wouldn't be able to unduplicate it for you. Events with the same uuid, event name, timestamp and distinct_id are considered duplicates and will be eventually unduplicated. Keep in mind that failures and retries happen, so your application or our library might send the same event multiple times, which if you don't set UUIDs would end up in PostHog as separate events.
