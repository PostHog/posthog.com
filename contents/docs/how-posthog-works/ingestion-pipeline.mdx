...
In its simplest form, the ingestion pipeline is a collection of services which listen for events as they are sent in, process them, and then store them for later analysis.
...
This first step runs any workloads that come from [destinations and transformations](/docs/cdp) you installed and which export a `processEvent` function. This is the only chance for apps to transform or exclude an event before it is written into ClickHouse.
...
> **Further reading:** [How to set and use person properties](/docs/product-analytics/user-properties)
...
The final step in the ingestion pipeline is calling the `onEvent` handler from any destinations that we have enabled.
