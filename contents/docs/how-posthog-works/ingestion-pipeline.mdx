...
We recommend sending a UUID with every request, and some of our libraries such as `posthog-js` will do this automatically. If you don't send UUIDs for every event, it's possible you may see duplicate events in PostHog. 
