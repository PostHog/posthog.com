...
- Its events are rerouted to the overflow lane and spread across partitions, so they no longer hold up a single partition. This relaxes the ordering guarantee for that distinct ID while the limit is active.
...
If you have a legitimate high-volume identifier that must keep strict ordering and person processing, [contact support](https://posthog.com/talk-to-a-human) and we can add an override for it.
