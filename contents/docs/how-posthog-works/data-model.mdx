...
| **last_seen_at** | `timestamptz` | The timestamp of the person's most recent event, rounded to the hour                                                         |
...
By default, `last_seen_at` is updated whenever a person sends an event. You can prevent this by setting the `$update_person_last_seen_at` event property to `false`. This is useful for backend enrichment jobs or server-side events that update person properties but don't represent actual user activity.
...
When `$update_person_last_seen_at` is `false`, the event is processed normally but the person's `last_seen_at` timestamp remains unchanged. This only applies when updating existing persons – `last_seen_at` is always set when a person is first created.
