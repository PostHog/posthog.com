...
<span class="text-base font-medium mb-0">프로덕트 엔지니어를 위한 제품 개발 도구</span>

<CTAs />

궁금한 점이 있으신가요? [데모 보기](/demo) 아니면 [상담 요청](/talk-to-a-human).
...
## PostHog 데이터 스택, 데이터 팀을 위해 만들고 프로덕트 팀에게 사랑받는
...
고객이 제품을 어떻게 사용하는지 분석할 때는 _전체 데이터_를 기반으로 해야 합니다.

여기에는 _제품 외부에서_ 발생하는 고객 정보도 포함됩니다:

-   Stripe 결제 내역
-   에러 트래킹 도구의 예외 정보
-   고객 지원 플랫폼의 티켓

모든 데이터를 한 곳에 모으면 다음에 무엇을 만들지 더 나은 결정을 내릴 수 있습니다.

PostHog의 Product OS는 단순한 분석 도구가 아닙니다 – 고객을 한눈에 파악할 수 있게 해주는 도구 모음이에요.
...
저희 철학은 간단해요: 가격 걱정 없이 쓰셔야 한다는 겁니다.

유료 제품도 쓰신 만큼만 내시면 되고, 월 무료 한도도 넉넉해요. 실제로 고객 98%는 무료로 쓰고 계세요.

저희는 스케일 키워도 제일 싸게 쓰실 수 있게 할 거예요. PostHog 쓰는 건 당연한 선택이어야 하고, 영업이랑 "잠깐 통화"할 필요 없어요.
...
PostHog AI는 정보 모으고 요약하는 반복 작업을 자동화하고, 곧 버그 수정과 코드 개선까지 할 수 있게 될 거예요.
...
저희가 다른 회사랑 다른 점이 몇 가지 있어요:

-   **투명해요.** [회사 핸드북](/handbook), [영업 매뉴얼](/handbook/growth/sales/overview), [회사 전략](/handbook/why-does-posthog-exist) 그냥 다 열어볼 수 있어요.
-   **빨리 씁니다.** [변경 로그](/changelog) 보시면 돼요.
-   **_진짜_ 기술 지원.** 저희 <SupportSmallTeamLink />은 다 엔지니어 출신이에요.
...
아직 여기 계신가요? 그럼 재미있을 수도 있는 링크 몇 개 드릴게요:
