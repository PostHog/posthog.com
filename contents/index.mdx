...
<<<<<<< HEAD
{/*
  Not routed. `template: custom` keeps this MDX out of createPages, and the
  English homepage is now rendered by src/pages/index.tsx using the Sections/
  components. This file exists as the GraphQL data source that src/pages/ko
  (KoreanHome -> KoreanMDXViewer) queries via useStaticQuery and renders with
  translated strings + Korean JSX descriptors. Don't delete without first
  porting /ko to render the new homepage directly.
*/}

<div class="text-center @xl:text-left mb-12">

<HeroImage />

<h1 class="[&_p]:m-0 flex gap-1 flex-wrap justify-center @xl:justify-start !text-2xl mb-8 pt-2">
<Logo />
</h1>
...
## The PostHog Context Warehouse: Built for data teams, loved by engineers
...
Whether you're analyzing customer usage or directing an AI, you should be operating with the _full context_.
...
Combine it with all the other tools and PostHog isn't just a single source of truth. It's a single source of _everything_.
