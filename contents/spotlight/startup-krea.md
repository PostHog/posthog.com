...
This quick pivot was only possible because Diego had stayed close to the data. “Especially in generative AI," says Diego, "any increase in user activity can greatly impact the whole system. It means paying attention and moving quickly."
 ...
 ## 3. LLM Observability are the new frontier

It's not the usual round-up of [pirate metrics](/product-engineers/aarrr-pirate-funnel) alone when we're building in AI. In the realm of LLM Observability, there are entirely new stats to track.
...
It's quite possible that a founder might not know to ask these questions, let alone where to look to answer them, and it's why staying alert matters. Towards this, PostHog has just launched a new product in this category, introducing [LLM Observability](/docs/llm-observability) to help you follow more closely what's happening with your LLM calls. 
