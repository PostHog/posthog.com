...
He regularly creates product events in PostHog and routes them directly to ad platforms or Customer.io using destinations. This allows Kilo Code to experiment quickly with onboarding flows, activation campaigns, and new segments, often without engineering involvement.
...
Because Kilo Code ships so frequently, traditional funnel analysis isn’t always enough to explain what’s working and what isn’t. Multiple changes often go live at once, which makes it hard to isolate impact purely through numbers. [Session replay](/session-replay) fills that gap.
...
This approach helped refine Kilo Code’s pricing, team, and enterprise offerings in ways that would have been difficult to spot through aggregate metrics alone.

## Telemetry for users who never sign up

A large portion of Kilo Code’s product can be used without creating an account. Developers can bring their own API keys and start immediately through IDE extensions like VS Code and JetBrains.

PostHog is used to power telemetry across those environments, giving the team visibility into usage patterns, model adoption, and errors, even for anonymous users. That data is critical for understanding real-world behavior and for working closely with model providers on partnerships and launches.
