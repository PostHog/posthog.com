...
PostHog is an open source, all-in-one platform that combines business intelligence with several other developer-focused tools, such as [product](/product-analytics) and [web analytics](/web-analytics), [session replays](/session-replay), [experimentation](/experiments), [error tracking](/error-tracking), and [surveys](/surveys).

Its business intelligence stack contains three key pieces:

1. A [data warehouse](/data-warehouse) that stores data captured into PostHog as well as syncs with external sources you already use like [Stripe](/docs/data-warehouse/sources/stripe), [Hubspot](/docs/data-warehouse/sources/hubspot), [Postgres](/docs/data-warehouse/sources/postgres), [Google Sheets](/docs/data-warehouse/sources/google-sheets), [S3](/docs/data-warehouse/sources/s3), [Snowflake](/docs/data-warehouse/sources/snowflake), and more.
...
For example, not only can you analyze [A/B tests from warehouse data](/docs/experiments/data-warehouse) in PostHog, but you can actually set up and run them natively using our experimentation and feature flag products. You don't need to set up 4 different products, just PostHog.
