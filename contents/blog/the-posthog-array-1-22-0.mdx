...
<Link to='https://app.posthog.com/signup'><Button className='center' type='primary' size='large'>Try PostHog Cloud Now</Button></Link>
...
From this release (1.22.0) onwards, if your plugin server is not running, this circle will turn amber. You can click on the server to verify if your plugin server is indeed the problem.

**Important:** next release we will move all our event ingestion to the plugin server, meaning that you **will not be able to ingest events** if your plugin server isn't running.
