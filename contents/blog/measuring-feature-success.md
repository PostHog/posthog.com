...
To evaluate success after launch, we did [real user monitoring](/blog/real-user-monitoring) by watching session replays and tracking metrics that matching criteria we care about. For example, we might look at the session recordings of power users who are in our ICP and paying customers. We also might create a dashboard to show usage and the feature’s impact.
