...
- The bill is somehow 4x what you expected. 

None of this shows up in your normal monitoring, because your normal monitoring was built for software that does the same thing every time.

That's the gap **AI observability** fills. It's the layer that watches what your LLMs are actually doing in production – what prompts you sent, what they returned, how long it took, how much it cost, and whether the output was any good.

This guide walks through what AI observability is, the components it's built from, how the tools actually instrument your code, and which ones are worth using depending on your stack and stage.
...
It's a category that didn't really exist until the last couple of years, because the problems it solves didn't exist either. Traditional APM tools like [Datadog](/blog/best-datadog-alternatives) and New Relic were built to monitor software that behaves the same way every time.
...
|---|---|
| Whether your API call succeeded | What the model actually said |
...
Both layers are useful and you need both if you're running AI features in production, but they answer different questions.

The data model is different too. APM is built around requests, services, and traces of synchronous code paths. AI observability is built around *generations* (individual LLM calls), traces (the chain of generations and tool calls that make up an agent loop), and spans (the steps inside a trace).
...
## What are the best AI observability tools available
...
[PostHog's AI observability](/ai-observability) is bundled with [product analytics](/product-analytics), [web analytics](/web-analytics), [session replay](/session-replay), [feature flags](/feature-flags), [experiments](/experiments), [surveys](/surveys), [error tracking](/error-tracking), [logs](/logs), and more – which means the same user model flows through all of them. 
...
**Langfuse** is the open-source darling of the category. It's MIT-licensed, framework-agnostic, with depth across tracing, evaluations, and prompt management. Acquired by ClickHouse in early 2026.
...
**Best for:** Teams who want full control over their data through self-hosting, or who are building anything beyond raw API calls (agents, multi-step workflows, RAG pipelines). Self-host is free; the Core cloud plan starts at $29/mo.
...
[Datadog](/blog/best-datadog-alternatives) added LLM Observability as a product line in 2024, and it's evolved into one of the strongest enterprise option for teams already using Datadog for the rest of their stack.
...
**Best for:** Teams already invested in the Datadog ecosystem who want LLM monitoring in the same pane as their backend services. The catch is pricing: the free tier covers 40K LLM spans/mo, but Datadog automatically activates LLM Observability charges the moment it detects LLM spans, so be careful if you're already on Datadog for other workloads. Hard to justify at the MVP stage.
...
**Best for:** Teams who want cost tracking and basic logs with minimal setup, or who are juggling multiple LLM providers and want a unified routing layer. The trade-off, as with any proxy-based tool, is that you get less detail than SDK-based tracing for complex multi-step agents. Advanced features (some observability, security, governance) are gated to the cloud product. Free tier covers 10K requests/mo; paid plans start at $49/mo.
...
- **Bundled vs unbundled.** Standalone tools are flexible but fragment user context. Bundled platforms ([PostHog](/) being the obvious example) keep everything in one user model, which makes correlation analyses possible.
...
PostHog covers all six core layers (tracing, costs, errors, evals, feedback, prompts) in one tool, on one free tier (100K events/mo), with the same user model flowing through product analytics, session replay, feature flags, logs, surveys, and more. That means:
...
Yes. The setup is fast, and the free tiers will cover you well past a thousand users. [Here's what to set up if you're at the MVP stage](/blog/ai-observability-for-mvp-ai-apps).
