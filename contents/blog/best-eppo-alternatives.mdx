...
-   PostHog uses an integrated data warehouse: you sync or ingest data into PostHog and run queries. This [supports many warehouse-native workflows](/warehouse-native) but, rather than running queries on your existing Snowflake or Postgres, runs them with PostHog compute. PostHog combines product analytics, [pipelines](/docs/cdp), and warehousing in one platform, which eliminates the need for stitching together integrations and searching just for warehouse-native platforms. This makes it easier for engineers to work with data.
