...
2. **Spin up a sandbox per candidate query**, the same sandboxes we use to run [PostHog Desktop](/desktop), our coding agent and product editor (currently in beta).

3. **Run `pi-autoresearch` in each sandbox**, each with its own benchmark target and budget.

4. **Have an LLM dedup the suggestions and spawn a PostHog Desktop session for each surviving idea.** Different sandboxes often land on the same idea, so the LLM collapses those before dispatching. PostHog Desktop writes the actual change against the real codebase, with tests and benchmarks.
