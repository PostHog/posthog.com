...
PostHog combines [product analytics](/product-analytics) and [web analytics](/web-analytics) with other useful tools, such as [session replay](/session-replay), [feature flags](/feature-flags], [A/B testing](/ab-testing), and [surveys](/surveys) into one tightly integrated platform. Everything you need from a single vendor with a single contract for less than buying separately. And you can self-serve, too.

### 2. We build for developers first

This means you get support from the engineers who _actually build the product_. We also have [extensively documented APIs](/docs/api) and a [SQL query builder](/docs/hogql), so you can you can analyze data how you want, and you can query external sources by importing them into [our data warehouse](/docs/data-warehouse).
