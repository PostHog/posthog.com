...
Regardless of CPU contention, we need these processes to wait for each other and only progress in lockstep. Turns out Chrome has a mode for exactly this scenario: [`HeadlessExperimental.beginFrame`](https://chromedevtools.github.io/devtools-protocol/tot/HeadlessExperimental/#method-beginFrame). It stops the compositor from producing frames on its own. Instead, it waits for an explicit `beginFrame` command over CDP, renders exactly one frame, and returns once that frame is complete. In this mode we can ensure that Chrome does not render the next frame until the capture process has taken the previous one and added it to the ffmpeg stream. In case of CPU contention, rendering would simply halt until the capture process is ready for another frame.
