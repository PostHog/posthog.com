...
<!--
DRAFT STATUS: rough draft with the Sep 24 review applied (intro fixed, title, role names unified, typos, 60% story and 170k example in prose, PR count made honest, PR links inline, balance paragraph before the close). Headings are placeholders, rename freely.

Still open before "Ready for review":
- [gap] in the 60% paragraph: the one-line reason the cut made sense.
- Hero image, then add featuredImage + featuredImageType: full to the frontmatter.
- Re-check the linked PR bodies for customer names before publishing (none in the first lines as of Sep 24).
- Unused Slack note: the quota calculator (Stripe + Salesforce + customer analytics into a per-rep comp scenario). Could be a third example in the AI section.
-->

If you open the internet or look at the billboards in any major city right now, you get the impression that AI is about to take over all jobs, sales included. The truth is that this is not really new.
...
I spent most of my career selling HR and finance software, and the data in that space has been clear in recent years that there is a general push towards "Superjobs", powered by new productivity technology.
...
In my last enterprise job, I've built up a whole team in order to figure out how to scale our sales-led approach to more and more customers. It was a massive undertaking, but the reality is it is really hard to pull off once you are organizationally set up to grow by acquiring customers by a contract first, pay first, then implement and use the method.
...
PostHog on the other side decided deliberately to implement a usage-based cost system, which adds complexity upfront, but for sales creates enormous transparency. You only pay for what you use! So that encourages the sales teams to pursue genuine adoption and fight for projects till the end, which again in turn creates a more relevant, long-term situation for everyone.

This year I cut one customer's spend with us by 60% because [gap: the one-line reason, what was over-provisioned or inefficient in their setup] and still hit my quota. In my old world that is not something you do. Here it makes long term sense for both sides, they pay for what they actually use, and the relationship is built on that instead of on a contract someone signed years ago. That full autonomy is very unusual in sales.

## AI accelerates the operational work

On the other hand, AI is a massive accelerator for operational sales work. In the past, I've spent days in Salesforce, building out demo environments, doing feature requests, and more.

Thanks to the Salesforce MCP, I have not logged into the Salesforce UI in over a month. Only to verify the work and for closed-won or closed-lost. My entire opportunity review is now a skill in PostHog Desktop (or Claude) that looks at my notes and then interviews me opp by opp for an update and writes it directly into Salesforce. This has condensed that operational time by 80% (my estimates).
...
- With the services glasses on, I not only advise on the ideal set up but can literally help them through MCP prompts. And I can measure the impact of my deployment consulting by the actual usage numbers going up.
...
One thing that took me a while was figuring out where the line is. I have automated a lot of my sales work with AI, but not the human part. The dinner with a customer, the call where someone tells you what is actually going on in their company, the judgment call on whether to push for the annual now or wait, none of that is automated and I don't think it should be. Pretty much everything around it is. I think I have found a good balance now, ask me again in a year.

So in the Sales Venn Diagram, the PostHog Sales role is in the middle. It is a superjob - but it's not stressful. It is actually less stressful due to the set-up and support than any other sales job I had so far. You are empowered to succeed - and you are working with people who want to ACTUALLY get stuff done - which is rewarding.
