...
The most [natural place to include an AI chatbot](/newsletter/building-ai-features) would be in our [docs](/docs). But I was hesitant to start here, as the conversational nature of AI meant we'd lack control over the quality and accuracy of the responses. So I decided to try something different.
...
1. In the response, we get a `confidence` score.
    ...
    <p><strong className="text-green">If the confidence score is high enough,</strong> we show the answer to the user.</p>
...
Now that this is live, we're monitoring feedback in Slack. PostHog's recently introduced support for [realtime destinations](/changelog?id=2069) in data pipelines.
