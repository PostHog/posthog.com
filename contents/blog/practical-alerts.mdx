...
For a more detailed walkthrough, you can also check out our [Alerts docs](https://posthog.com/docs/alerts), which we'll also update as new alerting features get added.
...
For example, we can get an alert when the number of pageviews for our [pricing page](/pricing) rises above 8,000 views for the week. First, we can [create a new trend insight](/docs/product-analytics/trends/overview#1-creating-a-new-trend) to track pageviews where "Path Name" equals "/pricing". Then, filter by the year-to-date and group by week.
...
With feature flags, you can set up [canary releases](/tutorials/canary-release) to slowly roll out changes and make sure your new features work well before they're released to all your users.
