...
PostHog's [LLM Observability](/docs/llm-observability/start-here) is one way to build that observability layer.
...
For such complex cases, we can use an LLM to act as an evaluator. This is known as an [LLM-as-a-Judge](/docs/llm-observability/evaluations#llm-as-a-judge-evaluations), and with it, we can capture things like:
...
Running your evaluators on all production traces sounds expensive, because it is, especially if you depend on LLM-as-a-Judge evaluators. This is why it's important to use code-based evaluators where possible. At PostHog, code-based evaluators for online evaluations use [Hog](/docs/llm-observability/evaluations#code-based-evaluations-hog). You can't, however, only depend on code-based evaluators, so you can also:
...
2. Look for error spikes and the [traces that have those errors](/docs/llm-observability/errors)
3. Look at traces that have been flagged as problematic via a support ticket
4. If you've shipped a new feature, look for traces that are using that feature, for example by looking at the [traces that have called a specific tool](/docs/llm-observability/tools) belonging to that feature
5. If your LLM observability platform supports it, look at traces that have a [negative sentiment](/docs/llm-observability/sentiment) associated with them
6. Also if supported, look at outlier traces or small clusters from the [clusters generated for your traces](/docs/llm-observability/clusters)
...
PostHog also has [Reviews and Review Queues](/docs/llm-observability/trace-reviews), which help you build these lists of traces and review them one by one.
