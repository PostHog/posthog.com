...
First, you [observe real behavior](/newsletter/stop-being-the-code-review-bottleneck#4-verify-by-observation-not-reasoning) with tracing. Then you turn the failures you see into offline evals you can run during development. Next, you run online evals on sampled production traffic so you can catch the problems your curated datasets miss. Finally, you keep reviewing real traces, because manual review is where the next generation of evaluators comes from.
