...
<ProductComparisonTable
    ...
    rows={[
        ...
        'surveys.branching.multi_step_surveys',
        {
            ...
            values: ['Beta', true, true, true, false, false],
        },
        {
        ...
        },
        {
            ...
            values: ['1,500 responses/mo', '20 responses/mo', '1 survey/mo', '25 responses/mo', 'NPS only', false],
        },
...
PostHog (that's us, hi 👋) is an all-in-one product engineering platform that includes surveys alongside [product analytics](/product-analytics), [session replay](/session-replay), [feature flags](/feature-flags), [experiments](/experiments), [error tracking](/error-tracking), and much, much more.

This means you can do more than just collect feedback. When a user submits an NPS response, you can pull up their session replay to see exactly what experience preceded it, join the response to their event history, or use a feature flag to target them with a follow-up experiment. Everything lives in one place, and all the products are designed to play nice together.

Surveys in PostHog support web and mobile apps, can be triggered by events or feature flags, and include templates for NPS, PMF, CSAT, and more. The free tier includes 1,500 responses per month with no credit card required.

PostHog is also investing heavily in AI features. Surveys have built-in AI summaries and a chat interface for deeper analysis — both currently in beta.

**Strengths:**

- Surveys linked to session replay, analytics, and feature flags
- Event-triggered and feature flag-targeted surveys
- Web and mobile support (iOS, Android, React Native, Flutter)
- AI analysis: summarize responses and dig deeper via chat (beta)
- 1,500 free responses/month with usage-based pricing
- Open source and self-hostable
- Unified suite: analytics, experiments, error tracking, surveys, and more
...
Hotjar is one of the most widely deployed tools for understanding user behavior on websites. Its surveys sit inside a broader toolkit that includes heatmaps, session recordings, and user interviews — making it easy to triangulate between what users say and what they do.
...
The main limitation is that Hotjar is web-only. There's no mobile SDK for native apps. See our [PostHog vs Hotjar](/blog/posthog-vs-hotjar) comparison for a deeper breakdown.
...
Product, UX, and marketing teams focused on web apps _only_ who want qualitative feedback paired with visual behavior insights.
...
Sprig supports web and most mobile platforms (iOS, Android, but not Flutter). There's no meaningful free tier beyond one active survey per month, and pricing is usage-based without public information on rates.
...
The free tier allows 25 responses per month, with paid plans starting at $79/month.
...
Pendo is a product experience platform that combines in-app surveys with analytics, session replay, in-app guides, and product roadmaps. It's primarily aimed at product managers and customer success teams at mid-to-large companies.

Surveys in Pendo are targeted by user segment or behavior, support web and mobile, and connect to its broader analytics model. The free plan includes Pendo-branded NPS surveys for up to 500 monthly active users, but full survey functionality requires a paid plan.

Pendo's pricing is very opaque and quite hefty. Paid plans start at roughly $15,900/year, which puts it out of reach for most smaller teams and startups. See our [PostHog vs Pendo](/blog/posthog-vs-pendo) comparison for more detail.
...
- Want surveys integrated with analytics, replay, feature flags, and AI analysis? **[PostHog](/surveys)**
- Need web surveys alongside heatmaps and session replay? **Hotjar**
...
- **PostHog** for a generous free tier, transparent pricing, and an all-in-one platform that replaces multiple tools
...
**PostHog**, **Survicate**, **Pendo**, and **SurveyMonkey** all offer native mobile SDKs. **Sprig** supports iOS and Android but not Flutter. **Hotjar** is web-only and has no mobile SDK.
...
- **Sprig**: 1 active survey/month free
