...
![Andrej Karpathy's tweet: "The hottest new programming language is English"](https://res.cloudinary.com/dmukukwp6/image/upload/karpathy_tweet_8e8a64e5ed.png)
...
This is the deep dive. If you just want the short version, the [docs](/docs/start-here) walk the whole [self-improving loop](/docs/start-here/self-improving-loop) – scouts, signals, reports, and the inbox – at a glance. And if you just want to see scouts in action, skip to [two scouts, end to end](#two-scouts-end-to-end) – there's an ~8 minute video walkthrough of a real scout run there too.
...
| Tools for the agent | Mostly just the [PostHog MCP](https://1c173e83.posthog-preview.pages.dev/docs/model-context-protocol) — the same one you can connect to Claude Code or Cursor today. The aim is to build as little custom code as possible; the agent reads your data through the exact tools anyone else gets. |
