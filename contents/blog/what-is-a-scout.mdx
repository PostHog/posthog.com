...
A scout is a small agent (don't overthink that word) that looks at, wrangles, and makes sense of any data available in PostHog. It's a core component of [Self-driving](/docs/self-driving) in PostHog (now in open beta), [an autonomous loop](/newsletter/agent-autonomy#level-3-self-driving-mode) that monitors your product for patterns and drafts fixes. 
...
This is the deep dive. If you just want the short version, the [Self-driving docs](/docs/self-driving) walk the whole [self-improving loop](/docs/self-driving/self-improving-loop) – scouts, signals, reports, and the inbox – at a glance. 
...
2. **If it is, the scout emits a [signal](/docs/self-driving/signals)** – a semi-structured finding with evidence, and a suggested action.
3. **Signals get picked up by the [self-driving pipeline](/docs/self-driving/self-improving-loop)**, grouped, and turned into **[reports](/docs/self-driving/reports)** that land in your PostHog [Inbox](/docs/self-driving/inbox).
4. **Reports get actioned** – by you, or automatically by a [self-driving](/docs/self-driving) task that picks up the report and ships a fix or improvement.
...
Or even easier just press one of the suggested questions here on the scouts page in PostHog Desktop. "Make a scout" will use the [authoring-signals-scouts](https://github.com/PostHog/posthog/blob/master/products/signals/skills/authoring-signals-scouts/SKILL.md) skill, scan your project and suggest some custom scouts for you.
...
Because a scout is just a skill plus the MCP, you can run one manually in any agent surface where the PostHog MCP is available – PostHog Desktop, Claude Code, Codex, Cursor, whatever. 
...
A [self-driving](/docs/self-driving) agent picked up the report and opened the PR on its own – introducing a writable `SyncFrequencyField` so the cadence flows into the generated PATCH body, and adding three regression tests (a writable-field guard, a round-trip update, and an invalid-value rejection).
