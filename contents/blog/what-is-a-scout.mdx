...
A scout is, at its core, **a skill** — and that skill becomes the main interface for everyone who touches or uses it, human or agent.

Core business logic that used to be a sprawl of `if/else` branches and helper functions in an old-school data pipeline starts life as a few hundred lines of English that anyone can read, write, and improve (and scales up to bundled reference docs and scripts with progressive disclosure when a scout needs them).

Code moves into language and instruction (obligatory [Karpathy tweet](https://x.com/karpathy/status/1617979122625712128)), and the skill becomes the place where the logic actually lives and breathes.

Of course you can't get away with this for all data pipelines (we still need determinism for a lot of crucial stuff), but for those where you have the flexibility the trade-offs are increasingly worth it.
...
In a traditional data pipeline, the logic that decides "is this interesting?" lives in code: thresholds, `if/else` branches, helper functions, a scheduler, a pile of glue. Changing what you care about means a code change, a review, a deploy. This is of course still needed and not going away, but for more subjective and flexible pipelines, the agent-pilled approach can offer many benefits.
...
`signals-scout-mcp-feedback` (you can see [it's a fairly simple skill](https://gist.github.com/andrewm4894/4aef57ab4b31fe78b889d1a2f94fa490)) watches a single event: `mcp feedback submitted`. When an AI agent (Claude Code, Cursor, etc.) hits friction using our MCP — a confusing schema, a tool that returns the wrong thing, a missing capability — it can file a structured report via the MCP's own `agent-feedback` [tool](https://github.com/PostHog/posthog/blob/master/services/mcp/src/tools/feedback/submit.ts). 
...
**Us, roasted, repeatedly.** The scout also keeps a running *"PostHog is too complex / bloated"* theme — and the genuinely useful part is watching it *decide what's worth your attention*. Most of these never clear the bar (one voice, low reach, a matter of taste), so the scout logs each with a deadpan verdict and a *"promote if a second voice echoes"* trigger, and stays quiet. Pop these open for the actual public roasts — and the scout's clinical reasoning for holding them:
...
The fastest way to get hands-on is [PostHog Code](/code), our desktop coding agent — download it, sign in, and the PostHog MCP is wired up for you. From there you can navigate to the Agents section and explore the set of canonical scouts PostHog ships with as well as make your own custom scouts.
