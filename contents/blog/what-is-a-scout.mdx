...
4. **Reports get actioned** – by you, or automatically by a PostHog Desktop task that picks up the report and ships a fix or improvement.
...
A PostHog Desktop agent picked up the report and opened the PR on its own – introducing a writable `SyncFrequencyField` so the cadence flows into the generated PATCH body, and adding three regression tests (a writable-field guard, a round-trip update, and an invalid-value rejection).
