...
This allows the dog to take *itself* for a walk. For each stage of an integration, the agent is prompted with a set of tasks. Once those tasks are complete, the agent is told to load the next piece of the prompt.
...
Now the problem is designing just enough context that the Wizard can serve way more languages and frameworks. So while [we crack away at that](https://posthog.com/teams/wizard-and-docs#goals-for-q1-2026), steal what’s interesting:
