...
[PostHog](/) is an all-in-one developer platform that combines [LLM observability](/llm-observability) with several other developer-focused tools, such as [product](/product-analytics) and [web analytics](/web-analytics), [session replay](/session-replay), [feature flags](/feature-flags), [experiments](/experiments), [error tracking](/error-tracking), and [surveys](/surveys).

Its LLM observability product (known as [LLM Observability](/docs/llm-observability)) integrates with popular LLM providers, captures details of generations, provides an aggregated metrics dashboard, and more.
...
- **[Prompt management](/docs/llm-observability/prompt-management) (beta):** Create and version prompts directly in PostHog. Prompts are fetched at runtime via the SDK with caching and fallback support, so you can update them without code deploys. Non-engineers can iterate on prompts from the UI, and every change creates an immutable version you can compare, restore, or link to traces to see which prompt versions drive which outputs.

- **[Evaluations](/docs/llm-observability/evals) (beta):** Score LLM outputs automatically or with human review to track quality over time – not just whether API calls succeed, but [whether they're actually good](/blog/stop-ai-slop).
...
[Langfuse](/docs/llm-observability/integrations/langfuse-posthog) (recently acquired by ClickHouse) is an open source LLM engineering platform. It provides LLM call tracking and tracing, prompt management, evaluation, datasets, and more. These give LLM app developers tools they need for their entire workflow.
...
It's completely free to get started – no credit card required. Our [setup wizard](/wizard) handles configuration in minutes, or you can check out [our docs](/docs/llm-observability) to do it yourself.
...
No. PostHog's [LLM observability](/docs/llm-observability) product is built into the platform, so if you're already using PostHog for product analytics or session replay, you can add LLM observability without any additional setup or contract. You get 100k LLM events free per month.

[Getting started](docs/llm-observability/start-here) is easy; once you install the SDK, it will handle all the heavy lifting. Use your LLM provider as normal and we'll capture everything automatically.
...
- [PostHog LLM observability documentation](/docs/llm-observability)
