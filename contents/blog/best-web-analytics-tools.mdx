...
PostHog is an all-in-one developer platform that combines [web analytics](/web-analytics) with [product analytics](/product-analytics), [session replay](/session-replay), [feature flags](/feature-flags), [experiments](/experiments), [error tracking](/error-tracking), [surveys](/surveys), and [LLM observability](/llm-analytics) in one place.
...
You can also run [A/B tests](/experiments) using web vitals as goals, track ad spend and ROAS with [marketing analytics](/docs/web-analytics/marketing-analytics), and [embed analytics](/blog/customer-facing-analytics-examples) into your own product using [endpoints](/endpoints).
