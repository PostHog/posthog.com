...
More examples of this flexibility include PostHog's simpler [custom event capture](/docs/getting-started/send-events#2-capture-custom-events), the ability to [import and use data from external sources](/context-warehouse), and [direct SQL querying](/docs/sql).
...
Error and performance monitoring is the main focus of Sentry. PostHog has a full error tracking product as well as [network performance recording](/docs/session-replay/network-recording), [distributed tracing](/tracing), and [metrics](/docs/metrics) (in alpha), though Sentry's tracing and monitoring features are more mature.
...
Many tools like **Sentry** offer both. **PostHog** has both too – error tracking next to [distributed tracing](/tracing) and [metrics](/docs/metrics) (alpha) – and adds the product analytics context that shows you who the slowdown affected.
