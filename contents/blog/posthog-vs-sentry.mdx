...
Nicely, PostHog and Sentry also have a [two-way integration](/docs/libraries/sentry) that adds a direct link in Sentry to the person in PostHog and captures an `$exception` event in PostHog with a direct link to Sentry. 
