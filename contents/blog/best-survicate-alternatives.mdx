...
PostHog and Survicate have similar capabilities when it comes to surveys but differ on other features. PostHog includes entire features Survicate doesn't have, like product analytics, A/B testing, and session replays. 

On the other hand, Survicate supports sending surveys on more platforms, such as mobile app, emails, and shareable-links. PostHog doesn't yet offer surveys on mobile – although all of its other features are available on apps.
...
3. **It has pre-built reporting tool:** Reviewers appreciate that they can easily interpret results and gain actionable insights from the collected data.
...
> #### Bottom line
> 
> Hotjar is a good option for product and marketing teams who want to do in-depth analysis on user journeys within web apps. However, it's best to look elsewhere if you need to support on other channels.
...
When it comes to surveys, Lucky Orange does many of the same things that Survicate. However, it's designed specifically for optimizing conversion on e-commerce websites, so doesn't support any of the other platforms that Survicate does. 
