...
But these tools don't actual remove dev work, they defer it. No-code tools inject changes on top of the rendered page. When an experiment wins, someone still has to rebuild the winning variant. The total dev effort is the same, it just happens after the experiment instead of before it.
...
## How the daily workflow changes

With no-code tools, growth designs the experiment visually, runs it, finds a winner, and then creates a ticket for engineering to rebuild it in real code. 

With AI code generation, growth writes an experiment brief, AI generates a PR, engineering reviews it, and the winner ships instantly. The work happens upfront instead of after the experiment, and it leads to a faster, more iterative process:
...
You don't have to write it alone. [PostHog AI](/ai) can help. It knows how to set up experiments and how users are using your product. You can use it to hone your brief before you pass it to your agent to start generating code.

You'll want to include these four things in your final prompt: 
...
You don't need to get everything right in one message. The agent will look at your code, ask about edge cases, and suggest things you might have missed. After a few back-and-forth messages, it generates the code change, the experiment in PostHog, and all the configuration. Review what it produced, then open the PR.

![Claude and PostHog experiments](https://res.cloudinary.com/dmukukwp6/image/upload/w_1600,c_limit,q_auto,f_auto/vibeexperiment_0f464de9d4.png)

## Previewing before you launch

Many rely on the visual preview no-code experiments provide and might be worried about losing them, but again thanks to agents, this is solvable. Don't underestimate the power of LLMs when it comes to debugging and [helping "non-technical" people understand what's going on](/newsletter/engineeringification-of-everything).

Experiments in PostHog deploy behind a disabled feature flag. No users see it. From there, three ways to preview:
...
2. **PostHog toolbar.** Edit the feature flag to release to 0% of users, ship the code for your experiment, open your production site, launch the [toolbar](/docs/toolbar), and force-toggle the experiment flag to the test variant. You see it live with real data, only in your browser. Nobody else is affected.
...
- [Validating what you ship: Did anyone use it? Did it work?](/tutorials/validating-what-you-ship)
