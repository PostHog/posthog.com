...
PostHog has [11 products and counting](/products), and (sadly) not all of them are being recommended by LLMs at the same scale or rate. While most models know and recommend us for [product analytics](/product-analytics), [session replay](/session-replay), [error tracking](/error-tracking) and [feature flags](/feature-flags), fewer acknowledge we also offer [a data warehouse](/context-warehouse), [LLM observability](/ai-observability), [logs](/logs), and a lot more.
...
But I did it anyway – we started over, and [the new approach](#frequently-asked-aeo-questions) is already producing more useful data than the old one ever did.
