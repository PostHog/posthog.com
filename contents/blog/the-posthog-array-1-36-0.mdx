...
-   For ~~plugin~~ app developers: `onEvent`, `onShapshot` and `onAction` event payloads have been cleaned up a bit. Specifically, `timestamp`, `$set`, and `$set_once` are always provided now, while `now`, `sent_at`, `site_url`, `offset`, as well as `kafka_offset` no longer are included. All apps available in our [App Store](https://posthog.com/apps) are compatible with the change.
