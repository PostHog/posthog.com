...
seo:
    {
        metaTitle: 'Workflows is now GA. Congrats, your product data just got a job',
...
We introduced [Workflows in alpha](blog/workflows-alpha) asking: could we let teams define actions directly on top of the same events and user data they already trust for analysis? [During the beta](blog/workflows-beta), we saw it used to automate onboarding, coordinate feature rollouts, and trigger messages based on real product behavior. 

With that feedback and usage in place, **[Workflows](https://app.posthog.com/workflows)** is now generally available. 

## What does Workflows do? 

**[Workflows](https://app.posthog.com/workflows)** lets you automate product-led actions directly from your PostHog data. When something happens in your product, you can respond immediately by sending an email, updating a user property, or triggering a Slack alert, without syncing tools, duct-taped integrations, or API key scavenger hunts.

Everything runs on the same event data you already track. Whether you’re sending a welcome email after onboarding or adding branching logic based on an upgrade path, it all happens inside PostHog, without shipping data off to third-party platforms.
...
Most automation tools start by asking you to import data from your analytics stack. That usually means connectors to maintain, delayed syncs, and logic scattered across multiple systems. Workflows skips that entire step by running directly inside PostHog.

Because your product data already lives there, Workflows can react to it in real time. You can trigger automation from:

- Product events and user actions
- User properties and cohort membership
...
- Update user properties for future targeting
