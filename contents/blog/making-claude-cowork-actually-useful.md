...
These are all lovely ideas. They are also, almost entirely, a list of low-leverage work that I already don't need help with or don’t want to do even better. 
...
* **Weekly Blitzscale ownership refresh.** Re-reads the handbook to work out which exec is currently responsible for which teams. This is what powers the "group by owner" bit of the coherence check \- this matters because we change stuff around a bunch at PostHog.   
...
## **6\. Things can only get betterrr**
...
Here's the rough shape of my `~/Documents/Claude/` folder. The agent reads all of this at the start of every session.

```
~/Documents/Claude/
├── CLAUDE.md                           # root index
│
├── Memory/                             # context the agent reads every session
│   ├── CLAUDE.md                       # memory index — people, projects, preferences
│   ├── glossary.md                     # acronyms, nicknames, tools, Slack channels
│   ├── people/                         # one file per person I work closely with
│   │   ├── james-hawkins.md
│   │   ├── tim-glaser.md
│   │   ├── simon-fisher.md
│   │   └── ... (16 files total)
│   └── context/
│       ├── company.md                  # what PostHog is, teams, current quarterly priorities
│       ├── company-details.md          # legal/finance reference (synced from a Google Sheet)
│       ├── exec-responsibilities.md    # which exec owns which Small Teams (auto-refreshed)
│       └── handbook/                   # local cache of PostHog's public handbook
│           ├── INDEX.md
│           ├── pages/                  # 14 root pages cached locally
│           └── sections/               # per-section paths for on-demand fetch
│
├── Scheduled/                          # recurring background tasks (one SKILL.md per folder)
│   ├── coherence-check/                # Mon — flag contradictions across team plans
│   ├── push-harder-monday-ritual/      # Mon — growth bets against our defaults
│   ├── weekly-1-1-prep/                # Mon — discussion topics per direct report
│   ├── company-internal-weekly-digest/ # Mon — GitHub threads I might have missed
│   ├── promise-tracker/                # Wed — what other people owe me
│   ├── weekly-achievements-report/     # Fri — where the week actually went
│   ├── monthly-memory-refresh/         # 8th — read "Things I care about" and update CLAUDE.md
│   ├── sync-company-details-memory/    # Mon — refresh legal/finance reference
│   ├── sync-handbook-root-pages/       # weekly — refresh handbook cache
│   ├── refresh-blitzscale-ownership/   # Mon — who owns what in the exec team
│   ├── weekly-downloads-organizer/     # Sun — tidy ~/Downloads
│   └── ... (15 folders total)
│
├── Skills/                             # on-demand skills I invoke ad-hoc
│   ├── charles-writing-style/          # write in my voice
│   ├── charles-blog-writing/           # long-form blog post structure
│   ├── dissent-farmer/                 # pressure-test controversial announcements
│   ├── sf-pipeline-review/             # Salesforce pipeline health check
│   ├── account-retro/                  # customer account post-mortem
│   ├── slack-engagement-audit/         # who's quiet in which channels
│   └── workload-analysis/              # PostHog workload classification
│
└── Projects/                           # folders the agent writes into
    ├── Writing/                        # blog drafts (this piece lives here)
    ├── Briefings/                      # weekly achievements + briefing rubric
    ├── Blitzscale/                     # exec growth work / push-harder outputs
    └── Sales/                          # sales skill bundle
```
...
I also use it for customer lookups. I route "who's the TAM for \[customer\]?" through the PostHog MCP because we keep Vitally synced into our warehouse. Every customer question Cowork answers starts with an `execute-sql` call against that data.
