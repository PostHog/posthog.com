...
You can also score outputs with [LLM-as-a-judge or code-based evals](/docs/ai-evals), [manage and version prompts](/docs/prompt-management) without redeploying code, and query trace data with SQL or through the [MCP server](/mcp) directly from your editor.
...
In August 2026, Dynatrace announced an agreement to acquire Arize, so Phoenix's roadmap will now be set by an enterprise observability vendor rather than an independent startup.
...
> #### Bottom line
>
> Arize Phoenix is a good fit for teams that want an open-source observability platform with strong evaluation and RAG capabilities. It doesn't act as a gateway like Helicone, so it's best suited to teams that care more about tracing and evaluation than request routing. Note that Arize is being acquired by Dynatrace.
