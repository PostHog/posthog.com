...
If you're a Helicone user, you've probably seen the news: [Mintlify acquired it](https://www.helicone.ai/blog/joining-mintlify) in March 2026, and the platform is now in maintenance mode. It'll keep running, getting security updates and receiving new model support, but the roadmap is frozen.

That's not an emergency – nothing is breaking tomorrow. But "no new features, ever" is a significant limitation in a space where the tools reinvent themselves every six months. 

Agent tracing, evals, OTel support: whatever the next must-have is, Helicone won't have it.
...
That means a bit more setup, no gateway features like caching or rate limiting, but also no proxy between you and your LLM provider, and much deeper context on the other side. Because traces are stored as regular PostHog events, you can connect AI behavior to what users actually did: whether people who hit a slow generation churned, or whether a new prompt version improved activation.
