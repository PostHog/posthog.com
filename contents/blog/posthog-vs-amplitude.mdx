...
Amplitude and PostHog are superficially similar. 

They both offer product analytics and a range of additional tools, like feature flags, A/B testing, and session replay. 

Deep down, however, Amplitude is an analytics tool with some extra bits grafted on. 

In contrast, PostHog is built from the ground up as an all-in-one platform for building successful products. Here's how they compare.
...
- It means they do support – all product teams have a [weekly support hero](/handbook/engineering/support-hero) dedicated to solving user issues. 
...
- It means you can sign up and start using PostHog without ever jumping on a quick call – see [how we do sales](/sales).
...
### 3. We're cheaper and offer for free

We're committed to [sustainably cheap pricing](/blog/analytics-pricing) on all our products. Every user get a [generous free allowance](#free-tiers) every month. Inputting a card unlocks almost every feature, but you'll pay nothing if you stay within the free tiers. Customers who switch to PostHog from Amplitude, and other tools, end up saving money. See the [price comparison section](#price-comparison) for more.
...
- Feature flags make it easy to roll out features to specific users or groups, and [safely test in production](/product-engineers/testing-in-production). 
...
- Surveys are great for tracking customer satisfaction scores like Net Promoter Score (NPS), customer satisfaction (CSAT), and customer effort score (CES).
...
- Amplitude offers a free Starter plan, but it has many arbitrary limitations and a low limit on usage.
...
Anonymous events are the default event type. You'll only be charged the identified event rate when you call `posthog.identify`, or to capture an event from an identified user. 
...
Use the [calculator on our pricing page](/pricing) to estimate how much you'll pay.
