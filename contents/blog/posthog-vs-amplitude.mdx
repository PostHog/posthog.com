...
PostHog autocaptures data, so you don't need to waste energy instrumenting basic events. You can choose between light and dark mode themes. Features like our custom SQL query builder, keyboard shorcuts, and command pallette, cater to power users. You can setup PostHog and start paying us without ever speaking to a sales rep, though our [customer success team](/contact-sales) is always available to answer your questions.
...
1. PostHog supports autocapture, which makes it much fast to implement as you don't to manually instrument every event. Don't want autocapture? Just [turn it off](/docs/product-analytics/autocapture) – we offer the best of both worlds.

2. PostHog has a [SQL-based query builder](/docs/hogql) for power users, which enables you to generate custom data analyses using PostHog event data. In future, our query builder will let you analyze data as you would in a Business Intelligence tool, like Metabase.
...
PostHog's testing features are free when you input a card and you get a generous 1 million API requests for free _every_ month. Amplitude's A/B testing isn't available on any self-serve plan and the price isn't disclosed. You must contact its sales team to use it.
...
Replays enable you watch how users experience your app, diagnose issues, improve support, and understand real user behavior in a way raw data can't. They are _not_ video recordings of users. We reconstuct sessions by capturing Document Object Model (DOM) mutations. All private and personal data is masked. 
...
<ComparisonTable column1="PostHog" column2="Amplitude">
  <ComparisonRow column1="250" column2={false} feature="Free usage" description="How much free usage do you get each month?" />
...
We charge progressively less per event, replay, API request the more you use – e.g. we charge $0.000100 per event up to 2 million events, but all events after 10 million a month only cost 
