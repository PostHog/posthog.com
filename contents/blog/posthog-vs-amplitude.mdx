...
At each tier, PostHog is around 22% to 33% cheaper than Amplitude. Sending more than 75 identified events per MTU will increase the cost of PostHog relative to Amplitude, and sending fewer events will reduce the cost. 

The benefit of charging per-event is that you can control how many events you send per user by only sending important events, whereas there is less desirable control for tracking fewer _users_. We provide [numerous ways](/tutorials/fewer-unwanted-events) to control how many events you send and reduce your costs, including the option to send cheaper **anonymous events**.
...
Anonymous events are the default event type. You'll only be charged the identified event rate when you call identifying functions like `posthog.identify()` or `posthog.group()`, or capture an event from an already-identified user. 
