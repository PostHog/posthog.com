...
The main exception is warehouse-native analytics: Amplitude can run queries directly on your data warehouse (Snowflake, BigQuery), while we mostly sync data into PostHog (either via [our CDP](/cdp) or by ingesting into our integrated data warehouse) and run queries on our compute. Find out more about ([how we do warehouse-native](/warehouse-native) in the docs).
