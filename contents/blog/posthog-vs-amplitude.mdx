...
**For data teams**
- **Amplitude** – Warehouse-native architecture lets you query directly on Snowflake, BigQuery, or Databricks without moving data. PostHog supports [warehouse-native workflows with its integrated warehouse](/warehouse-native) and can sync to and from external warehouses while running queries on PostHog compute, not on your external warehouse (Snowflake, Databricks, etc).
