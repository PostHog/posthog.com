...
**For data teams**
- **Amplitude** – Warehouse-native architecture lets you query directly on Snowflake, BigQuery, or Databricks without moving data. PostHog offers an integrated data warehouse that enables warehouse-native workflows across other PostHog tools, such as analytics, feature flags, and surveys. Alternatively, you can ingest data into PostHog from an external warehouse, such as Snowflake, and run queries on PostHog compute.
...
**Amplitude** if you need queries to run directly on your Snowflake/BigQuery. **PostHog** supports [warehouse-native workflows](/warehouse-native) with an integrated data warehouse, but queries currently run on PostHog compute.
