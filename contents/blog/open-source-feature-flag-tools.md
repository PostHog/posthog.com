...
Feature flags (or toggles) turn specific functionalities on and off during runtime. Rather than maintaining multiple feature branches in your source code, feature flags hide code or behaviors and allow you to enable or disable features on demand and without deploying new code.
...
- **Controlling access** to certain features on user criteria, such as location, device type or subscription level.
...
Feature flags are a core feature of all editions, including the open source release. While multivariate feature flags are limited to paid plans, most of PostHog's paid plans are [free up to 1 million events every month](/pricing).
