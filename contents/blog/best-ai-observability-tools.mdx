...
- **An aggregated dashboard:** Spot trends over time, including throughput, instead of reading [logs](/blog/best-log-monitoring-tools) one by one.
...
- Query traces and ship fixes [from Slack](/slack), your editor ([via MCP](/mcp) or [CLI](/docs/cli)), [PostHog Code](/desktop), or [PostHog AI](/products) in the app.
...
One thing to factor in: Dynatrace announced an agreement to acquire Arize in August 2026. Phoenix stays open source and self-hostable, but the roadmap and pricing for AX will now be shaped by a large enterprise observability vendor rather than an independent startup.
