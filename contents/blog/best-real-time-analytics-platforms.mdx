...
And when the spike dies down, the same live data is [queryable natively with SQL](/docs/sql).
...
GA4 is, for many teams, [still the default](/blog/ga4-alternatives), and its Realtime report is probably the most widely-used live dashboard on the web. 
...
GA4 also [requires a cookie consent banner in most jurisdictions](/blog/cookieless-analytics), and in several European countries its legality has been challenged. A meaningful chunk of traffic is simply not counted because users decline consent.
...
- Cookie-free and [GDPR-compliant by default](/blog/best-gdpr-compliant-analytics-tools)
