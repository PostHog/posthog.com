...
Due to the complex nature of the w eb, there are some limitations to what we can capture. Below are some of the most common ones.

* **Canvas** - Recording of canvas elements is technically possible but due to browser limitations we cannot currently support playback without introducing security issues. 
* **WebGL**
* **Iframes** 
  ...
  - iframes from a _different origin_ (e.g. `posthog.com` -> `app.posthog.com`)
