...
PostHog is an incredibly broad platform, with many distinct features and tools. As a result, it's hugely powerful -- but can be overwhelming if you're new to product analytics. So, we've created a glossary to explain many of the most common terms and acronyms. If a term is specific to PostHog then it's marked with a <img src={InternalTerm} alt="Internal Term" style={{ width: 25 }} /> to help reduce confusion. 
