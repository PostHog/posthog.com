...
Our basic app is now set up. Run `npm start` to see it in app action.
...
Update your `fetchResponse()` function in `App.js` to capture this event:
...
Note that it may show `0` if the total cost is less than `0.01`.
...
Note that it may show `0` if the number is less than `0.01`.
