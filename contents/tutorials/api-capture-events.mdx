...
## Authenticating with the project token

The first thing needed, like the [basic GET request tutorial](/tutorials/api-get-insights-persons), is to authenticate ourselves in the API. Unlike in the GET request tutorial, we can use the project token (the same key you use to initialize a PostHog library). This can be found in your project settings. 

The project token is a write-only key, which works perfectly for the POST-only endpoints we want to access.

## Basic event capture request

To capture events, all we need is the project token, the data we want to send, and a way to send a request. To capture a new event, you need to send a `POST` request to `<ph_client_api_host>/i/v0/e/` (or the `/i/v0/e` endpoint for your instance) with the project token, event name, and distinct ID.
...
You can also `POST` `$identify` events to add more details about those users. The API has no concept of state so the user information is not added as properties unless you send it in a request. It is not automatically created or included in the request like it is in the [JavaScript](/docs/libraries/js) library.
...
## Capturing AI Observability events

It's also possible to capture AI Observability events using the API. If you're using a language without [SDK support for AI Observability](/docs/ai-observability/installation), you can use the API to capture events.

To capture AI Observability, you need to capture 4 types of events:

| Event type   | Event name         | Documentation                          |
|--------------|-------------------|---------------------------------------------|
| Generations  | `$ai_generation`  | [What are generations?](/docs/ai-observability/generations) |
| Spans        | `$ai_span`        | [What are spans?](/docs/ai-observability/spans)             |
| Traces       | `$ai_trace`       | [What are traces?](/docs/ai-observability/traces)            |
| Embeddings   | `$ai_embedding`   | [What are embeddings?](/docs/ai-observability/embeddings)    |

You can find more information in our [manual capture guide](/docs/ai-observability/installation/manual-capture).
...
- [Manual AI Observability capture](/docs/ai-observability/installation/manual-capture)
