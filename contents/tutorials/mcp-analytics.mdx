...
The first step is to define an interface for an analytics provider. It defines a standard set of methods for sending analytics data from your MCP server. 
...
The next step is to create a wrapper that intercepts every tool call. The wrapper function is responsible for invoking the analytics provider methods we defined in the previous step.
