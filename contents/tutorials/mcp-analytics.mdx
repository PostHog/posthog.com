...
This tutorial will walk you through how product analytics and error tracking can be addedto any MCP server using a simple wrapper pattern. This implementation tracks every tool execution without touching core business logic. 
...
First, let's clone the repository of our MCP server.
...
Now that our MCP server is compiling successfully, let's take a look our MCP tools in the `tools.ts` file. 
