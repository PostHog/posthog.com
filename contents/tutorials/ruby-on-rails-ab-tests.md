...
To implement the A/B test, we fetch the `my-cool-experiment` flag using [`posthog.onFeatureFlags`](/docs/libraries/js/usage#ensuring-flags-are-loaded-before-usage) in a `DOMContentLoaded` event listener. Then, we update the button text based on whether the user is in the `control` or `test` variant of the experiment:
...
Next, we create an initializer for our PostHog client. Create a new file in `config/initializers` called `posthog.rb`. Then, initialize PostHog in this file using your project token and host:
