...
The challenge with time on page is measuring activity. Because of the [event-driven structure of PostHog](/docs/how-posthog-works/data-model), we can’t be 100% sure a session is active at any point. Instead, we can estimate time spent by getting the time between a first pageview event and a subsequent pageview or pageleave event, filtering out extreme values, and averaging these together.
