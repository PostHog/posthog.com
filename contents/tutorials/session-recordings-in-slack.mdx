...
This will enables us to send an event to PostHog from a user's browser when we detect a recording that we want to act on and have that converted to a message to a Slack channel.
...
Finally, you can change the message template. Here we add a button to jump to the person or straight to the recording:
