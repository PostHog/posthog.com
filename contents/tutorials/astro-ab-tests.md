...
To implement the A/B test, we use the [`posthog.onFeatureFlags`](/docs/libraries/js/usage#ensuring-flags-are-loaded-before-usage) callback to update the button text based on whether the user is in the `control` or `test` variant of the experiment.
