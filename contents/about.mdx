...
This includes agents and tools for building products, talking to customers, and making sense of all your customer data.
...
-   **Actually technical support.** We don't offshore our support. In fact, the entire <SmallTeam slug="support" /> has engineering backgrounds!
...
-   Our <Link to="/handbook/growth/sales/overview" state={{ newWindow: true}}>manual for our salespeople</Link>, <Link to="/handbook/marketing" state={{ newWindow: true}}>marketing team</Link>, and so on
...
PostHog is priced to match the cheapest major competitor, so you know you don't have to compare pricing for each new product.
...
<div className="flex justify-center @lg:float-right lg:-mr-4">
    <div className="@lg:ml-4 rotate-1 inline-block mx-auto mb-2 @lg:mt-2">
        <CloudinaryImage
            ...
            width={782}
            height={521}
            className="border border-primary shadow-xl rounded"
...
We have over _<CustomerCount /> teams_ using our products. Just under a quarter of a million engineers use us. That's far, far more than any of our competitors – most have around 1-3k customers. It's what happens when the internet loves you.
...
<div class="absolute right-2 top-4 @md:-top-8 rotate-3 inline-block">
<Link to="/merch?product=data-warehouse-t-shirt" state={{ newWindow: true }} className="block border border-primary shadow-xl rounded bg-primary p-1">
<img src="https://res.cloudinary.com/dmukukwp6/image/upload/h_500,c_limit,q_auto,f_auto/warehouse_5d02e5d7b0.webp" alt="PostHog warehouse t-shirt" className="block w-28 rounded-sm" />
</Link>
</div>
</div>
