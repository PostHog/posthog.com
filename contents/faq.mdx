...
PostHog is the leading platform for building self-driving products. With a full suite of developer tools – [AI observability](/ai-observability), [product analytics](/product-analytics), [session replay](/session-replay), [feature flags](/feature-flags), [experiments](/experiments), [error tracking](/error-tracking), [logs](/logs), [surveys](/surveys), and more – PostHog captures all the context agents need to diagnose problems, uncover opportunities, and ship fixes. 

A [data warehouse](/context-warehouse) and [CDP](/cdp) tie it all together, unifying that context into one source agents can read across. You can steer it all from [Slack](/slack), [the web app](/ai), [PostHog Desktop](/desktop), or your own editor via [the MCP](/mcp).
...
- [Error tracking](/docs/error-tracking) to capture and debug exceptions with full stack traces and session context
- [Logs](/docs/logs) to ingest, search, and analyze log data alongside your product data
- [Workflows](/docs/workflows) to automate actions based on events, like sending messages to your users
- [AI observability](/docs/ai-observability) to monitor, debug, and optimize AI features with trace inspection and cost tracking
- [Self-driving mode](/docs/self-driving) to turn signals in your product data (errors, rage clicks, failed queries, and more) into researched reports and pull requests you review and merge
...
You can work with all of it through four products: the [Slack app](/slack) (mention @PostHog in any channel), the [web app](/ai), [PostHog Desktop](/desktop) (a desktop app for driving parallel agents on top of your product data), and [the MCP](/mcp), which brings PostHog into Claude Code, Cursor, or whatever agent you already use.
...
See the [self-driving docs](/docs/self-driving) for how it works under the hood.

### How much does PostHog cost?

PostHog offers a generous free tier with no credit card required:

- **Self-driving**: reports are free, and your first 3 pull requests each month are free ($15 per PR after that, refunded if it wasn't worth paying for)
...
- **AI observability**: 100 thousand events/month
...
- [The MCP](/mcp), which works with Claude Code, Cursor, and any MCP-compatible agent
...
- **Self-driving** – Other tools stop at showing you a dashboard. PostHog closes the loop: it uses your product data to find problems, ship fixes as pull requests, and measure whether they worked. No other platform runs this loop end to end because no other platform holds all the context agents need like PostHog does.
- **All-in-one platform** – Instead of stitching together separate tools for analytics, session replay, feature flags, and experimentation, PostHog provides everything in one place. This means shared data, no integration headaches, and lower total cost – for you and your agents.
- **Built for AI engineers** – PostHog is designed with developers in mind. It offers SDKs in every major language, a powerful API, SQL access to raw data, and transparent documentation. Engineers can self-serve without waiting on analytics teams – and so can their agents, via the MCP.
- **Open source** – PostHog's core is open source with thousands of GitHub stars. This meanstransparency into how the product works, the ability to contribute, and community-driven development.
...
Yes. The [PostHog MCP](/mcp) brings your product data and tools into Claude Code, Cursor, and any other MCP-compatible agent. Your agent can query analytics, debug errors, build dashboards, measure the impact of changes without you leaving your editor, and a lot more.
...
Yes. We release continually and have a world-class team working daily on making the product better. Some of those pull requests are opened by PostHog itself – we run the [self-driving loop](/docs/self-driving) on our own product.
...
Alternatively, the [AI wizard](/docs/ai-engineering/ai-wizard) can handle the entire installation process for you with a single command. It automatically authenticates your account, installs the relevant SDKs, scans your codebase, and configures everything.
