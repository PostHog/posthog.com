...
In this post, I'll walk through what we learned through the process of building it, where we messed up, and lots of practical tips on how to make user behavior analysis work in production. All the code is [open-source](https://github.com/PostHog/posthog), and here's a 36-second [demo](https://res.cloudinary.com/sortafreel/video/upload/v1764510391/minivied-v2_onrxvy.mov) of how it works:

![Demo video](https://res.cloudinary.com/dmukukwp6/image/upload/Screenshot_2025_11_30_145133_70c7081276.png)
...
![Inputs](https://res.cloudinary.com/dmukukwp6/image/upload/single_session_filter_1_ceec2db1d8.jpeg)
...
- If you must segment, do it in one step, and then provide the context on all segments to each parallel call. Lots of context for the current segment, minimal context for the previous/next segments to keep LLM grounded.
...
![Video validation](https://res.cloudinary.com/dmukukwp6/image/upload/video_validatio_6d1a0375aa.jpeg)
...
- Don't analyze all the videos - there's a clear set of parameters (like event count, or active duration) that can be used to decide if it's worthwhile to check this session.
...
- Keep in mind that `puppeteer`, `playwright`, and similar libraries have different keyframe settings, so after a certain point, speeding up will result in data loss.
...
![Four-phase pipeline](https://res.cloudinary.com/dmukukwp6/image/upload/78071c82_b7bf_4883_b806_2a0fc1aa72db_06154e7b68.jpeg)
...
- Or even easier, as we load the video of the session at the moment it happened (actually, a couple of seconds before)
...
> __Temporal workflow logic on a small sample of sessions__
...
- We set failure thresholds instead of failing on the first error. Each stage allows ~20-25% of requests to fail (after exceeding the retries), so the process can move on.
...
![Basic retry logic](https://res.cloudinary.com/dmukukwp6/image/upload/2dc9059b_5887_4ae1_9cd2_e03da961609f_6b45fa47be.jpeg)

### “Phenomenal cosmic power, ity bity living space”

Temporal is amazing for orchestrating complex workflows, but it has limits. Event history size is capped at around 2MB. When we tried to use it as-is, we hit the limit even before making a single LLM call, just by getting the data to analyze from the DB (an obvious anti-pattern).
...
The beta of Session summaries has been released and is available to everyone right now. So, if you want to check our findings above, give it a try. Just ask PostHog AI to summarise your sessions. Worst case: you’ll notice where we messed up. Best case: you’ll learn something useful about your users and your product.
...
It’s still a beta, so we have lots of plans on how to make it better. Our goal is to ship a tool that does one thing well, then iterate. Expect the logic to evolve as we learn what gives the best results.

**What it will be able to do soon:**

- Full video understanding: Find sessions with a specific behaviour and answer free-form questions
...
In the meantime, try it, break it, and let us know. Thank you for reading.

---

P.S. If you are still reading and find it interesting, we're hiring [AI Product Engineers](https://posthog.com/careers/ai-product-engineer). We have tons of ideas to ship, and we would love to hear yours. Ping me if you have any questions, or [apply right here](https://posthog.com/careers/ai-product-engineer).