...
We, at PostHog, have a lot of user behavior data - what pages they visit, what buttons they click, and so, stored as sessions. And by "a lot" I mean billions of events and terabytes of S3 snapshots. We (and our clients) love watching these sessions, to understand how our product is used. However, there's an obvious problem - too many user sessions to watch manually. 

So, we created a tool to watch and analyze user sessions, highlighting the issues - so you can find what's broken without watching hundreds of hours of recordings yourself. It's called Session summaries, we release it in beta today. You can use it right now, for free, with PostHog AI. 
...
- If segmenting is required - go sequential, so when analyzing segment #2, it has context on what happened in segment #1. "what happened next" part will be lost anyway, and it will be crazy slow though.
