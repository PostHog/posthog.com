...
**Our approach:**
- We picked better quality. A report worth reading in 5 minutes beats a useless report in 30 seconds.
...
Right now, if you ask "what happened in these sessions?" - we'll tell you what went wrong. We focused on issue detection first: blocking errors, confusion, and abandonments. General summarization ("user browsed products, added to cart, checked out successfully") is the priority number two.
...
- General summarization is coming. The architecture supports it, we just need to tune our infra and output format a bit.
