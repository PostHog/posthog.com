...
## Using LLMs to enforce style

LLMs are effective at catching style guide violations that humans often miss. Consider using them to check your writing against this entire guide.
...
Lead with what readers need to know. For familiar concepts, start with the action. For unfamiliar terms, provide brief context first.

You can also split content into sections—one with the definition, one with instructions—to improve scannability.
...
Avoid paragraphs longer than 3-4 lines. Break up longer content with subheadings, lists, or visual elements as needed.
...
Use numbered lists only when sequence matters. Numbered lists are mainly used for UI instructions. Consider using steps if the list items are loaded or complex.
...
- **Simply, just, easily** - Don't use these. Say "Add the SDK to your project" not "Simply add the SDK to your project."

- **Please** - Don't use this. Say "Click **Save**" not "Please click **Save**."

- **Obviously, of course, clearly** - Don't use these. Say "This only works for authenticated users" not "Of course, this only works for authenticated users."
