...
See the [style guide](/handbook/content/style-guide) from the <SmallTeam slug="editorial" /> for additional writing guidelines.
...
- [Vale](/handbook/docs-and-wizard/vale) – enforces style guide rules through prose linting in PRs
...
| Term | Description |
|------|-------------|
| **Project token** | The public identifier (starts with `phc_`) used in SDKs and the snippet to send events. This is NOT an API key. Never call it `project API key`. |
