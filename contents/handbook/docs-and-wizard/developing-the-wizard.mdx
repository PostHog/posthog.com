...
First, identify cause of failure: run `npx -y @posthog/wizard@latest`. Review the logs at `/tmp/posthog-wizard.log`. This log can be quite verbose, so agent-driven analysis may be helpful to quickly pinpoint where things are going wrong. Include the below details to help the agent diagnose the issue.
...
Find [a previous release version number](https://github.com/PostHog/wizard/releases) and run `npx @posthog/wizard@<version>`. If the wizard runs successfully, you can compare the logs to the current release to see what changed. This will also confirm a safe rollback path.
