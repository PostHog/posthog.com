...
Find [a previous release version number](https://github.com/PostHog/wizard/releases) and run `npx @posthog/wizard@<version>` against an example project. You can find projects known to work in the [wizard-workbench](https://github.com/PostHog/wizard-workbench/tree/main/apps) repository. If the wizard runs successfully, you can compare the logs to the current release to see what changed. This will also confirm a safe rollback path.
