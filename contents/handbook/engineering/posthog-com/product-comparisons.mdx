...
_Example: If there's a global data retention for 2 years but in reference to heatmaps, you might want to say "Heatmap data retained for 2 years."_
...
For example, showing the `platform.integrations` section might make sense for the [Product Analytics comparison](/product-analytics#feature-comparison), but not for [AI Observability comparison](/ai-observability#feature-comparison) where that product doesn't really integrate with the tools that are otherwise integrated across the PostHog platform.
...
`/src/hooks/productData/ai_observability.tsx`:
