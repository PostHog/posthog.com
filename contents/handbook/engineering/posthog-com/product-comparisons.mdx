...
For example, showing the `platform.integrations` section might make sense for the [Product Analytics comparison](/product-analytics#feature-comparison), but not for [AI Observability comparison](/llm-analytics#feature-comparison) where that product doesn't really integrate with the tools that are otherwise integrated across the PostHog platform.
