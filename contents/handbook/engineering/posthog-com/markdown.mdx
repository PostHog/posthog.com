...
The `<Link />` component is used throughout the site, and is accessible within Markdown. (When used _internally_, it takes advantage of <Link to="https://nextjs.org/docs/pages/api-reference/components/link" external>Next.js `<Link />` features</Link> like prefetching and client-side navigation between routes).
...
There's also a `photo={true}` parameter which will inline their photo next to their name, but use sparingly as it can mess up the line height in paragraphs.
