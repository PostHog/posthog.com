...
The simplest entry point. It resolves the image (light + dark) and an optional named annotation set straight from a product hook, so the image stays in sync if it's ever updated. `ImageAnnotations` is a global MDX component, so no import is needed.
