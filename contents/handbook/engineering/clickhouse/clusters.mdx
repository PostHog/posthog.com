...
In order to upgrade ClickHouse, we need to bump the version on CI to test for regressions and compatibility issues. We do this by **adding** the ClickHouse version to the CI Matrix of ClickHouse versions.
...
One of the nice features of our data is that recent data is the hottest both for reads and writes. This is a perfect fit for tiered storage.
...
We will also want to create new coodinators for multi-tennant workloads in the future. This will allow us to scale up and down easily over time, and even potentially throughout the day as the workload rises and falls.
