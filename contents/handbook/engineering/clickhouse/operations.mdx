...
- Undocumented settings can be found in https://clickhouse.com/codebrowser/ClickHouse/src/Storages/MergeTree/MergeTreeSettings.h.html
...
At PostHog when deleting user data, we schedule for all deletions to occur at once per week to minimize the cost
of rewriting data.
...
Merges an be monitored via the following tables:
...
Note that not all parts are guaranteed to be merged if the size of parts exceeds maximum limits or if data is already in a single part.
...
One area where ClickHouse however differs from other databases is that schema changes are generally lazy and apply to only new data or merged parts. This applies to:
...
Some schema changes are however deceptively hard and frequently requires rewriting the whole table in some fashion or re-creating
...
Note that ALTER PARTITION commands only work if the two tables have identical structure: same columns and ORDER BY/PARTITION BY. It works by
...
Example async migration: [0005_person_replacing_by_version.py](https://github.com/PostHog/posthog/blob/50deeae355af35b3512097ebdc592c9fadd84bb4/posthog/async_migrations/migrations/0005_person_replacing_by_version.py)

### Resharding

At PostHog we've yet to need to reshard data, but the process would look likely similar to changing `ORDER BY` or `PARTITION BY`,
by requiring either to pause data or deduplicate at the end.
