...
- How choosing the correct `ORDER BY` and `PARTITION BY` in table definitions can affect query performance and why
...
The merge combines the two parts into a new one similar to how merge sort works and atomically replaces the two source parts.
...
Our `sensor_values` table above is set up in a way that queries similar to the following are really fast to execute.
...
1. can leverage table `ORDER BY (site_id, toStartOfDay(timestamp), event, uuid)` clause to skip reading a lot of data
2. is really fast and efficient about I/O and aggregation
...
As the dataset size or column sizes increase, this performance would get pathologically worse.
...
Always ensure your ClickHouse schema is aligned with queries that are being executed!
...
Now, ClickHouse would generate one **partition** per 10 years of data, allowing to skip reading even the primary index in some cases.

In the underlying data, each **part** would belong to a single **partition** and only parts within a partition would get merged.

One additional benefit of partitioning by a derivate of timestamp is that if most queries touch recent data,
...
3. Then, it loads and leverages the Primary key as before to narrow data down to 12 parts.
...
Loading the primary index/marks file might also not be the bottleneck you expect so benchmark different schemas against each other.
...
- Batching up GDPR- or other data deletions up and doing them on a schedule rather than immediately.

### No query planner

ClickHouse doesn't have a query planner in the sense PostgreSQL or other databases do.

On the one hand you often end up fighting the query planner in other databases. If we know how ClickHouse performs internally and how to translate queries into intuition
how they're executed, we're well-equipped to deal with issues as they arise.
...
Read more on `PREWHERE` here: https://clickhouse.com/docs/en/sql-reference/statements/select/prewhere/
...
Compression ratios have direct impact on query performance: IO is often the bottleneck, meaning that highly compressed data
can be read faster from disk at the cost of more CPU work for decompression.

By default columns are compressed by the `LZ4` algorithm. We've found good success using `ZSTD(3)` for storing JSON columns though - see
...
This means that at scale JOINs can kill performance. Read more on effect of removing JOINs from our events database here:
...
- https://clickhouse.com/docs/en/guides/improving-query-performance/sparse-primary-indexes/sparse-primary-indexes-design/
- https://kb.altinity.com/altinity-kb-schema-design/how-much-is-too-much/
- https://kb.altinity.com/engines/mergetree-table-engine-family/pick-keys/
