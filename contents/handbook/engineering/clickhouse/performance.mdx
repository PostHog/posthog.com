...
This document:
- details what tools are available to understand and measure query performance
- importance of page cache
- provides some general tips and tricks
...
At PostHog we also often add metadata about the query to the query as a comment to make it easier to analyze after the fact.

Example query to get recent slow queries:
...
Previous pages in this manual have [showed various examples](/handbook/engineering/clickhouse/data-storage#aggregation-supported-by-order-by)
of using ClickHouse `EXPLAIN` statement to your advantage.
...
Read more on `EXPLAIN` here: https://clickhouse.com/docs/en/sql-reference/statements/explain/
...
As most queries in ClickHouse are dependant on fast IO to execute fast, this can have a significant effect on query performance and a reason
why at PostHog our ClickHouse nodes have a lot of memory available.

### Effect on benchmarking

This behavior can however be a problem for profiling: Users constructing new queries might not hit the page cache and receive a worse
