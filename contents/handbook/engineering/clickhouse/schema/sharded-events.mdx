...
However we should now be sampling by `person_id` column as analytics-by-person is _likely_ the most important thing.

Note that at the time of writing, sampling is not yet supported by the PostHog app.

## `ReplacingMergeTree` with `uuid` as sort key

`ReplacingMergeTree` in allows "replacing" old data with new given identical `ORDER BY` values. In our case, since
...
Only way to make use of this is to regularly use `OPTIMIZE TABLE sharded_events FINAL`, but that could
...
PostHog javascript library has an autocapture feature, where we store actions users do on pages and DOM elements
