...
When designing the schema, we needed ingestion of these stats should be as 'cheap' as possible. On the flip side
...
Sorting sort by error_type also has a significance: `error_details` of a given error type should be similar and compress well.

In the future we also might introduce `TTL`s for the error columns if storage becomes a problem or periodically
...
Note that this runs the trade-off of counts being subtly off after deploys or restarts. If this becomes
a significant user concern, we may reduce the precision of the numbers shown in the UI in the future.
...
Note that even with all of this we still need to sum values in queries as merges may never occur.
...
storage similar number of events can require hundreds of gigabytes, or tens of thousands times more expensive.

Queries against this schema are also usually measured in milliseconds.

The reason we were able to do leverage pre-aggregation to this extent was since we needed to answer only a few different questions:
