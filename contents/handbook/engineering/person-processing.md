...
This triggers a **merge**: all events from the anonymous ID should be attributed to the same person as the identified ID.
...
When a user identifies themselves, we merge their anonymous person into their identified person. But there's a problem: historical events in ClickHouse still have the old `person_id`. If we do nothing, queries for that user would miss all their anonymous events.
...
Instead, we use a two-part strategy:

1. **Small overrides table for queries**: We maintain `person_distinct_id_overrides` which only contains distinct_ids whose person mapping has **changed** (i.e., been merged). This table is tiny compared to `person_distinct_id2` - typically just thousands of rows instead of millions. Queries LEFT JOIN to this small table, which is fast.
