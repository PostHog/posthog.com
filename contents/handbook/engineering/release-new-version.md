...
- [ ] Tag the version in GitHub. **This will immediately mark that a new version is available for users. Do this when you're sure the new release is ready.** This will also build and push the `release-[version]`, `latest-release`, `foss-lastest-release` & `foss-release-[version]` Docker images to Docker Hub.
  ```bash
