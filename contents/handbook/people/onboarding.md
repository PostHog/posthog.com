...
-   Please make sure you spend at least 3 days together, working through the [first week onboarding list](/handbook/people/onboarding#your-first-week) and spending time working on any role-specific tasks that are outlined in the new joiner's personal onboarding checklist.
    -   Add the details of the in-person onboarding to the [In-person Onboarding Calendar](https://calendar.google.com/calendar/u/0?cid=Y19lMzdjNjE0NzM0NDAyOGJlY2ZkMTc0Y2MxZjUxZjY5ZTkxMzY0NzVlNTFjMTE3MjA4OGM2NWQwNjE1YTczNmRlQGdyb3VwLmNhbGVuZGFyLmdvb2dsZS5jb20) so that other PostHog team members can join, if possible. To do this, create the event on your own Google Calendar as normal, then add the in-person onboarding calendar as a guest to that event. Subscribing to the calendar only gives you read-only access, so adding the calendar as a guest is what gets your event onto the shared calendar. If that doesn't work, ask in `#team-people-and-ops` for edit access to the calendar.
