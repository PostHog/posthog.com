...
Benchmark adjustments should still be seen as performance adjustments and are not guaranteed if your performance is not keeping up with others at a similar level. They are analagous to yearly raises at other companies for doing a "good job" (though are not guaranteed yearly as we follow the market rates instead of a calendar schedule). 
