...
The newsletter is long-form, original copy that focuses on product and business lessons and information for engineers. It's consistently our top-performing content surface, so we invest a lot of effort in it. Our quality bar for it is very high.

We currently publish biweekly. The content in the newsletter is not directly tied to PostHog, even if the branding is obviously tied. Topics should be able to stand alone; it is NOT a surface for marketing our products.
...
The process of publishing a newsletter looks like this. (This is not a strict set of rules you have to follow.)

### Part 1: Come up with an idea

There are a few common ways we source [good content ideas](#topic):

- **[#content-and-video-ideas](https://posthog.slack.com/archives/C015CRUQR7Y)**: A place for anyone and everyone at PostHog to post random shower thoughts. Things that get a lot of buzz here are usually promising.
- **Reader polls**: Sometimes we poll readers for interest in future topics. We compile and maintain the [results](https://github.com/PostHog/posthog.com/issues/10212) as a list of potential topics for anyone to pick up.
- **Old blogs**: Newsletters are often based on successful blogs we already wrote, just converted into a newsletter-y format. We still always try to add a twist to make the content fresh.

### Part 2: Form an opinion

This sounds obvious, but we mean it. 

Once you've picked a topic, you should have at least one strong opinion or hypothesis. Product for Engineers thrives because we are opinionated; sitting on the fence isn't interesting.

This opinion becomes the angle of the article and the starting point for your research in Part 2.

### Part 2: Gather concrete examples
...
Research some observable, real-world data related to the topic. Compare what you find with your original opinion. As you immerse yourself in examples, you'll naturally refine the narrative and angle of the piece.

This is really important if you are NOT already an expert on the topic because otherwise, why should the reader listen to your opinion on it?

You don't have to come up with whole dissertation at this point, but you really need to do do _some_ research before outlining. These examples are literally the foundation of the piece and will inform everything else like the headline, structure, and hook.

For example, if you're writing an article about how to write great job posts, search for real job posts and develop a sense of what makes one good or bad. Look for patterns and themes.

#### (B) Lived experience

Another category of examples is lived experience, but this really only works if you have legitimate authority to speak on the topic. You wouldn't necessarily trust a chocolate chip cookie recipe by Jane Goodall, for example, but you probably would try one by Gordon Ramsay.

We have several newsletters and blogs by execs who introduce the piece by saying "I worked on this for 18293129 years, here's what I learned". That's the source of their authority on the topic.
...
Congrats, it's time to start actually writing! (Revisit [How to write a good newsletter](#how-to-write-a-good-newsletter) for style and writing tips.)

This is one of the few times where we don't collaborate on GitHub. This is because we want the content preview to look like a Substack article, so we use a customized template to mimic this format.
...
1. Write your draft in it in Markdown.
    - Include sketches, descriptions, or samples of graphics that you plan to use in this stage (e.g., diagrams, memes, charts). 
    - For diagrams, we often use Figma or FigJam to create our own, and then request a designer to polish it later.
    - For artwork like hero images or hedgehog stuff, you can just include a description or a sketch or AI-generated artifact to communicate the concept.
...
> Pro tip: You can enable Markdown formatting in Google Docs settings. Some of us also like to draft first in Notion for the UI; it's totally up to you as long as the version you request review is on Google Docs.

### Part 5: Request visuals and assets

Diagrams are in <PrivateLink url="https://www.figma.com/board/b0ttECQMiQ6LlGKpIICUcM/Marketing-Jam?node-id=0-1&p=f&t=Uy3T3a4MXGY266zM-0">FigJam</PrivateLink>.

1. [Open an art request](/handbook/brand/art-requests) to request a hero image. (Give the team at least 1-2 weeks to complete it! The more heads up, the better)

- Having a good post preview image is important. Either create one using hedgehogs from the [Hoggies file in Figma](https://www.figma.com/design/I0VKEEjbkKUDSVzFus2Lpu/Hoggies?node-id=1-196&t=UZQMXMddH0DMLxqX-0). 
    - It needs to be 1200x630 px for the `posthog.com` OG image and 1456x1048 px for the Substack preview image.
    - Art from previous newsletters can be found in <PrivateLink url="https://www.figma.com/design/tNuNQ0STmx0ve4f1sAv4Ka/Blog-images?node-id=0-1&p=f&t=qHbYLFRHbl0v8wqW-0">Figma</PrivateLink>. 

1. [Open an art request](/handbook/brand/art-requests) to request a "polish" review. (Give the team at least 1 week to complete it!)
    - This is for things like diagrams and visuals you created in FigJam, etc.

### Part 6: Export to Substack

1. Go to the Substack dashboard and port your content over!
1. There are a few more sections you can add to the newsletter at this stage.
    - Required:
      - "More reads". External blogs and articles you enjoyed recently. They can be things you found while researching, or other PostHog blogs. They don't have to be related to the newsletter topic itself. Channels like #dev, #devel, and #industry-news are also good sources of inspiration here.
    - Optional:
      - Reader polls. Add a poll for future newsletter topics (only if it fits in well or flows with the piece)
      - Job posts. Ping #team-talent to see if they need a boost for any job listings recently.
1. It's time to add the final touches.
    - Check for formatting mistakes
    - Minor line edits, typos, etc.
    - Double check all images and assets work
    - Make sure all links work
    - For any `posthog.com` links, include UTMs like `?utm_source=posthog-newsletter&utm_medium=post&utm_campaign=enter_name_here`.
1. Fill in Substack-specific fields including the title, description, SEO stuff, and hero image settings. (Note: We don't really worry about SEO for newsletters so you can just reuse the title/description.)
1. Send a test email to yourself to make sure it all looks good!
1. You can configure a couple of titles to A/B test.
1. PUBLISH! Your creation is now live and reaching thousands of inboxes around the world. No pressure.

### Part 7: Create a GitHub PR
After publishing on Substack, we still do a final GitHub PR for recordkeeping.

1. Create a PR on GitHub
1. Request a quick review from peer for glaring mistakes
1. Merge!