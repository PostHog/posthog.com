...
- Focus on the main element, elements like the left and right sidebar should be out of frame unless they are relevant to the screenshot.
- Double check that your screenshot doesn't contain any user data, like user emails or names.
- Use the following settings for clean screenshots in Chrome:
  - Enter devtools and use a device width of 1000px - 1400px. This avoids unnecessary empty space and keeps text readable.
  - Use DPR (device pixel ratio) of 3, this will upscale rendering to 3x the size of the screen.
  - Prefer taking **node screenshots** of the DOM node element you're trying to capture. This gives you a cleanly cropped screenshot and upscales by the DPR setting. Even zoomed in screenshots will be sharp.
  - You can also use Chrome's **Capture screenshot** button to take a full screen screenshot. This will upscale by the DPR setting, so even when zoomed in, the screenshot will be clear.

Here's an example of Chrome devtools for node screenshots:
...
In tutorials, you can identify the file name of the code block by adding `filename=` to the code block. This will be displayed in the code block's title.
...
Especially in long tutorials, you can highlight the important differences between code blocks using highlighting comments. It's much easier to read visual diffs than reading through the code block line by line.

- `// +` adds a green highlight, usually representing additions in diffs
- `// -` adds a red highlight, usually representing removals in diffs
- `// HIGHLIGHT` adds a yellow highlight without special meaning
...
In some cases, such as large nested config files, you need readers to focus on a specific part of the code block without losing context. You can do this by adding `focusOnLines=` to the code block. This will collapse the code block and only show the lines of code you specify.
...
Code blocks can also be used to show mermaid UML diagrams. When using these diagrams, make sure to also include a text description of the diagram for accessibility and LLMs.
