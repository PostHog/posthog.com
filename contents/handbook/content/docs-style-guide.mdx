...
To add a preloaded question to docs or tutorials, you can use the PostHogAIPrompt component and add the question you'd like to preload in the question field, like so:
...
The `<AskPostHogAI>` component opens the PostHog AI chat window directly on the website. Use this for documentation pages where users might need help understanding concepts or troubleshooting. Unlike `<PostHogAIPrompt>` which links to the PostHog app, this keeps users in the docs context.
