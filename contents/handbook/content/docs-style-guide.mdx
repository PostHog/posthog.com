...
> **Note:** Decision trees can easily grow far too complex, so keep your logic minimal. Aim for 2-3 questions max with 5-6 options per question. If you need more, consider restructuring your content instead. A complicated decision tree is worse than a simple list.
...
<DecisionTree
    questions={[
        {
            ...
            description: 'Select your hosting platform.',
            options: [
        ...
        },
        {
            ...
            condition: (answers) => answers.platform === 'vercel',
            options: [
    ...
    getRecommendation={(answers) => {
        if (answers.platform === 'vercel' && answers.framework === 'nextjs') {
            return {
                title: 'Use Next.js rewrites',
                path: '/docs/advanced/proxy/nextjs',
                reason: 'Next.js rewrites work seamlessly on Vercel.',
            }
        }
        return {
            title: 'Use the default option',
            path: '/docs/advanced/proxy',
            reason: 'This works for most setups.',
        }
    }}
...
-   **questions**: array of question objects with `id`, `question`, `options`, and optional `description` and `condition`
-   **getRecommendation**: function that takes answers and returns `{ title, path, reason }`
-   **buttonText**: optional, defaults to "Follow the guide", can change as needed

The `condition` function receives all previous answers and returns `true` if the question should be shown. Use this for branching logic.

To see this implemented, see `contents/docs/advanced/proxy/_snippets/proxy-decision-tree.tsx` or [Deploy a reverse proxy](https://posthog.com/docs/advanced/proxy) on the live site.
