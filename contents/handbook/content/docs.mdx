...
The in-context example is more verbose, but much more useful. It shows how PostHog fits into application logic, which helps developers understand *when* and *where* to use it. 

Inline comments and UI enhancements like syntax highlighting and collapsible sections will keep the code scannable.

### 2. Treat docs like a product

We treat our docs like a product, because they are. They have users (readers), use cases (implementation, product education, troubleshooting, etc.), and success metrics. PostHog's goal is to help customers build successful products, and docs play a huge part in that.
...
So when working on the docs, take some time to ask yourself:
...
For documentation writing especially, we prioritize clarity, precision, and consistency because they contribute to a smoother reading experience. Nitpicking isn't just allowed, it's encouraged! Nits, semantics, formatting – they're all part of the fun (and challenge) of technical writing. Attention to detail is what turns good docs into great ones, so don't shy away from it.

This does *not* mean our docs have to be dry or academic. In fact, they should be opinionated and have a natural flow that makes them easy to read. Be open, direct, and conversational. Don't be afraid to add humor and style when there's opportunity.
