...
7. On the right of the screen there is a sidebar, select the **Variables** tab and populate them as follows:
    ...
    - **Startup credits** - If the customer [qualifies for the 2 free months](/handbook/growth/sales/contract-rules#startup-plan-discounts) when rolling off the startup plan, add up their total and discount as normal, and then add a note about the free credits in this format: "An additional credit in the amount of `<amount>` to be applied to Customer's account upon signature with an expiration 12 months from the start date." For example, if a customer is signing a standard $20k annual contract to get the 20% discount, the total will be $25k, 20% discount of $5k, total cost to the customer would be $20k. In the notes, you would write: "An additional credit in the amount of USD $4,166.67 to be applied to Customer's account upon signature with an expiration 12 months from the start date."
