...
- **Take-home sandboxes for prospects.** Sometimes a prospect wants to click around before they instrument anything. DemoHog hands them a populated project to explore Product Analytics, Session Replay, flags, and experiments with zero setup.
...
- **It doesn't feel like the real product.** The data's plausible but it isn't theirs. Numbers won't match anything they recognize, edge cases in their funnel are missing, and the "aha" moment from spotting something real in their traffic never happens.
...
**Discovery & Education**
- `market_viewed` — with properties like `category`, `market_id`, `source` (browse, search, deeplink, push notification, email), `time_to_close`, `total_volume`, `is_trending`. This is your impression-level data and it's gold for understanding what catches attention vs. what converts.
- `search_performed` — `query_text`, `results_count`, `result_clicked` (bool). Zero-result searches are a direct product roadmap input — people are telling you what markets they want.
- `market_shared` — `share_method` (link copy, twitter, embed), `market_id`. Viral loops on prediction markets are underexplored.
- `education_tooltip_viewed` / `explainer_opened` — tracks whether users are engaging with your "what is this?" affordances, segmented by `user_tenure_days`.
**Pre-Trade Deliberation (the funnel nobody instruments)**
- `order_book_viewed` — `depth_viewed`, `time_spent_ms`. Someone staring at the order book for 30 seconds is a fundamentally different user than someone who glances and bounces.
- `position_calculator_used` — if you have any payout/profit calculator, instrument it. This is the "shopping cart equivalent" for a prediction market.
- `price_chart_interacted` — `timeframe_selected`, `zoomed`, `hovered_on_event_marker`. People studying price history are warming up to trade.
- `order_started_but_abandoned` — `amount_entered`, `price_entered`, `abandon_reason` (if detectable — navigated away, changed mind, insufficient balance). This is your cart abandonment.
- `contract_details_expanded` — someone reading the resolution rules is a high-intent signal.
**Portfolio & Position Management**
- `portfolio_viewed` — `total_positions`, `unrealized_pnl`, `frequency_per_day`. Daily portfolio checkers are your power users.
- `position_closed_early` — `pnl_at_close`, `time_held`, `market_time_remaining`. Understanding *why* people exit early (profit-taking vs. panic) is critical.
- `limit_order_placed` vs. `market_order_placed` — different sophistication levels, different user segments entirely.
- `deposit_initiated` / `deposit_completed` / `deposit_failed` — with `method`, `amount`, `is_first_deposit`, `time_since_signup`. The deposit funnel is arguably your most important funnel.
- `withdrawal_requested` — `amount`, `remaining_balance`, `trigger_event` (did they just have a big loss?). Early warning for churn.
**Engagement & Return Triggers**
- `notification_received` / `notification_opened` — `notification_type` (price alert, market resolution, new market in category), `time_to_open`. Which notifications actually bring people back?
- `watchlist_added` / `watchlist_removed` — `market_category`, `current_price`. Watchlist-to-trade conversion is a metric you should own.
- `market_resolved_viewed` — `user_had_position` (bool), `pnl_on_resolution`, `next_action_within_5min`. What do people do right after they win or lose? That's the emotional moment where you either retain them or lose them.
- `referral_link_generated` / `referral_converted` — obvious but often under-propertied. Track `referrer_lifetime_volume` to see if power users refer other power users.
**Trust & Friction Signals**
- `kyc_step_completed` / `kyc_step_abandoned` — `step_number`, `time_on_step`, `document_type`. KYC is where you hemorrhage users.
- `support_ticket_opened` — `category`, `user_tenure`, `recent_trade_count`. A spike in tickets after a controversial resolution is a leading indicator of trust erosion.
- `terms_or_rules_page_viewed` — `from_context` (during trade, during dispute, idle browsing). Context matters enormously here.
```
...
| **Project token** (`phc_…`) | PostHog → Project settings → Project API key (needed for upload; add it here or on Results) |
...
### 4. Results: upload, dashboard, demo site
 
On **Results**:
 
1. **Download JSONL** (optional) to inspect the file.
2. **Upload to PostHog.** Sends events to `/batch` using the project token. Needs the `phc_…` key if you didn't set it earlier.
...
- **First scheduled run:** backfills from the range **start** through **today** (or the range end, whichever's sooner).
