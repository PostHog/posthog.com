...
-   **Two or more `uncollectible` invoices block a customer from re-subscribing.** The unblock is automatic, but it only runs when a payment succeeds on the account and no `uncollectible` invoices are left. Clearing those invoices with credit notes does not trigger that check, so if you resolve someone that way, ask RevOps to confirm the block lifted and to clear it manually if it didn't.
