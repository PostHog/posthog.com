...
| Event | What fires it | Captured from |
|---|---|---|
| `insight created`, `dashboard created` (allowlist) | API layer, when an insight/dashboard is saved from any surface. The allowlist drops `source = 'wizard'` – insights and dashboards the setup wizard creates are setup, not engagement. The starter dashboard every new project gets fires no event at all. | Server – multi-surface |
