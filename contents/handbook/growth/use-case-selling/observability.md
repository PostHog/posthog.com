...
**Logs → Distributed tracing → Metrics (the OpenTelemetry path):** Once a team is exporting logs over OTLP, traces and metrics are a config change, not a new integration. The same collector, the same project token, three different endpoints. This is the cheapest expansion in the whole playbook to *describe* — but be honest that traces are beta and metrics is alpha, and check they're not depending on the gaps listed under pain points.
...
- [ ] If they already run OpenTelemetry, point a trace exporter at PostHog ([beta](/docs/distributed-tracing/start-here), free during beta)
...
| Logs over OpenTelemetry | Distributed tracing *beta* + Metrics *alpha* | The hard part (an OTel pipeline) is already done; the other two pillars are a config change | "You're already exporting OTel to us. Traces and metrics are the same collector and a different endpoint — and they're free while they're pre-GA." |
