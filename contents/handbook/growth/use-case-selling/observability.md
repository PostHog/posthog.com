...
- **[Health checks](/docs/health-checks)** — PostHog inspecting the customer's own PostHog setup: missing SDKs, traffic bypassing the reverse proxy, broken materialized views, ingestion warnings. Issues raise and resolve themselves, and can be fed to self-driving to be fixed automatically.
