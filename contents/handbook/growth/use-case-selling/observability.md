...
|---|---|---|
| Tracing is beta, Metrics is alpha | Setup details, including the ingestion endpoint, may change before GA. Don't build a migration plan around them yet. | Be honest that tracing is beta (free during beta) and metrics is alpha. They're usable today and a real reason to start exporting OTel to PostHog, but a team mid-migration off Datadog should keep their existing tool running. |
...
| Logs over OpenTelemetry | Distributed tracing *beta* + Metrics *alpha* | The hard part (an OTel pipeline) is already done; the other two pillars are a config change | "You're already exporting OTel to us. Traces and metrics are the same collector and a different endpoint — and tracing is free during beta." |
