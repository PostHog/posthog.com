...
- **OpenTelemetry-native, no proprietary SDK.** Logs, traces, and metrics all ingest over standard OTLP. There's nothing to rip out if they leave, which is a genuinely disarming thing to say to a platform team that's been locked into an agent-based vendor. One OTel setup covers all three pillars.
