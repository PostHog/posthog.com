...
| AI reply agent isn't available yet | Teams comparing against Intercom's Fin won't find a like-for-like answer | It's coming and will be opt-in and separately billed, so nobody gets surprised on their bill. In the meantime, position [Workflows](/docs/support/workflows) (rules they control) plus self-driving (recurring issues become PRs) — a different and arguably better answer to ticket volume. |
