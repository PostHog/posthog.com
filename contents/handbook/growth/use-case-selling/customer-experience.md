...
| Tracing is beta, metrics is alpha | Can't fully replace backend performance monitoring for complex microservice architectures | Be honest: [tracing](/docs/distributed-tracing) is beta and [metrics](/docs/metrics) is alpha, both free while pre-GA, with no service map and no profiling. Position PostHog as the user-facing debugging layer; heavy backend monitoring stays in their existing tool for now. See the [Observability playbook](/handbook/growth/use-case-selling/observability) for the full gap list. |
