...
-   **Draft, coordinate, and send any messages.**
    ...
    -   Direct users to the status page or post-mortem (if available) as the source of truth. 
     > **When do we need to notify users immediately?** In cases of data breach and security incidents, like the [download of malicious packages](/blog/nov-24-shai-hulud-attack-post-mortem), in case the incident lead has identified that users can take action to prevent the malicious packages from spreading further, we should notify users immediately with clear steps how to act on their side. Product downtime that doesn’t involve security breaches/attacks should be addressed after the incident is closed and we have the context needed to inform users.
