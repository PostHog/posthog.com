...
- We generally let influencers decide what the ad read is like so it best fits their audience. We can provide guidance on what to talk about though – our [brand foundations](/handbook/brand/foundations) go deeper, but the key points to get right:
  ...
  - PostHog is the platform for [self-driving products](/self-driving) – a suite of products that gives developers (and their AI agents) everything they need to build successful products. Don't call it "an analytics platform" (we've grown well beyond that) or pitch it as a single product.
