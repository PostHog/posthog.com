...
PostHog Session Replay records every user session across web and mobile. From any analytics insight, jump to the sessions behind it. From any error in Error Tracking, watch exactly what the user was doing when it happened. From any rage click pattern, let Signals research it and hand an agent a fix PR.
...
Session replay used to be reactive – you watched what went wrong *after* a user complained. In the [product autonomy loop](/blog/self-driving-product), replay is a proactive signal source. [Signals](/handbook/marketing/positioning/signals) reads replay patterns – rage clicks, dead ends, unexpected exits – and turns them into researched, ranked reports in your Inbox (which an agent can then take into a fix PR) before users ever file a ticket.
...
**Solution:** Signals connects to Session Replay as a signal source. Replay patterns – rage clicks, repeated form abandonment, dead-click clusters – are automatically surfaced, researched, and ranked in the Inbox (in PostHog Web, and piped to Slack), where an agent can take the worst offenders into a fix PR. AI summarization enables manual review to scale too, as PostHog AI can group and summarize sessions for you.

**Supporting features:**
- Signals uses Session Replay as a signal source (Inbox in PostHog Web, piped to Slack)
...
- Signals + agent integration – replays become inputs to ranked reports and automated fix PRs
...
- Signals uses replay as a signal source – LogRocket has no equivalent agent loop
...
The forward-looking pitch: replay as a signal source for Signals is a capability no other vendor offers. Teams that instrument replay properly now will have a fully automated UX investigation loop as the agent features mature.
