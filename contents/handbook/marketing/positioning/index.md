...
This section is the product-marketing detail: the vocabulary we hold ourselves to, and an opinionated playbook for each tool – what it does, who it's for, why it beats the obvious alternative, and how it fits the [self-driving product](/blog/self-driving-product) argument. These aren't datasheets. They're opinionated playbooks.

> **The house position:** **PostHog makes _your_ product self-driving.** Every tool page ladders up to that – each one is a signal source, a way agents act, or the evaluation that proves it worked. Keep the customer's product as the subject: they get a product that improves itself; PostHog is how. Don't write "PostHog is self-driving software" in customer-facing copy. We do use the same capabilities on PostHog itself – that's a proof point, not the pitch.
...
We've repositioned around self-driving. That only works if we all use the same words for the layers of what we offer – across marketing, content, and positioning. The [four layers](/handbook/brand/foundations#how-we-describe-posthog) (products, tools, context, context warehouse) are defined in brand foundations. This is how we keep the language tight. When in doubt, match it.
