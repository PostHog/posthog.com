...
6. **Granola or it didn't happen.** Every video meeting needs notes shared somewhere the whole company can find them — [Granola](https://granola.ai) is our default, and it captures the conversation so you can actually engage in the call instead of frantically typing. After the meeting, drop the share link in the most relevant Slack channel (or link from the meeting's GitHub issue or Google Doc), along with any decisions and to-dos. This way anyone can skim across meetings to stay in the loop without sitting in or asking what they missed — which is the whole point. If a meeting is too sensitive to share notes from company-wide, that's a strong hint it belongs in [Company Internal](#company-internal) — or didn't need to be a meeting at all.
