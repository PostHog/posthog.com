...
As much as possible, each project should be owned by an individual within a single small team. However, some projects affect multiple other teams and require work from them. For example, the performance work owned by Karl in product analytics which requires support from the pipeline and infrastructure team.
