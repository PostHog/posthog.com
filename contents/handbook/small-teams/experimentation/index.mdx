...
Imagine Alice is a product manager, and Bob is an engineer, both of whom love using PostHog. They're building a new feature together, but they're unsure of the impact, so Alice & Bob decide to gate the feature via a feature flag.
...
Bob feels this isn't enough, and also adds a condition for Sentry errors increasing by 30%. This should result in an automatic rollback as well.

Once the feature is rolled out, thankfully, nothing goes wrong, but what's disappointing is that the feature doesn't seem to have moved any of the core company metrics. This doesn't fit into either of Bob's or Alice's model, so they want to dig deeper why this was the case.
...
As it turns out, people in USA and India love their new feature, having a 40% increase in conversion, but other countries, specially the UK, seem to dislike it, so much so that it negatively affects conversion. In the end, these forces balance out, leading to similar total conversion rates.
