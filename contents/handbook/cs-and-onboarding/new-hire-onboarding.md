...
- Nailing the product fundamentals - use the [framework](#learning-posthog) as a guide and work through the [onboarding exercise](/handbook/cs-and-onboarding/new-hire-onboarding-exercise). If you want a guided version of this, the [hands-on labs](#learning-posthog) set the project up for you.
...
**If you learn better by doing than by reading, the CSM labs are built for that.** The [labs](https://github.com/PostHog/onboarding-csm-lab) are a self-contained course you can take or leave. You create your own free PostHog project, instrument a small app from zero, and use product analytics, session replay, feature flags, and experiments against data you generated yourself - so the first time you see a funnel drop off or a replay with no events, it's your own, not a customer's. From there you work through twelve real customer failure modes drawn from [health checks](/handbook/cs-and-onboarding/health-checks) and the [basic implementation review](/handbook/cs-and-onboarding/foundation-check), each ending in the reply you'd actually send. They're modular, so you can do the foundations and stop there, or flex toward whatever your book uses.
