...
-   PostHog AI being as single-loop agent <TeamMember name="Georgiy Tarasov" />
-   Unify MCP server and PostHog AI tools to the extent sensible <TeamMember name="Emanuele Capparelli" />
...
Then: Figure out line between PostHog AI and Array if not clear yet
Then: Figure out MCP vs PostHog AI, if not clear yet
Then: Jobs to be done, if not clear yet

Where is the line between PostHog AI and Array, what is the handover, is PostHog AI read, and Array write?
...
MCP vs PostHog AI, where do they overlap, where not? Venn diagram style
