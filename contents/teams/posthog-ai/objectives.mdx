...
🏆 Product autonomy (ultimate goal)

- API for product teams to create reports (or what is consumable by PostHog AI for analysis)
  - Before coding: Experiment has finished -> Create a report -> PostHog AI analyzes the report and researches it (Zendesk/session summaries) -> PostHog AI makes a decision on the next steps (drop, ask human for approval, yolo).
  - Before merging a PR: Instrumentation (our problem?)
  - After merging a PR: Create a report -> CRON -> Research the problem with a reconstructed conversation -> Make a decision on the next steps (drop, ask a human for approval, yolo).
- Background agents
...
- Improve data retrieval experience for larger organizations
- New tool and query examples for optimized property and property value searches
