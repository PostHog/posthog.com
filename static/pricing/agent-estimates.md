...
Now – and only now – ask the remaining questions, batched into one message. Ask each one only if Steps 3-4 left it unanswered:
...
1. **A summary table:** each product, the monthly volume you used, where that number came from, and the estimated monthly cost from current rates in pricing.md.
2. **The total,** as a range when any mapping was uncertain, with the free-tier amounts already subtracted.
3. **Your assumptions,** each one falsifiable: "I split events 70/30 anonymous/identified based on your answer," not "I made reasonable assumptions."
4. **The calculator link,** with a note that the user should confirm the sliders match before trusting or sharing it.
