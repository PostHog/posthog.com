...
const EventsPage: React.FC = () => {
    ...
    const columns = [
        {
            name: (
                <button onClick={() => handleTableSort('name')} className="flex items-center gap-1 hover:text-accent">
                    Event
                    {tableSortBy === 'name' && (
                        <span className="text-xs">{tableSortDirection === 'asc' ? '↑' : '↓'}</span>
                    )}
                </button>
            ),
        ...
        {
            name: (
                <button onClick={() => handleTableSort('date')} className="flex items-center gap-1 hover:text-accent">
                    Date
                    {tableSortBy === 'date' && (
                        <span className="text-xs">{tableSortDirection === 'asc' ? '↑' : '↓'}</span>
                    )}
        ...
        {
            name: (
                <button onClick={() => handleTableSort('format')} className="flex items-center gap-1 hover:text-accent">
                    Format
                    {tableSortBy === 'format' && (
                        <span className="text-xs">{tableSortDirection === 'asc' ? '↑' : '↓'}</span>
                    )}
        ...
        {
            name: (
                <button onClick={() => handleTableSort('vibe')} className="flex items-center gap-1 hover:text-accent">
                    Vibe
                    {tableSortBy === 'vibe' && (
                        <span className="text-xs">{tableSortDirection === 'asc' ? '↑' : '↓'}</span>
                    )}
                    ...
                    <div className="flex items-center justify-center">
                        <div className="flex">
                            {[...Array(5)].map((_, i) => (
                                <span
                                    key={i}
                                    className={`text-lg ${i < event.vibeScore ? 'text-yellow' : 'text-muted'}`}
                                >
                                    ★
                                </span>
                            ))}
                ...
                <section className="max-w-6xl">
                    ...
                    <div className="flex flex-col @lg:flex-row @lg:items-start gap-8 mb-12">
                        <div className="@lg:flex-1">
                            ...
                            <p className="text-secondary text-lg">
                                We run all sorts of events and parties around the world. From AI meetups to founder
                                breakfasts, we just want to have a good time and spend all this VC money on the
                                community. In a responsible, safe way. Probably.
                            </p>
                            <blockquote className="border-l-4 border-accent pl-4 mt-4 italic text-secondary">
                                Want to collab? Chat to our party planner, <TeamMember name="Daniel Zaltsman" photo />,
                                or{' '}
                                <a href="https://github.com/PostHog/meta/issues" className="text-accent">
                                    open an issue
                                </a>
                                .
                            </blockquote>
                            ...
                            <div className="grid grid-cols-1 @md:grid-cols-2 @lg:grid-cols-3 gap-6">
                                {featuredEvents.map((event, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="mt-4">
                                            <CallToAction to={event.link} externalNoIcon size="sm" width="full">
                                                Learn More
                                            </CallToAction>
                                        </div>
                                    </div>
                        ...
                        <div className="flex flex-col @md:flex-row @md:items-center @md:justify-between mb-6">
                            <div>
                                ...
                                <p className="text-secondary">
                                    Browse our complete calendar of events. {filteredAndSortedEvents.length} events
                                    found.
                                </p>
                            </div>
                            ...
                            tabs={[
                                {
                                    label: 'Table View',
                                    value: 'table',
                                    content: <OSTable columns={columns} rows={rows} size="md" className="text-sm" />,
                                },
                                {
                                    label: 'Map View',
                                    value: 'map',
                                    content: <EventsMap events={filteredAndSortedEvents} />,
                                },
                            ]}
                            value={viewMode}
                    ...
                    <div className="bg-accent border border-primary rounded-lg pt-4 pb-8 px-8 text-center">
                        ...
                        <p className="text-secondary mb-6">
                            Rewards include free merch, invites to exclusive organizer events, PostHog credits, and
                            more. Plus, we're always willing to collab with interesting companies.
                        </p>
                        <div className="flex flex-col @sm:flex-row gap-4 justify-center">
                            <CallToAction to="//community/profiles/34023" size="md">
                                Chat to our party planner
                            </CallToAction>
                            <CallToAction to="/community-incubator" size="md" type="secondary">
