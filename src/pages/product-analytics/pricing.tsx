        ...
        >
          <div className="flex flex-col-reverse md:flex-row md:gap-12">
                <div className="flex-1">
                    <p className="">
                        Use {productMenu.children.find((item) => item.slug === slug)?.name} free. Or enter a credit card for advanced features.{' '}
                        <br className="hidden lg:block" />
                        Either way, your first {productMenu.children.find((item) => item.slug === slug)?.freeTier?.toLocaleString()} are free – every month.
                    </p>
                </div>
