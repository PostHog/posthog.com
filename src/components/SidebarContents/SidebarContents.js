...
import { onSetSidebarOpen, onSetSidebarContentStructure, onSidebarContentExpanded } from '../../actions/layout'
...
class SidebarContents extends Component {
    ...
    render() {
        const { selectedKey, expandedKeys, entry, selectedEntry, contentTree, contentDir } = this.props.sidebar
                ...
                render={(data) => {
                    ...
                    const convertToTree = (entry, parent) => {
                        ...
                        const child_dir = rootEntry.child_entries
                            ? rootEntry.child_entries.map((item) => convertToTree(item, rootEntry.id))
                            : null
