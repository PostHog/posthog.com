...
const ProductItem = ({ product, translate = identity }: { product: any; translate?: TranslateFn }) => {
...
export const OnePlaceSlide = ({ translate = identity }: { translate?: TranslateFn }): JSX.Element => {
    ...
    const { allDestinations } = useStaticQuery(graphql`
        query KoreanHeroDestinationData {
    ...
    return (
        <div className="rounded p-4 flex flex-col h-full">
            <div className="mb-6">
                ...
                />
                <h2 className="mt-0 mb-2">더 나은 데이터로 더 나은 제품을 만드세요</h2>
                <p className="text-secondary text-[15px]">
                    흔한 데이터 통합인 줄 알았죠? 아닙니다. 서드파티 데이터는 PostHog CDP와 웨어하우스로 가져와 1급
                    데이터로 다뤄집니다. 따라서 서드파티 데이터와 <em>제품 사용 데이터까지 함께</em> 쿼리해 더 나은
                    의사결정을 할 수 있습니다.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 @lg:grid-cols-2 @2xl:grid-cols-3 gap-8 @2xl:gap-6 @3xl:gap-8 flex-1">
                ...
                <div className="flex flex-col gap-3 order-2 @lg:order-3 @2xl:order-2 @lg:col-span-2 @2xl:col-span-1">
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-2 @lg:justify-center @lg:max-w-xs @lg:mx-auto @xl:max-w-none">
                        {manageProducts.map(
                            (product: any) =>
                                product && <ProductItem key={product.handle} product={product} translate={translate} />
                        )}
                    </div>
                    <div className="@lg:text-center mt-auto leading-tight">
    ...
    const renderSlots = (breakpoint: '@xl' | '@2xl' | '@3xl') =>
        productSlots.map(({ handle, ...pos }, i) => {
            ...
            return (
                ...
                >
                    <ProductStatusLink product={product} translate={translate} />
                </div>
            )
        })
        ...
        <>
            ...
            <div className="z-10 @xl:hidden grid grid-cols-2 gap-x-4 gap-y-1 mb-2">
                {productSlots.map(({ handle }) => {
                    const product = productMap[handle]
                    if (!product) return null
                    return <ProductStatusLink key={handle} product={product} translate={translate} />
                })}
            </div>
            {/* @xl to @2xl */}
...
export const UnderstandUsageSlide = ({ translate = identity }: { translate?: TranslateFn }): JSX.Element => {
    const allProducts = useProduct() as any[]
    ...
    return (
        <div className="rounded p-4 relative h-full flex flex-col bg-[#F3F4F0] dark:bg-[#131316]">
            <h2 className="my-0">사용자가 무엇을 하는지 이해하세요</h2>
            <div className="grid grid-cols-1 @lg:grid-cols-2 gap-x-8 gap-y-4 mb-2">
                <div>
                    <p className="text-secondary text-sm">
                        참여도를 측정하고, 전환을 추적하며, 개인, 회사, AI 기능 단위의 사용 패턴을 이해하세요.
                    </p>
                </div>
                <div>
                    <p className="text-secondary text-sm">
                        데이터를 분석하고 제품을 만드는 방식은{' '}
                        <Link to="/tooling" state={{ newWindow: true }} className="font-semibold">
                            크게 달라졌습니다
                        </Link>
                        . <IconSparkles className="size-4 text-purple inline-block" />{' '}
                        <Link to="/ai" state={{ newWindow: true }} className="font-semibold">
                            PostHog AI
                        </Link>{' '}
                        를 사용해 데이터에 관한 질문에 바로 답을 얻으세요.
                    </p>
                </div>
            </div>

            <div className="relative mt-auto flex-1 min-h-[160px]">
                <ArcProducts products={products} translate={translate} />
...
export const DebugFixSlide = ({ translate = identity }: { translate?: TranslateFn }): JSX.Element => {
    const allProducts = useProduct() as any[]
    const products = debugHandles.map((h) => allProducts.find((p: any) => p.handle === h))

    return (
        <div className="rounded pt-4 px-4 bg-primary">
            <h2 className="mt-0 mb-2">문제를 분류하고 자동으로 해결하세요</h2>
            <p className="text-secondary text-sm">
                PostHog 디버깅 도구로 문제를 빠르게 찾고 해결에 필요한 컨텍스트를 확보하세요.
            </p>
                ...
                {products.map((product: any) => {
                    ...
                    return (
                        <div key={product.handle} className="rounded p-2">
                            <ProductStatusLink product={product} className="top-0" translate={translate} />
                            <p className="text-secondary text-[13px] mt-1 ml-[calc(16px+.375rem)] mb-0 leading-snug">
                                {translate(product.shortDescription || product.description)}
                            </p>
                        </div>
                    )
                })}
...
export const TestRolloutSlide = ({ translate = identity }: { translate?: TranslateFn }): JSX.Element => {
    const allProducts = useProduct() as any[]
    ...
    return (
        <div className="rounded p-4 pb-6">
            ...
            />
            <h2 className="mt-0">기능을 안전하게 출시하고 피드백을 받으세요</h2>
            <p className="text-secondary text-sm">
                무슨 일이 일어나는지 알게 되었으니 무엇을 만들지 더 정확하게 판단할 수 있습니다.
            </p>
            <p className="text-secondary text-sm">
                PostHog는 특정 사용자나 그룹에 기능을 배포하고, 전체 출시 전에 아이디어를 테스트하고 검증할 수 있게
                돕습니다.
            </p>
            <p className="text-secondary text-sm">
                이 모든 것을{' '}
                        ...
                        >
                            ...
                            >
                                {group.products.map((product: any) => (
                                    <ProductStatusLink key={product.handle} product={product} translate={translate} />
                                ))}
                            </div>
                        </div>
                    )
