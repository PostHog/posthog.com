...
export default function KoreanHeroCarousel({ translate = translateKo }: { translate?: TranslateFn }): JSX.Element {
    const tabs = [
        {
            value: 'understand-usage',
            label: '제품 사용량 이해',
            content: <UnderstandUsageSlide translate={translate} />,
            color: 'bg-blue',
        ...
        },
        {
            value: 'one-place',
            label: '프로덕트 데이터를 한곳에',
            content: <OnePlaceSlide translate={translate} />,
            color: 'bg-teal',
        ...
        },
        {
            value: 'debug-fix',
            label: '문제 디버깅 및 해결',
            content: <DebugFixSlide translate={translate} />,
            color: 'bg-salmon',
            activeText: 'text-white',
        ...
        {
            value: 'test-rollout',
            label: '변경 사항 테스트 및 배포',
            content: <TestRolloutSlide translate={translate} />,
            color: 'bg-purple',
            activeText: 'text-white',
            progressBar: 'bg-white shadow-[0_0_6px_2px_rgba(255,255,255,0.4)]',
        },
                            ...
                            <Tooltip
                                trigger={
                                    <button
                                        ...
                                        aria-label={isPaused ? '캐러셀 다시 시작' : '캐러셀 일시정지'}
                                    >
                                        {isPaused ? (
                                ...
                                delay={0}
                            >
                                <span>{isPaused ? '캐러셀 다시 시작' : '캐러셀 일시정지'}</span>
                            </Tooltip>
                        </span>
                        {tabs.map((tab) => (
                            <Tabs.Content
