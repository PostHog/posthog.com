...
export default function Dock() {
    ...
    return (
        <div ref={ref} data-scheme="tertiary" className="bg-primary w-full p-1 relative z-50 md:hidden block">
            ...
            >
                <Folder
                    label={translateKo('Product OS')}
                    items={productLinks.map((item) => ({ ...item, label: translateKo(item.label) }))}
                    onOpen={setOpenFolder}
                    open={openFolder === translateKo('Product OS')}
                />
                <Folder
                    label={translateKo('Browse all apps')}
                    items={apps.map((item) => ({ ...item, label: translateKo(item.label) }))}
                    onOpen={setOpenFolder}
                    open={openFolder === translateKo('Browse all apps')}
                />
            </div>
        </div>
    )
