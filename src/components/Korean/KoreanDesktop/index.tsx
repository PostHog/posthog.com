...
const STORAGE_KEY = 'ko-desktop-icon-positions'
...
export default function Desktop(): JSX.Element {
    ...
    const handleScreensaverDismiss = () => {
        addToast({
            title: translateKo('Screensaver dismissed'),
            description: translateKo('Want to disable it permanently?'),
            duration: 10000,
            actionLabel: translateKo('Disable screensaver'),
            onAction: () => {
                updateSiteSettings({ ...siteSettings, screensaverDisabled: true })
                addToast({
                    title: translateKo('Screensaver disabled'),
                    description: (
                        <>
                            {translateKo('Change this setting in')}{' '}
                            <Link
                                to="/display-options"
