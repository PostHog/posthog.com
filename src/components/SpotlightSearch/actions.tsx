...
export type SpotlightAction = {
    ...
    keepOpen?: boolean
    // With keepOpen, also wipe the trigger query — for actions where re-running
    // the old query makes no sense (e.g. the search-mode toggle, where it would
    // immediately fire the trigger words at the newly selected engine)
    clearQuery?: boolean
    perform: () => void
}
...
export const useSpotlightActions = (): SpotlightAction[] => {
    ...
    const toggleSearchMode = () => {
        const next = searchMode === 'semantic' ? 'keyword' : 'semantic'
        setSearchMode(next)
        toast(
            next === 'semantic' ? (
                <IconSparkles className="size-5 inline-block mr-1" />
            ) : (
                <IconSearch className="size-5 inline-block mr-1" />
            ),
            next === 'semantic' ? 'Switched to semantic search' : 'Switched to keyword search'
        )
    }
