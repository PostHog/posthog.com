...
export default function Cher({ active }: { active: boolean }) {
    ...
    return active ? (
        <motion.button
            initial={{ opacity: 0, translateX: 0, translateY: '-40%', rotate: 5 }}
            animate={{ opacity: 1, translateX: '33%', translateY: '-50%', rotate: 5 }}
            onClick={() =>
                addWindow(<MediaPlayer newWindow location={{ pathname: `cher` }} key={`cher`} videoId="nZXRV4MezEw" />)
            }
            whileHover={{ translateY: '-51%', translateX: '34%' }}
            whileTap={{ scale: 0.96 }}
