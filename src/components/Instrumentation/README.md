...
**Unter**: a fake website (a fictional Uber for hedgehogs), instrumented for real with every PostHog tool. It renders inside a fake browser frame, and every instrumented element carries a numbered marker that opens the code behind it.

Its only consumer is `<UnterFigure>` (`components/PocketGuides/UnterFigure.tsx`), which puts a live frame in the PostHog pocket guide's reading column.

## What it is

Unter has four pages (Shuffle, Host, Help, Safety), switched by React state rather than routes, since the OS window system owns the URL. Each page instruments a different set of tools.

**Inspect** starts off, so you first see Unter as its users do; turning it on drops numbered markers on the instrumented elements. Selecting one expands its explanation, code, and docs link.

The demo's survey is toggled by a "Quick survey" badge on the footer's top edge, not on a timer, so it never covers what you're reading. Its annotation points at that badge rather than at the popover, which keeps the marker on the page whether the survey is open or not. Opening it changes layout without resizing either container, so the consumer bumps `useAnnotationPositions`'s `revision` argument to force a re-measure.

Marker numbers and any list of rows derive from the same annotation array, so ordering is automatic.
...
overlay/
  ...
  InstrumentationBlocks.tsx CODE/CONFIG and DATA blocks for an annotation body
```

This folder is the demo only. It owns no page-level layout: whoever renders it supplies the
height, the inspect toggle, and wherever the selected annotation gets read. It came from the
`posthog-code/interactive-instrumentation` branch, which also had a full-screen `/instrumentation`
page (`index.tsx`, `AnnotationSidebar.tsx`, `ToolbarBar.tsx`). That page is not part of this repo;
those three files live in that branch's history if anyone wants them back.
...
2. Add an entry to the right `overlay/annotations/*.tsx`:
   - `target`: that `data-unter-id`. `dx`/`dy`: fractional position within the target's box (0 to 1; outside that range sits outside the element, e.g. `dy: 1.15` is just below, `dx: 0` straddles the left edge). Avoid negative values on full-width elements, or the marker lands outside the frame. Fractions scale with the box, so on wide targets check the marker at more than one window width: `dx: 0.315` on the nav sat in empty space at one size and on top of a link at another.
   - `tool`: a key from `tools.ts`, which drives color, icon, and docs link.
   - `body`: `{ why, input?, output?, after? }`. Every touchpoint pairs an `input` (what you instrument) with an `output` (the PostHog data it produces), rendered under `CODE`/`CONFIG` and `DATA` badges by `InstrumentationBlocks.tsx`. `input` is either a code block (`{ kind: 'code', language, snippet, context? }`) or a config table (`{ kind: 'config', rows, context? }`); `output` is a table (`{ table, context?, footnote? }`) that's either `kind: 'columns'` (header + rows) or `kind: 'fieldValue'` (field/value rows). Keep both consistent across all annotations, and don't mix instrumented code and result data in a single block. The column is ~384px, so keep snippet lines and table cells short.
