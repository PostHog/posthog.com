...
function Spine({ volume, count }: BookProps): JSX.Element {
    ...
    return (
        <article className={`flex flex-col gap-0.5 rounded-sm bg-${volume.token} px-4 py-3 shadow-sm`}>
            <h3 className="m-0 truncate text-sm font-bold leading-tight text-white">{volume.title}</h3>
            <p className="m-0 flex items-baseline gap-2 text-[10px] uppercase tracking-[0.16em] text-white/70 tabular-nums">
