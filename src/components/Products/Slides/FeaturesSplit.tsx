        ...
        <div className={`h-full flex flex-col @2xl:flex-row @2xl:gap-4 bg-${bgColor} ${textColor}`}>
            <div className="@2xl:flex-1 pt-8 px-8 @2xl:py-8">
                <div className="mb-8">
                    ...
                    <h2 className="text-6xl @2xl:text-5xl mb-0">{headline}</h2>
                    {description && (
                        <p
                    ...
                    <div className="space-y-2">
                        {features.map((feature: Feature, index: number) => (
                            <div key={index} className="flex gap-4 items-start">
                                {feature.icon && (
                                    <div className="">
                                        <div className="size-12 @2xl:size-10">{feature.icon}</div>
                                    </div>
                                )}
            ...
            <aside className="@2xl:max-w-[40%] flex">{children && <div>{children}</div>}</aside>
        </div>
    )
}
