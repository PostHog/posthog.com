...
/**
 * A single entry in a product's `marketingMenu`. Defined on the product data
 * (e.g. `src/hooks/productData/session_replay.tsx`) and consumed by
 * `ProductReaderView` + the `MarketingNav` / `CrossPageMarketingNav` components.
 *
...
/**
 * Resolves a `MarketingNavItem` to the template key that should render it,
 * falling back to the slug.
 */
export const resolveTemplate = (item: MarketingNavItem): string => item.template || item.slug
