...
// Tells us which rows people use, so we can see if the menu earns its place.
const trackDocsMenuClick = (label: string, link: string) => {
    if (typeof window === 'undefined') return
    window.posthog?.capture?.('docs_nav_item_clicked', { docs_nav_label: label, docs_nav_link: link })
}
...
const processMenuItemWithGrouping = (item: DocsMenuItem): any => {
    ...
    // Depth prune: an entry with a page of its own stops here. It becomes a link, and its
    // children stay in the docs sidebar instead of another panel in this menu.
    if (item.url) {
        const baseItem: any = {
            type: 'item' as const,
            label: item.name,
            link: item.url,
            onClick: () => trackDocsMenuClick(item.name, item.url as string),
        }
        if (item.icon) {
