...
const SlideButton = ({ title, Icon, color, activeSlide, index }) => {
    ...
    return (
        <li className="h-[calc(100%_-_.25rem)] pb-1 border-b border-primary/25 dark:border-primary-dark/25">
            <button
                className={`flex flex-col items-center mt-1 p-2 w-full rounded-md transition-opacity transition-colors border border-b-3 border-transparent relative space-y-1 h-full ${
                    active
