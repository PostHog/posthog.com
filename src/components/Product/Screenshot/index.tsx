...
export const Screenshot = ({ icon, product, slug, order, className = '', children }: ScreenshotProps): JSX.Element => {
    ...
    useEffect(() => {
        const messageHandler = (event: MessageEvent) => {
            if (event.data.type === 'session-replay-data' && event.data.ready) {
                setIframeReady(true)
            }
        }
    ...
    useEffect(() => {
        if (iframeReady && replayEvents.length > 0) {
            replayRef.current?.contentWindow?.postMessage({ type: 'session-replay-data', snapshots: replayEvents }, '*')
        }
    }, [iframeReady, replayEvents])
            ...
            >
                <g clipPath="url(#a)">
                    ...
                    <rect
                        ...
                        x="330.531"
                        y="84"
                        className="fill-border dark:fill-border-dark"
                        rx="4"
                    />
