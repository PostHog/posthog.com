...
const Select = (props: SelectProps) => {
    ...
    return (
        <Combobox as="div" className="relative" value={props.value} onChange={props.onChange}>
            {({ open }) => (
                <>
                    <Combobox.Label className="text-sm">{props.label}</Combobox.Label>
                    <Combobox.Input
                        onChange={(event) => setQuery(event.target.value)}
                        className="relative block w-full max-w-md text-left bg-white px-2.5 py-1.5 rounded border border-gray-accent-light text-xs select-none mt-1.5 focus:ring-1 focus:ring-orange focus:border-orange"
                    >
                    ...
                    <Transition
                        ...
                        leaveTo="opacity-0"
                    >
                        <Combobox.Options className="absolute top-full mt-1 w-full bg-white rounded z-50 p-0 text-xs max-h-[12rem] overflow-y-scroll py-1 focus:outline-none space-y-1 shadow-lg">
