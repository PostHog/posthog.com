...
export const WIZARD_DROP_FLAG = 'wizard-drop'

/**
 * Whether the GitHub "drop" flow should take over the wizard hero.
 *
 * Gated on the `wizard-drop` experiment: only the `test` variant enables it; `control` and
 ...
 * <WizardDrop /> only when this is true, so the component itself never needs to re-check.
 *
 ...
 * is the only gate.
 */
export function useWizardDropEnabled(): boolean {
    const flags = useActiveFeatureFlags()
    const posthog = usePostHog()
    return process.env.NODE_ENV === 'development' || (!!flags && posthog?.getFeatureFlag?.(WIZARD_DROP_FLAG) === 'test')
}
