...
export default function OSTabs({
    ...
    return (
        <>
            ...
            >
                <Tabs.List className="ml-2 flex-shrink-0">
                    {tabs.map((tab) => (
                        <Tabs.Trigger
                            key={tab.value}
                            value={tab.value}
                            data-scheme={triggerDataScheme}
                            className="data-[state=active]:bg-primary px-2 py-1 border border-transparent data-[state=active]:border-primary border-b-0 rounded-tr-md rounded-tl-md relative -bottom-px z-10 text-sm select-none"
                        >
                            {tab.label}
                        </Tabs.Trigger>
                    ))}
