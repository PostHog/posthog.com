...
export default function TeamPatch({ name, imageUrl, color, textColor, textShadow, frame, plaque, size }: TeamPatchProps) {
  return (
    <div className={`size-${size === 'lg' ? '72' : size === 'md' ? '56' : size === 'sm' ? '36' : '72'} mx-auto flex justify-center`}>
      <div
        className={`relative ${size === 'lg' ? 'w-48 h-64' : size === 'md' ? 'w-36 h-48' : size === 'sm' ? 'w-24 h-32' : 'w-48 h-64'} shadow-xl border-4 mx-auto flex items-center justify-center border-team-${color}-border bg-team-${color}-bg ${frame === 'half-round'
      ...
      >
        {plaque === 'straight' && (
          <>
            <div
              className={`left-round-behind w-4 h-1.5 border-l-2 border-b-2 border-team-${color}-border bg-team-${color}-plaque-shadow rounded-bl-full absolute left-[calc(-1rem_-_0px)] bottom-1.5 z-[-1]`}
            />
            <div
              className={`right-round-behind w-4 h-1.5 border-r-2 border-b-2 border-team-${color}-border bg-team-${color}-plaque-shadow rounded-br-full absolute right-[calc(-1rem_-_0px)] bottom-1.5 z-[-1]`}
            />
            <div
              className={`banner-text-bg bg-team-${color}-plaque-bg bg-opacity-80 border-2 border-team-${color}-border h-8 rounded-tl rounded-tr absolute bottom-4 -left-4 -right-4 z-20 flex items-center justify-center text-center leading-none uppercase ${size === 'lg' ? 'text-xl' : size === 'md' ? 'text-lg' : size === 'sm' ? 'text-sm' : 'text-xl'} font-bold font-squeak [font-variant:none] text-${textColor ? textColor : 'white [text-shadow:0_1px_0_rgba(0,0,0,.5)]'
                } ${textShadow === 'light'
            ...
            <div className="fake-corners absolute bottom-4 -left-3.5 -right-3.5">
              <div className="relative mt-[-2px]">
                ...
                />
                <div
                  className={`left-rounded-corner bg-team-${color}-plaque-shadow border-l-2 border-t-2 rounded-tl-full border-team-${color}-border w-3 h-1.5 absolute -left-0.5 -bottom-1.5 z-20 `}
                />
                <div
                ...
                />
                <div
                  className={`right-rounded-corner bg-team-${color}-plaque-shadow border-r-2 border-t-2 rounded-tr-full border-team-${color}-border w-3 h-1.5 absolute -right-0.5 -bottom-1.5 z-20`}
                />
              </div>
            </div>
          </>
