...
| File                 | What it is                                                                                     |
| -------------------- | ---------------------------------------------------------------------------------------------- |
| `InboxReplica.tsx`   | The main event: the Inbox scene – header + subtitle, the real tab bar (Pull requests / Reports / Not actionable / Runs / Archive) with the For-you/Entire-project scope picker, the Sort/Scout/Priority filter bar, the centered list, a full-width detail takeover for the open item, and the Inbox Zero state. The "Pull requests" tab count is the live countdown. All state is in React memory – no localStorage. |
