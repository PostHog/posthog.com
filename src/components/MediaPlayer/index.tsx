...
export default function MediaPlayer({ videoId, source = 'youtube', startTime = 0 }: MediaPlayerProps) {
    ...
    }, [playerState.player, playerState.isPlaying, isScrubbing, source])
    ...
    const handleSeek = (seconds: number) => {
        if (playerState.player) {
            if (source === 'youtube') {
                ...
                const currentTime = playerState.player.time()
                playerState.player.time(currentTime + seconds)
            }
        }
    }
    ...
    const handleScrubEndMouse = (e: React.MouseEvent<HTMLInputElement>) => {
        ...
        if (playerState.player) {
            if (source === 'youtube') {
                ...
                playerState.player.time(newTime)
            }
        }
    }
