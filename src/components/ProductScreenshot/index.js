...
export const ProductScreenshot = ({ imageLight, imageDark, alt, classes }) => {
    return (
        ...
        >
            {imageLight && <img src={imageLight} alt={alt} className={`dark:hidden max-w-full  ${classes}`} />}
            {imageDark && <img src={imageDark} alt={alt} className={`hidden dark:block max-w-full ${classes}`} />}
        </div>
    )
}
