...
const components = {
    valueselect: ({ title, required, path, options }: { title: string; required: boolean; path: string; options: Array<{ label: string; value: string }> }) => (
    ...
    file: ({ title, required, path }: IResumeComponentProps) => {
        ...
        return (
            <div className="w-full">
                ...
                <div
                    {...getRootProps()}
                    className={`relative h-24 w-full border border-primary ${isDragActive ? 'bg-blue/10' : 'bg-accent'
...
const Form = ({ setSubmitted, info, id }: { setSubmitted: (submitted: boolean) => void; info: any; id: string }) => {
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-2 gap-3">
                        {info?.applicationFormDefinition?.sections?.map(({ fields }: { fields: any[] }) => {
                            return fields.map(({ field, isRequired, descriptionPlain }: { field: any; isRequired: boolean; descriptionPlain: string }) => {
...
const ApplicationSuccess = () => {
    ...
    return (
        <>
            <div data-scheme="secondary" className="w-full bg-primary text-primary rounded-md relative">
                ...
                <div className="mx-6 md:mx-12 pt-6 pb-2 border-t border-primary">
                    ...
                    <p className="text-sm mb-4">
                        ...
                        <Link to="/cool-tech-jobs" state={{ newWindow: true }} className="text-red dark:text-yellow font-semibold">
                            cool tech job board
                        </Link>{' '}
...
export default function Apply({ id, info }: { id: string; info: any }) {
    ...
    useEffect(() => {
        if (!submitted) {
            ...
            addWindow(windowElement as any)
        }
    }, [submitted, addWindow])
